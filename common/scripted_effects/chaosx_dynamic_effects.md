...
Outputs: contact reconciliation grants the hidden operational alien-infantry technology, creates and relocks the ten-battalion `D’Rhondan Landing Cohort`, and exposes laser production while at least one receipt remains. A successful spawn creates exactly one fully equipped cohort, marks the state, inserts the state scope into the caller country’s sparse `alien_infantry_landing_state_registry`, and records the persistent landing counters. `alien_infantry_landing_spawn_succeeded` is a country-scoped one-or-zero result reset at the start of each spawn call, so bounded callers may read it after a nested scripted effect returns. D’Rhondan revolt transfer consumes only the pact host’s registry; no global cross-provider state array is used.
