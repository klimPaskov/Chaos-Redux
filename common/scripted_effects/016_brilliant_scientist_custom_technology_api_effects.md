...
Portal weaponization authorizes the existing portal facility raid for any country that also has the rebuilt `Quantum Transit Raiders` template. The native outcome calls `brilliant_scientist_portal_raid_establish_beachhead` to commit one full-equipment and full-manpower `Quantum Transit Raiders` formation with 0.50 starting experience and raise one scope-less reconstruction receipt. The native `division_effects` block consumes the assigned formation only against that receipt, preserving one formation and the locked six-battalion baseline budget without a target-race loss or duplicate. Damage and experience normalize to the breach-cadre profile because the documented `teleport_armies` effect is STATE-scoped and cannot safely relocate only the selected raid division. Kruger's presence still increases the raid's AI weight, but it is not an access requirement.
