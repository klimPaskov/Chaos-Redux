...
The normal row order is host remnant, primary claimant, frozen ordinary secondary, and frozen Event 006 secondary, with optional rows appended only when their independent receipts already exist. The host remnant uses the distinct `administrative_continuity` objective, while the primary, nested, independence, and same-tag rows retain their route-specific terminal objectives.
