...
| `independence_wave_cleanup_iw093_asante` / `_iw098_sokoto` | Matching original tag and package ID | Removes package decisions, focus receipts, shared focus/formable profiles, route exclusions, staged ideas, package-local values, and the four Event 006 fictional commanders, clears the external runtime-content admission receipt for the torn-down generation, then restores the generic tree if the Event 006 tree was loaded. SOK cleanup never retires vanilla Siddiq or clears Event 012 focus or lifecycle flags |
