...
Set `alien_infantry_contact_source_id` to one numeric source constant, then call `alien_infantry_grant_contact = yes` or `alien_infantry_revoke_contact = yes`. The API stores one receipt variable per source and derives the aggregate `alien_infantry_has_contact` state from the sum. Revoking one source never removes another source’s entitlement.

The current source constants are `kruger_pact`, `mengele_expedition`, `event019_provider_508`, `dhrondan_sovereignty`, and `future_event`. A future event must add a source constant and receipt field before granting access; it must not reuse another event’s receipt.

Example:

```text
set_temp_variable = { alien_infantry_contact_source_id = constant:alien_infantry_contact_source.future_event }
...
`alien_infantry_reconcile_country = yes` is the idempotent setup/reload helper. It recreates the locked template only when the country has valid contact or D’Rhondan sovereignty, restores contact-gated laser production, and removes stale pending reservations or templates without changing unrelated contact receipts. It never grants human manpower, ordinary infantry equipment, or a free training path.
...
- State-targeted calls must use `dhrondan_landing_state_id`; callers must clear or let the API clear this target after completion, cancellation, expiry, or invalid ownership.
