...
```txt
set_temp_variable = { cbrn_requested_use_policy = constant:cbrn_use_policy.battlefield_authorization }
...
| `cbrn_hq_stop_operation_benefits` | Character scope. Removes every active CBRN headquarters status trait but deliberately retains the operation code and commitments until planned cleanup. This prevents a stale delayed event from crossing into a newer operation. |
