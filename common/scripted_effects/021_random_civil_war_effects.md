...
`event021_cleanup_crisis`, `event021_cleanup_global_registry`, and `event021_clear_global_threat_source` remove temporary state after a completed or rejected transaction. Crisis cleanup also clears every transient Event 021 AI role profile so settled or rejected actors cannot retain crisis behavior. Decision-state cleanup clears the priority-front mirror and rail receipt, while global cleanup clears the persistence carrier. Clearing the Evolution III threat also empties the Critical queue, removes its country receipts, and resets its cursor and count.
