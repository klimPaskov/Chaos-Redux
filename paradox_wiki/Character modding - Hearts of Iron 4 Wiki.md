...
| allowed | Triggers | *(example below)* | Triggers that are checked when trying to assign the trait to a unit leader, making it fail to assign if false. Checked in the scope of the unit leader. | FROM is the country that recruited the character. |
| ai_will_do | [MTTH block](<AI modding - Hearts of Iron 4 Wiki.md>) | *(example below)* | Decides the weight that AI has for picking this trait. | A weight of 0 will result in AI never picking it. |
| new_commander_weight | [MTTH block](<AI modding - Hearts of Iron 4 Wiki.md>) | *(example below)* | Decides the weight that the trait has for new randomly-generated unit leaders. | A weight of 0 will result in it never appearing for randomly-generated unit leaders. Only can be defined for traits with the personality_trait type. |
...
| Argument | Value type | Example | Effects | Notes |
| --- | --- | --- | --- | --- |
| modifier | Modifiers | *(example below)* | Assigns the [modifiers](<Modifiers - Hearts of Iron 4 Wiki.md>) that the trait grants to the divisions that the unit leader leads. | Possible to specify the terrain by scoping into it. Terrain types are defined in `/Hearts of Iron IV/common/terrain/*.txt`. |
| non_shared_modifier | Modifiers | *(example below)* | Assigns the [modifiers](<Modifiers - Hearts of Iron 4 Wiki.md>) that the trait grants to the unit leader themselves. | While in theory identical to the prior modifier, this has a different tooltip, so it shows up differently in-game. |
| corps_commander_modifier | Modifiers | *(example below)* | Assigns the [modifiers](<Modifiers - Hearts of Iron 4 Wiki.md>) that the trait grants to the army leader when in the role of a corps commander, i.e. leading units directly rather than leading other generals. | If a field marshal is assigned to lead divisions directly rather than other generals, this will apply on them. |
| field_marshal_modifier | Modifiers | *(example below)* | Assigns the [modifiers](<Modifiers - Hearts of Iron 4 Wiki.md>) that the trait grants to the army leader when in the role of a field marshal, i.e. leading other generals that lead divisions. | If a field marshal is assigned to lead divisions directly rather than other generals, this will *not* apply on them. |
| sub_unit_modifiers | Modifiers | *(example below)* | Assigns the [modifiers](<Modifiers - Hearts of Iron 4 Wiki.md>) that the trait grants to the division brigades that make up the divisions that the army leader leads or, similarly, ships that make up the fleet that the navy leader leads. | Brigades are defined in `/Hearts of Iron IV/common/units/*.txt`. What's used in the definition as arguments is possible to apply as a multiplicatory sub-unit modifier. |
...
| on_add | Effects | *(example below)* | Defines the effects that would be executed on the unit leader when the trait is added. |  |
| on_remove | Effects | *(example below)* | Defines the effects that would be executed on the unit leader when the trait is removed. |  |
| daily_effect | Effects | *(example below)* | Defines the effects that would be executed on the unit leader every day if they have the trait. |  |
...
**Example: corps_commander_modifier**
...
**Example: field_marshal_modifier**
...
| prerequisites | Triggers | *(example below)* | Triggers that must be met in order for assigning the trait to be possible. | Checked in the unit leader scope. |
...
| gain_xp | Triggers | *(example below)* | Triggers that must be met in order to gain experience that'd make assigning this trait possible. | Checked in the [combatant](<Triggers - Hearts of Iron 4 Wiki.md#Combat>) scope. |
| gain_xp_leader | Triggers | *(example below)* | Triggers that must be met in order to gain experience that'd make assigning this trait possible. | Checked in the unit leader scope. |
...
| trait_xp_factor | Triggers | *(example below)* | Modifies the amount of experience gained towards other traits if the unit leader has this trait. | 0.1 would mean that the unit leader gains 10% more experience, not that it gains 10% as much. |
