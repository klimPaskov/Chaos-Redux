...
*Offline snapshot of the Hearts of Iron IV Wiki page "Data structures", captured 2026-09-19.*
...
- [Flags](#Flags)
  - [Example](#Example)
  - [Non-boolean usage](#Non-boolean_usage)
  - [Debugging](#Debugging)
- [Event targets](#Event_targets)
  - [Global event targets](#Global_event_targets)
  - [Regular event targets](#Regular_event_targets)
- [Country tag aliases](#Country_tag_aliases)
- [Variables](#Variables)
  - [Variable types](#Variable_types)
  - [Scoping](#Scoping)
  - [Trigger usage](#Trigger_usage)
  - [Localisation](#Localisation)
  - [Additional usage](#Additional_usage)
  - [Operators](#Operators)
  - [Math expressions](#Math_expressions)
  - [Math functions](#Math_functions)
  - [Usage examples](#Usage_examples)
  - [MTTH variables](#MTTH_variables)
  - [Debugging variables](#Debugging_variables)
  - [Token-valued variables](#Token-valued_variables)
  - [Advanced use of tokens](#Advanced_use_of_tokens)
- [Modifier tokens](#Modifier_tokens)
  - [Examples](#Examples)
...
- [Arrays](#Arrays)
  ...
  - [Scorers](#Scorers)
    - [Example](#Example_2)
- [Game variables](#Game_variables)
  - [Game arrays](#Game_arrays)
...
Note that neither of the effects that modify country flags have tooltips, so [a custom effect tooltip can be used to notify the player](<Effects - Hearts of Iron 4 Wiki.md>) in some cases.
Everything is the exact same for flags of other scope types.

### Non-boolean usage <a id="Non-boolean_usage"></a>
...
| --- | --- | --- | --- |
| set_country_flag | `<flag>` The name of the flag to set. | `set_country_flag my_flag` | Sets the country flag for the country controlled by the player. |
| set_global_flag | `<flag>` The name of the flag to set. | `set_global_flag my_flag` | Sets the global flag. |
| list_flags | `<scope>`Optional. Changes the scope from the currently-selected scope to what's specified. | `list_vars` | Lists currently active flags in the console window, with their value. Context senstive if nothing (global_flag), country (country_flag) or state (state_flag) is selected when entering this command. |

## Event targets <a id="Event_targets"></a>
...
### Global event targets <a id="Global_event_targets"></a>
...
For example, by using [start_civil_war](<Effects - Hearts of Iron 4 Wiki.md>) as an effect block, the following will assign the TAG_democratic_side event target to the revolter:
...
It would make sense to clear the event target after it no longer exist, such as by using the [on_civil_war_end on action](<On actions - Hearts of Iron 4 Wiki.md>).
...
### Regular event targets <a id="Regular_event_targets"></a>
...
## Country tag aliases <a id="Country_tag_aliases"></a>
...
- `original_tag = TAG` - This puts every country that originates from TAG (created with [start_civil_war](<Effects - Hearts of Iron 4 Wiki.md>) or [create_dynamic_country](<Effects - Hearts of Iron 4 Wiki.md>)) as a possible target for the alias. In this case, further specification is needed with triggers. This is equivalent to the [original_tag trigger](<Triggers - Hearts of Iron 4 Wiki.md>), but this is treated as an argument here as it does make certain errors go away.
...
- `country_score = { ... }` - This is a [MTTH block](<AI modding - Hearts of Iron 4 Wiki.md#MTTH_blocks>) evaluated for each country. The country which has the highest total score will be picked for the country tag alias, provided the conditions imposed by every other argument are met.
...
This is most commonly used with [start_civil_war](<Effects - Hearts of Iron 4 Wiki.md>) or [create_dynamic_country](<Effects - Hearts of Iron 4 Wiki.md>). For example:
...
### Trigger usage <a id="Trigger_usage"></a>
...
However, there's one aspect of triggers that's important to know: *when* each scope stops evaluating. If a scope stops evaluating, then the variable will not be modified anymore. In general, [trigger scopes](<Scopes - Hearts of Iron 4 Wiki.md#Trigger_scopes>) that can select multiple targets are divided into two groups: `all_<scope type>` and `any_<scope type>`. The first group, requiring all targets to meet triggers, evaluates each target in order and stops when it encounters one where the triggers aren't met. The second group, requiring any target to meet triggers, evaluates each target and stops when it encounters a true one.
Empty trigger blocks are considered true by default, so one would instantly stop the validation of the second group (`any_` triggers). For this reason, [since scope limits are not supported in trigger scopes](<Scopes - Hearts of Iron 4 Wiki.md>), then the proper usage for modifying temp variables would be an [if statement](<Triggers - Hearts of Iron 4 Wiki.md>) that modifies the variable if the limit is met with no other triggers that can come up as true or false coming up.

### Localisation <a id="Localisation"></a>

*Parts of this section are transcluded from [Localisation § Formatting variables](<Localisation - Hearts of Iron 4 Wiki.md#Formatting_variables>)*
...
By default, variable-related arguments such as `set_variable` or `check_variable` do not have a tooltip. For *some* of these arguments, such as `add_to_temp_variable`, it is possible to set a tooltip with an optional `tooltip = localisation_key` argument within. Others, such as `check_variable`, do not support this and a custom tooltip must be used instead, whether it's for [effects](<Effects - Hearts of Iron 4 Wiki.md#Tooltip_manipulation>) or for [triggers](<Triggers - Hearts of Iron 4 Wiki.md>).

This tooltip argument is notable since the value of that localisation key allows using two [strictly-internal variables](<Localisation - Hearts of Iron 4 Wiki.md>) marked with dollar signs: `$LEFT$` and `$RIGHT$`.
...
### Additional usage <a id="Additional_usage"></a>

By default, an unset variable is equal to zero when checked. However, the game also provides a null coalescing operator for variables, used with a question mark. For example, `cost = var_name?100` within a decision will result in the cost of the decision being the same as var_name for the country. However, if that variable was never set or was cleared beforehand, then the decision will cost 100 ![Political Power](media/building-modding-hearts-of-iron-4-wiki_811161eae8__img1.png)Political Power instead of 0.
...
### Math functions <a id="Math_functions"></a>
...
| --- | --- | --- | --- |
| less_than | Returns 1 if the accumulator is less than the value, otherwise 0. | `{ value = 2 less_than = 10 }` |  |
| greater_than | Returns 1 if the accumulator is greater than the value, otherwise 0. | `{ value = 2 greater_than = 10 }` |  |
...
| every_collection | Iterates over a named collection, applying statements to the accumulator for each element. | `{ value = 0 every_collection = { named_collection = my_collection add = element_value } }` |  |
...
Since temporary variables can be set in triggers, they can be used as an alternative version of [count_triggers](<Triggers - Hearts of Iron 4 Wiki.md>), to count the amount of scopes where the trigger is fulfilled instead of counting the amount of fulfilled triggers:
...
When using variables in this sense, **it is better to use [scripted effects](<Effects - Hearts of Iron 4 Wiki.md#Scripted_effects>) to modify them**, in order to make changes to the variable easier. For example, a small gain in beavers could be the following:
...
After creating these scripted effects, these can be used within any effect block to modify the variable at will. [Showing its value in localisation to the player](#Additional_usage) would also be helpful, such as within a [decision category](<Decision modding - Hearts of Iron 4 Wiki.md>) or in [scripted GUI](<Scripted GUI modding - Hearts of Iron 4 Wiki.md>). **Variables cannot be used within idea modifiers directly, [dynamic modifiers can be used instead.](<Modifiers - Hearts of Iron 4 Wiki.md#Dynamic_modifiers>)**
...
### MTTH variables <a id="MTTH_variables"></a>

MTTH variables allow to dynamically create a [MTTH block](<AI modding - Hearts of Iron 4 Wiki.md#MTTH_blocks>) to be used as a variable. These are stored within `/Hearts of Iron IV/common/mtth/*.txt` files, where each MTTH variable is a separate block, the name of which gets taken as the MTTH variable.
...
### Debugging variables <a id="Debugging_variables"></a>
...
| print_variables | `file = <string>` The name of the file, not counting the file extension. Defaults to "variable_dump".  `text = <string>` What will be printed in the beginning of the file. Defaults to "No Header".  `append = yes` If set, will append the current variables (and the text) to the end of the file instead of overwriting.  `print_global = yes` If true, will print the variables from the global scope as well.  `var_list = { ... }` A whitespace-separated list of variables the values of which should be printed. Optional: every variable in the scope (and global if set) if left out. | `print_variables = { file = my_file text = "These variables are printed at [GetDateText]" append = yes print_global = yes }`  `print_variables = { file = my_file_2 var_list = { temp1 temp2 temp3 } }` | Dumps the specified variables from the current scope and optionally the global scope into a log file with the specified name. | The log will be within `/Hearts of Iron IV/logs/variable_dumps/` in the [user directory](<Modding - Hearts of Iron 4 Wiki.md>). |
...
### Token-valued variables <a id="Token-valued_variables"></a>
...
When the variable contains a token value, it is possible to use the `GetTokenKey` [localisation function](<Localisation - Hearts of Iron 4 Wiki.md#Functions>) (such as `[?some_var_with_token_value.GetTokenKey]`) to get the token string value of the variable, `token:` prefix not included. This is useful for pasting in [meta-effects](<Effects - Hearts of Iron 4 Wiki.md#Meta_effects>) and [meta-triggers](<Triggers - Hearts of Iron 4 Wiki.md#Meta_triggers>), however note that the `token:` prefix is still required if using it within the `set_(temp_)variable` operator. With some token-valued variables (specifically, those backed by named database objects of a tokenizable datatype), it is possible to use `GetTokenLocalizedKey` to retrieve the value of the localisation key with the same name as the token itself. This can be used to produce and/or define human-readable names, but it can also be taken advantage of to pass around arbitrary localisation values.
...
### Advanced use of tokens <a id="Advanced_use_of_tokens"></a>
...
| Name | Parameters | Examples | Description | Notes |
| --- | --- | --- | --- | --- |
| is_in_array | `array = <array>` The array to check.  `value = <decimal>/<variable>` The value to check for. | `is_in_array = { array = global.my_countries value = THIS.id }`  `is_in_array = { temp_states = THIS }` | Checks if any value within the array is the same as the specified value. | Shortened version exists with `is_in_array = { <array> = <value> }`. |
| any_of | `array = <array>` The array to check.  `value = <variable>` The name of the temporary variable where the value of the currently-selected element is stored.  `index = <variable>` The name of the temporary variable where the index of the currently-selected element is stored.  `<triggers>`  An AND trigger block. | `any_of = { array = temp_numbers value = v index = i multiply_temp_variable = { v = i } check_variable = { v > political_power } # Checks if the value multiplied by the index is larger than the political power of the country this is checked in. }` | Checks if any value within the array fulfills the triggers, halting and returning true if that's the case. | The scope does not change from the one this is checked in by default, requiring one of [scopes](<Scopes - Hearts of Iron 4 Wiki.md>). |
...
| for_loop_effect | `start = <decimal>/<variable>` The value at which the evaluation starts. Default to 0.  `end = <decimal>/<variable>` The value at which the evaluation ends. Default to 0.  `compare = <compare type>` How the currently-evaluated value must compare with the `end` for the for loop to continue. Defaults to `less_than`, leading to the end value never being picked.  `add = <variable>` How much is being added to the `start` value for every iteration. Defaults to 1.  `break = <variable>` The temporary variable that can be set to be not 0 to instantly break the loop.  `value = <variable>` The name of the temporary variable where the current value of evaluation is stored.  `<effects>`  An effect block. | `for_loop_effect = { end = 4 value = temp if = { limit = { NOT = { count_triggers = { amount = 2 check_variable = { temparr^temp < temparr^0 } check_variable = { temparr^temp < temparr^1 } check_variable = { temparr^temp < temparr^2 } check_variable = { temparr^temp < temparr^3 } } } } add_to_array = { potential_max = temp } } }` | Runs the effects as long as the limit is fulfilled. | The limit is only checked at the beginning and once the effect block is executed. Can't run for more than 1000 times by default[5]. Comparison types are the same as used in check_variable. |

### Scorers <a id="Scorers"></a>

Scorers are defined within `/Hearts of Iron IV/common/scorers/country/*.txt` files, serving as a sorted list of targeted countries, based on how well they did in the specified [MTTH block](<AI modding - Hearts of Iron 4 Wiki.md>). The scorer has the primary country which the scorer is used for and targeted countries.
...
Scorers target countries similarly to [targeted decisions](<Decision modding - Hearts of Iron 4 Wiki.md#Targeted_decisions>) but with differences, with the following arguments being used within of `targets = { ... }`:
...
- `targets = { TAG, TAG }` - A list of country tags, including other [dual scopes](<Scopes - Hearts of Iron 4 Wiki.md#Dual_scopes>) such as [overlord](<Scopes - Hearts of Iron 4 Wiki.md>), that can be targeted by this scorer. Unlike targeted decisions, this seems to be comma-separated.
...
| Name | Parameters | Examples | Description | Notes |
| --- | --- | --- | --- | --- |
| get_highest_scored_country | `scorer = <scorer>` The scorer to use.  `var = <variable>` The variable where the result will be stored. | `get_highest_scored_country = { scorer = asian_countries_owned_states_scorer var = largest_asian_country }`  `get_highest_scored_country_temp = { scorer = asian_countries_owned_states_scorer var = largest_asian_country }` | Sets the specified variable to be the highest-scoring country within the scorer, as checked for the country where this is used. |  |
| get_sorted_scored_countries | `scorer = <scorer>` The scorer to use.  `array = <variable>` The array where the countries will be stored.  `scores = <variable>` The array where the scores will be stored. Optional. | `get_sorted_scored_countries = { scorer = asian_countries_owned_states_scorer array = asian_countries_by_size scores = asian_country_size_scores }`  `get_sorted_scored_countries_temp = { scorer = asian_countries_owned_states_scorer array = asian_countries_by_size scores = asian_country_size_scores }` | Creates two arrays of countries and their corresponding values, sorted in the ascending order. | Values with the same index are connected: in the example, `asian_countries_by_size^0` is the country that got `asian_countries_scores^0` points in the `majors_owned_states` scorer. |
...
## Game variables <a id="Game_variables"></a>
...
| num_days | None. | The amount of days passed since the start of year 0. The value on the 1st of January 1936 is 706640. |
...
| pc_turn | None. | The turn in the currently-ongoing peace conference. |
| power_balance_value | BoP, such as `@ITA_power_balance` | The current value of the balance of power on the scale from -1 to 1. |
| power_balance_daily_change | BoP, such as `@ITA_power_balance` | The current daily change in the value of the balance of power. |
| power_balance_weekly_change | BoP, such as `@ITA_power_balance` | The current weekly change in the value of the balance of power. |
...
| has_casualties_war_support | None. | The malus from casualties towards war support on the scale from 0 to 1. |
| has_convoys_war_support | None. | The malus from sunk convoys towards war support on the scale from 0 to 1. |
| has_bombing_war_support | None. | The malus from bombing towards war support on the scale from 0 to 1. |
...
| autonomy_ratio | None. | How close the country is to taking the next autonomy level. 0 if can be downgraded, 1 if can be upgraded, -1 if independent. `compare_autonomy_progress_ratio` can be used in a similar way. |
...
| country_leader | None. | The leader of the current country. |
...
| resource_exported | Resource, such as `@oil`. | The amount of resource of the specified type currently set to be exported with the [min_export modifier](<Modifiers - Hearts of Iron 4 Wiki.md>). |
...
| original_research_slots | None. | The amount of research slots that the country had in the beginning of the game, as set in the [history file](<Country creation - Hearts of Iron 4 Wiki.md#Effects>). |
...
| days_since_capitulated | None. | The amount of days since the country has capitulated. If the country has never capitulated, returns 2147483. |
...
| total_constructed_<building> | None. | Amount of buildings of a certain type that have been constructed after the game's start by the country. The following options exist:  - Name: total_constructed_civilian_factory - Name: total_constructed_infrastructure - Name: total_constructed_military_factory - Name: total_constructed_dockyard - Name: total_constructed_port - Name: total_constructed_air_base - Name: total_constructed_rocket_site - Name: total_constructed_radar - Name: total_constructed_anti_air - Name: total_constructed_refinery - Name: total_constructed_fuel_silo - Name: total_constructed_supply_node - Name: total_constructed_nuclear_reactor - Name: total_constructed_land_fort - Name: total_constructed_naval_fort - Name: total_constructed_other |
...
| num_of_supply_nodes | None. | Amount of supply nodes. |
| num_of_nukes | None. | Amount of nukes. |
| num_researched_technologies | None. | Number of technologies a tag has researched. |
...
| has_legitimacy | None. | Legitimacy of the government in exile on the scale from 0 to 100. Defaults to 0 if none. |
| pc_total_score | None. | Total score of the country on the currently-ongoing peace conference where it is a winner. |
| pc_current_score | None. | Current score of the country on the currently-ongoing peace conference where it is a winner. |
| days_left | Country flag such as `:my_flag` | The current number of remaining days for a country flag. Must be targeted with a `:` rather than a `@` with the position of the target and the variable being swapped unlike in most other game variables, e.g. `my_flag:days_left`. The value of the variable cannot be directly passed into another variable; instead, a [meta effect](<Effects - Hearts of Iron 4 Wiki.md#Meta_effects>) must be used. |
...
| airforce_intel | Country. | Air intel against the target country.(If you need to use this variable as a trigger for checking, please write a localization yourself.) |
| civilian_intel | Country. | Civilian intel against the target country. |
| encryption_strength | None. | Total encryption strength of the country. Only without the ![La Résistance](media/ai-modding-hearts-of-iron-4-wiki_04cfcba22e__img6.png)La Résistance DLC. |
| decryption_speed | None. | Total decryption speed of the country. Only without the ![La Résistance](media/ai-modding-hearts-of-iron-4-wiki_04cfcba22e__img6.png)La Résistance DLC. |
...
| total_equipment_produced_<type> | None. | The total amount of produced equipment with the specified type of equipment archetypes. The following options exist:  - Name: total_equipment_produced_infantry; Base game notes: Includes infantry equipment, support equipment, artillery (non-rocket), anti-air, and anti-tank. - Name: total_equipment_produced_artillery; Base game notes: Also includes anti-air, anti-tank, and rocket artillery. - Name: total_equipment_produced_anti_air - Name: total_equipment_produced_anti_tank - Name: total_equipment_produced_rocket - Name: total_equipment_produced_support - Name: total_equipment_produced_motorized; Base game notes: Also includes armoured cars, rocket artillery, and support equipment. - Name: total_equipment_produced_mechanized - Name: total_equipment_produced_capital_ship - Name: total_equipment_produced_submarine - Name: total_equipment_produced_carrier - Name: total_equipment_produced_screen_ship - Name: total_equipment_produced_fighter - Name: total_equipment_produced_heavy_fighter - Name: total_equipment_produced_cas - Name: total_equipment_produced_interceptor - Name: total_equipment_produced_strategic_bomber - Name: total_equipment_produced_tactical_bomber - Name: total_equipment_produced_naval_bomber - Name: total_equipment_produced_maritime_patrol_plane - Name: total_equipment_produced_scout_plane - Name: total_equipment_produced_air_transport - Name: total_equipment_produced_suicide; Base game notes: Suicide bombers. - Name: total_equipment_produced_missile - Name: total_equipment_produced_convoy - Name: total_equipment_produced_armor - Name: total_equipment_produced_amphibious; Base game notes: Only includes amphibious tanks and not amphibious mechanised equipment. - Name: total_equipment_produced_flame - Name: total_equipment_produced_train - Name: total_equipment_produced_railway_gun - Name: total_equipment_produced_floating_harbor |
...
| Variable | Target | Description/Notes |
| --- | --- | --- |
| ai_attitude_allied_weight | Country. | The total amount of the `attitude_allied` AI attribute. |
| ai_attitude_friendly_weight | Country. | The total amount of the `attitude_friendly` AI attribute. |
| ai_attitude_hostile_weight | Country. | The total amount of the `attitude_hostile` AI attribute. |
| ai_attitude_is_threatened | None. | Returns 1 if the AI is threatened, 0 otherwise. |
| ai_attitude_neutral_weight | Country. | The total amount of the `attitude_neutral` AI attribute. |
| ai_attitude_outraged_weight | Country. | The total amount of the `attitude_outraged` AI attribute. |
| ai_attitude_protective_weight | Country. | The total amount of the `attitude_protective` AI attribute. |
| ai_attitude_threatened_weight | Country. | The total amount of the `attitude_threatened` AI attribute. |
...
| ai_strategy_activate_crypto | Country. | Total sum of the [AI strategy](<AI modding - Hearts of Iron 4 Wiki.md#AI_strategies>) values of the type `activate_crypto` towards the target country. |
| ai_strategy_alliance | Country. | Total sum of the [AI strategy](<AI modding - Hearts of Iron 4 Wiki.md#AI_strategies>) values of the type `alliance` towards the target country. |
| ai_strategy_antagonize | Country. | Total sum of the [AI strategy](<AI modding - Hearts of Iron 4 Wiki.md#AI_strategies>) values of the type `antagonize` towards the target country. |
| ai_strategy_befriend | Country. | Total sum of the [AI strategy](<AI modding - Hearts of Iron 4 Wiki.md#AI_strategies>) values of the type `befriend` towards the target country. |
| ai_strategy_conquer | Country. | Total sum of the [AI strategy](<AI modding - Hearts of Iron 4 Wiki.md#AI_strategies>) values of the type `conquer` towards the target country. |
| ai_strategy_consider_weak | Country. | Total sum of the [AI strategy](<AI modding - Hearts of Iron 4 Wiki.md#AI_strategies>) values of the type `consider_weak` towards the target country. |
| ai_strategy_contain | Country. | Total sum of the [AI strategy](<AI modding - Hearts of Iron 4 Wiki.md#AI_strategies>) values of the type `contain` towards the target country. |
| ai_strategy_declare_war | Country. | Total sum of the [AI strategy](<AI modding - Hearts of Iron 4 Wiki.md#AI_strategies>) values of the type `declare_war` towards the target country. |
| ai_strategy_decrypt_target | Country. | Total sum of the [AI strategy](<AI modding - Hearts of Iron 4 Wiki.md#AI_strategies>) values of the type `decrypt_target` towards the target country. |
| ai_strategy_dont_defend_ally_borders | Country. | Total sum of the [AI strategy](<AI modding - Hearts of Iron 4 Wiki.md#AI_strategies>) values of the type `dont_defend_ally_borders` towards the target country. |
| ai_strategy_force_defend_ally_borders | Country. | Total sum of the [AI strategy](<AI modding - Hearts of Iron 4 Wiki.md#AI_strategies>) values of the type `force_defend_ally_borders` towards the target country. |
| ai_strategy_ignore | Country. | Total sum of the [AI strategy](<AI modding - Hearts of Iron 4 Wiki.md#AI_strategies>) values of the type `ignore` towards the target country. |
| ai_strategy_ignore_claim | Country. | Total sum of the [AI strategy](<AI modding - Hearts of Iron 4 Wiki.md#AI_strategies>) values of the type `ignore_claim` towards the target country. |
| ai_strategy_influence | Country. | Total sum of the [AI strategy](<AI modding - Hearts of Iron 4 Wiki.md#AI_strategies>) values of the type `influence` towards the target country. |
| ai_strategy_invade | Country. | Total sum of the [AI strategy](<AI modding - Hearts of Iron 4 Wiki.md#AI_strategies>) values of the type `invade` towards the target country. |
| ai_strategy_occupation_policy | Country. | Total sum of the [AI strategy](<AI modding - Hearts of Iron 4 Wiki.md#AI_strategies>) values of the type `occupation_policy` towards the target country. |
| ai_strategy_prepare_for_war | Country. | Total sum of the [AI strategy](<AI modding - Hearts of Iron 4 Wiki.md#AI_strategies>) values of the type `prepare_for_war` towards the target country. |
| ai_strategy_protect | Country. | Total sum of the [AI strategy](<AI modding - Hearts of Iron 4 Wiki.md#AI_strategies>) values of the type `protect` towards the target country. |
| ai_strategy_send_volunteers_desire | Country. | Total sum of the [AI strategy](<AI modding - Hearts of Iron 4 Wiki.md#AI_strategies>) values of the type `send_volunteers_desire` towards the target country. |
| ai_strategy_support | Country. | Total sum of the [AI strategy](<AI modding - Hearts of Iron 4 Wiki.md#AI_strategies>) values of the type `support` towards the target country. |
...
| Variable | Target | Description/Notes |
| --- | --- | --- |
| id | None. | The internal ID of the state. While regular `set_variable = { my_state_pointer = 123 }` would still work as a pointer to the state, this can remove ambiguity within the code whether the number is used as a state ID or the number that refers to the state with the state ID of 123. **Not the same value as the state ID** defined with `id=123` in the file, typically returns some negative number instead. For fetching the state ID, a [meta effect](<Effects - Hearts of Iron 4 Wiki.md>) using [the GetID namespace](<Localisation - Hearts of Iron 4 Wiki.md>) can be used. |
| building_level | Building such as `@nuclear_reactor`. | The level of a building with the target type. |
...
| damaged_building_level | Building such as `@industrial_complex`. | Damaged building level of a building with target type. |
| non_damaged_building_level | Building such as `@arms_factory`. | Non-damaged building level of a building with target type. |
| modifier | Modifier's name, such as `@political_power_gain`. | The total value of the current [modifier](<Modifiers - Hearts of Iron 4 Wiki.md>) as the country has in total. Targeted modifiers have separate definitions. |
...
| days_left | State flag such as `:my_flag` | The current number of remaining days for a state flag. Must be targeted with a `:` rather than a `@` with the position of the target and the variable being swapped unlike in most other game variables, e.g. `my_flag:days_left`. The value of the variable cannot be directly passed into another variable; instead, a [meta effect](<Effects - Hearts of Iron 4 Wiki.md#Meta_effects>) must be used. |

Unit leader-scoped game variables:

| Variable | Target | Description/Notes |
| --- | --- | --- |
| leader_modifier | Modifier's name, such as `@navy_max_range`. | The total value of the specified [modifier](<Modifiers - Hearts of Iron 4 Wiki.md>) as the unit leader has in total, applied towards the leader. This is represented with `non_shared_modifier = { ... }` in [unit leader traits](<Character modding - Hearts of Iron 4 Wiki.md#Unit_leader_traits>). |
| unit_modifier | Modifier type, such as `@army_attack_factor` | The total value of the specified [modifier](<Modifiers - Hearts of Iron 4 Wiki.md>) as the unit leader has in total, applied towards the units. This is represented with `modifier = { ... }` in [unit leader traits](<Character modding - Hearts of Iron 4 Wiki.md#Unit_leader_traits>). |
...
| attack_level | None. | The attack level of the leader. `attack_skill_level` exists as a synonym. |
| coordination_level | None. | The coordination level of the leader. |
| defense_level | None. | The defense level of the leader. `defense_skill_level` exists as a synonym. |
| logistics_level | None. | The logistics level of the leader. `logistics_skill_level` exists as a synonym. |
| maneuvering_level | None. | The maneuvering level of the leader. |
| planning_level | None. | The planning level of the leader. `planning_skill_level` exists as a synonym. |
| skill_level | None. | The general skill level of the leader. `skill` exists as a synonym. |
...
| avg_units_acclimation | Acclimatization type, such as `@hot_climate` | Average unit acclimatization for the specified climate, which is defined in `/Hearts of Iron IV/common/acclimatation.txt` |
...
| research_bonus | None. | Research bonus of the MIO when assigned to a research slot. |
| size | None. | Size of the MIO. |
| funds | None. | Total MIO funds. |
| number_of_currently_assigned_tasks | None. | Amount of tasks that the MIO is assigned to. |
| max_task_capacity | None. | Maximum task capacity of the MIO. |
| number_of_unlocked_traits | None. | Unlocked traits in the MIO. |
| number_of_unused_trait_points | None. | Unused trait points in the MIO. |

### Game arrays <a id="Game_arrays"></a>
...
1. ↑ `NDefines.NGame.EVENT_TIMEOUT_DEFAULT = 13` in [Defines](<Defines - Hearts of Iron 4 Wiki.md>)
2. ↑ `NDefines.NAI.FOCUS_TREE_CONTINUE_FACTOR = 1.5` in [Defines](<Defines - Hearts of Iron 4 Wiki.md>)
3. ↑ `NDefines.NGame.END_DATE = "1949.1.1.1"` in [Defines](<Defines - Hearts of Iron 4 Wiki.md>)
4. ↑ `NDefines.NMilitary.UNIT_EXP_LEVELS = { 0.1, 0.3, 0.75, 0.9 }` in [Defines](<Defines - Hearts of Iron 4 Wiki.md>)
5. ↑ 5.0 5.1 `NDefines.NGame.MAX_EFFECT_ITERATION = 1000` in [Defines](<Defines - Hearts of Iron 4 Wiki.md>)
...
- **Documentation**: [Effects](<Effects - Hearts of Iron 4 Wiki.md>) • [Triggers](<Triggers - Hearts of Iron 4 Wiki.md>) • [Defines](<Defines - Hearts of Iron 4 Wiki.md>) • [Modifiers](<Modifiers - Hearts of Iron 4 Wiki.md>) • [List of modifiers](<List of modifiers - Hearts of Iron 4 Wiki.md>) • [Scopes](<Scopes - Hearts of Iron 4 Wiki.md>) • [Localisation](<Localisation - Hearts of Iron 4 Wiki.md>) • [On actions](<On actions - Hearts of Iron 4 Wiki.md>) • [Flags](<Data structures - Hearts of Iron 4 Wiki.md#Flags>) • [Event targets](<Data structures - Hearts of Iron 4 Wiki.md#Event_targets>) • [Country tag aliases](<Data structures - Hearts of Iron 4 Wiki.md#Country_tag_aliases>) • [Variables](<Data structures - Hearts of Iron 4 Wiki.md#Variables>) • [Arrays](<Data structures - Hearts of Iron 4 Wiki.md#Arrays>)
