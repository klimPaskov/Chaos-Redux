...
After the mandatory key and job-root gates, when no authoritative user-supplied image exists, search the Internet for a suitable unit-specific modern visual artwork before creating any model reference from scratch. Eligible source families include game concept art, game character or unit art, tabletop or miniature concept art, fantasy or horror illustration, and professional character design sheets. Do not select archival photographs, museum paintings or drawings, historical plates, antiquities, archaeological images, or documentary references as the source-first model reference; they may provide contextual research only. Prefer official artist, studio, publisher, portfolio, or asset pages and sources with explicit reusable licensing or recorded user authorization. Use the repository's normal web-search workflow and record the search scope, queries, date, candidate URLs, rights decisions, and exact rejection reasons in `refs/source/source_search.md`; reject unclear provenance, unclear modification rights, or terms that do not support the intended mod use.
