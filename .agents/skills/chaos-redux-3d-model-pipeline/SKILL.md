...
For every animated runtime consumer, require substantive skeletal animation for every requested role. On new non-firearm production jobs, allow one Meshy rig submission per model and one Meshy animation submission per required role on a usable rig. Retain included walking/running clips when they pass the same semantic and anatomy gates; do not pay to duplicate them. Inspect support before submission: an unsupported profile or role goes directly to Blender. After the first failed or unusable rig, manually rig and weight the accepted geometry in Blender; after the first failed or unusable action, manually author that role in Blender while retaining valid Meshy outputs. Explicit non-firearm existing-model repairs use Blender directly; firearm rebuilds take the weapon-free body route before direct Blender authoring. Missing, unverified, static, transform-only, whole-rig-only, or semantically aliased actions cannot pass.
