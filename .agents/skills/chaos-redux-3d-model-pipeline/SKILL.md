...
Choose the route before provider calls.
Firearm-bearing units, including existing firearm models, require a fresh Meshy 7 weapon-free body from exactly one weapon-free prepared input, then direct `gpt-6-astra` Blender rigging, weights, manual animation, and separately modeled/attached firearms and held props; no Meshy rig/animation attempt is required.
Existing non-firearm repairs proceed directly in Blender.
An explicit user request for Text to Motion may supply a standalone clip for Blender retargeting/refinement on an existing rig; it does not authorize replacing accepted geometry, rigs, or actions.
Other new models try Meshy rigging once and each required animation once on a usable supported rig, then switch failed, unusable, or unsupported stages to Blender.
This firearm regeneration route takes precedence over the general existing-model repair route.

Always create or restore every missing required model component, anatomy element, equipment piece, weapon, or held tool in Blender without renewed approval, preserving intended identity and source-informed design.
Record an inventory that maps each required element to its modeled geometry, material, rig/attachment, action usage, and export/reimport evidence.
Use verified bounded modeling and authoring operations; absent capability is a blocker, not permission to omit the element or replace it with a static/action alias.
The authorization covers required component completion and modest budget-preserving reduction, while other provider/source substitutions retain their separate approval rules.
Never store credentials in job files, skill files, manifests, prompts, logs, or handoffs.
...
- for every firearm-bearing unit, an explicit `fires_in_combat` or `non_firing` declaration. The Meshy input must be weapon-free, with source firearm identity archived for separate Blender modeling; assembled firing actions must pass grip, stock, muzzle, and recoil/contact gates, while a declared non-firing unit receives no firing effects
...
For firearm-bearing units, use the weapon-free body route: retain source firearm identity, proportions, materials, and equipment references as Blender reconstruction evidence, but remove all wielded weapons from the single prepared Meshy input. A source firearm need not already be held in a firing pose; Blender supplies the final grip and action. Preserve the intended body, clothing, armour, period, non-anime style, and unit identity while cleaning the pose and restoring source-obscured body areas. Declare `fires_in_combat` or `non_firing`; only firing consumers require discharge actions/effects.
...
Meshy receives exactly one approved, fully colored, full-subject image at `refs/original/meshy_input.png`, with genuine transparency or the required plain background. For firearm units this must be a weapon-free body image; source weapons and props remain reconstruction evidence for Blender, not provider inputs. Preserve intended identity and distinctive non-equipment traits while allowing authorized weapon removal, pose cleanup, and restoration. Record source URL, creator, terms, rights/AI-use status, source/prepared checksums, prompt, comparison, pose mode, and parent review. Never submit multi-view boards, comparison sheets, search-result pages, or extra equipment images; never overwrite immutable source evidence. A source-free fallback still needs separate approval after documented search failure. An unavailable required provider route remains a blocker.

The one submitted image must pass the hard non-anime style gate and preserve complete readable anatomy, intended source identity, and approved color and material cues.
For the firearm route, require zero firearms or wielded props in that image; authorized body-pose normalization and restoration do not need to preserve source weapon contacts.
Validate the complete trigger-hand, support-hand, stock/shoulder, and clear-muzzle relationship on the assembled Blender model and every firing action, rejecting unintended barrel-held, slung, fused, obscured, floating, or otherwise non-fireable contacts there.
If the applicable input or assembled-model gate cannot pass, mark the job `needs_user_review` or `blocked`.
A declared `non_firing` firearm-bearing unit must have no firing roles or effects.

For a sourced artwork with an unwanted opaque background, request genuine transparency in the ImageGen preparation and preserve the alpha channel in the prepared input. A clean plain background is acceptable when transparency is not required by the consumer. Preserve the source and prepared evidence with checksums, reject halos, matte pixels, clipped geometry cues, internal alpha holes, cast-shadow remnants, or design drift. A verified local background-removal or chroma-key process is a documented fallback only when the ImageGen preparation fails transparency. It must not replace the preparation or silently alter the design.
...
Preflight the one input for non-anime style, intended period and identity, full body coverage, limb separation, alpha/background quality, complete color and material cues, provenance/AI-use rights, and pose preparation. For the firearm route, verify zero firearms or wielded props in the actual Meshy input and a complete reconstruction inventory linked to source equipment evidence. Hand/weapon/stock and muzzle checks apply to the assembled Blender model and every firing action, not to the weapon-free input. Restore missing specified elements through the authorized source-informed preparation and Blender completion routes; do not hide omissions. Preserve immutable sources, prompts, comparisons, checksums, and parent review before generation.

### Portrait fidelity gate

For a portrait-inspired custom unit, compare the prepared candidate against the source portrait before any paid Meshy call and preserve the exact head or face identity, silhouette, proportions, clothing or anatomy, distinctive attachments, palette, and intended unit identity. Preserve the source pose unless the approved derivative records `pose_preparation_mode = t_pose` or `pose_preparation_mode = a_pose` without unintended identity drift; the firearm route intentionally removes source weapons for separate Blender reconstruction and may restore hidden body details. Do not accept a generic redesign that preserves only the unit category. Require explicit parent or user acceptance of the source-to-prepared comparison. Without it, mark the job `needs_user_review` or `blocked` and do not call Meshy. Record the source/prepared comparison, exact prompt, source and prepared checksums, pose mode and reason, approval or status, and revision lineage in the job manifest, with each revision identifying its predecessor. Reference review is separate from provider generation: after approval, exactly one approved provider image is sent to Meshy, while the source and comparison views remain evidence only.
...
Use Text to Motion for suitable required roles within the accepted provider plan, generating at most once per role and retaining usable clips for Blender retargeting/refinement on the accepted rig.
Ordinary existing-model repairs remain direct Blender work unless the user explicitly requests this provider route; firearm bodies still require manual Blender rigs, separately modeled weapons, and Blender-authored contact corrections.
A failed or unusable motion or retarget goes to `gpt-6-astra` Blender without another paid attempt unless the user explicitly authorizes one; changing endpoints does not reset an exhausted attempt allowance.

Verify the locked MCP compatibility route's live `tools/list` for creation, status, download, and library discovery before use; use only returned tool names and record an unavailable route as a blocker rather than calling REST directly.
The [Text to Motion API](https://docs.meshy.ai/en/api/text-to-motion) uses `POST /openapi/v1/text-to-motion` to create a standalone clip without a character model: prompt at most 400 characters, duration 2–10 seconds in 0.5-second increments, `prime` at 10 credits for FBX or `swift` at 3 credits for BVH.
Recheck current pricing and live balance before the authorized call; retain prompt, mode, duration, task receipt, actual credits, format, and checksum, and download immediately within the three-day asset window.

The [Animation API](https://docs.meshy.ai/en/api/animation) takes `rig_task_id` plus exactly one of preset `action_id` or generated `motion_task_id`.
Provider retargeting of generated motion requires a successful biped rig task and an unexpired motion task; an existing Blender rig does not supply `rig_task_id`.
Prefer local retargeting for accepted Blender rigs and do not create a provider rig merely to consume the clip.
Generated-motion retargeting may return only GLB; requesting FBX-dependent post-processing then fails, so omit it unless compatibility is verified and inspect actual returned formats.
Use the free [animation library](https://docs.meshy.ai/en/api/animation-library) `GET /openapi/v1/animations/library` to discover preset IDs and previews; a preset remains a library action, not bespoke motion.
...
Save stable checkpoint stages in `blender/checkpoints/` for source import, normalization, component modeling/repair, materials, rig/weights, props/attachments, actions, and pre-export. Preserve immutable source checkpoints, versions, hashes, and stage transitions. Creating or restoring missing required anatomy, equipment, weapons, tools, and other model components is standing authorized Blender work; complete those elements without renewed approval. Record source-informed design and added geometry separately, preserve the intended identity, and report an exact capability blocker if required modeling operations are unavailable.
...
Existing-body material rebinding preserves the inspected body atlas budget up to 2048 pixels; the default for new components remains 1024 pixels unless fresh installed references prove otherwise.
...
Resize packed RGBA material-data textures by splitting R, G, B, and A into independent scalar channels, resampling each channel, then merging them without color conversion or alpha premultiplication.
Alpha stores glossiness in these packed specular maps and normal data in packed normal maps, not opacity; ordinary RGBA color-image resizing can corrupt RGB as alpha varies.
Verify channel ranges and required constant channels after resizing and the DDS round-trip before accepting the material.
The current `io_pdx_mesh` importer displays packed specular alpha as Blender Roughness, which differs from the engine shader's glossiness interpretation.
Keep the engine-packed artifact correct and treat any corrected Blender preview as a separate interpretation; a plausible importer preview does not validate runtime channel packing.
...
For every firearm-bearing unit, generate a fresh weapon-free body in Meshy 7 from exactly one prepared weapon-free image, then directly use `gpt-6-astra` for Blender rigging, weights, manual skeletal animation, and separately modeled firearms and held props. Do not submit Meshy rig or animation attempts on this route. Existing firearm models must be regenerated through this body route rather than merely repairing their fused weapons. Model required equipment from recorded source identity, add weapon/prop bones or rigid parenting and constraints, and establish trigger-hand, support-hand, shoulder/stock, muzzle, recoil, and ground contacts. Final animation must preserve all required components. A declared `non_firing` unit still gets its equipment but no firing effects.
...
Blender's native split-normal encoding can introduce measurable angular drift; retain the default 0.25-degree guard.
A reviewed per-job tolerance up to 0.5 degrees is supported only with the exact source hash, per-corner before/after and reopened-checkpoint evidence, and final visual review; never replace those checks with a broad normal reset.
...
- selected source and source-informed model-ready preparation or documented failed-search fallback, preflight, non-anime style decision, source URL/page, title, creator/publisher, stated license/terms, retrieval date, provenance and AI-use decision, explicit user authorization, declared period-fit review and intentional exceptions, source color status, palette direction, colorization evidence when applicable, source checksum or identifying fingerprint, prepared checksum, preparation prompt, source-to-prepared comparison, `pose_preparation_mode` and reason, parent approval, and evidence that any archived source bytes are non-shipping
- for every firearm-bearing unit, firearm/body route selection, `fires_in_combat` or `non_firing`, weapon-free prepared-input proof, fresh Meshy body task/checksum, source weapon identity and Blender reconstruction evidence, complete prop inventory, attachment/rig/weight evidence, and assembled per-frame trigger-hand, support-hand, stock/shoulder, muzzle and recoil checks. Every firing state requires discharge timing, locator, synchronized effects/light, licensed weapon-matched sound and checksums; `non_firing` records zero firing-state/effect/sound requirements
...
- firearm-body route, general Blender fallback, or existing-model repair basis, complete required-component inventory and added-geometry/prop provenance, gpt-6-astra authoring evidence, exact operations/arguments, rig/weight/action checkpoint hashes, and before/after deformation and role-motion review
...
Archive source bytes only where terms permit as non-shipping evidence. When archival is forbidden, transiently hash the retrieved bytes when permitted, discard them after review, and retain an identifying fingerprint. Record the source URL/page, title, creator/publisher, stated license/terms, retrieval date, source checksum or identifying fingerprint, explicit authorization, prepared checksum, exact ImageGen preparation prompt, source-to-prepared comparison, and `pose_preparation_mode` with its reason. Meshy 7 must receive exactly one approved source-informed, fully colored image at `refs/original/meshy_input.png`. Sources and comparison sheets remain evidence only. Meshy 7 generates weapon-free bodies for firearm units, followed directly by gpt-6-astra Blender rigging, weights, manual animation, and separately modeled equipment, without Meshy rig/action attempts. Other new models retain the first-attempt Meshy policy; existing non-firearm repairs proceed directly in Blender. All missing required model components are authorized Blender completion work without renewed approval. Keep geometry-recovery rules, live credit preflight for provider calls, and recorded attempt/source lineage. Spawn it with `fork_turns="none"` through Codex `collaboration.spawn_agent`. Put every needed conversation constraint into the prompt or named repository files.
