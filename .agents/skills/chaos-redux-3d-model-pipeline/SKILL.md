...
When a user requests the newest Meshy model, inspect the live `meshy_image_to_3d` declaration before choosing `ai_model`. Prefer the declared `latest` alias only when the live route exposes it, and record the provider's resolved model in the task evidence. Never pass an assumed identifier such as `meshy-7`; if it is not declared by the live route, report that it is unavailable and retain the verified supported model. Tie credit preflight to the resolved live model, not to an assumed version number.
