...
- one ready reference image path and checksum, or enough brief input for the documented modern-designed-artwork source search and one model-ready ImageGen refinement; a source-free reference remains a separate explicit fallback only after documented search failure
- source mode (`licensed_search` or `reference_only_user_authorized`), immutable Internet source path or documented search record, source URL/page, title, creator/publisher, stated license/terms, retrieval date, provenance/rights and AI-use status, source checksum or identifying fingerprint, explicit user authorization where reference-only use is required, and refinement approval evidence
...
For a licensed or explicitly user-authorized source, use native ImageGen to create a substantially original, model-ready refinement informed by the selected concept traits. This is the normal sourced route, not a faithful-edit-only requirement or a from-scratch fallback. The refinement may clarify or complete model readability and adapt pose, composition, silhouette, background, or other production-facing presentation while preserving the explicitly selected concept traits and intended unit identity. It need not be pixel-faithful, but it must not become an unrelated unit redesign or lose required identity cues. Request genuine transparency when the consumer leaves unused canvas transparent and preserve the alpha channel. Keep the source and refinement separate, record the exact prompt, source-to-refinement comparison, refined checksum, processing record, and parent approval, and send only the approved refinement to Meshy.
...
Meshy receives exactly one approved clean final reference image. Never send a search-result page, comparison sheet, or multi-view board; only `refs/original/meshy_input.png` may be submitted. On the normal sourced route, that file must be the approved model-ready refinement informed by one eligible sourced artwork, with source URL/page, title, creator/publisher, stated license/terms, retrieval date, source checksum or identifying fingerprint, authorization where applicable, refinement prompt, source-to-refinement comparison, refined checksum, and approval. The refinement may be substantially original and production-adapted; it is not required to be a pixel-faithful edit. A source-free from-scratch fallback requires its own explicit approval after documented source-search failure. Sources and comparison sheets remain evidence only. If the required provider route is unavailable, mark the job as `required installation/verification` or `blocked` rather than substituting.

For a sourced artwork with an unwanted opaque background, request genuine transparency in the ImageGen refinement and preserve the alpha channel in the immutable final input. Preserve the source and refinement evidence with checksums, reject halos, matte pixels, clipped geometry cues, internal alpha holes, cast-shadow remnants, or loss of selected identity traits. A verified local background-removal or chroma-key process is a documented fallback only when the refinement fails transparency; it must not replace the refinement or silently alter the design.
...
Preflight the one input for silhouette, cropped parts, limb/component separation, dark gaps, strong shadows, alpha quality or background complexity, thin structures, painted details that may be mistaken for geometry, symmetry/asymmetry, unseen-side ambiguity, intended unit identity, and source rights. An approved refinement may improve resolution, clarify or complete anatomy and equipment readability, remove scenery, text, a display base, or extra figures, and adapt pose, composition, or silhouette for production while preserving the selected concept traits and intended unit identity. It may not become an unrelated redesign. Require explicit parent/user approval of the final image and source-to-refinement comparison before the Meshy call. Keep the source evidence, final input, refinement prompt, processing record, and all checksums separate.

### Portrait fidelity gate

For a portrait-inspired custom unit, compare the candidate refinement against the source portrait before any paid Meshy call and preserve the selected identity traits, distinctive attachments, intended unit identity, and readable silhouette; pose or composition may adapt for production. Do not accept a generic redesign that preserves only the unit category. Require explicit parent or user acceptance of the comparison; without it, mark the job `needs_user_review` or `blocked` and do not call Meshy. Record the source/refinement comparison, prompt, approval or status, and revision lineage in the job manifest, with each revision identifying its predecessor. Reference review is separate from provider generation: after approval, exactly one approved provider image is sent to Meshy, while the source and comparison views remain evidence only.
...
- selected source and model-ready refinement or documented failed-search fallback, preflight, source URL/page, title, creator/publisher, stated license/terms, retrieval date, provenance and AI-use decision, explicit user authorization, source checksum or identifying fingerprint, refined checksum, refinement prompt, source-to-refinement comparison, parent approval, and evidence that any archived source bytes are non-shipping
...
For a selected source, native ImageGen must create one substantially original, model-ready refinement informed by the selected concept traits. It may clarify or complete model readability and adapt pose, composition, silhouette, background, or production-facing presentation while preserving the explicitly selected traits and intended unit identity; it need not be a pixel-faithful edit. Require genuine transparency when the consumer leaves unused canvas transparent, preserve alpha, record the prompt, source-to-refinement comparison, refined checksum, and parent approval, and never use source pixels as runtime art.
