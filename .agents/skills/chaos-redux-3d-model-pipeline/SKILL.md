...
For a workflow-generated `meshy_input.png`, request a genuine transparent background in the initial ImageGen call and preserve the alpha channel in the immutable source. Do not generate an opaque or chroma backdrop for routine later removal. If native transparency fails validation, first use a targeted ImageGen edit-to-transparency; use a verified local background-removal process only as a documented fallback, or when an inherited, sourced, or user-provided reference has an unwanted opaque backdrop. Preserve both the untouched source and repaired derivative with checksums, reject halos, matte pixels, clipped geometry cues, internal alpha holes, or cast-shadow remnants, and mark the reference blocked if no safe verified fallback is available.
...
Preflight the one input for silhouette, cropped parts, limb/component separation, dark gaps, strong shadows, native-alpha quality or background complexity, thin structures, painted details that may be mistaken for geometry, symmetry/asymmetry, unseen-side ambiguity, and source rights. An approved derived image may clarify exposure, perform fallback background removal, or fix an accidental seam; it may not silently redesign the subject. Keep the original and both checksums.
...
When a humanoid job declares `humanoid_rig_route = blender_failure_recovery_humanoid_v1`, the batch keeps that unit's own Meshy 7 geometry and uses the allowlisted `author_humanoid_rig` and `author_humanoid_actions` operations. The route creates the HOI4-compatible 24-bone skeleton, bounded same-region vertex weights, and real idle, move, attack, and death actions on that unit's own geometry. Export the mesh and every action, reimport each output, and record the local route, source Meshy 7 task, checkpoints, checksums, and proof scenes. This is an explicit failure-driven rig recovery route, never a static animation substitute or a geometry-sharing shortcut.
...
For humanoids, provider rigging is a candidate only for a clear standard humanoid biped within the verified endpoint's constraints; inspect and map it in Blender. If the same provider task-ID rig input fails twice, do not repeat it unchanged; after the separately authorized signed-artifact recovery route also fails, stop paid provider-rig retries and use the explicit local `blender_failure_recovery_humanoid_v1` route when the job manifest selects it. That route creates the repository's 24-bone HOI4 skeleton and four required skeletal actions through the allowlisted adapter. For nonhumanoids, create a custom rig with a written rig map. For mechanical assets, use rigid components and deliberate pivots: turrets, barrels, recoil, propellers, rotors, doors, wheels, and tracks must not bend from blended weights.
