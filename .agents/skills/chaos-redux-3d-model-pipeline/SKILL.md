...
After the mandatory key and job-root gates, when no authoritative user-supplied image exists, search the Internet for suitable unit-specific designed artwork before creating any model reference from scratch. Build the candidate pool from game concept art, game character or unit art, game production or promotional art, tabletop or miniature concept art and renders, fantasy or horror illustration, and professional character or creature design sheets. Do not use archival photographs, museum works, historical paintings or drawings, historical plates, antiquities, archaeological images, ethnographic records, reenactment photography, or documentary imagery as model-reference candidates, shortlist entries, comparison images, or selected sources. Prefer official artist, game-studio, publisher, portfolio, or asset pages and sources with explicit reusable licensing or recorded user authorization. Use the repository's normal web-search workflow and record the search scope, queries, date, eligible artwork candidate URLs, rights decisions, and exact rejection reasons in `refs/source/source_search.md`. A copyrighted source may proceed as `reference_only_user_authorized` when the user explicitly directs the project to use actual game or artwork references; preserve that authorization and provenance, and still reject unclear provenance or any explicit `NoAI` or equivalent restriction.

Archive every selected source's original bytes unchanged into `refs/source/untouched.<ext>`, record its source page and direct URL, title, creator or publisher, rights or license terms, rights/reuse status, explicit AI-use restrictions, original dimensions/format, retrieval date, and SHA-256 in `refs/source/provenance.json`, and keep it beside every derivative. A parent/user-supplied image is an authoritative user-authorized source only when that authorization is explicit: record the authorization and never replace or overwrite the source with a search result or generated image. Reject a source carrying a `NoAI` or equivalent restriction. When rights are unclear and no explicit user authorization covers reference-only use, record the uncertainty and choose another eligible artwork source rather than silently converting it into generated replacement art.

For a licensed or explicitly user-authorized source, use native ImageGen in faithful edit mode to derive the model-ready single-subject input from the actual sourced artwork. Limit the edit to resolution recovery, subject isolation, genuine transparent background, scenery/display-base/irrelevant-text removal, alpha-edge repair, compression cleanup, and modest exposure/contrast/colour normalization. Preserve the exact subject identity, design, silhouette, pose, anatomy, clothing, armour, weapons, proportions, materials, palette, and distinctive details. Do not restyle, replace, complete, or substantially redesign the subject. If essential anatomy or equipment is cropped, obscured, missing, or unusable, reject that source and select a better artwork rather than inventing it. Keep the untouched source, faithful edited input, comparison sheets, prompt, processing record, and source-to-derivative checksums as separate evidence surfaces; only the approved faithful edit may proceed to the Meshy gate.

Only when the source-search record shows that a reasonable search for suitable unit-specific modern artwork found no acceptable source may a generated reference be proposed, and it remains forbidden unless the parent or user explicitly requests that specific fallback. A reference-only candidate, an excluded archival or historical image, or a generic category result is not by itself proof that the search failed. Ordinary model-production authorization does not authorize ImageGen reference creation. Without explicit fallback direction, continue source research or mark the reference `needs_user_review` or `blocked`.

## Exactly one Meshy reference image

Meshy receives exactly one approved clean final reference image. Never send a search-result page, comparison sheet, or multi-view board; only `refs/original/meshy_input.png` may be submitted. That file must be a faithful ImageGen edit of one archived eligible sourced artwork, with its provenance, edit prompt, processing record, source checksum, derivative checksum, visual-fidelity comparison, and approval. A from-scratch or redesigned reference is allowed only after the explicit fallback gate above. If the required provider route is unavailable, mark the job as `required installation/verification` or `blocked` rather than substituting.

For a sourced artwork with an unwanted opaque background, request genuine transparency in the faithful ImageGen edit and preserve the alpha channel in the immutable final input. Preserve both the untouched source and edited derivative with checksums, reject halos, matte pixels, clipped geometry cues, internal alpha holes, cast-shadow remnants, or design drift. A verified local background-removal or chroma-key process is a documented fallback when the faithful edit fails transparency while otherwise preserving the source exactly.
...
Preflight the one input for silhouette, cropped parts, limb/component separation, dark gaps, strong shadows, alpha quality or background complexity, thin structures, painted details that may be mistaken for geometry, symmetry/asymmetry, unseen-side ambiguity, and source rights. An approved faithful edit may improve resolution, clarify exposure, and remove scenery, text, a display base, or extra figures without changing the selected subject; it may not complete geometry, invent missing parts, replace equipment, or redesign the subject. Require explicit parent/user approval of the final image and source-to-derivative comparison before the Meshy call. Keep the untouched source, final input, edit prompt, processing record, and all checksums.
