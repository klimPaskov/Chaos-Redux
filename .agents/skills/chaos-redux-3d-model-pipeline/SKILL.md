...
After install and patch, require two consecutive `tools/list` probes and one concurrent probe pair through the locked wrapper, recording matching patched-file checksums or sizes across the consecutive probes to prove idempotence. Every response must expose `meshy_image_to_3d` with `meshy-7` as the accepted image model, followed by a live `meshy_check_balance` probe through the same route. After those probes, enumerate only processes belonging to the exact wrapper and provider entrypoint and require zero matches before any paid call; a match is a failed route-health gate. On Windows, attach every stdio wrapper process to a kill-on-close Job Object or equivalent process-tree owner and close it after the response or timeout because wrapper descendants can remain after JSON-RPC completes. A successful response without zero exact-route processes is not healthy.
...
- source mode (`user_authorized`, `licensed_search`, or `from_scratch_after_failed_search`), immutable source path or documented search record, provenance/rights status including `reference_only` when applicable, and parent/user approval state
...
## Designed-artwork source-first reference gate — read [the model reference source policy](references/source-reference-policy.md)

After the mandatory key and job-root gates, when no authoritative user-supplied image exists, search the Internet for suitable unit-specific designed artwork before creating any model reference from scratch. Build the candidate pool from game concept art, game character or unit art, game production or promotional art, tabletop or miniature concept art and renders, fantasy or horror illustration, and professional character or creature design sheets. Do not use archival photographs, museum works, historical paintings or drawings, historical plates, antiquities, archaeological images, ethnographic records, reenactment photography, or documentary imagery as model-reference candidates, shortlist entries, comparison images, or selected sources. Prefer official artist, game-studio, publisher, portfolio, or asset pages and sources with explicit reusable licensing or recorded user authorization. Use the repository's normal web-search workflow and record the search scope, queries, date, eligible artwork candidate URLs, rights decisions, and exact rejection reasons in `refs/source/source_search.md`; reject unclear provenance, unclear modification rights, or terms that do not support the intended mod use.

Archive every selected source's original bytes unchanged into `refs/source/untouched.<ext>`, record its source page and direct URL, title, creator or publisher, rights or license terms, rights/reuse status, original dimensions/format, retrieval date, and SHA-256 in `refs/source/provenance.json`, and keep it beside every derivative. A parent/user-supplied image is an authoritative user-authorized source only when that authorization is explicit: record the authorization and never replace or overwrite the source with a search result or generated image. For a copyrighted game or art source that lacks reusable rights, record `reference_only` in the provenance and manifest, and do not pass its pixels directly to ImageGen or Meshy; use only high-level, non-proprietary cues from it in a text-described adaptation prompt. For `reference_only` research, use native ImageGen from that text-described brief rather than an image-edit input, and subject the resulting sheet to the same originality, transparency, source-to-derivative, and approval checks.

For a licensed or explicitly user-authorized source, use native ImageGen in edit/adaptation mode to produce a substantially original, model-ready single-subject sheet with complete subject geometry, a usable pose, genuine transparency, and no scenery, text, base, or extra figures. Inform the sheet with the source's broad silhouette, pose, material/function cues, and readable unit role, but do not directly copy distinctive protected character designs, logos, symbols, exact costumes, proprietary details, or recognizable signature features. Keep the untouched source, adapted input, comparison sheets, prompt, and source-to-derivative checksums as separate evidence surfaces; only the approved adapted input may proceed to the Meshy gate.

Only when the source-search record shows that a reasonable search for suitable unit-specific modern artwork found no acceptable source may native ImageGen generate the reference from scratch. A reference-only candidate, an excluded archival or historical image, or a generic category result is not by itself proof that the search failed. Record the exact blocker, failed-search evidence, proposed fallback, and absence of source provenance; obtain explicit parent/user approval before that fallback generation and again before sending its final image to Meshy. Without that approval, mark the reference `needs_user_review` or `blocked` rather than substituting.

## Exactly one Meshy reference image

Meshy receives exactly one approved clean final reference image. Never send the untouched source, a search result, a comparison sheet, or a multi-view board; only `refs/original/meshy_input.png` may be submitted. A selected source must pass checksum and rights preflight and its ImageGen adaptation must be approved before the provider call. If no source exists, the approved from-scratch fallback must retain its prompt, source mode, search record, blocker, checksum, and approval note. If the required ImageGen or provider route is unavailable, mark the job as `required installation/verification` or `blocked` rather than substituting.

For any ImageGen-produced `meshy_input.png`, including source adaptation and an approved from-scratch fallback, request a genuine transparent background in the ImageGen call and preserve the alpha channel in the immutable final input. Do not generate an opaque or chroma backdrop for routine later removal. If native transparency fails validation, first use a targeted ImageGen edit-to-transparency; use a verified local background-removal process only as a documented fallback, or when an inherited, sourced, or user-provided reference has an unwanted opaque backdrop. Preserve both the untouched source and repaired derivative with checksums, reject halos, matte pixels, clipped geometry cues, internal alpha holes, or cast-shadow remnants, and mark the reference blocked if no safe verified fallback is available.
...
Preflight the one input for silhouette, cropped parts, limb/component separation, dark gaps, strong shadows, native-alpha quality or background complexity, thin structures, painted details that may be mistaken for geometry, symmetry/asymmetry, unseen-side ambiguity, and source rights. An approved adapted image may clarify exposure, remove scenery/text/base/extra figures, complete subject geometry, perform fallback background removal, or fix an accidental seam; it may not silently redesign the subject. Require explicit parent/user approval of the final image and source-to-derivative comparison before the Meshy call. Keep the untouched source, final input, and all checksums; source images and comparison sheets remain evidence only.
...
If a provider rig or required animation role fails, preserve the unit's own Meshy 7 geometry and retry only through the authorized Meshy rig/`meshy_animate` route or an explicitly user-approved professional source. A local adapter may prepare or validate the skeleton and process an approved source action, but it must never generate missing final motion. If no approved source survives recovery, mark the role `blocked`, retain the failure evidence, and seek approval rather than filling the gap with a simple local action.
...
For every skeletal 3D unit or entity package, primary motion for every required runtime action must come from a verified `meshy_animate` operation or another explicitly user-approved professional animation source. Record the source task or action id, semantic role, and approval in job evidence. Manually keyed or simple procedural Blender actions, whole-rig rotations or translations, transform-only motion, static-pose aliases, and semantic reuse of one action for another role are forbidden as final animation.
...
If Meshy lacks or repeatedly fails a required role, regenerate, re-rig, or re-animate through the authorized Meshy route, or obtain explicit approval for a professional source. Otherwise mark the role `blocked`; never fill it with a simple local action.
...
For humanoids, provider rigging is a candidate only for a clear standard humanoid biped within the verified endpoint's constraints; inspect and map it in Blender. If the same provider task-ID rig input fails twice, do not repeat it unchanged; retry the authorized Meshy rig and `meshy_animate` route or use an explicitly approved professional animation source, then stop and mark the role `blocked` if no approved source survives. Blender may prepare the skeleton and process the approved action, but may not author replacement motion. For nonhumanoids, create a custom rig with a written rig map. For mechanical assets, use rigid components and deliberate pivots: turrets, barrels, recoil, propellers, rotors, doors, wheels, and tracks must not bend from blended weights.

For an armed humanoid recovery, call `chaosx_blender_hoi4_isolate_humanoid_weapon` only with the explicitly reviewed component ids. It joins only those components, uses deterministic principal-axis alignment to place the original recovered weapon at the shoulder, and marks it for the dedicated `weapon` bone; inspect the resulting pose and require visible alignment with both hands and the shoulder before rigging or export. The armed route excludes weapon geometry from body bounds and body weights, builds the T-pose 24-bone body plus one weapon bone, and binds the recovered weapon rigidly to that bone. Every declared armed role, including `idle`, `move`, `laser_attack`, `defend`, `support_attack`, `retreat`, `training`, and `death` when required by the job, must use its own Meshy or explicitly approved professional source action; Blender may only process that source and may not author final armed motion. These recovery operations remain job-root-bounded and forbid arbitrary Python, shell, URL, or unrestricted path inputs.

When an accepted Meshy 7 armed humanoid is one fused character mesh whose hands and firearm already form a correct two-hand shoulder grip, do not cut the hands or rifle out merely to satisfy the isolated-weapon route. Preserve the accepted provider rig and process only approved source actions through import, retarget, cleanup, contact correction, validation, and export; never create final motion by keying a shared root, translating the held assembly, or rotating the whole rig. Require per-frame grip-distance and muzzle-axis proxy evidence, visible trigger-hand, foregrip, and shoulder-stock contact, genuine role motion, and a substantive articulated death source. Reject generic provider walk/run actions when they swing, sling, detach, or reorient the firearm, and block any missing role until an approved source is available.
...
- source reference or documented failed-search fallback, preflight, provenance, license/authorization, source URL, creator/publisher, rights/reuse status, source and adapted-input checksums
