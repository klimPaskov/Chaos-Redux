...
- one ready reference image path and checksum, or enough brief input to complete the documented source search and request an explicitly approved from-scratch fallback when necessary
- source mode (`licensed_search` or `reference_only_user_authorized`), immutable Internet source path or documented search record, source URL, creator/publisher, provenance/rights status, original-byte checksum, and parent/user authorization where reference-only use is required
...
Archive every selected source's original bytes unchanged into `refs/source/untouched.<ext>`, record its source page and direct URL, title, creator or publisher, rights or license terms, rights/reuse status, explicit AI-use restrictions, original dimensions/format, retrieval date, and SHA-256 in `refs/source/provenance.json`, and keep it beside every derivative. A copyrighted source may proceed only as `reference_only_user_authorized` when the user explicitly directs the project to use that actual game or artwork reference; preserve that authorization and provenance. Reject unclear provenance, unclear reuse terms, and any source carrying a `NoAI` or equivalent restriction. A non-Internet user file is not a substitute for this source gate.

For a licensed or explicitly user-authorized source, use native ImageGen in faithful edit mode to derive the model-ready single-subject input from the actual sourced artwork. Limit the edit to upscaling or resolution recovery, subject isolation, genuine transparent background, scenery/display-base/irrelevant-text removal, alpha-edge repair, and compression or exposure cleanup. Preserve the exact subject identity, design, silhouette, pose, anatomy, clothing, armour, weapons, proportions, materials, palette, and distinctive details. Do not restyle, replace, complete, create a substantially original replacement, or substantially redesign the subject. If essential anatomy or equipment is cropped, obscured, missing, or unusable, reject that source and select a better artwork rather than inventing it. Keep the untouched source, faithful edited input, comparison sheets, prompt, processing record, and source-to-derivative checksums as separate evidence surfaces; only the approved faithful edit may proceed to the Meshy gate.

Do not create a model reference from scratch, generatively redesign the selected source, or use a generic category result unless the parent or user explicitly approves that from-scratch fallback after the documented search across eligible artwork families finds no suitable source. Without that explicit fallback approval, if no eligible source survives provenance, rights, AI-use, or visual-fidelity review, mark the job `needs_user_review` or `blocked` and stop provider work. Ordinary model-production authorization does not waive this gate.

## Exactly one Meshy reference image

Meshy receives exactly one approved clean final reference image. Never send a search-result page, comparison sheet, or multi-view board; only `refs/original/meshy_input.png` may be submitted. On the normal sourced route, that file must be a faithful ImageGen edit of one archived eligible sourced artwork, with its provenance, edit prompt, processing record, source checksum, derivative checksum, visual-fidelity comparison, and approval. Only an explicitly approved from-scratch fallback after documented source-search failure may use a newly generated image, and its search failure and approval must be recorded rather than presented as a sourced faithful edit. If the required provider route is unavailable, mark the job as `required installation/verification` or `blocked` rather than substituting.
...
When Meshy cannot provide a suitable required motion, rig, or action family, or its capability is incompatible or incomplete, research compatible licensed professional animation packages and request explicit parent/user approval for acquisition and use before obtaining one; never authorize acquisition or purchase silently. For an approved package, record the package or source, license and permitted mod use, access method and cost, action-role coverage, skeleton/retarget compatibility, and required tooling. Preserve the immutable source archive and license receipt, retarget each approved action to the accepted rig, then export/reimport and validate every action independently. If no suitable approved source survives, mark the affected role or package `blocked`; never simplify.
...
If Meshy lacks or repeatedly fails a required role after the authorized recovery path, use the licensed professional-package path above only after explicit approval. If no approved source survives, mark the role `blocked`; never fill it with a simple local action.
...
Use `chaosx_3d_model_pipeline` only when the parent provides the exact job root, approved source and model-input paths with checksums, output folders, handoff path, profile, named vanilla references, scale relationship, required actions, custom-unit sound roles and Internet-source requirements, custom-unit counter consumers and tokens, inspected vanilla counter definition/DDS paths, matching skill-local counter family, counter-artist handoff path, synchronization requirements where applicable, source permissions, dependency lock, and forbidden simplifications.
The prompt must require the modern-artwork source-first gate: search unit-specific game concept, character or unit, tabletop or miniature, fantasy or horror, or professional design-sheet artwork, and select actual Internet-sourced modern designed artwork; reject archival, museum, historical, antiquities, archaeological, and documentary references as model sources; prefer official or explicitly reusable sources; allow a copyrighted source only as `reference_only_user_authorized` when the user explicitly authorizes that actual artwork; and reject any explicit `NoAI` or equivalent restriction.
For a selected source, native ImageGen must use faithful edit mode only to upscale or recover resolution, isolate the subject, create genuine transparency, remove scenery/display base/irrelevant text, repair alpha edges, and clean compression or exposure while preserving the exact subject identity, silhouette, pose, anatomy, clothing, armour, weapons, proportions, materials, palette, and distinctive details; it must not create a substantially original replacement or redesign.
If the documented search across eligible artwork families finds no suitable source, the prompt may permit a from-scratch reference only after explicit parent/user approval and must record both the search failure and approval; without that approval, mark the reference `needs_user_review` or `blocked`.
Archive the source URL, creator/publisher, rights/reuse status, source checksum, adapted-input checksum, and source image. Meshy 7 must receive exactly one approved cleaned final image at `refs/original/meshy_input.png`; sources and comparison sheets remain evidence only. Meshy 7 generates, rigs, and animates weapon-bearing characters without manual Blender attachment or replacement animation. Planned operations and failure-driven provider recovery both proceed without further credit confirmation while the live balance and provider capability permit them, with every attempt recorded. Spawn it with `fork_context=false`; put every needed conversation constraint into the prompt or named repository files.
