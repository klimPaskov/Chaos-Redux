...
Do not approve a black-and-white, grayscale, desaturated monochrome, clay-render, or untextured source directly as a Meshy input. Prefer a complete eligible color artwork. When the best eligible source is monochrome, use a dedicated native ImageGen faithful colorization pass as an explicit cleanup exception: preserve the exact design, silhouette, pose, anatomy, clothing, armour, weapons, equipment, proportions, material boundaries, framing, lighting, and value structure while applying the parent-approved period-, role-, species-, and material-appropriate palette. Do not let colorization redraw, restyle, reshape, re-pose, complete, invent, or simplify the subject. Reject a drifting colorization and select a better color source when faithful recovery fails. Record the colorization prompt, palette direction, source-to-color comparison, checksum, review, and approval.
...
Meshy receives exactly one approved clean, fully colored final reference image. Never send a black-and-white, grayscale, desaturated monochrome, clay-render, untextured source, search-result page, comparison sheet, or multi-view board; only `refs/original/meshy_input.png` may be submitted. On the normal sourced route, that file must be the approved faithful ImageGen cleanup of one eligible Internet-sourced artwork, with source URL/page, title, creator/publisher, stated license/terms, retrieval date, source checksum or identifying fingerprint, authorization where applicable, cleanup or colorization prompt, source-to-cleanup comparison, cleaned checksum, and approval. The cleaned image must preserve the source artwork's exact subject identity, pose, anatomy, equipment, proportions, materials, distinctive design, and palette unless the documented monochrome-source colorization exception applies. A source-free from-scratch fallback requires its own explicit approval after documented source-search failure. Sources and comparison sheets remain evidence only. If the required provider route is unavailable, mark the job as `required installation/verification` or `blocked` rather than substituting.
...
Preflight the one input for silhouette, cropped parts, limb/component separation, dark gaps, strong shadows, alpha quality or background complexity, thin structures, painted details that may be mistaken for geometry, symmetry/asymmetry, unseen-side ambiguity, intended unit identity, declared period fit, explicit anachronism exceptions, source rights, full-color status, material color separation, and palette approval. An approved cleanup may improve resolution, exposure, contrast, sharpness, alpha edges, and source-visible detail, and may remove scenery, text, a display base, or extra figures without changing the selected subject. It may apply only the documented faithful colorization exception to an approved monochrome source. It may not complete geometry, invent missing parts, replace or periodize equipment, alter pose, or redesign the subject. Require explicit parent/user approval of the final image and source-to-cleanup comparison before the Meshy call. Keep the source evidence, final input, cleanup or colorization prompt, processing record, and all checksums separate.
...
If a provider rig or required animation role fails, preserve the unit's own Meshy 7 geometry and continue automatically through the authorized Meshy rig/`meshy_animate` route. A local adapter may prepare or validate the skeleton and process the verified action, but it must never generate missing final motion. Check the live balance before every paid retry and append the operation, estimate, response, and checksum; do not ask for a recovery-spend confirmation. Continue until the role passes or the skeletal animation source gate below proves that Meshy is genuinely incapable of producing the required result.
...
For every skeletal 3D unit or entity package with an animated runtime consumer, animation is mandatory and Meshy is the required first animation provider. Run the verified Meshy rig route, retain its included walking and running actions when they pass the semantic and anatomy gates, and use verified `meshy_animate` operations for every other required runtime role. Every required role must have its own dedicated, substantive action from Meshy unless the documented fallback gate below has passed; no completion or runtime promotion is allowed with a missing, omitted, unverified, or aliased role. Record the rig task id, animation task id, library action id, semantic role, and approval in job evidence. Aliases or semantic reuse of one action for another role, manually keyed or simple procedural Blender actions, local replacements, whole-rig rotations or translations, transform-only motion, and static-pose substitutes are forbidden as final animation.

For animation production, Blender is limited to importing, retargeting, non-destructive cleanup, contact or root correction, scale normalization, baking, sound-event synchronization, validation, and PDX export or reimport. Cleanup must preserve the Meshy action's substantive motion and may not become a manual replacement; a static preview or transform-only companion remains evidence only and never becomes a runtime action.

Use the verified Meshy rig and animation route first for humanoid, creature, quadrupedal, multi-limbed, mechanical, and other articulated profiles. Treat every returned rig and action as a candidate: verify anatomy, joint placement, deformation, contacts, root behavior, semantic motion, and action completeness before acceptance.
...
Use an external animation package only after evidence proves that Meshy is genuinely incapable of producing the required rig or action: the live API lacks the necessary capability or distinct semantic action, or materially different authorized Meshy attempts repeatedly fail the anatomy or motion gates. The fallback package must be free to access, downloadable in a usable source format, clearly licensed for the intended mod use, and approved by the parent or user before acquisition. Record the Meshy failure evidence, source URL, license, checksum, action coverage, and retarget compatibility, then validate every fallback action through the same Blender export and reimport gates. If neither Meshy nor an approved free package can supply the role, mark it `blocked`; never buy an animation package, alias another role, procedurally replace the motion, omit it, or simplify it.
...
Provider rigging is a candidate for every animated profile within the verified endpoint's constraints; inspect and map it in Blender and reject anatomy-incompatible results. If the same provider task-ID rig input fails twice, do not repeat it unchanged; continue through a materially different authorized Meshy recovery attempt when the live route supports one, then apply the skeletal animation source gate if no Meshy result survives. Blender may prepare the skeleton and process the verified action, but may not author replacement motion. For nonhumanoids, require a written rig map and prove that the rig matches the intended anatomy; never accept a humanoid armature merely because the provider returned one. For mechanical assets, require suitable articulation and preserve rigid components and deliberate pivots: turrets, barrels, recoil, propellers, rotors, doors, wheels, and tracks must not bend from blended weights.
...
- selected source and faithful model-ready cleanup or documented failed-search fallback, preflight, source URL/page, title, creator/publisher, stated license/terms, retrieval date, provenance and AI-use decision, explicit user authorization, declared period-fit review and intentional exceptions, source color status, palette direction, colorization evidence when applicable, source checksum or identifying fingerprint, cleaned checksum, cleanup prompt, source-to-cleanup comparison, parent approval, and evidence that any archived source bytes are non-shipping
...
The prompt must reject black-and-white, grayscale, desaturated monochrome, clay-render, or untextured images as final Meshy inputs. Prefer a complete eligible color source. If the best eligible source is monochrome, require a dedicated native ImageGen faithful colorization that preserves exact design, pose, anatomy, equipment, proportions, material boundaries, framing, lighting, and value structure while applying a parent-approved period-, role-, species-, and material-appropriate palette; reject any colorization that redraws or simplifies the subject. Record source and final color status, palette direction, colorization prompt, source-to-color comparison, cleaned checksum, review, and parent approval.
