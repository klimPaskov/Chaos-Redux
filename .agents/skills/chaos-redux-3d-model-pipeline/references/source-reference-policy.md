...
Prefer official artist, game studio, publisher, portfolio, or asset pages. Record rights and provenance. If suitable copyrighted game or artwork lacks reusable rights, mark it `reference_only`, extract only high-level non-proprietary design cues in text, and create a substantially original ImageGen reference without passing the source pixels to generation.

Creating the model reference from scratch is allowed only after a documented search across the eligible artwork families finds no suitable source and the required approval is recorded.
