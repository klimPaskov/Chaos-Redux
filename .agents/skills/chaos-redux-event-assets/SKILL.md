...
Prepare each native `65x67` PNG with `.tools/create_advisor_icon.py`. Use the canonical `portraits/advisors/advisor_template.png` as one exact, untouched top layer rather than reconstructing its frame, paper, or shadows from separate elements. Load the portrait first, center-crop it to the requested aspect ratio without distortion, apply its fractional size, rotation, and opening-center offset through one uniform affine transform, then alpha-composite the template on top. When a task supplies exact fractional placement values, preserve them without integer rounding. Record the source crop, fractional dimensions, center, offset, rotation, sepia strength, template hash, processed PNG hash, and final DDS hash in the asset package.
