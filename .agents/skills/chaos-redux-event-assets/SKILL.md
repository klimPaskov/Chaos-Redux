...
For animated units, follow the one-attempt contract in `chaos-redux-3d-model-pipeline`: one Meshy rig attempt per new model and one animation attempt per required role on a usable rig, retaining valid outputs. A failed, unusable, or unsupported stage switches to manual Blender rigging, weights, and action authoring by `chaosx_3d_model_pipeline` running `gpt-6-astra`, without renewed approval or paid retries. Explicit existing-model repairs may use Blender directly. Both sources require genuine articulated motion, anatomy/deformation/contact checks, FPS/frame/loop/root review, and real `.anim` export/reimport proof. Static poses, transform-only or whole-rig substitutes, and semantic aliases remain forbidden. Other providers or professional sources require separate explicit approval; unavailable Blender operations remain blockers.

For armed units, the approved source must depict the complete weapon-bearing character and Meshy 7 must generate its geometry. After the first unusable rig/action, Blender may create or repair weapon bones, parenting, constraints, skin weights, and skeletal motion on that accepted geometry to preserve trigger-hand, secondary-hand/foregrip, and shoulder/stock contacts. Preserve immutable source geometry and passing actions. Missing weapon geometry still needs geometry recovery or an explicit modeling decision; Blender rig/animation authorization does not approve replacement geometry. Do not require a T-pose image for normal armed generation.
...
31. Every armed skeletal action identifies its passing Meshy source, preauthorized Blender fallback/repair with gpt-6-astra authoring evidence, or separately approved professional source, visibly aims, discharges, recoils/reacts, and recovers where applicable, preserves trigger-hand, secondary-hand/foregrip, and shoulder/stock contact, and has reimport evidence. Non-firing actions are not assigned particles or gunshot audio merely because the unit is armed.
32. Every runtime entity state that visibly fires or discharges independently identifies its exact frame/time, verified muzzle or weapon locator/node, matching particle/beam/muzzle effect, light where applicable, licensed weapon-identity-matched `soundeffect`, source/license/checksum, runtime consumer, and evidence/status. Attack, defend, support_attack, and other firing states cannot be silent or particleless, and the parent owns final entity/particle/sound-definition wiring and evidence review. The user performs live-consumer validation.
