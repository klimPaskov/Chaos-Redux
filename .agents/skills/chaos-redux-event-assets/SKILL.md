...
Use `.agents/skills/chaos-redux-event-assets/tools/create_advisor_icon.py` for native `65x67` advisor-template composition. Use the canonical `portraits/advisors/advisor_template.png` as one exact, untouched top layer rather than reconstructing its frame, paper, or shadows from separate elements. Load the complete source portrait without cropping or pre-warping it. Measure the opening's center, rotated width and height, and angle from the actual template; canonical cards must use that exact opening-fill plane, match the angle within `0.05` degrees, and use a `0 0` center offset. Resize the complete portrait uniformly to contain within the measured plane while preserving its source aspect ratio. Fill only the residual strip with a matte sampled from the source's upper corners and bottom-align the contained portrait so shoulders remain at the frame base. Never crop or stretch the portrait itself. Safety-clip the fitted composition only for irregular edge antialiasing and paper overlap, then composite the untouched template on top. Generate and retain a separate placement study for every person and inspect the face, head, and shoulders at native size and `4x`. The alignment overlay must show the measured opening in red, the fill plane in green, and the contained full portrait in yellow. The selected transform must match the retained study candidate. Near-zero rotation is rejected by default; use the explicit override only for an independently reviewed alternative template whose measured opening is actually unrotated. Retain transform and hash metadata for the source, template, measured opening geometry, selected placement, contained content dimensions, source-derived padding, crop/stretch booleans, study, alignment overlay, processed PNG, and runtime DDS. The reusable command contract and review checklist are documented under `tools/README.md` in the advisor and high-command dossier section.
