...
When supplied advisor layers contain baked shadows, keep the canonical source files unchanged but remove the baked shadow alpha from working copies before composition. Reconstruct a narrow antialiased core from an explicit alpha threshold, derive one consistent soft dark shadow from each cleaned core, and composite that generated shadow below its cleaned layer. Do not retain both the baked and generated shadows, and do not use a hard portrait-exclusion gap to hide a fringe.
