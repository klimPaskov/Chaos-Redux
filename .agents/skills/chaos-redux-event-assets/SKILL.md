...
Use `.agents/skills/chaos-redux-event-assets/tools/create_advisor_icon.py` for native `65x67` advisor-template composition. Use the canonical `portraits/advisors/advisor_template.png` as one exact, untouched top layer rather than reconstructing its frame, paper, or shadows from separate elements. Resize the complete source portrait without cropping to `65x67` first, then apply the selected portrait dimensions, opening-center offset, and visible rotation to that portrait layer, clip the transformed layer to the template's enclosed portrait opening so no source pixels remain outside it, and only then composite the untouched template on top. Generate and retain a separate placement study for every person, inspect the face, head, and shoulders at native size and `4x`, and never reuse one blanket transform across a batch merely because source dimensions match. The selected transform must match one candidate in the retained study. Near-zero rotation is rejected by default; use the explicit override only for an independently reviewed and documented exception. Retain transform and hash metadata for the source, template, selected placement, study, processed PNG, and runtime DDS, including the complete-source resize, dimensions, center, offset, rotation, and sepia strength. The reusable command contract and review checklist are documented under `tools/README.md` in the advisor and high-command dossier section.
