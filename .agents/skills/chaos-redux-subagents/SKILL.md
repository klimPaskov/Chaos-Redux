...
Its allowed scope is source/reference preservation, provider candidates, downloaded GLB/FBX and lineage, Blender source and checkpoints, bounded geometry/material/rig/weight work, and non-destructive processing of approved action sources through import, retarget, cleanup, contact or root correction, scale normalization, baking, sound synchronization, validation, export, and reimport. Every required skeletal runtime action must retain primary motion from verified Meshy `meshy_animate` or another explicitly user-approved professional animation source; manually keyed or simple procedural Blender actions, whole-rig rotations or translations, transform-only motion, static-pose aliases, and semantic reuse of one role for another are forbidden as final animation. Require genuine multi-frame attack or fire aim/discharge/recoil/recovery evidence where applicable and articulated death collapse/impact/settling evidence. If Meshy lacks or repeatedly fails a role, regenerate, re-rig, or re-animate through authorized provider work or use an explicitly approved professional source; otherwise mark the role blocked and do not route `author_humanoid_actions` or any local adapter operation as final motion. It must not create audio from scratch or use generated, synthesized, recorded, placeholder, or unlicensed audio, and it must not edit gameplay, GFX, `.gfx`, `.gui`, `.asset`, entity, sound definitions, localisation, events, focuses, decisions, country/history/AI, on-actions, or spreadsheets.
