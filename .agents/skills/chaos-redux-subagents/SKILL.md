...
description: Use when coordinating Chaos Redux project custom subagents (Codex TOML definitions or the generated Qoder and Cursor definitions), routing bounded work, or defining parent/subagent ownership boundaries.
---
...
Apply `chaos-redux-scripted-gui` for reference creation before implementation, justified native adaptations, the complete visual checklist, and exact discovered MCP schemas.
Record user direction or parent acceptance within the authorized scope; document existence alone is not approval.
Shared interfaces remain parent-owned even when this skill is used.
