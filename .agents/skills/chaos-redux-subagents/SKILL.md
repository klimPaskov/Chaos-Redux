...
Stage 2's opt-in `helper_expansion` mode for `hoi4.event_inspect` and `hoi4.tech_inspect`, persistent jobs, `hoi4.job_inspect`/`hoi4.job_cancel`, and negotiated native tasks were introduced in the 3.1.0 candidate. Before using them, verify the installed version and live schemas; request native tasks only when the client negotiates support, otherwise use ordinary calls. Keep opaque job IDs and helper continuation URIs intact. A completed job can still contain a tool-level error, and cancellation is a request whose final outcome must be inspected. For paged helper evidence, retain the source revision, selected roots, bounds, `finished`/`complete` status, and unresolved findings; resume only with the returned continuation under the same revision and authorized principal. Neither a job receipt nor a finished page proves full lifecycle or runtime behavior. The owning event, technology, focus, probability, GUI, or map workflow still supplies its required inspect, render, compare, and scenario evidence. Keep task-specific mechanics and fixtures in their profiles and handoffs, not this routing skill.
