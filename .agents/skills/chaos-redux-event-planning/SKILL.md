...
Plan firearm-bearing units, including current firearm models, as fresh Meshy 7 weapon-free bodies from exactly one prepared weapon-free input, followed directly by `gpt-6-astra` Blender rigging, weights, manual actions, and separately modeled/attached firearms and held props; no Meshy rig/action attempt is required. Archive source equipment identity and permit weapon removal, pose cleanup, and source-informed restoration of body detail. Always plan completion of every missing required component in Blender without renewed approval, with an element-to-geometry/material/attachment/action/export matrix and calibrated budgets that permit modest reduction to accommodate additions. Existing non-firearm repairs use Blender directly without a Meshy key. Other new models get one Meshy rig attempt and one action attempt per role on a usable rig, then Blender after the first failure, unusable result, or unsupported stage. Keep valid outputs, no paid rig/action retries, provider balance/provenance gates, verified bounded modeling support, semantic/deformation/contact checks, and export/reimport proof. Other provider/source substitutions retain separate approval.
...
Route production to `chaosx_3d_model_pipeline` on `sol` at high reasoning effort, or on `astra` only with explicit user permission for that run, using Codex `collaboration.spawn_agent` with `fork_turns="none"`. Supply job root, selected firearm-body/direct-repair/general route, input/reference status, source equipment identity, component inventory, profile, vanilla scale, actions, sound/effect synchronization, counter consumers, dependency lock, provider preflight where needed, calibrated budgets, and handoff path. Meshy 7 owns body generation; Blender owns required component completion and the route-specific rigs/actions. Require preserved source lineage, actual modeling/authoring routes, contact/deformation/semantic review, export/reimport proof, and no renewed approval for required components or authorized Blender work.

The coding-agent prompt must state that the main implementation agent owns `.asset`, entity, `.gfx`, unit/building/gameplay wiring, valid province and state placement, final source and MCP validation, and review of available in-game evidence. The user performs live runtime validation.
...
When `chaos-redux-improvement-loop` produces an expansion addendum, treat it as proposed event-planning input until its acceptance basis is recorded. Fold accepted design into the main spec pack with the same seriousness as the original user idea. Record the explicit user decision or parent acceptance within the user-authorized scope that supports each accepted claim. A spec path, status label, old handoff, or implementation evidence does not establish approval. Keep unsupported or conflicting claims unresolved and use the AGENTS.md Specs and Plans dispositions.
