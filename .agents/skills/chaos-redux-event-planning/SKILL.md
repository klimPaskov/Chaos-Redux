...
Plan one Meshy rig attempt per new model and one animation attempt per required role on a usable rig, retaining valid included locomotion and other passing outputs. The first failed, unusable, or unsupported stage switches to manual Blender rigging, weights, and articulated action authoring under `gpt-6-astra` without renewed approval or paid retries; polling an existing task is continuation. Explicit existing-model repairs use Blender directly and require no fresh Meshy attempt or Meshy key. Normal geometry generation/remesh/retexture/conversion and authorized geometry recovery retain live cost/balance preflight and immutable attempt lineage. Require verified Blender authoring operations, role/deformation/contact QA, and export/reimport proof. Missing Blender support is a blocker; other provider/source fallbacks require separate explicit approval.
...
Route production to `chaosx_3d_model_pipeline` running `gpt-6-astra` with Codex `collaboration.spawn_agent` using `fork_turns="none"`. Supply the exact job root, reference status, profile, vanilla references, scale, action list, sound roles, counter consumers and inspected vanilla paths, dependency lock, paid preflight, one-attempt Meshy rig/action limits, Blender fallback or existing-model repair mode, preserved source lineage, visual/deformation/export/reimport evidence requirements, and handoff path. Meshy 7 remains the geometry provider. The Blender fallback and authorized repairs need no renewed approval.
