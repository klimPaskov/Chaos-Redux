...
For a new or redesigned scripted GUI, use `chaos-redux-scripted-gui` to create reference image(s) before native implementation and carry the image-to-element mapping, supported HOI4 adaptations, acceptance basis, assets, states, and matched MCP scenarios into the implementation handoff.
A plan or image file does not establish approval; record explicit user direction or parent acceptance within the authorized scope.
