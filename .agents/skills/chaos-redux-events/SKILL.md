...
Use bounded `hoi4.event_inspect` pages with `mode = helper_expansion` when a large helper closure needs distinct source-linked call paths. Continue with the returned opaque URI until `finished`, and report `complete`, source revision, depth stops, cycles, dynamic or missing references, and source coverage separately. This page stream complements ordinary event inspection for direct references and aggregate diagnostics; it does not clear the focused-analysis lifecycle boundary or prove runtime outcomes. Follow the shared job/task handling in `chaos-redux-subagents`.
