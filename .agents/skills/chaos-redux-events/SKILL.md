...
When the event specifically adds a dedicated scripted GUI or mechanic window, route its bounded layout implementation and visual-quality pass to `chaosx_event_ui_worker`. The worker must use `hoi4.gui_inspect`, thorough `hoi4.gui_render` state/resolution/hierarchy/click-region views, an in-scope `hoi4.gui_rewrite`, and post-change comparison evidence, treat the production render as the one-to-one in-game visual view, fix every defect visible there, and follow the full layout contract in `chaos-redux-decisions-missions`. Never send the shared event log, event-details framework, settings, super-event framework, shared registries, or unrelated existing UIs to this worker. Eligibility requires accepted spec or source evidence that the named event introduces and owns the UI.
