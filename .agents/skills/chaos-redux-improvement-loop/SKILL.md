...
Route bounded production to `chaosx_3d_model_pipeline` running `gpt-6-astra` with Codex `collaboration.spawn_agent` using `fork_turns="none"`. Require Meshy 7 geometry, one Meshy rig attempt per model and one animation attempt per required role on a usable rig, then preauthorized manual Blender work after the first failed, unusable, or unsupported stage. Existing-model repairs may use Blender directly; retain valid source outputs and do not repeat paid rig/animation attempts. Keep actual adapter capability, deformation, role-motion, export/reimport evidence, Internet-sourced unit audio, and bespoke vanilla-green counter gates. Preserve parent ownership of `.asset`, entity, `.gfx`, gameplay wiring, and final review.
