...
| `sync_claude_agents.py` | Claude Code subagents, skill links, and MCP registration | `.claude/agents/*.md`, `.claude/skills/<name>`, `.claude/agent-map.md`, repo-root `.mcp.json` |

`sync_claude_agents.py` writes more than agent definitions because Claude Code has no configuration option that points at an external skills folder or a shared MCP file.
