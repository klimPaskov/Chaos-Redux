...
Surviving host alien formations are deleted without refund. The host laser stockpile transfers to DHR. One-time expedition stores are granted and consumed for an initial force of `max(5, min(15, marked_states + floor(arrivals / 2)))` locked cohorts. Every disconnected enclave receives a cohort before the remainder concentrates at the capital. Later uprisings join an existing DHR, and reinitialization after annexation cannot duplicate the country or its characters.
