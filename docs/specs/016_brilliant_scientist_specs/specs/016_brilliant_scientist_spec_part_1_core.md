...
The accepted country-specific institutional settlement addendum promotes one bounded four-country pilot inside the existing `chaosx.nr16.5` assistant-conflict event and the existing `.7` and `.8` reaction reports. Static source inspection confirms the named runtime identifiers and the bounded source implementation is complete in the working tree; targeted balance, transfer, cleanup, and live-acceptance evidence remain pending.
