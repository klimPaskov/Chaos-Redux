...
The accepted country-specific institutional settlement addendum promotes one bounded four-country pilot inside the existing `chaosx.nr16.5` assistant-conflict event and the existing `.7` and `.8` reaction reports. Static source inspection now confirms the named runtime identifiers, but source promotion does not claim complete implementation; the current disposition is accepted / implementation in progress pending targeted balance, transfer, cleanup, and live-acceptance evidence.

The existing generic `.5.a`, `.5.b`, and `.5.c` options remain available with their current triggers, effects, and AI weights. Four additional options are conditional on the current host tag and context: Britain (`ENG`) in public-science or distributed-research context, the United States (`USA`) in industrial-mobilization or distributed-research context, the Soviet Union (`SOV`) in strategic-security or industrial-mobilization context, and Japan (`JAP`) in strategic-security or industrial-mobilization context. Germany (`GER`), France (`FRA`), Italy (`ITA`), China (`CHI`), Poland (`POL`), Czechoslovakia (`CZE`), and every unnamed country retain the complete generic option pool and existing country presentation clause.

Every national option clears `brilliant_scientist_context_assistant_conflict_pending`, sets `brilliant_scientist_assistant_conflict_resolved`, calls one existing bounded base resolver plus its named settlement resolver, and calls `brilliant_scientist_try_schedule_impossible_lecture` exactly once. The four named settlement resolvers apply these fixed total vectors after the base resolver:
