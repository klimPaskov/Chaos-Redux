...
The accepted country-specific institutional settlement addendum promotes one bounded ten-country layer inside the existing `chaosx.nr16.5` assistant-conflict event and the existing `.7` and `.8` reaction reports. Static source inspection confirms the named runtime identifiers and the bounded source implementation is complete in the working tree; targeted balance, transfer, cleanup, and live-acceptance evidence remain pending.

The existing generic `.5.a`, `.5.b`, and `.5.c` options remain available with their current triggers, effects, and AI weights. Ten additional options are conditional on the current host tag and context: Britain (`ENG`) and France (`FRA`) in public-science or distributed-research context, the United States (`USA`) and Czechoslovakia (`CZE`) in industrial-mobilization or distributed-research context, the Soviet Union (`SOV`), Japan (`JAP`), Germany (`GER`), Italy (`ITA`), and China (`CHI`) in strategic-security or industrial-mobilization context, and Poland (`POL`) in public-science, distributed-research, or strategic-security context. Every unnamed country retains the complete generic option pool and existing country presentation clause.

Every national option clears `brilliant_scientist_context_assistant_conflict_pending`, sets `brilliant_scientist_assistant_conflict_resolved`, calls one existing bounded base resolver plus its named settlement resolver, and calls `brilliant_scientist_try_schedule_impossible_lecture` exactly once. The ten named settlement resolvers apply these fixed total vectors after the base resolver:
...
| German research board | `+10` | `+20` | `-10` | `+20` | `-15` | `+20` |
| French laboratories | `+10` | `-10` | `+15` | `+5` | `+25` | `-20` |
| Italian procurement compact | `+20` | `+10` | `+15` | `+15` | `+5` | `-5` |
| Chinese technical bureau | `+5` | `+25` | `-10` | `+20` | `-20` | `+15` |
| Polish university shelter | `+15` | `0` | `+10` | `+5` | `+15` | `0` |
| Czechoslovak research charter | `+5` | `-15` | `+15` | `+15` | `+20` | `-15` |

All numeric inputs belong in `common/script_constants/016_brilliant_scientist_country_settlement_constants.txt` under `brilliant_scientist_country_settlement_delta`, and resolvers use the existing bounded `brilliant_scientist_change_*` effects rather than direct unclamped arithmetic. The ten settlement receipts are host-local institutional history, are mutually exclusive because `.5` resolves once, and are not copied by ordinary transfer or Kruger State formation. Transfer before `.5` uses the recipient's current tag; transfer after `.5` preserves the former host receipt locally while the recipient receives only already-supported generic assistant state; unresolved `.7` or `.8` ownership continues to move under the existing reaction contract without inheriting the former country's national clause or AI momentum.

The finite layer does not add an event ID, fire path, delay, evolution, cluster entry, decision, focus, GUI, country, scientist, project reward, or asset package. The `.7` and `.8` player-facing three-way choices remain unchanged; settlement receipts add only the specified facility or custody description clause and bounded AI preference/caution factors. Existing `.5`, `.7`, and `.8` images are reused, and no model or visual substitute is part of this tranche.
