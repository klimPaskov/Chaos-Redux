...
Stage 0 is complete from the exact approved `portrait_generic_biowarfare_europe_male_01` base. Its runtime leader or scientist DDS and `65x67` advisor DDS are registered as `GFX_portrait_KRG_doctor_warren_kruger_stage_0` and `GFX_idea_doctor_warren_kruger_stage_0`. All fourteen later-stage leader or scientist portraits and their matching advisor cards are also present under their registered Stage I through IV sprite contracts. Every advisor card uses `.agents/skills/chaos-redux-event-assets/tools/create_advisor_icon.py`: the complete source portrait is resized to a native `65x67` intermediate, uniformly transformed without warping, and then covered once by the untouched canonical `advisor_template.png`. The standard cards use transformed size `33x46`, the clone cards use `36x50`, and the temporal cards use `39x54`; all use rotation `-6`, offset `-1 -1` from opening center `25 32.5`, and sepia strength `0.18`. Final gameplay state-selection review remains separate from asset presence and sprite registration. Other visual families retain their own production status elsewhere in this package.
