...
| 3D model status | Mixed: clone installed; Alien Infantry action tranche partially accepted; Portal Raider model rejected; remaining packages queued | The installed clone infantry model/entity package is reusable by generic consumers. Alien Infantry uses the Meshy R2 rig with Quaternius Universal Animation Library Standard CC0 `Pistol_Shoot`, idle, and walk actions exported and actual-byte reimported through the locked adapter; runtime entity promotion remains blocked by the missing supported muzzle locator and distinct defend, support-attack, retreat, and death actions, so particles, light, firing audio, and entity bindings remain unwired. Portal Raider counter art is complete and wired, but its runtime model/entity, actions, and sounds remain rejected and unwired after the failed semantic generation task and require its own recovery. The remaining route-specific model packages stay a future approval-dependent backlog under the current scope. No model, final GUI, or full Event 016 completion claim is made. |
...
| Visual asset status | Fifteen Kruger leader/scientist and advisor portrait states, six real severe-stage frame sheets, static fallbacks, Directorate UI, 100 focus icons, 51 achievement states, seven flag triplets, decision/category packages, twenty-five report images, six super-event images, seven Event 016 news images, and all 34 Event 016 idea/national-spirit DDS files are present and registered. The 21-icon KRG country-idea extension has complete source, processing, decode, and header evidence. Portal Raider counter art is complete and wired through `interface/portal_raider_system.gfx`; the clone infantry model/entity package is installed and reusable. Alien Infantry has the accepted Meshy R2 rig and Quaternius action exports/reimports documented, while its runtime entity, muzzle effects, firing audio, and remaining action roles remain blocked. The Portal Raider runtime model/entity, actions, and sounds remain rejected and unwired pending its own recovery. The country-tag clauses reuse existing report art and introduce no new asset requirement. |
