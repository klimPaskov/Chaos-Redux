...
| 3D model status | Mixed: clone and Alien Infantry static packages promoted; Portal Raider model rejected; remaining packages queued | The installed clone infantry model/entity package is reusable by generic consumers. Alien Infantry uses the accepted Meshy V13 model, 24-bone rig, seven genuine actions, actual-byte reimports, and promoted static entity/GFX/animation/sound registrations; supported muzzle/effect binding, strict audio-role provenance, positional playback, and live acceptance remain open. Portal Raider counter art is complete and wired, but its runtime model/entity, actions, and sounds remain rejected and unwired after the failed semantic generation task and require its own recovery. The remaining route-specific model packages stay a future backlog under the current scope. No full Event 016 completion claim is made. |
...
| Visual asset status | Fifteen Kruger leader/scientist and advisor portrait states, six real severe-stage frame sheets, static fallbacks, Directorate UI, 100 focus icons, 51 achievement states, seven flag triplets, decision/category packages, twenty-five report images, six super-event images, seven Event 016 news images, and all 34 Event 016 idea/national-spirit DDS files are present and registered. The 21-icon KRG country-idea extension has complete source, processing, decode, and header evidence. Portal Raider counter art is complete and wired through `interface/portal_raider_system.gfx`; the clone infantry model/entity package is installed and reusable. Alien Infantry has the accepted Meshy V13 model, seven action exports/reimports, and promoted static entity/GFX/animation/sound registrations, while its supported muzzle/effect locator, particle/light binding, strict audio roles, positional playback, and live acceptance remain open. The Portal Raider runtime model/entity, actions, and sounds remain rejected and unwired pending its own recovery. The country-tag clauses reuse existing report art and introduce no new asset requirement. |
