...
## Existing-mesh recovery supersession — 2026-08-27

The owner recovery report `docs/plans/016_brilliant_scientist_plans/subagent_handoffs/2026-08-27_existing_mesh_animation_recovery_owner_report.md` is the latest model status. It supersedes any earlier wording that called Clone Infantry or Autonomous Robot runtime packages complete: their firing-action, locator, and synchronized-effect gates remain blocked. It also supersedes any wording that treats Alien training or wounded aliases as valid; those aliases were removed from the runtime entity. Portal Raider, Paleogenetic Creature, Xenobiological Assault Organism, and Temporal Guard remain unwired until their provider-authored action, anatomy, counter, provenance, and parent-consumer gates pass.

## Binding reconciliation

> Core-runtime inventory reconciled through 2026-08-27. The exact stage-0 `156x210` leader or scientist DDS and `65x67` advisor DDS are produced and registered as `GFX_portrait_KRG_doctor_warren_kruger_stage_0` and `GFX_idea_doctor_warren_kruger_stage_0`. The fourteen Stage I through IV leader or scientist portraits and matching advisor cards are present under registered sprite contracts, and every advisor card uses the canonical advisor-template workflow. Six severe route animation sheets contain real frame-by-frame art and have registered static fallbacks. The Directorate UI, 100 focus icons, 51 achievement states, seven flag triplets, decision/category packages, twenty-five report images, six super-event images, seven news images, and six sound cues are present and wired. Eight breakthrough-family cards and eight incident/security cards are dynamically routed, with four aftermath-family cards and the shared remnant card retained for surviving-ledger hearings. The clone infantry model/entity package is installed and reusable, and Portal Raider counter art is complete and wired through `interface/portal_raider_system.gfx`; a Meshy 7 Portal Raider model, rig, nine real actions, audio provenance, and counter evidence are preserved, but its firing action failed every permitted automatic deformation route, so no runtime model/entity/actions/sounds are promoted. Alien Infantry has the accepted V13 model, seven actual-byte-reimported actions, and static entity/GFX/animation/sound registrations promoted for parent review, while its supported muzzle/effect locator, particle/light binding, strict audio roles, positional playback, and live acceptance remain open. These package statuses do not claim whole-event completion.
...
| Portal Raider | Counter art complete and wired through `interface/portal_raider_system.gfx`; Meshy 7 model, rig, nine provider-authored actions, audio provenance, and source records are preserved in the blocked package | Runtime model/entity, actions, and sounds remain rejected and unwired because the genuine firing action deformed the firearm-bearing pack under every permitted automatic retarget route; see `subagent_handoffs/portal_raider_meshy7_handoff_2026-08-27.md`. |
