...
| Alien Infantry | Reusable 2D unit, counter, equipment, technology, tactic, decision, event, focus, flag, portrait, and country-interface assets are installed and wired; sourced entity audio and reusable muzzle-particle/light definitions are installed but intentionally unbound | Independent V3 through V7 Meshy lineages retained the integrated weapon in neutral geometry, but every audited firearm action catastrophically deformed the arms, torso, and rifle. No accepted seven-action set, muzzle node, discharge timestamp, packed model, PDX export/reimport evidence, entity, or synchronized runtime binding exists. |
