...
Use `prompts/016_brilliant_scientist_super_event_prompt.md`. Six-package title, button, quotation, and audio research is complete. Six final Event 016-owned WAVs are ready at IDs `90` through `95`. Images, final descriptions, localisation, shared sound definitions, settings-aware playback, and live presentation wiring remain blocked until their dedicated handoffs and implementation exist.
