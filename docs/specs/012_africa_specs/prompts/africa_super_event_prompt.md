...
- `specs/012_africa_spec_part_5_high_chaos_world_order.md`
...
- `research/012_africa_historical_research_and_bibliography.md`
- `research/012_africa_host_country_research_addendum.md`
- `matrices/012_africa_asset_animation_matrix.csv`
- the repository super-event skill
...
1. **Africa is one**
   ...
   - Host variation: use the original host only where its capital, historical claim, or diplomatic legacy remains central to the completed order.
...
4. **The World**
   ...
   - Tone: finality, political exhaustion, and the irreversible end state.
   - Avoid: a generic planet explosion, an invented quote, or a victory image that ignores the cost of the continental wars.

## Text research

For each approved role, compare several candidates and document:

- title direction and researched final title recommendation
- selected main quote
- author or speaker
- source work, speech, scripture, document, or archive
- date or period
- exact source URL
- attribution confidence
- public-domain or copyright note
- selected button remark or short cultural allusion
- source and copyright-risk note for the remark
- backup quote and remark candidates
...
For each approved role, find a unique final musical track unless the user explicitly approves a named reuse.
...
- editing plan
- final 44.1 kHz OGG path
- suggested unique audio ID and sound wrapper
- why the pacing fits the role and its allowed route variants

Reject unclear licensing, commercial film or game music without permission, YouTube uploads without usable terms, generated test tones, drones, stingers, abstract noise beds, and oscillator output.

## Image handoff

Use generated super-event art for these alternate-history and terminal roles unless a final direction requires a real historical artifact. Coordinate image production through the generated event-art worker. Use the exact rows in the asset matrix and provide the final source, processed PNG, DDS, manifest, and sprite handoff.

Use a small, justified variant set. Prefer one strong base composition with route-state alternatives for genuinely different institutions over separate images for every host. Every image variant needs a named trigger role and a static final file.
