...
This disposition resolves the stale-plan bookkeeping problem only. It is not an Event 012 completion claim. Remaining known blockers include targeted scenario validation, live GUI/animation render proof, prompt-equivalent Continental Congress GUI/static/animated asset coverage beyond the wired charter seal, cohesion warning border, Green Covenant seal, and formable-ready emblem packages, stale plan cleanup, deeper host and created-country route consequences, AI/balance/exploit validation, and final proof that the World Is One terminal path only opens after all continental-unifier prerequisites. The static Continental Congress GUI wiring has been audited against its control names, visibility hooks, sprite registration, DDS dimensions, and localisation references; that audit does not replace live in-game render proof and does not prove the broader prompt-named GUI/animation asset package complete.
...
- The current triggerable-scenario tranche moves Africa to `SCN-008` with only `Africa Is One` and `World Is One` types. The launch gate allows the scenario even if Event 012 has already fired or another world-end branch is active. `Africa Is One` immediately forms a strong Africa, opens the Charter League, paper claims, regional authorities, Authority Atlas, sponsor, proof-ledger, and post-unification systems, and starts unification wars against remaining African-capital candidates and outside holders of African land. `World Is One` starts from that opening, seats Middle East/Asia/Europe/South Atlantic readiness, spawns supported scenario continent-unifier actors with intensity-scaled logistics, verifies the proof/certification markers, applies the Congress of Continents identity, starts terminal wars, and sets `world_end_africa_world_is_one` through the scenario terminal helper.
- The live Africa super-event audio package was refreshed on `2026-06-21`: slots `68-80` now use African-source live-performance recordings instead of the rejected Europe-heavy anthem/classical/military direction. The legal/source evidence and the source-diversity caveat are recorded under `docs/assets/012_africa/audio/2026-06-21_africa_direction_refresh/`, while the live `music/`, `sound/`, and canonical `docs/assets/012_africa/super_events/audio/final/` files were promoted from that package.
...
- Earlier targeted scenario-validation notes for Standard Unifier, RSA Civil War, Fragile Unifier, Ally Under Attack, Liberation League, High-Chaos Covenant, and Continental Pole are superseded for the manual triggerable scenario by the `SCN-008` two-type selector. The underlying RSA civil-war branch and other gameplay mechanics remain in Event 012 proper, but the shared scenario window now exposes only `Africa Is One` and `World Is One`.
...
- The Continental Congress scripted GUI static wiring audit closes the known missing-fallback-hook risk for the Charter banner and records that all six original visual-strip controls have matching scripted visibility hooks, sprite registrations, existing alpha-capable DDS assets, and localisation. The prompt-named `africa_charter_seal_animated`, `africa_cohesion_warning_border_animated`, `africa_green_covenant_seal_animated`, and `africa_formable_ready_emblem_animated` packages are wired with static fallbacks, sprite registration, scripted-GUI placement, visibility hooks, and tooltip localisation. The current visual strip remains useful and wired, but it is not accepted as full prompt-equivalent coverage for the asset prompt's background/header/meters/regional cards or broader state families unless later files prove that mapping. `docs/plans/012_africa_plans/2026-06-21_continental_congress_gui_animation_gap_handoff.md` remains current for the broader GUI queue, but is superseded for the prompt-named animation package rows. The remaining UI blockers are prompt-equivalent coverage beyond those wired packages and live render/animation proof in-game.
- The historical source-asset blocker is narrowed by the latest source-research pass: Benin, Sokoto, Kuba, Luba, Adal/Ifat, Ajuran, Zanzibar, and Bunyoro/Kabalega now have local raw and processed source files plus rights notes. The Bunyoro/Kabalega row uses a rights-clean 1874 *Ismailia* period plate as dossier/source evidence, with a colonial-engraving caveat for any future leader crop.
- The Event 012 workbook/catalog alignment tranche previously updated `docs/spreadsheets/chaos_redux_events_catalog.xlsx` Main Sheet row `13` for older route-variant wording. It needs a follow-up spreadsheet alignment pass for the SCN-008 two-type scenario wording and terminal scenario behavior.

These tranches reduce the active blocker list, but they do not close Event 012. Remaining known blockers still include live scenario validation for the static validation matrix, live GUI/animation render proof, deeper route-specific country-package consequences beyond the selected-unifier origin profile/case, regional-authority mandate/package cycle, current dossier slot families, and created-actor role packages, AI/balance/exploit validation, and live proof that the World Is One terminal path only opens after all continental-unifier prerequisites. The accepted Africa super-event package is blocker-free for the live slots: visible slots `68-79` and the root-terminal hybrid using shared slot `72` text/image plus dedicated audio id `80` are sourced, wired, documented, and file-reconciled. The high-chaos capstone parity gap for `BON`, `HYR`, `BIR`, and `SAO` is closed by their kinship-boundary, signal-omen, terracotta-citadel, and tag-specific capstone focuses.
...
- `docs/assets/012_africa/source_research/manifest.md` should be read as current for historical source confidence. The latest source pass leaves no documented-only row in that manifest, though several medium-confidence vector or attribution rows still require implementation judgment before direct final use.
...
- `docs/plans/012_africa_plans/subagent_handoffs/2026-06-21_012_africa_completion_audit_followup.md` remains current for the non-completion verdict and validation/depth blockers, but its Bunyoro/Kabalega source-blocker row is superseded by the latest source-research manifest entry and local *Ismailia* plate files.
