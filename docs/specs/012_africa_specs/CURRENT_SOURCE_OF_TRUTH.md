...
- The 2026-06-20 v5 icon packages are the current live icon source for Event 012 Africa. They supersede the earlier icon-regeneration pass with regenerated transparent focus/goal DDS files under `docs/assets/012_africa/icon_regen_goal_icons_no_white_bg_v5_2026_06_20/` and regenerated distinct idea/national-spirit DDS files under `docs/assets/012_africa/icon_regen_idea_icons_distinct_no_white_bg_v5_2026_06_20/`. The `.gfx` sprite names and live texture paths remain unchanged.
...
- The current regional-authority package tranche adds the first tag-specific post-mandate follow-up actions for WAC, SAH, and IOC. Each action requires a successful mandate, loyal subject status, capital control, and a one-time target flag; spends concrete non-PP resources through the custom-cost path; transfers equipment, manpower, convoys, infrastructure, dockyard capacity, or guard divisions to the authority; moves visible Africa values; fires local reports `chaosx.nr12.55` through `chaosx.nr12.57`; and exposes `africa_regional_package_action_count` against the configured goal in the Charter diplomacy header.
