...
- The Continental Congress scripted GUI static wiring audit closes the known missing-fallback-hook risk for the Charter banner and records that all six original visual-strip controls have matching scripted visibility hooks, sprite registrations, existing alpha-capable DDS assets, and localisation. The prompt-named `africa_charter_seal_animated` package is also wired with `GFX_africa_charter_seal` and `GFX_africa_charter_seal_animated`. The current visual strip remains useful and wired, but it is not accepted as full prompt-equivalent coverage for the asset prompt's background/header/meters/regional cards/warning border/Green Covenant seal/formable emblem or the remaining prompt-named animations unless later files prove that mapping. `docs/plans/012_africa_plans/2026-06-21_continental_congress_gui_animation_gap_handoff.md` is the current queue for those missing GUI/static/animated families. The remaining UI blockers are both prompt-equivalent coverage and live render/animation proof in-game.
...
- The Event 012 workbook/catalog alignment tranche updates `docs/spreadsheets/chaos_redux_events_catalog.xlsx` Main Sheet row `13`: `I13` now describes the late sponsor, Congress of Continents, dynamic union, external proof/certification, and prepared World Is One gate chain; `Y13` describes the SCN-012 route variants; and `AA13` records the scenario intensity behavior without claiming any intensity prepares proofs, certification, or the World Is One gate.
