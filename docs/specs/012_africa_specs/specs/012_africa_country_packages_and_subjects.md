...
Current implementation note: Event 012 registers 15 explicit Bestiary actor tags with direct fictional/supernatural/nonhuman public identities. The original actor tags are `GHP` Gorilla Highlands, `BBS` Baobab Senate, `TDM` Tidemark Dominion, `ANW` Ananse Web, `OVN` Orisha/Vodun Nature Courts, and `CRR` Crocodile Rivers; the expanded actor tags are `CTL` Chimpanzee Telegraph League, `OKP` Okapi Court, `TRM` Termite Citadel Engineers, `HGD` Honeyguide Commons, `GHC` Great Herds, `BON` Bonobo Kinship Congress, `HYR` Hyena Radio Dominion, `BIR` Bird of the Walls, and `SAO` Sao Terracotta Host. They have country/history files, ideology and party localisation, nonhuman/special classification, unique seat-state transfer hooks, setup-package effects, high-chaos focus-tree access, AI posture coverage, generated flag/portrait assets, actor-target decisions from the unifier decision layer, and local consequence events for actor decisions. The high-chaos companion tree includes tag-specific post-World-Witness capstones for all 15 Bestiary actors, including Bonobo gentle vetoes, Hyena night broadcasts, Bird verified wall warnings, and the Sao Terracotta Line. The four expanded `BON`/`HYR`/`BIR`/`SAO` actor action events also include post-action choice branches that set actor/root flags and move Event 012 values, so those packages are not just single-option receipts.

Current implementation note: `africa_generate_created_country_role_staff` gives every created regional authority and Bestiary actor two generated role advisors when its setup package applies, and `africa_generate_created_country_command_staff` adds one role-specific generated corps commander plus naval commanders for the nine actors with naval OOBs. The advisor and commander names are functional staff/court/body labels, not invented historical human personnel; nonhuman and supernatural actors receive explicit fictional/nonhuman staff wrappers. The roles use vanilla slots and traits: political advisors for organisers/builders/omen keepers, high command for route and forest logistics, army chiefs for muster/river/herd command, navy chiefs and naval commanders for maritime actors, theorists for survey/signal roles, and corps commanders for local defense or package-specific warfare. This covers the first staff and command surface, while broader bespoke minister rosters, deeper country-specific naval and air branches, and full bespoke focus trees remain future country-package depth.

Current implementation note: the selected unifier now receives one opening origin profile through `africa_apply_unifier_origin_package`. Registered hosts map to Highland Legacy (`ETH`), Atlantic Return Route (`LIB`), Union Rupture (`SAF`), Nile Sea Gate (`EGY`, `SUD`, `ERI`, `DJI`, `SOM`), Western Congress Ports (`WAC`, `GHA`, `MLI`, `SEN`, `NGA`, `GNA`, `VOL`, `DAH`, `IVO`, `SIE`, `GAM`, `TOG`, `NGR`), Congo River-Forest Mandate (`CBC`, `COG`, `ANG`, `CMR`, `EQG`, `GAB`), Indian Ocean Gate (`IOC`, `MAD`, `MZB`, `KEN`, `TZN`), or General Congress Mandate for other valid hosts. Each profile has a visible spirit, live Congress header label, value movement, logistics grant, cleanup path, and AI posture. A successful origin mandate case opens `chaosx.nr12.65`, which sets a route-specific settlement flag or a provisional-review flag and moves visible Event 012 values. This closes the first opening identity pass and selected-host follow-up for selected hosts, while deeper route-specific long-form branches remain future country-package depth.

Current implementation note: all ten regional authorities have tag-specific package actions after a successful mandate and matching companion-tree capstone. WAC can convene a port congress, SAH can open caravan columns, MAG can reopen harbor dockets, NHR can chart highland warrants, EAC can lock its railway timetable, GLK can muster lake guards, CBC can arm river quartermasters, ZSC can open stone-city yards, SLC can secure the mine-port belt, and IOC can secure sea lanes. These actions spend concrete resources, transfer material or forces to the target authority, move visible Event 012 values, record one-time target flags, increment `africa_regional_package_action_count`, fire visible consequence events `chaosx.nr12.55` through `chaosx.nr12.64`, and gate sponsor readiness plus terminal World Is One certification through the shared package-count trigger. They are a regional-authority depth layer, not a replacement for the shared companion focus tree or for deeper bespoke long-form branches where future country-package depth requires them.
