...
| Flag | Near-black or pure-black fictional flag. No text. It may use subtle cloth/void texture for readability but the map color remains pure black. |
| Starting state | Random eligible remote island state. |
| Capital | Origin state at creation. It can move to latest mainland foothold if the origin is occupied and engine needs a controlled capital. |
...
| Elections | None. |
| Diplomacy | No normal diplomacy before reveal. After reveal, automatic war with neighbors and public crisis behavior. World-threat behavior starts only after continent consumption. |
...
| `Public Death` | Mainland reveal | Enables withering and neighbor wars. World-threat source remains locked until continent consumption. | Replaced by the world-end Death idea at world-end. |
...
| `The Island Pattern` | Repeated maritime errata begin to rhyme across harbors while no public enemy is named. |
...
| `The Mainland Smell` | Mainland target weighting opens once enough island pressure exists. If a living country has declared war after Second Shore, it can record Mainland Hunger early and try the same mainland step. |
...
`Public Death` unlocks after the mainland reveal trigger. It should not be completed before reveal. It updates ideas, event logs, public pressure, and war behavior. It does not set world-threat state until continent consumption.
