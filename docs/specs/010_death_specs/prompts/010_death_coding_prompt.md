...
- No initial world notification. Use delayed marine-office packets months later without revealing Death.
...
- Every consumed state: set population to zero, add consumed population and civilian deaths when enabled, delete or neutralize industry, infrastructure, ports, airbases, rail/supply, and resources, transfer and core to Death, apply wasteland effects, and prevent resistance.
...
- After reveal, Death auto-declares war on neighbors, can wither undefended neighboring states, and can coastal-jump with cooldown if pushed back.
...
- Evolution flow: baseline consumes one small island and waits. Second Shore unlocks slow island spread through focus choices. Mainland Hunger unlocks faster island pressure and the mainland reveal focus path. Weak passive custom ghosts appear around the 600 tier. Stronger but still inferior custom ghosts appear around the 800 tier. Infantry-parity aggressive hosts appear only at world-end.
...
- Decisions/missions: Maritime Errata, Death Country, Living Containment Compact, Wasteland Outposts, World-End Emergency. Dark Methods and Black Oath must be fully implemented or explicitly queued/hidden, not half-visible.
...
- Triggerable scenario `SCN-006` with one Instant Outbreak type. It consumes a hidden origin, intensity-scaled extra islands, at least one mainland reveal state, and intensity-scaled ghost hosts. It must not set the Chaos Meter, bypass natural evolution records, or start world-end.
