# Super-Event Prompt — Event 010 Death

Research and prepare the super-event presentation package for Event 010 `Death`. Follow `chaos-redux-super-events`. Use `chaosx_super_event_text_researcher` for quote, title, button text, and cultural remark research. Use `chaosx_super_event_audio_researcher` for licensed or public-domain audio research and `.ogg` conversion. Coordinate images with the Event 010 asset prompt.

Do not invent titles, button text, cultural remarks, or quotes. Do not treat any planning label or role label as final player-facing text. Verify wording, attribution, source, and copyright or public-domain status where possible. Modern copyrighted references must be very short if used as button text. Every completed super-event needs unique final audio unless exact reuse is explicitly approved.

The functional role labels below are not final titles. They exist only so the researcher can separate the required packages.

## Required super-event roles
...
- Image: `super_event_death_reveal`.
- Audio: sparse grim music, 1-2 minutes, verified license.
...
- Image: `super_event_death_world_end`.
- Audio: unique finality track, licensed or public domain, not a drone or test tone.
...
- Image: `super_event_death_defeat_aftermath`.
- Audio: reflective, unique, licensed or public domain.
...
- Image: `super_event_death_world_consumed`.
- Audio: sparse final track, unique, licensed or public domain.

### Optional 5. Herald oath reveal

Only research and prepare if the Herald route is implemented.

- Trigger: a major country or player-led country publicly pledges to Zol.
- Role: hidden route reveal and betrayal.
- Tone: government euphemism and cultic surrender.
- Title research direction: find a short title about oath, names, surrender, or pledged service to Death.
- Button or cultural remark research direction: find a short sourced line, allusion, or historically grounded remark about signatures, names, bargains, betrayal, or state surrender.
- Quote research direction: find a real quote about bargains, names, betrayal, vows, or death.
- Image: `super_event_death_black_oath`.

## Research note output

Create or update `docs/super_events/010_death_super_event_research.md` with title candidates, quote candidates, cultural remark/button candidates, selected text package for each role, source links, attribution confidence, copyright notes, audio candidates, selected audio, license, duration, final `.ogg` path, suggested audio ids, image asset references, and implementation notes.
