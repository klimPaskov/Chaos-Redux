...
Follow `AGENTS.md`, `chaos-redux-events`, `chaos-redux-subagents`, `chaos-redux-event-assets`, `chaos-redux-super-events`, and the relevant offline HOI4 wiki/vanilla docs for implementation syntax.
...
- Stage IV: `+50 chaos`, `+100` world tension, severe hidden side effects and one-time non-terminal super-event.
- No world-end scenario. Never set `world_end` from Event 8.
...
- optional `Diplomatic Panic` cluster only if it can be fully registered and logged, otherwise document it as queued
...
- Add Stage IV super-event only as a first-time escalation, with full image/audio/quote/localisation research completed before claiming complete.
...
Use task-specific validation, not boilerplate claims. Verify stage gating, WT=100 behavior, pulse replacement/cap, timed opinion cooldowns, delayed report non-recursion, one-time super-event, event log/evolution details, and absence of world-end or direct war-goal behavior.

Before claiming completion, run a completion audit or equivalent review. Report files changed, systems touched, implemented vs queued pieces, asset status, spreadsheet status, validation evidence, and every simplification or blocker. Do not claim completion with missing localisation, unwired assets, placeholder audio, skipped achievements, unlogged evolutions, or unreported queued cluster work.
