...
The seven specification parts remain the accepted design authority, while the source-of-truth map and resume packet carry the current implementation ledger. Event 006 remains **HOLD / PARTIAL** at 32 content-attested packages, 29 compatible reservation groups, 40 runtime adapters, and 161 unattested selectable rows. The dated portrait-consumer evidence records 110 supplied PNG/DDS pairs, 70 Event 006 runtime DDS files, 38 exact supplied runtime matches, 64 unique Event 006 GFX portrait pairs, and 47 resolved character-registry references, with NAV Aguirre and GLC Castelao as the only current `styled_final` consumers and 13 supplied candidates intentionally unmapped; that 47-reference snapshot is superseded for current source routing by the 2026-08-29 portrait-consumer reconciliation and the current inspection of 25 guarded country blocks, 65 `recruit_character` calls, and 58 `portraits` blocks. The 2026-08-29 absent-country release-scope repair and the SCN-008 receipts `3ce4b3468` and `713306f2e` are source/static evidence; the latter closes success-only publication while fresh MCP/live-runtime evidence remains unavailable. The 2026-08-30 localisation and cost-display receipts `e2a1403ed` and `cbee847f8` leave the nine named Event 006 localisation files without indented key rows, while production render and exact visible-consumer review remain unavailable. Remaining admission, localisation, probability, live-event, GUI, focus, and super-event rights gates remain preserved in the plan ledger.

The current non-portrait asset authority is the 2026-08-29 audit plus the 2026-08-30 rights and path handoffs `../../plans/006_independence_wave_plans/subagent_handoffs/006_event6_asset_rights_research_2026-08-30.md` and `../../plans/006_independence_wave_plans/subagent_handoffs/006_event6_asset_docs_path_reconcile_2026-08-30.md`. They confirm the consolidated `interface/006_independence_wave_small_assets.gfx` receiver while leaving ASSET-004 grayscale, AEX collision, NWE alias, ASSET-046 emblem, BWX/chunk-3 source, animation-manifest, and super-event rights issues open; no GFX registry edit is claimed by this documentation reconciliation.
