...
## Current reconciliation snapshot (2026-08-29)

The seven specification parts remain the accepted design authority, while the source-of-truth map and resume packet carry the current implementation ledger. Event 006 remains **HOLD / PARTIAL** at 32 content-attested packages, 29 compatible reservation groups, 40 runtime adapters, and 161 unattested selectable rows. The current portrait evidence records 110 supplied PNG/DDS pairs, 70 Event 006 runtime DDS files, 38 exact supplied runtime matches, 64 unique Event 006 GFX portrait pairs, and 47 resolved character-registry references, with NAV Aguirre and GLC Castelao as the only current `styled_final` consumers and 13 supplied candidates intentionally unmapped. The 2026-08-29 absent-country release-scope repair is source-only evidence. The current localisation audit also reports 1,880 leading-whitespace keys across nine dedicated Event 006 localisation files and 140 numeric cost strings without dynamic constant tokens. Remaining admission, localisation, probability, live-event, GUI, focus, and super-event rights gates remain preserved in the plan ledger.

The current non-portrait asset audit is `../../plans/006_independence_wave_plans/subagent_handoffs/006_event6_assets_audit_current_2026-08-29.md`. It confirms the consolidated `interface/006_independence_wave_small_assets.gfx` receiver and records open ASSET-004 grayscale, AEX collision, NWE alias, ASSET-046 emblem, BWX/chunk-3 source, animation-manifest, and super-event rights issues. The asset manifests and GFX handoffs now point to the consolidated receiver; no GFX registry edit is claimed by this documentation reconciliation.
