...
The tag audit locks identifiers only. The thirteen registered-tag compatibility adapters and thirteen additive overlay hooks remain implementation blockers. A bounded IW-022 Dalmatia source adapter now observes the vanilla CRO-origin `dalmatia` dynamic carrier with visible route values, costed actions, a timed mission, lifecycle ideas, and D01-D50 identity-gated hooks; it creates no tag or history and is not a complete package admission. Its meaningful-tree, network, league, formable, save/load, and live-runtime evidence remain open, as do the eleven other exact route-overlay hooks. The `IW-153 POK` adapter must preserve registered history, characters, cores, `INS` releasable membership, and `indonesia_transfer_POK` behavior while retaining the named-community restriction. No source document may describe those adapters or overlays as implemented without gameplay evidence and route-preservation validation.
