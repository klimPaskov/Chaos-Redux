...
The tag audit locks identifiers only. The thirteen registered-tag compatibility adapters and thirteen additive overlay hooks remain implementation blockers. Bounded IW-022 Dalmatia and IW-025 Vojvodina source adapters now observe the vanilla CRO-origin `dalmatia` and HUN-origin `vojvodina` dynamic carriers with route-specific visible values, costed actions, timed missions, lifecycle ideas, and shared D01-D50 identity-gated hooks; they create no tags or history and are not complete package admissions. Their meaningful-tree, network, league, formable, symbol, save/load, and live-runtime evidence remain open, as do the ten other exact route-overlay hooks. The `IW-153 POK` adapter must preserve registered history, characters, cores, `INS` releasable membership, and `indonesia_transfer_POK` behavior while retaining the named-community restriction. No source document may describe those adapters or overlays as implemented without gameplay evidence and route-preservation validation.
