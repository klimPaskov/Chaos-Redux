...
The 2026-08-31 IW-095 security-cost repair aligns `iw095_organize_civic_guard` and `iw095_authorize_emergency_directorate` with the shared four-resource security payment text. Their existing one-factory project reservation remains active and is stated in each decision description, so the reservation is disclosed without the verbose two-line spend/commit cost row. The trigger, payment effect, duration, route, AI, admission, and absolute no-pre-event contract remain unchanged. The formable revolutionary/military seven-group commit palettes and other broader over-budget bundles remain open design work.
...
IW-051 YAK and IW-060 KUR have package-local source tranches but are not admitted packages. YAK uses the existing state-574/Yakutsk anchor and optional 644/876/877 states, with the source-backed `YAK_pavel_pevznyak` opening consumer behind an unset identity-clearance flag; its identity/origin, flag, host-remnant, typed-probability, and parent-admission gates remain open. KUR follows the package-local state-421 research anchor while installed state 1001 and vanilla capital state 800 remain an unresolved map/Form-18 policy; its identity, roster, flag, portrait, typed-probability, and whole-event MCP gates remain open. Neither package may enter central adapter, attestation, preflight, scenario, or Join surfaces without a complete independent admission packet.
