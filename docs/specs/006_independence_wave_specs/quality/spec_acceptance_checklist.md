...
| Automatic wave sizes are exactly 3, 4, 5, 7, and 10 | Part 1 exact wave ladder, `006_wave_tuning_model.csv` | Complete in source design | Tune only the supporting package strength, not these counts |
...
IW-022 has a bounded CRO-origin `dalmatia` source adapter, IW-025 has a bounded HUN-origin `vojvodina` source adapter, IW-035 has a bounded living-LIT `LIVONIA` adapter, and IW-059 has a bounded formed `neo_mesopotamia` adapter, but their meaningful-tree, network, league, formable, symbol, save/load, and live-runtime evidence are intentionally not counted toward either unchecked acceptance gate; eight other exact route-overlay hooks remain absent.
