...
The post-P0 audit supersedes the 2026-08-29 localisation audit's prior structural count: the nine named Event 006 localisation files now have zero indented key rows after the current source repair. The parent-owned 2026-08-29 wording pass removes the scoped player-facing `package` terminology from Scotland/Wales, Asante/Sokoto, Rhineland/Bavaria, Balkan, and rival-bloc tooltips while leaving internal keys and comments unchanged. The broader audit indicator still reports 95 cost-key rows containing numeric literals without a dynamic constant token, and production render plus exact visible-consumer review remain unavailable. Handoff: `../../../plans/006_independence_wave_plans/subagent_handoffs/006_event6_localisation_cleanup_2026-08-29.md`.
