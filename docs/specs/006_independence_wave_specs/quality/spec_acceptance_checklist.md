...
Current implementation authority after the IW-045 admission remains 32 content-attested selectable packages across 29 compatible reservation groups, 161 unattested selectable rows out of 193 non-overlay rows, and 40 runtime adapters. Eight adapter-only rows remain fail-closed: IW-013 NAV, IW-015 GLC, IW-043 CHU, IW-058 ASY, IW-093 DOX, IW-098 SOK, IW-177 FIJ, and IW-179 FSM. IW-040, IW-044, and IW-045 are inserted after IW-038 and before IW-033 in deterministic Join order; IW-045 passes the central content-attestation and normal/scenario preflight gates under its exact dormant BSK/state-651 contract. Commit `314642488` applies the coordinate-only focus closure, preserving 184 focuses and 195 connectors while fresh MCP evidence reports zero crossings, zero node intersections, zero long connectors, zero too-close same-row pairs, and zero Event 006 layout diagnostics; the only remaining focus diagnostic is the unrelated vanilla `continuous_restrict_freedom_desc` localisation reference. The bounded focus-overlay audit found no safe patch without changing accepted geometry or widening package admission. The startup registry is the committed current source at `history/general/006_independence_wave_character_recruitment_registry.txt` under commit `5895d0b69`, with 25 guarded country blocks and 54 `recruit_character` calls. FSM focus lifecycle hardening is committed under `35ab74212`, but FSM remains adapter-only and fail-closed. The latest portrait audit accepts NAV Aguirre and GLC Castelao as the only current `styled_final` Event 006 consumers with exact existing runtime matches; GLC Bóveda remains an unmapped supplied styled output. IW-095 has a package-local implementation tranche but remains centrally blocked, while the first-footprint addendum remains queued for central admission; typed probability evidence remains blocked by the recorded `Transport closed` auditor route. Super-event 23 remains rights-blocked with no approved replacement or runtime audio. The whole Event 006 disposition remains HOLD / PARTIAL.

The 2026-08-28 portrait-consumer revalidation supersedes the dated 2026-08-24 wiring note. It confirms 110 supplied PNG/DDS pairs, 70 Event 006 runtime DDS files, 38 exact supplied runtime matches, 64 unique Event 006 GFX portrait pairs, and 47 resolved character-registry portrait references, while accepting NAV Aguirre and GLC Castelao as the only current `styled_final` consumers and retaining GLC Bóveda as an unmapped supplied styled output. No additional portrait was wired or promoted because identity, role, rights, or package-admission gates remain unresolved; see `../../../plans/006_independence_wave_plans/subagent_handoffs/006_event6_portrait_consumer_audit_2026-08-28.md`. The 2026-08-24 research-table schema repair and static refresh cover fixed-width table parsing and the FORM-48, FORM-01/02/04, FORM-03, and IW-035 cost-localisation checks, but remain source/static evidence and do not close live event, GUI, tooltip, probability, balance, or whole-event gates.

The current 2026-08-29 localisation audit supersedes the 2026-08-27 static wording receipt for current structural status. It reports 1,880 leading-whitespace keys across nine dedicated Event 006 localisation files and 140 numeric cost strings without dynamic constant tokens, with no source repair claimed by this documentation pass.
