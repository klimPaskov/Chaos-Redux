...
- Intentionally shared resolved tags: **2** (`CHU` and `BIA`)
- Unintended duplicate resolved tags: **0**
...
- Every custom or reused resolved tag is a valid three-character uppercase tag. Overlay rows intentionally keep `resolved_tag` blank.
- Every custom Event 6 tag matches `[A-Z][A-Z]X`.
...
- Custom remaps are locked as `IW-087 HYX`, `IW-124 HZX`, `IW-161 IAX`, and `IW-162 IBX`; `IBX` replaces the engine-reserved `GFX` graphics namespace.
...
- The superseded all-row ledger records **149** bound and **57** unbound packages.
- Excluding 11 bound overlays and 2 unbound overlays leaves **138** bound and **55** unbound selectable country packages.
...
The machine-readable state result is under `../../../plans/006_independence_wave_plans/package_bindings/`. Its all-row totals remain historical map evidence, while the candidate registry controls selectable representation. The two cross-group findings remain explicit implementation decisions.
...
- implement and validate all thirteen registered-tag compatibility adapters
- implement and validate all thirteen additive vanilla route-overlay hooks
