...
> Current status, 2026-07-18: this directory remains the accepted design and provenance package. The current mechanic entry point is `docs/events/015_utopia_manifesto/overview.md`, current implementation proof is in `matrices/completion_coverage_matrix.md`, and closing state is in `docs/plans/015_utopia_manifesto_plans/015_utopia_manifesto_source_of_truth_and_resume_2026_07_15.md`. The fresh read-only whole-event audit passes in `docs/plans/015_utopia_manifesto_plans/completion_audit.md`, SHA-256 `5a90b637478872d6f960c7e67630e0efd0fda3e17869bad2c094473596a12183`, with 53-file runtime-text manifest SHA-256 `395873f4821fdf159cfb2f6edb9eecc0790a724f151cb0df62f89b969649d4b2`. References below to replacing World Tension Subsides describe historical catalog provenance, not the current event identity.
...
- `prompts/` contains historical asset, super-event, achievement, decision, coding, and goal prompts. They are reproducible provenance, not an active implementation queue.
- `prompts/subagents/` contains historical project-subagent prompts. Do not rerun them as finalization instructions. Use the current resume packet and dated final audit handoffs instead.
...
## Historical limits of the planning run

The following limitations describe the original unmounted planning environment only. They do not describe the current repository state and must not be used to reopen completed implementation tranches.
...
A manual improvement-loop and anti-bloat pass is included as provenance. The formal improvement addendum was implemented and promoted, the 2026-07-18 final improvement-loop closure says to stop broad expansion, and the fresh whole-event completion audit passes with no open completion finding.
