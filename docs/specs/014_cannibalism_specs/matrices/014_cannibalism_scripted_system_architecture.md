...
| mark cannibal world threat | global | active countries, cult nodes, territory, Hannibal | world threat source state | updates shared world threat | Evolution III refresh |
...
- global.cannibalism_hannibal_network_power
...
Use global event targets only for persistent world-threat or Hannibal-linked references that cannot be represented by flags or variables. If global event targets are used, every cleanup path must clear them.

Potential global targets:

- cannibal_primary_country
- cannibal_hannibal_actor, only after Hannibal exists
...
- Hannibal resonance if the future Hannibal event needs to read past exploitation
