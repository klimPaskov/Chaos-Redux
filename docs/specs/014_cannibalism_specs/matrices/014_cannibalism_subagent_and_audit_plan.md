...
| chaosx_improvement_loop_planner | mandatory near-completion depth and anti-bloat pass, plus optional earlier use after major implementation tranches that create new mechanics, countries, focus routes, assets, super-events, or design questions |
...
All project subagents must be spawned with `fork_context=false`. The parent prompt must pass Event 014, slug `cannibalism`, the relevant spec paths, user constraints, implementation status, unresolved handoffs, and the exact question the subagent must answer.

The near-completion `chaosx_improvement_loop_planner` pass is mandatory. If it returns an expansion addendum, the parent must resolve it before completion by implementing it, folding accepted design into the source spec, queuing it with a reason, or rejecting it with a reason. If it returns a closure handoff, record that closure before the final completion audit.
...
| chaosx_improvement_loop_planner | before the final completion audit and before any completion claim |
| chaosx_event_completion_auditor | after the mandatory improvement loop addendum or closure handoff is resolved |
