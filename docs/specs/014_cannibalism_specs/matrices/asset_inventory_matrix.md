...
| Regional warlord portraits | 56 | Internet-sourced modern fictional game/concept-art reference crops with 156x210 placeholder PNG/DDS derivatives; authorized HOI4-style finals remain pending | Eight origin-agnostic slots across Europe, Asia, Africa, Middle East, North America, South America, and Oceania; source hashes, crops, and runtime aliases are recorded in the portrait manifest | No prison settings, Hannibal likeness, or living ceremonial dress |
...
| Bespoke unit counters and equipment art | 9 counter triplets (27 DDS); no equipment art | Nine inactive custom subunits retain distinct gameplay identities and locked-template consumers | Gameplay/CXT registration and the 27 counter DDS/registry consumers are present. Feast Guard, Feast Cohort, Bone Guard, Siege Eaters, March Predation Column, Island Reavers, and Scavenger Warband have parent-installed mesh, eight action exports, material maps, entity/GFX bindings, and runtime sound packages. Bone Riders reuses vanilla `sprite = cavalry`, and Network Cadre reuses vanilla `sprite = infantry`; no custom model/action/entity/provider package is required for either. Scavenged Elephant Column uses installed vanilla `elephantry` and adds no custom elephant token | No additional reveal surface exists; all custom tokens remain Event 014-gated |
