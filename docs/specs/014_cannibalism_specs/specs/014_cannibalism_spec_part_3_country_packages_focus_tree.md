...
A cannibal country is a special chaos country. It begins as a human death-cult or military commune. It should become an actual nonhuman country only if Evolution III or hidden-leader mechanics create a clear monstrous transformation.
...
| Party names | Use route-specific cult, commune, military kitchen, or the hidden leader network names. Final names belong to localisation. |
| Leader | Generated fictional leader or council. If the hidden leader has been revealed and takes over, use the hidden leader package from the Event 014 hidden-leader track. |
| Leader portrait | Generated fictional portrait or council portrait with gore and ritual imagery. the hidden leader portrait comes from the hidden-leader track if implemented. |
...
| Diplomacy | Hostile to normal countries. Can form cannibal pacts with other cannibal actors. hidden-leader branch can coordinate globally. |
...
- the hidden leader-linked cadres after the reveal route is active
...
| Butcher packs | shock detachments | prison, exploit mutiny, the hidden leader | small, high attack, low staying power, high casualty and death-system side effects |
...
| the hidden leader cadres | elite special branch | the hidden leader only | rare and dangerous, should require the hidden leader power and global cult strength |
...
- receive hidden-leader cadres after the reveal route is active
...
| --- | --- | --- | --- | --- | --- |
| Commune of Hunger | start | defines the cannibal government and gives irregular war bonuses with severe diplomatic isolation | can become ordered cult under the hidden leader or devolve into packs | if discipline collapses, units splinter and raid randomly | the hidden leader order, island commune, broken packs |
...
| the hidden leader's Discipline | hidden-leader branch only | transforms loose cults into a strategic network | strengthened by the hidden leader victories | lost if the reveal is blocked or the leader is killed before terminal unification | world-end preparation |
...
Command hierarchy fork
  ...
  -> sealed-unifier discipline route, hidden until the hidden leader has been revealed
...
Military branch
  ...
  -> scavenger parties
  -> prison processions
  -> the hidden leader cadres if hidden route active
...
World threat branch
  ...
  -> the hidden leader takeover or leader council hardening
  -> Last Table formable path
  -> world-end preparation if chaos and the hidden leader power allow
```
...
sealed-unifier discipline route:

- hidden until the hidden leader has been revealed or is connected
- changes leader or overlord relation if the hidden-leader track owns the actor
- unlocks global courier, elite cadres, and world-end preparation
- should not be available if the hidden leader is dead, absent, or blocked by Event 014 route rules
...
- the hidden leader cadre branch if hidden route is active
...
- stronger the hidden leader takeover chance

Solitary rampage route:

- stronger individual war goal generation
- cannibal actors can fight each other
- lower the hidden leader coordination unless the hidden leader conquers them

AI should prefer pact if weaker or if the hidden leader has been revealed. AI can prefer solitary rampage if strong, isolated, and high chaos.
...
### the hidden leader dominion transformation

Working label, not final localisation: `the hidden leader Dominion`.

This transformation belongs to Event 014 and becomes available only after the hidden leader has been revealed through the network-strength gate.

Requirements direction:

- the hidden leader has been revealed and is active
- the hidden leader has access to a cannibal country or enough cult pressure in multiple countries
...
- the hidden leader takes over, inspires, puppets, or coordinates the cult depending on the hidden leader implementation
- global world-threat source becomes severe
- hidden hidden-leader branch opens in cannibal focus tree
...
- authoritarian terror focus can unlock exploitation path but raises hidden-leader resonance
