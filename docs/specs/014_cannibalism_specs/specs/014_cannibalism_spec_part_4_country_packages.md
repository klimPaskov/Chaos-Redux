...
### Elite contract ordering

The Warlord and unified Bone Guard action is one paid, state-aware elite contract rather than three free unit grants. After the corresponding focus or decision opens the route, the first successful contract fields Bone Riders and consumes the route's `*_bone_riders_open` flag. The second successful contract fields the Scavenged Elephant Column and consumes the route's `*_elephantry_open` flag. Later successful contracts field ordinary Bone Guards.

All three outcomes increment the shared Bone Guard raised counter and use the same Larder, exact Deaths-backed population, infantry-equipment, support-equipment, artillery-equipment, state-control, and commander-capacity gate. The base Warlord cap of two is therefore deliberately exhausted by the Rider and elephant contracts; ordinary Bone Guards require the cap-upgrade route. A successful transaction creates an empty locked formation and lets ordinary reinforcement draw from the held equipment reserve. Failed or partial population transactions leave Larder, the counter, sequence flags, and transaction flags unchanged. Warlord rollback and reusable-slot release clear the sequence and transaction flags, remove all locked Event 014 templates, and reset the Event 014 activation technologies without removing vanilla `elephantry`.
