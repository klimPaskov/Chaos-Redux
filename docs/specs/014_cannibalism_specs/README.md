...
- Unit gameplay and model consumers: Event 014 defines nine inactive custom irregular-infantry/cavalry sub-units with stable sprite tokens, real locked-template consumers, and an additive CXT registration bus. It adds no equipment archetype and reuses ordinary infantry, support, artillery, and motorized equipment. The locked Scavenged Elephant Column uses installed vanilla `elephantry` and its real infantry- and artillery-equipment need without adding a custom elephant token. The 27 counter DDS files and registry consumers are present. Five families have parent-installed model/entity/action/material/audio packages; four remain explicitly blocked or pending their provider gates.
...
- Catalog status: `Events!N15` and `Scenarios!F10` are `Fully Functional`.
