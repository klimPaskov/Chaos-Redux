...
The stale-receipt behavior above remains the accepted cleanup contract. The committed-row cleanup gate uses the generic country registry so same-generation ownership, control, category, or terrain drift can release an already-scheduled cleanup row. The generation-reset abort effect now provides the static route for pre-result stale cancellation and generation-drift cleanup before runtime arrays are rebuilt. Runtime execution and save recovery remain unproven. The exact engine-native all-valid-province thermonuclear sweep remains outside this chain and is still blocked. The manual Fallout survival contract remains a direct `90` to `95` percent prestrike population-loss band, with the seven-day rewrite reconciling only the remaining grade-specific delta.
