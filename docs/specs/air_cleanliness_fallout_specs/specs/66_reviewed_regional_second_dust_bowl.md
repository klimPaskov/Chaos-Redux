...
The current implementation review records repaired opening-target rehydration, scoped result-variable access, callback ledger coverage, AI state-pressure scope, dynamic-modifier localisation, branch-preparation grading, committed-row cleanup after same-generation target drift, and a generation-reset abort route that cancels stale opening and delayed rows before runtime arrays are rebuilt. Runtime engine acceptance and save recovery remain open implementation boundaries. Those findings do not alter this accepted source design.
...
History `9171` records the four opening choices, twelve result payloads, three callback payloads, and authenticated cancellation. The country is the primary actor and the authenticated plains state is the secondary actor. This Fallout chain has no workbook or catalog row.

Candidate `FALLOUT-656` is an internal identity and is absent from the workbook and exported catalogs. The source covers the four branch choices, twelve result outcomes, three callback outcomes, authenticated cancellation, and visible budget cost `3`. The wording pass removed internal state-variable references and matches the current player-facing terms.
...
The stale-receipt behavior above remains the accepted cleanup contract. The committed-row cleanup gate uses the generic country registry so same-generation ownership, control, category, or terrain drift can release an already-scheduled cleanup row. The generation-reset abort effect provides the static route for pre-result stale cancellation and generation-drift cleanup before runtime arrays are rebuilt. The exact engine-native all-valid-province thermonuclear sweep belongs to the completed consequence core. The manual Fallout survival contract remains a direct `90` to `95` percent prestrike population-loss band, with the seven-day rewrite reconciling only the remaining grade-specific delta.
