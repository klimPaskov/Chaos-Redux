...
The tranche remains dormant and outside release-floor credit until scheduler activation and live delayed delivery are proven. Static proof must cover the native infrastructure read and damage effect, the Air Winter highland receipt, Deaths-system request, exact `35` and `210` day offsets, branch affordability recheck, hidden-AI parity, Event Log history `9168`, dedicated asset hashes, workbook row `FALLOUT-635`, and generation-safe cleanup. The exact all-valid-province thermonuclear sweep, full-screen blackout host authority, save recovery, normal-map visual cold route, wasteland conversion, successor allocation, and focus packages remain separate blockers.
