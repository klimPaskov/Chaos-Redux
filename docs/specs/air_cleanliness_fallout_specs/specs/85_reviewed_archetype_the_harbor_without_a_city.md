...
The human opening, delayed result, and callback use `GFX_report_event_fallout_harbor_without_city`. The dedicated fictional report card is documented in `docs/assets/782_harbor_without_city/manifest.md` and registered in `interface/fallout_world_end.gfx`. Hidden AI lanes remain silent. No zombie asset, sprite, audio, path, or ordinary super-event surface is reused.
