...
The B7 follow-up now also supplies a dormant fragmented-footprint pilot for NZL. It accepts only five current candidate states whose AI owners each hold one state and whose controllers match their owners. It records those five sources, transfers the exact states to NZL, retires each emptied source as `retired_landless`, chooses state 284 as the deterministic capital candidate, and then reuses the existing NZL assignment and package producer. It does not create a tag, switch a player, or mark global allocation complete. The general allocator and missing-tag path remain blocked.

## Coordinator and idempotence

`fallout_successor_b7_run_vertical_slice` is the only B7 entry point. It is dormant and is not called by an on action, decision, ordinary event, scheduler activation flag, or public scenario. It first repairs the player reservation and conflict inventory prerequisites, begins the existing successor transaction once, applies the USA receipt, assigns the continuity row, probes fragmentation sources, and then attempts the fail-closed NZL existing-tag or exact fragmented-transfer path. A current generation marker makes repeated calls no-ops.
...
The next tranche must add a general allocator materializer for a missing or fragmented reviewed successor, prove player candidate assignment, and record package and tag-conflict cleanup for every changed owner. The NZL transfer pilot is intentionally narrower because it accepts only five one-state AI sources and does not materialize a tag. The global allocator must remain fail-closed until that path and the remaining successor matrix are complete.
