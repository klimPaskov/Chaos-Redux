...
The implemented major-arc reservation substrate records its top-level payload contract in `2026-07-26_major_arc_candidate_reservation_addendum.md`. Major rows may reserve only through the exact arc identity, parent-ticket, actor-shape, and current-scope proof, then enter the existing capped arc ledger at the opening stage. No authored stage consumer exists yet, so major rows skip ordinary dispatch and remain dormant and outside release-floor credit.
