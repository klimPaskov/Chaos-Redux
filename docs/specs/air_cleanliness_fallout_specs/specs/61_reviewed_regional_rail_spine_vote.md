...
The chain does not invoke thermonuclear strikes, rewrite the normal map, convert a state to wasteland, assign successor tags, change government, create a second world iterator, or enable the Fallout scheduler. The exact sweep, blackout, rewrite, successor allocation, and universal package belong to the completed core. Scheduler release receipts and delayed-delivery producers remain separate dormant-content obligations.

The implementation proof must record the constants, target trigger, branch costs, native railway reads and damage effects, Deaths request, delayed result and callback receipts, Event Log history `9166`, dedicated asset hashes, and the final dormant release-floor count. Candidate `FALLOUT-621` is an internal identity with no workbook or catalog row. No release-floor credit is granted while scheduler activation remains unset.
