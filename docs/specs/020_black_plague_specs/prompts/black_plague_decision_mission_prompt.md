...
> Historical accepted-design prompt, reconciled 2026-08-02: the current Crown Strike and Seal Royal Burrows surfaces are shared timed state actions, not native `activate_mission`/`days_mission_timeout` owners. Do not duplicate them until the parent decides whether the API simplification is accepted. Hold the Line and Secure the Refuge are already native `activate_mission`/`days_mission_timeout` missions. SCN-012 repeat signals are reconciliation-only and its live intensity postcondition verifies RTA/RTX floors. The current two-tag/no-model boundary, promoted weapon-delivery icon/source-frame Rat King-seal packages, and implemented `.57-.59`, `.64-.65`, and `.71-.75` surfaces are recorded in the consequence/aftermath addendum.
