...
- Dock `Tide Court` and `Cross-Sea Cargo` add dock Brood Mass and a capped division-cap bonus. The route remains gated by Evolution II. Its overseas exposure bonus applies only while the shared `black_plague_overseas_spread_enabled` flag is active.
