# Event 006 FORM-08: Danubian Confederation

FORM-08 is the researched Danubian Confederation family in the Event 006 regional formable registry. Its scope is the Danube corridor and the compact anchors in the accepted registry: Transylvania (TRA, state 84 with optional North Transylvania state 76), Banat (AXX, state 82), and Macedonia (MAC, state 106). The Vojvodina row remains the vanilla HUN-origin dynamic-country overlay and is not converted into a new Event 006 country.

## Identity and admission

The adapter reuses the already registered vanilla `HUN_EMPIRE` cosmetic identity and its complete vanilla flag package. No new country tag, cosmetic tag, flag, history file, or artwork is introduced by FORM-08. A global identity lock prevents two simultaneous Danubian formations from claiming the same existing identity.

The family is fail-closed until the shared registry has at least three active Event 006 members, three unique anchors, and the required consent rows. Every candidate must be an active sovereign Event 006 country with a selected FORM-08 family, a matching anchor variable, an owned and controlled capital anchor, and no active or Soviet-origin replacement state. The congress ledger freezes generation, invitation, family, sequence, anchor, and consent data before mutation.

## Formation and protected origins

The carrier receives `HUN_EMPIRE` only after the generic transaction lock and FORM-08 runtime proof pass. A consenting member with explicit full-integration authorization can transfer only its researched anchor states: TRA 84/76, AXX 82, or MAC 106. A consenting member without that authorization enters autonomous membership through directional military-access and guarantee relations. Its Event 006 origin, focus content, ideas, territory, and national identity remain intact. Cleanup removes only relations and ideas created by this generation.

The carrier keeps the best surviving member capital in deterministic order: state 84, then 82, then 106. The generic allocator and congress vote remain responsible for host survival, reserve-state protection, unique-anchor allocation, and collision rerolls.

## Visible post-formation system

After a successful commit the carrier tracks federal cohesion, arbitration capacity, transport authority, and minority settlement. The four values are shown live in the Danubian decision-category ledger, and all four participate in charter promotion: the coordinated charter requires every ledger to reach the coordinated threshold, while the integrated charter requires every ledger to reach the integrated threshold. Three material-costed decisions expose the public work loop:

* `Convene the River Congress` spends strategic and administrative reserves over a long congress window.
* `Arbitrate the Border Communities` spends strategic and security reserves over a public arbitration window.
* `Standardize Rail and River Authority` spends strategic and administrative reserves over a transport convention window.

The decisions alter Event 006 country values as well as the FORM-08 ledgers; they are not passive checklist missions or free unit rewards. FORM-08 also refuses to claim `HUN_EMPIRE` while the vanilla Austria-Hungary formation flag is set or any living country already carries that cosmetic identity.

## Registry reconciliation still required

The accepted registry names Transylvania, Banat, Vojvodina, Slavonia, and compatible members. The current audited adapter remains narrower (TRA, AXX, and MAC anchors) because the Vojvodina/Slavonia package and installed-map crosswalk are not yet attested. This tranche makes the existing four-ledger loop visible and meaningful without admitting an unresearched member or silently changing the accepted geography.

## Asset and localisation wiring

FORM-08 intentionally reuses the vanilla `HUN_EMPIRE` identity and flags. Its lifecycle ideas use existing Event 006 idea pictures. Localisation lives in `localisation/english/006_independence_wave_formable_registry_l_english.yml`. No custom advisor or leader portrait is added.

## Future work

The family remains runtime-inadmissible until Banat and Macedonia have complete researched Event 006 package adapters and their source, territory, symbols, leaders, and collision evidence. The adapter does not claim live-game, save/load, or whole-event AI validation.
