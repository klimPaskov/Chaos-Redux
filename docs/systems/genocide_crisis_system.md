...
The concentration-camp and extermination-camp buildings use distinct custom artwork on both building-icon surfaces. The Gulag labor camp network deliberately reuses the concentration-camp artwork. The camp family follows the vanilla HOI4 building language: compact ochre pictograms and the standard charcoal beveled tile on the indexed strip.
...
- `GFX_building_gulag_labor_camp_network` reads the same native `27x23` texture as `GFX_building_concentration_camp`.
- `GFX_buildings_strip` reads `gfx/interface/buildings/building_icon_strip.dds` as a `35`-frame strip of `46x46` frames.
- Strip frame `34` belongs to both `concentration_camp` and `gulag_labor_camp_network`; frame `35` belongs to `extermination_camp`.
