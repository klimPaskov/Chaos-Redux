...
All tuning introduced here is under `common/script_constants/016_brilliant_scientist_directorate_constants.txt`. Timing, political costs, equipment gates and spends, construction burdens, persistent modifiers, and AI weights are separated by category. The shared `brilliant_scientist_measure_delta` ladder remains the source for minor, moderate, and major meter changes.
...
- Establishing a secondary laboratory requires the recruited research cohort and a distinct owned, fully controlled core state with at least two infrastructure levels, an industrial complex or military factory, and either a home-area connection or a supply node. It adds exactly one infrastructure level, then saves that state as `event_target:brilliant_scientist_secondary_facility`.
- Relocating the primary laboratory is a state-targeted transfer plan, not an unconditional success effect. The destination must be a prepared owned and fully controlled core state with at least two infrastructure levels and existing industry. A land transfer requires a home-area connection; an overseas transfer requires a naval base and consumes convoys in addition to trucks and fuel. Completion records a persistent request context and starts the relocation cooldown.
