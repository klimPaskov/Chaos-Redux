...
The exact-state chemical air-raid adapter and selected-state nerve-suppression route consume real payloads, resolve the equipment-backed protection receipt, and pass their result through the shared consequence dispatcher. The shared calculator remains fail-closed for any route that cannot prove an exact target state, real payload debit, resolved protection, and verified conditions. Continuous ordinary-air missions are rejected because the installed engine exposes no verified eligible-activity hook; idle chemical-capable aircraft cannot enter the exposure path.
