...
When a Fallout request is admitted, the durable `fallout_air_cleanliness_disabled` flag pauses new Air Cleanliness work before host reconciliation and immediately clears already-applied state modifiers and country pressure ideas. A temporary request-ownership flag is removed if validation rejects the envelope, which restores the ordinary modifier surface. A locked consequence keeps the durable flag, pauses Air Winter and treaty updates, clears Air Winter state and country modifiers plus ordinary-map entities, clears the natural-source reservoir, rejects later contamination writes, and survives save migration. A pre-admission validation failure leaves the ordinary modifier surface intact. An admitted manual sweep failure keeps the durable Air shutdown so a partial consequence cannot silently resume Air Cleanliness.
