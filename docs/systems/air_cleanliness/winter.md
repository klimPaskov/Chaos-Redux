...
- The dormant manual scenario substrate has an exact 10,154-province manifest, 41 native batches, and seven-day ledger. Static source proof is complete; native every-valid-province acceptance and scenario release remain unobserved, so the public scenario is not live.
...
- The universal generic focus tree, survivor decision category, thirty-tag fracture consumer, and Year Ten route are implemented statically. Bespoke survivor country trees, additional country packages, and their dedicated leaders, units, diplomacy, AI layers, and assets remain outside this core tranche.
