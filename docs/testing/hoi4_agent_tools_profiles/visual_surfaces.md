...
| `chemical_warfare_folder` | `Screenshot 2026-09-20 172056.png` | All placements; 40 in the reference source | Check every placement before comparing the visible section, including the separate `mobile_cbrn_hospitals` gridbox when present, and connector routing. |
| `biowarfare_folder` | **No supplied in-game screenshot** | 12 nodes | Check source placement and render coverage only; in-game visual agreement is unmeasured. |
