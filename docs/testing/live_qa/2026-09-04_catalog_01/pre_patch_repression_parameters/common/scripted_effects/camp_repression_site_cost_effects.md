...
`camp_rework_prepare_site_cost_quote = { STATE = <state scope> }` is a country-scope scripted trigger declared in `../scripted_triggers/camp_repression_site_cost_triggers.txt`.
It performs deterministic temporary arithmetic and returns true for a valid supplied state scope.
The GUI passes `STATE = var:camp_selected_state_id`, decisions pass `STATE = FROM`, and execution passes the validated selected/action state.
...
camp_rework_prepare_site_cost_quote = { STATE = var:camp_selected_state_id }
check_variable = {
...
`camp_rework_can_pay_site_labor`, `camp_rework_can_pay_site_inspect`, `camp_rework_can_pay_site_dismantle`, `camp_rework_can_pay_site_evidence`, and `camp_rework_can_pay_site_restricted` take the same `STATE` parameter in country scope.
...
They take `STATE`, initialize retained temporary outputs, recompute the same quote, negate separate scratch amounts, and debit each quoted resource once.
