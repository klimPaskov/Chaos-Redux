...
Critical queue admission is independent from current theater capacity. Capacity remains mandatory in `event021_launch_critical_country`, so a valid Critical country stays queued while blocked and launches only after a later bounded pulse finds capacity.
...
`event021_parent_commit_scenario_country` reuses the immediate ordinary transaction for each selected eligible country.

The Maximum intensity caller owns the explicit all-eligible-country commit required by the scenario specification.
