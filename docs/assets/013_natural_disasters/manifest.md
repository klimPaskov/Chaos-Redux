...
| Super-event images | Present | `gfx/super_events/013_natural_disasters/` | Six slots are registered in the shared `interface/chaosx_super_events.gfx`; they are general super-event presentation and are not part of the direct Event 013 disaster GUI. |
...
The accepted Event 013 GUI uses frame-sheet animation with static fallbacks. The eight live pairs below are registered in `interface/013_natural_disasters.gfx` and reside under `gfx/interface/animated/013_natural_disasters/`:
...
The following five auxiliary marker pairs are retained in the source archive but are not registered or consumed by the live Event 013 GUI:
...
The auxiliary pairs remain available for a future, explicitly accepted UI extension and must not be treated as live wiring evidence.
