...
| `GFX_portrait_NZL_fallout_radio_service_coordinator_small` | no runtime DDS because frozen processor produced no candidate | 65x67 target | **BLOCKED / needs_user_review.** Authorized v14 source SHA `a1085e9c2839f740b6f34deab8f587ffc8333bf6f787507f299294d42014a342` added a real radio panel, tuning dials, ledger, cables, and desk edge while preserving v12 identity/placement, but frozen v5 still rejected it: closest `paper_mean` `198.848766` below band floor and `bottom_area_variation` `15.683616` above band ceiling, minimum margin `-0.020252`. No candidate PNG, review sheet, metadata, DDS, or sprite wiring exists. See `reviews/advisors/NZL_fallout_radio_service_coordinator_blocker.md` for exact handles and rejection counts. |

The storm candidate and dairy review/approval files are included in `source_masters/portraits/` and `reviews/advisors/`. `contact_sheets/portraits_contact_sheet.png` retains the approved portrait set beside the historical v7 radio source; `reviews/advisors/NZL_fallout_radio_service_coordinator_remediation_contact_sheet.png` retains the v10-v14 remediation alternatives for review only. No radio candidate passed the frozen gates, so the stable small-advisor sprite remains unwired. The storm DDS is present only after the separate `/root` approval record.
