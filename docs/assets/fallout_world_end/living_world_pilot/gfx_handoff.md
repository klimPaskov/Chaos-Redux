...
The Fallout event ID ledger reserves visible roots `chaosx.fallout.100`, `chaosx.fallout.107`, and `chaosx.fallout.114` for these three sprites. Final localisation keys remain pending. The main agent should bind each sprite inside its matching Fallout-owned event block in `events/fallout_world_end_events.txt`.
