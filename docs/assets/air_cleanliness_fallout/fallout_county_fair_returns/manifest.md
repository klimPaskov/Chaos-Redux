...
- Working event identity: `chaosx.fallout.572` / `FALLOUT-572` (collision-rescanned and owned by the County Fair Returns tranche).
...
- Visual constraints honored: cold but hopeful frost-browned rural recovery, repaired timber barns and sheet-metal pens, seed, preserved-food, and repaired-tool tables, guarded entry, blank community and remembrance boards, no people, flags, readable text, logos, zombies, corpses, radioactive creatures, modern props, amusement rides, or super-event composition.
...
- Target `.gfx`: `interface/fallout_world_end.gfx`, registered by the parent as `GFX_report_event_fallout_county_fair_returns`.
...
- Status: `wired` (source, processed preview, runtime DDS, prompt, hashes, handoff, `.gfx` registration, and collision-checked gameplay wiring are complete for this dormant tranche).
...
- Contact-sheet note: the retained sheet documents the alternate candidate. The selected people-free source is documented separately by its source and processed paths and hash pair.
...
| --- | --- | --- | --- |
| One dedicated static fictional Fallout report image for The County Fair Returns | `source_png/report_event_fallout_county_fair_returns_source.png` to `processed_png/report_event_fallout_county_fair_returns.png` | `gfx/event_pictures/fallout_world_end/report_event_fallout_county_fair_returns.dds` to `GFX_report_event_fallout_county_fair_returns` | Wired in `interface/fallout_world_end.gfx` |

The retained alternate is provenance and review evidence only. It does not replace the selected runtime source above.
