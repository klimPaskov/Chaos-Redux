...
Status: ready for main-agent sprite registration.

This package supplies one dedicated Air Winter response-category icon and seventeen dedicated decision icons. It does not edit `.gfx`, gameplay, localisation, specifications, or unrelated assets. The main agent owns registration and final in-context wiring review in `interface/air_cleanliness_winter.gfx`.
...
## Ready-copy sprite registration
...
| --- | --- |
| Response category | The shield, black winter sun, icy rim, and ash/snow base remain distinct at 52x40; silhouette is suitably wider and heavier than the decisions. |
| Reception | Open shelter and warm doorway read immediately; bedroll and crate reinforce reception without crowding the 32x32 mark. |
...
| Final evacuation | Loaded period truck departs frozen ruins; its direction and luggage load survive reduction. |
...
- `_tooling/process_air_winter_decision_icons.py` requires the exact eighteen expected assets and rejects missing or extra source, master, processed, decoded, or DDS files.
- The category is 52x40; all seventeen decisions are 32x32. Every DDS contains one image level in uncompressed 32-bit BGRA with masks `00FF0000`, `0000FF00`, `000000FF`, and `FF000000`.
...
- Raw sources, transparent masters, processed PNGs, and runtime DDS files are each unique across all eighteen concepts. The full decoded contact sheet SHA-256 is `61866d2617af2aace18af829736d90c6f67434bdf16bd2975b92105cca2e7349`.
