...
Composition/framing: wide horizontal report composition, archivist on the right third, ledger wall occupying the left and center, readable silhouette and strong focal light. Leave no empty modern UI-like space. Use a slightly rotated paper photograph presentation like vanilla HOI4 report art, with a clean transparent outside border if possible.
...
Constraints: fictional scene. No real identifiable person, readable invented text, flags, military insignia, gore, bodies, weapons, or modern props. Preserve the memorial and census focus and match the vanilla report-art documentary print aesthetic.
...
Processing note: the generated RGB source was passed through `.agents/skills/chaos-redux-event-assets/tools/process_report_event_image.py` with the report-card crop, tilt, shadow, sepia tone, and grain settings. The resulting 210x176 RGBA preview has transparent corners and a restrained paper card. It was converted with `.agents/skills/chaos-redux-event-assets/tools/convert_to_dds.py` to the runtime DDS path. The selected preview and DDS hashes are recorded in `manifest.json`.
