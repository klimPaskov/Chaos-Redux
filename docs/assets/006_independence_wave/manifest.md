...
- Scope: ASSET-007 through ASSET-038, ASSET-049 Post-Release Instability,
  the researched FORM-01 through FORM-04 child packages of
...
ASSET-049 is the eighth idea-family row required by IDEA-06 in the accepted idea
lifecycle matrix. It remains distinct from the seven earlier lifecycle families.
...
`FORM-05` and `FORM-48` now have separate researched flag/emblem packages and
stable runtime consumers; their package manifests below are authoritative for
those families. The remaining `FORM-06` through `FORM-47` identities and the
per-family UI emblem set remain blocked until each family has a final gameplay
tag, public identity, researched motif, approved palette, and stable UI consumer.
Creating generic emblems would be a prohibited substitute.
...
scripted-GUI `independence_wave_status_toggle_animation_click` action swaps the
live strip for the authored animated sibling without changing gameplay values.
...
DDS files under the existing sprites. IW-017 passed the fresh post-wiring
country-package audit and is exact compile-time attested; portrait conversion
alone remains insufficient evidence for any other package.
Prempeh II's
...
`portraits_generated_png/`. It contains exactly 56 original-size RGB source-locked
...
commander and Pietro Pinna Parpaglia crown-route repaints. Raw source photographs
and exact crops remain in their dated provenance workspaces. The Musa Dzhalil
and Ahmet Zeki Velidi Togan masters have independent visual likeness/style/
framing PASS results but remain rights/provenance/role-gated evidence only; no
DDS or `.gfx` consumer is authorized for either candidate. Generated-only
fictional or institutional portraits, DDS files, advisor icons, dossier/small
derivatives, and contact sheets are excluded from this shelf. Use its
`README.md`, `MANIFEST.md`, and
`PRE_RESIZE_MANIFEST.md` for the status, source path, SHA-256, dimensions, and
provenance boundary of every master; processed copies remain in their originating
package workspaces rather than this shelf. The original package manifests and
...
low-resolution group-photo caveat. v94 promotes only the existing concordat-council
DDS consumer. That narrow promotion does not admit IW-058 or authorize an
advisor/small portrait. Parent release acceptance remains separate from this
and no runtime promotion occurred. IW-030 Janko Vukotić is source/research-only
with `needs_user_review` provenance/date gates.

The preceding dated sentence is superseded by the current v94 promotion record:
the existing ASY concordat-council DDS is the only promoted Barsoum consumer.
There is no advisor, dossier, small portrait, or IW-058 package admission.

Shelf-status clarification (2026-08-02): the CHU evidence rows and the two MNT
masters remain `needs_user_review` for rights, provenance, date, or role gates.
The scoped v94 DDS promotion does not change those evidence dispositions or admit
any wider package.

The earlier sentence saying that no runtime promotion occurred is retained as
dated package-boundary wording; the current v94 record is the sole exception and
promotes only the already-existing concordat-council DDS consumer.

The current exact compile-time content-attestation set contains fourteen packages:
IW-001 Scotland, IW-002 Wales, IW-004 Brittany, IW-006 Wallonia, IW-007 Frisia,
IW-008 Rhineland, IW-009 Bavaria, IW-010 Saar, IW-012 Iceland, IW-017 Corsica,
IW-018 ARX Sardinia, IW-019 Sicily, IW-173 Hawai'i, and IW-184 California. These
fourteen packages span thirteen compatible reservation groups and fourteen
distinct anchors. IW-018 ARX is admitted by the post-wire package audit; IW-173
HAW remains admitted as an additive non-ruling Samuel Wilder King role while the
vanilla David Kalakaua Kawananakoa roster remains intact. IW-043 CHU, IW-058 ASY,
IW-093 DOX, IW-098 SOK, IW-179 FSM, IW-030 MNT, IW-177 FIJ, and IW-014 CAT remain
