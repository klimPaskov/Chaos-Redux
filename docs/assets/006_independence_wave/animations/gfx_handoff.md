...
The four ASSET-040 through ASSET-043 families are ready for parent wiring. All frames are 64x64, centered, alpha-transparent at unused corners, and authored as state-specific source art. Use the DDS sheet paths below; GIFs are review-only.
...
Recommended parent-side pattern, to be verified against the target GUI surface:
...
The parent implementation owns the final `.gfx` file, `.gui` placement, scripted-GUI context, visibility predicates, state-to-frame mapping, and static fallback behavior. `play_on_show = no` is intentional so a state trigger controls visibility; if the target surface requires autoplay, parent may change that setting after GUI validation.
...
No runtime `.gfx`, `.gui`, gameplay, localisation, or advisor/dossier assets are included in this handoff.
