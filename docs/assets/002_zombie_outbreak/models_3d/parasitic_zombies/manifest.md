# Parasitic Zombies model manifest
...
- Sub-unit: `parasitic_zombies`.
- Mesh stem: `chaosx_parasitic_zombies`.
...
- Mesh: `export/mesh/chaosx_parasitic_zombies.mesh` and `gfx/models/units/chaosx_parasitic_zombies/chaosx_parasitic_zombies.mesh`.
- Actions: `export/anim/chaosx_parasitic_zombies_{idle,move,attack,death}.anim` and matching runtime files.
...
- Reimport evidence: `validation/reimport_chaosx_parasitic_zombies_*.json`.
- Entity/GFX: `gfx/entities/chaosx_parasitic_zombies.asset` and `gfx/entities/chaosx_parasitic_zombies.gfx`.
- Unit binding: `common/units/zombies.txt#parasitic_zombies` uses `sprite = chaosx_parasitic_zombies`.
