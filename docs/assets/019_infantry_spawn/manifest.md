...
- One compact 960 by 640 muster-board background is processed under `processed_png/gui/` and `gfx/interface/019_infantry_spawn/`; the current authored composition and runtime proof are retained in `gui_background_compact_2026_07/` with its source PNG, processed PNG, review sheet, manifest, and GFX handoff.
