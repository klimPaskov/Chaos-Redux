...
- One richer compact 960 by 640 muster-board background is processed under `processed_png/gui/` and `gfx/interface/019_infantry_spawn/`; the current authored composition and runtime proof are retained in `gui_background_richer_2026_07/` with its source PNG, processed PNG, review sheet, manifest, coverage crosswalk, and GFX handoff.
...
- `interface/019_infantry_spawn_muster_board.gui` consumes the 960 by 640 background, all three animated sprites, all three static fallbacks, and the selected claimant army/muster scene through its unchanged portrait-slot widget.
...
- GUI review: `contact_sheets/event_019_gui_background_contact_sheet.png` and `gui_background_richer_2026_07/review/infantry_spawn_muster_board_background_richer_contact_sheet.png`.
