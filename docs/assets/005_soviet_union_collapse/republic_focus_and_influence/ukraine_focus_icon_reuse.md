...
The tree currently uses 112 focuses and 12 branch-level sprites defined in `interface/005_soviet_collapse_ukraine_icons.gfx`. Each sprite points to a dedicated branch DDS derived from existing generated Event 005 focus art with matching subject matter. This is a deliberate branch-icon reuse package for the current expanded implementation, not evidence that the older literal 208-focus target is implemented.
