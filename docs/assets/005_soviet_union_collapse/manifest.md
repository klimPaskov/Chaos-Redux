...
Latest regeneration pass: all 38 custom and factory leader portraits were regenerated with Codex built-in `image_gen` from four archetype-specific HOI4-style fictional leader sheets so the set varies by age, gender, ethnicity, role, clothing, and regime style. The generated source sheets are preserved as `docs/assets/005_soviet_union_collapse/source_png/regenerated_event005_leaders_varied_a_source.png`, `regenerated_event005_leaders_varied_b_source.png`, `regenerated_event005_leaders_varied_c_source.png`, and `regenerated_event005_leaders_varied_d_source.png`. The factory-country leader portraits for `CFR` and `MFR` are non-human symbolic factory portraits generated from `docs/assets/005_soviet_union_collapse/source_png/regenerated_event005_factory_leader_portraits_source.png`, matching their collective leader names, `The Construction Board` and `The Arsenal Board`. Per-tag source cells, processed PNG previews, and final DDS files were refreshed in the normal source, processed, and `gfx/leaders/005_soviet_collapse/` locations. Active idea icons and all 38 custom/factory idea icons were regenerated from dedicated chroma-key sheets, cleaned with the official `imagegen` chroma-key helper, then fitted into the 60x68 canvas with strict alpha-bounds centering and an enforced 8px top/bottom safety margin before DDS conversion. The user-reported custom idea icons also receive per-icon visual-mass nudges before conversion so their painted weight sits at the canvas center in the wired `gfx/interface/ideas/soviet_collapse/` DDS files. A targeted custom idea pass replaced `TSC`, `BEC`, `BLT`, `CFR`, `COU`, `DSC`, `FEV`, `ILU`, `IRA`, `LID`, `MFR`, `NRF`, `OGB`, `PRA`, `RCD`, `RLD`, and `SZA` with dedicated per-tag `image_gen` sources, official chroma-key cleanup, strict 60x68 alpha fitting, and no top/bottom clipping. The cleaned idea source sheets are preserved as `docs/assets/005_soviet_union_collapse/source_png/regenerated_event005_ideas_aligned_alpha_source.png` and `docs/assets/005_soviet_union_collapse/source_png/regenerated_event005_custom_ideas_aligned_alpha_source.png`; the targeted per-tag source PNGs are preserved as `docs/assets/005_soviet_union_collapse/source_png/005_<tag>_custom_splinter_idea_source.png`. The reported clipped focus icons remain on their accepted complete-symbol sources. Event 005 achievement icons use generated achievement-frame source art preserved as `docs/assets/005_soviet_union_collapse/source_png/achievement_frame_base_source.png`; completed, grey, and not-eligible red-cross DDS variants live under `gfx/achievements/`. Final DDS files are converted through `.tools/convert_to_dds.py` as BGRA 32-bit DDS, and review sheets are available at `docs/assets/005_soviet_union_collapse/contact_sheets/custom_country_leaders_alignment_debug.png`, `custom_country_leaders_labeled.png`, `soviet_collapse_idea_icons_aligned.png`, `custom_country_idea_icons_aligned.png`, `event005_requested_custom_idea_regen.png`, `event005_achievement_icons_complete.png`, and `event005_achievement_icons_not_eligible.png`.
...
| Shared breakaway focus icons | dedicated `GFX_focus_soviet_collapse_*` sprites in `interface/005_soviet_collapse_icons.gfx` | complete for active shared focus tree |
...
## Required Active Shared Focus Icons

The shared breakaway focus tree is active gameplay content in `common/national_focus/005_soviet_collapse_republics.txt`. Active focus icons are generated at 94x86, processed, converted, registered in `interface/005_soviet_collapse_icons.gfx`, and referenced by the shared focus tree. The regional republic runtime trees derive dedicated focus-id assets from this generated art, but now use their own PNG/DDS files and sprite definitions recorded in the republic focus and influence expansion asset ledger.

| Focus | Current icon | Final status |
| --- | --- | --- |
...
| The Baltic Restoration Pact | `GFX_super_event_baltic_restoration_pact` | internet source image, source tracking | `docs/assets/005_soviet_union_collapse/source_png/super_event_baltic_restoration_pact_source.png` | `docs/assets/005_soviet_union_collapse/processed_png/super_event_baltic_restoration_pact.png` | `gfx/super_events/super_event_baltic_restoration_pact.dds` | `interface/chaosx_super_events.gfx` | wired, `sourced` |
| The Caucasus Defense Compact | `GFX_super_event_caucasus_defense_compact` | internet source image, source tracking | `docs/assets/005_soviet_union_collapse/source_png/super_event_caucasus_defense_compact_source.png` | `docs/assets/005_soviet_union_collapse/processed_png/super_event_caucasus_defense_compact.png` | `gfx/super_events/super_event_caucasus_defense_compact.dds` | `interface/chaosx_super_events.gfx` | wired, `sourced` |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| The Black Banner Returns | `15` | `GFX_super_event_black_banner_returns` | `docs/assets/005_soviet_union_collapse/source_png/optional_super_event_sheet_source.png` | `docs/assets/005_soviet_union_collapse/processed_png/optional_super_events/super_event_black_banner_returns.png` | `gfx/super_events/super_event_black_banner_returns.dds` | `FTH_extreme_path` | wired, generated |
| The Dead Are Citizens | `16` | `GFX_super_event_dead_are_citizens` | `docs/assets/005_soviet_union_collapse/source_png/optional_super_event_sheet_source.png` | `docs/assets/005_soviet_union_collapse/processed_png/optional_super_events/super_event_dead_are_citizens.png` | `gfx/super_events/super_event_dead_are_citizens.dds` | `RMC_extreme_path`, `RCD_extreme_path` | wired, generated |
| The World as One Factory | `17` | `GFX_super_event_world_as_one_factory` | `docs/assets/005_soviet_union_collapse/source_png/optional_super_event_sheet_source.png` | `docs/assets/005_soviet_union_collapse/processed_png/optional_super_events/super_event_world_as_one_factory.png` | `gfx/super_events/super_event_world_as_one_factory.dds` | `ILU_extreme_path` | wired, generated |
| Every Port a Council | `18` | `GFX_super_event_every_port_a_council` | `docs/assets/005_soviet_union_collapse/source_png/optional_super_event_sheet_source.png` | `docs/assets/005_soviet_union_collapse/processed_png/optional_super_events/super_event_every_port_a_council.png` | `gfx/super_events/super_event_every_port_a_council.dds` | `KRS_extreme_path`, `NRF_extreme_path` | wired, generated |
...
| The Steppe Federation | `24` | `GFX_super_event_steppe_federation` | `docs/assets/005_soviet_union_collapse/source_png/super_event_steppe_federation_source.png` | `docs/assets/005_soviet_union_collapse/processed_png/super_event_steppe_federation.png` | `gfx/super_events/super_event_steppe_federation.dds` | `Steppe Federation formation` | wired, `sourced` |
| The Baltic Restoration Pact | `25` | `GFX_super_event_baltic_restoration_pact` | `docs/assets/005_soviet_union_collapse/source_png/super_event_baltic_restoration_pact_source.png` | `docs/assets/005_soviet_union_collapse/processed_png/super_event_baltic_restoration_pact.png` | `gfx/super_events/super_event_baltic_restoration_pact.dds` | `Baltic Restoration Pact formation` | wired, `sourced` |
| The Caucasus Defense Compact | `26` | `GFX_super_event_caucasus_defense_compact` | `docs/assets/005_soviet_union_collapse/source_png/super_event_caucasus_defense_compact_source.png` | `docs/assets/005_soviet_union_collapse/processed_png/super_event_caucasus_defense_compact.png` | `gfx/super_events/super_event_caucasus_defense_compact.dds` | `Caucasus Defense Compact formation` | wired, `sourced` |
...
The compact 12-focus `soviet_collapse_belarus_focus_tree` is active gameplay for Event 005-created Belarus. Its branch-level sprites are wired in `interface/005_soviet_collapse_blr_icons.gfx` and point at dedicated 94x86 DDS files derived from existing generated Event 005 focus art.
...
`GAC` uses generated custom-splinter asset reuse from the Basmachi green-mountain slot so the Green Army Congress country package loads with stable filenames while bespoke Green Army artwork is still pending. `DHC` uses generated custom-splinter asset reuse from the Brotherhood of the Last Tsar slot so the Don Host Emergency Circle package loads with stable filenames while bespoke Don Host artwork is still pending. `KHC` uses generated custom-splinter asset reuse from the Don Host placeholder set so the Kuban Host Provisional Council package loads with stable filenames while bespoke Kuban Host artwork is still pending. `FEV` uses the same generated-reuse placeholder set so the Far Eastern Republic Revival package loads with stable filenames while bespoke Far Eastern artwork is still pending. `SZA` uses the same generated-reuse placeholder set so the Siberian Zemstvo Authority package loads with stable filenames while bespoke Siberian artwork is still pending. `MRC` uses the same generated-reuse placeholder set so the Mountain Republic of the Caucasus package loads with stable filenames while bespoke Mountain Republic artwork is still pending. `TNC` and `ALA` use generated custom-splinter asset reuse from the Basmachi slot so the Turkestan National Council and Alash Restoration Authority packages load with stable filenames while bespoke Turkestan and Alash artwork is still pending. `UWD` uses generated custom-splinter asset reuse from the Iron Liturgy slot so the Ural Workers' Directorate package loads with stable filenames while bespoke Ural Workers artwork is still pending. `IUL` uses generated custom-splinter asset reuse from the Ural Workers slot so the Idel-Ural League package loads with stable filenames while bespoke Idel-Ural artwork is still pending. `BAC` uses generated custom-splinter asset reuse from the Far Eastern placeholder set so the Birobidzhan Autonomous Commune package loads with stable filenames while bespoke Birobidzhan artwork is still pending. Their replacement paths are already registered and should keep the same filenames when unique art is generated. `TRS` uses generated custom-splinter asset reuse from the Brotherhood of the Last Tsar slot so the Third Rome Emergency Synod package loads with stable filenames while bespoke Third Rome artwork is still pending. `ICD` uses generated custom-splinter asset reuse from the Iron Liturgy slot so the Iron Commissariat of the Dead package loads with stable filenames while bespoke Iron Commissariat artwork is still pending. `NLC` uses generated custom-splinter asset reuse from the Arctic naval slot so the Northern Lights Commune package loads with stable filenames while bespoke aurora artwork is still pending. `SEP` uses generated custom-splinter asset reuse from the Red Martyrs slot so the Sepulchre Soviet package loads with stable filenames while bespoke sepulchre artwork is still pending. `DSC` uses generated custom-splinter asset reuse from the Iron Commissariat slot so the Dead Soldiers' Congress package loads with stable filenames while bespoke dead-soldier artwork is still pending.
...
| `MRC` | `docs/assets/005_soviet_union_collapse/processed_png/flags/MRC_normal.png` | `gfx/flags/MRC.tga` | `gfx/flags/medium/MRC.tga` | `gfx/flags/small/MRC.tga` | wired, generated reuse |
| `UWD` | `docs/assets/005_soviet_union_collapse/processed_png/flags/UWD_normal.png` | `gfx/flags/UWD.tga` | `gfx/flags/medium/UWD.tga` | `gfx/flags/small/UWD.tga` | wired, generated reuse |
| `IUL` | `docs/assets/005_soviet_union_collapse/processed_png/flags/IUL_normal.png` | `gfx/flags/IUL.tga` | `gfx/flags/medium/IUL.tga` | `gfx/flags/small/IUL.tga` | wired, generated reuse |
| `BAC` | `docs/assets/005_soviet_union_collapse/processed_png/flags/BAC_normal.png` | `gfx/flags/BAC.tga` | `gfx/flags/medium/BAC.tga` | `gfx/flags/small/BAC.tga` | wired, generated reuse |
...
| `FEV` | `docs/assets/005_soviet_union_collapse/processed_png/flags/FEV_normal.png` | `gfx/flags/FEV.tga` | `gfx/flags/medium/FEV.tga` | `gfx/flags/small/FEV.tga` | wired, generated reuse |
| `SZA` | `docs/assets/005_soviet_union_collapse/processed_png/flags/SZA_normal.png` | `gfx/flags/SZA.tga` | `gfx/flags/medium/SZA.tga` | `gfx/flags/small/SZA.tga` | wired, generated reuse |
...
| `MRC` | `gfx/interface/goals/005_mrc_custom_splinter_focus.dds` | `gfx/interface/ideas/005_mrc_custom_splinter_idea.dds` | `gfx/interface/decisions/005_mrc_custom_splinter_decision.dds` | `gfx/leaders/005_soviet_collapse/MRC_leader.dds` | wired, generated reuse |
| `UWD` | `gfx/interface/goals/005_uwd_custom_splinter_focus.dds` | `gfx/interface/ideas/005_uwd_custom_splinter_idea.dds` | `gfx/interface/decisions/005_uwd_custom_splinter_decision.dds` | `gfx/leaders/005_soviet_collapse/UWD_leader.dds` | wired, generated reuse |
| `IUL` | `gfx/interface/goals/005_iul_custom_splinter_focus.dds` | `gfx/interface/ideas/005_iul_custom_splinter_idea.dds` | `gfx/interface/decisions/005_iul_custom_splinter_decision.dds` | `gfx/leaders/005_soviet_collapse/IUL_leader.dds` | wired, generated reuse |
| `BAC` | `gfx/interface/goals/005_bac_custom_splinter_focus.dds` | `gfx/interface/ideas/005_bac_custom_splinter_idea.dds` | `gfx/interface/decisions/005_bac_custom_splinter_decision.dds` | `gfx/leaders/005_soviet_collapse/BAC_leader.dds` | wired, generated reuse |
...
| `GAC` | `gfx/interface/goals/005_gac_custom_splinter_focus.dds` | `gfx/interface/ideas/005_gac_custom_splinter_idea.dds` | `gfx/interface/decisions/005_gac_custom_splinter_decision.dds` | `gfx/leaders/005_soviet_collapse/GAC_leader.dds` | wired, generated reuse |
| `DHC` | `gfx/interface/goals/005_dhc_custom_splinter_focus.dds` | `gfx/interface/ideas/005_dhc_custom_splinter_idea.dds` | `gfx/interface/decisions/005_dhc_custom_splinter_decision.dds` | `gfx/leaders/005_soviet_collapse/DHC_leader.dds` | wired, generated reuse |
| `KHC` | `gfx/interface/goals/005_khc_custom_splinter_focus.dds` | `gfx/interface/ideas/005_khc_custom_splinter_idea.dds` | `gfx/interface/decisions/005_khc_custom_splinter_decision.dds` | `gfx/leaders/005_soviet_collapse/KHC_leader.dds` | wired, generated reuse |
| `FEV` | `gfx/interface/goals/005_fev_custom_splinter_focus.dds` | `gfx/interface/ideas/005_fev_custom_splinter_idea.dds` | `gfx/interface/decisions/005_fev_custom_splinter_decision.dds` | `gfx/leaders/005_soviet_collapse/FEV_leader.dds` | wired, generated reuse |
| `SZA` | `gfx/interface/goals/005_sza_custom_splinter_focus.dds` | `gfx/interface/ideas/005_sza_custom_splinter_idea.dds` | `gfx/interface/decisions/005_sza_custom_splinter_decision.dds` | `gfx/leaders/005_soviet_collapse/SZA_leader.dds` | wired, generated reuse |
| `UDC` | `gfx/interface/goals/005_udc_custom_splinter_focus.dds` | `gfx/interface/ideas/005_udc_custom_splinter_idea.dds` | `gfx/interface/decisions/005_udc_custom_splinter_decision.dds` | `gfx/leaders/005_soviet_collapse/UDC_leader.dds` | wired |
| `SDZ` | `gfx/interface/goals/005_sdz_custom_splinter_focus.dds` | `gfx/interface/ideas/005_sdz_custom_splinter_idea.dds` | `gfx/interface/decisions/005_sdz_custom_splinter_decision.dds` | `gfx/leaders/005_soviet_collapse/SDZ_leader.dds` | wired |
| `RMC` | `gfx/interface/goals/005_rmc_custom_splinter_focus.dds` | `gfx/interface/ideas/005_rmc_custom_splinter_idea.dds` | `gfx/interface/decisions/005_rmc_custom_splinter_decision.dds` | `gfx/leaders/005_soviet_collapse/RMC_leader.dds` | wired |
| `RCD` | `gfx/interface/goals/005_rcd_custom_splinter_focus.dds` | `gfx/interface/ideas/005_rcd_custom_splinter_idea.dds` | `gfx/interface/decisions/005_rcd_custom_splinter_decision.dds` | `gfx/leaders/005_soviet_collapse/RCD_leader.dds` | wired |
| `ILU` | `gfx/interface/goals/005_ilu_custom_splinter_focus.dds` | `gfx/interface/ideas/005_ilu_custom_splinter_idea.dds` | `gfx/interface/decisions/005_ilu_custom_splinter_decision.dds` | `gfx/leaders/005_soviet_collapse/ILU_leader.dds` | wired |
| `PRA` | `gfx/interface/goals/005_pra_custom_splinter_focus.dds` | `gfx/interface/ideas/005_pra_custom_splinter_idea.dds` | `gfx/interface/decisions/005_pra_custom_splinter_decision.dds` | `gfx/leaders/005_soviet_collapse/PRA_leader.dds` | wired |
| `TSC` | `gfx/interface/goals/005_tsc_custom_splinter_focus.dds` | `gfx/interface/ideas/005_tsc_custom_splinter_idea.dds` | `gfx/interface/decisions/005_tsc_custom_splinter_decision.dds` | `gfx/leaders/005_soviet_collapse/TSC_leader.dds` | wired |
| `BLT` | `gfx/interface/goals/005_blt_custom_splinter_focus.dds` | `gfx/interface/ideas/005_blt_custom_splinter_idea.dds` | `gfx/interface/decisions/005_blt_custom_splinter_decision.dds` | `gfx/leaders/005_soviet_collapse/BLT_leader.dds` | wired |
| `NRF` | `gfx/interface/goals/005_nrf_custom_splinter_focus.dds` | `gfx/interface/ideas/005_nrf_custom_splinter_idea.dds` | `gfx/interface/decisions/005_nrf_custom_splinter_decision.dds` | `gfx/leaders/005_soviet_collapse/NRF_leader.dds` | wired |
...
| `DSC` | `gfx/interface/goals/005_dsc_custom_splinter_focus.dds` | `gfx/interface/ideas/005_dsc_custom_splinter_idea.dds` | `gfx/interface/decisions/005_dsc_custom_splinter_decision.dds` | `gfx/leaders/005_soviet_collapse/DSC_leader.dds` | wired, generated reuse |
...
- Final generated shared breakaway and Black Banner focus icons: `complete` for the active shared tree and Ukrainian Black Banner contingency focuses.
- Custom tag flags: `complete` for `FTH`, `KRS`, `BSC`, `UDC`, `SDZ`, `RMC`, `RCD`, `ILU`, `PRA`, `TSC`, `BLT`, and `NRF`; `wired, generated reuse` for `GAC`, `DHC`, `KHC`, `FEV`, `SZA`, `MRC`, `TNC`, `ALA`, `UWD`, `IUL`, `BAC`, `ARD`, `BBH`, `TRS`, `ICD`, `NLC`, `SEP`, and `DSC`.
- Custom tag focus icons, idea icons, decision icons, faction-style emblems, and fictional leader portraits: `complete` for the implemented custom tags except `GAC`, `DHC`, `KHC`, `FEV`, `SZA`, `MRC`, `TNC`, `ALA`, `UWD`, `IUL`, `BAC`, `ARD`, `BBH`, `TRS`, `ICD`, `NLC`, `SEP`, and `DSC`, which are wired through generated reuse pending bespoke Green Army, Don Host, Kuban Host, Far Eastern, Siberian, Mountain Republic, Turkestan, Alash, Ural Workers, Idel-Ural, Birobidzhan, Arctic Naval Directorate, Black Banner Host, Third Rome Emergency Synod, Iron Commissariat of the Dead, Northern Lights Commune, Sepulchre Soviet, and Dead Soldiers' Congress art.
- Optional branch super-event images: `complete` for Black Banner, Dead Citizens, One Factory, Port Council, Ukraine's A Map Larger than the Union branch reveal, Kazakhstan's Steppe Beyond History branch reveal, Belarus's Corridors Decide branch reveal, Ukraine's Bread State branch reveal, Ukraine's League of Equal Republics branch reveal, the Steppe Federation formation reveal, the Baltic Restoration Pact formation reveal, the Caucasus Defense Compact formation reveal, and the Eastern Buffer Coalition formation reveal.
