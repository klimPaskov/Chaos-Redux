...
`FTH_soviet_collapse_focus_tree`, `BBH_soviet_collapse_focus_tree`, `BSC_soviet_collapse_focus_tree`, `TNC_soviet_collapse_focus_tree`, and `ALA_soviet_collapse_focus_tree` each use 24 focuses in `common/national_focus/005_soviet_collapse_custom_splinters.txt`. They reuse the existing `GFX_focus_TAG_*` sprites and their generated custom-splinter DDS files from `interface/005_soviet_collapse_custom_icons.gfx`; no new focus DDS files are required for this pass.
