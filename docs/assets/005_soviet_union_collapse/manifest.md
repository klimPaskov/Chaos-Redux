...
- Final generated shared breakaway and Black Banner focus icons: `complete` for the active 53-focus shared fallback tree through deliberate reuse of the 22 wired shared sprites, for Ukrainian Black Banner contingency focuses, and for the 24-focus `FTH` and 24-focus `BBH` custom-splinter trees through their existing per-tag custom focus sprites.
...
`FTH_soviet_collapse_focus_tree` and `BBH_soviet_collapse_focus_tree` each use 24 focuses in `common/national_focus/005_soviet_collapse_custom_splinters.txt`. They reuse the existing `GFX_focus_FTH_*` and `GFX_focus_BBH_*` sprites and their generated custom-splinter DDS files from `interface/005_soviet_collapse_custom_icons.gfx`; no new focus DDS files are required for this pass.
