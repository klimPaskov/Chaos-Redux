...
PROCESSED = ROOT / "docs/assets/012_africa_priority_member_idea_icons/processed_png"
RUNTIME = ROOT / "gfx/interface/ideas/012_africa/priority_members"
DECODED = ROOT / "docs/assets/012_africa_priority_member_idea_icons/validation/dds_decoded"
REPORT = ROOT / "docs/assets/012_africa_priority_member_idea_icons/validation/dds_validation.json"
...
def decode(path: Path) -> tuple[Image.Image, dict[str, object]]:
	...
	checks = {
		...
		"header_ok": header_size == 124 and pf_size == 32 and pf_flags == 65 and fourcc == 0 and bits == 32 and [r_mask, g_mask, b_mask, a_mask] == [0x00FF0000, 0x0000FF00, 0x000000FF, 0xFF000000] and caps == 0x1000 and len(data) == expected,
	}
	if not checks["header_ok"]:
		raise ValueError(f"{path}: malformed legacy BGRA header: {checks}")
	pixels = bytearray(data[128:])
	for i in range(0, len(pixels), 4):
		pixels[i], pixels[i + 2] = pixels[i + 2], pixels[i]
	image = Image.frombytes("RGBA", (width, height), bytes(pixels))
	checks["alpha_min"] = image.getchannel("A").getextrema()[0]
	checks["alpha_max"] = image.getchannel("A").getextrema()[1]
	return image, checks


records: list[dict[str, object]] = []
for dds in sorted(RUNTIME.glob("idea_africa_priority_*.dds")):
