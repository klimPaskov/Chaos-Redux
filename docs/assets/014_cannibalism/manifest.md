...
- Hannibal Lecter's ordinary portrait is reveal-gated. Its 12 real source frames build the skull-lifting and licking loop.
- Wendigo Hannibal has a separate 16-frame transformed portrait sequence with an open winter battlefield backdrop and no borrowed sacred or living Indigenous motif.
...
| --- | --- | --- |
| `flags_refresh/` | 65 independently generated designs; 195 runtime TGAs at 82x52, 41x26, and 10x7 | Exact prompts, source PNGs, processed PNGs, hashes, contact sheets, manifest |
| `leader_portraits_refresh/` | 56 distinct warlord portraits; ordinary Hannibal 12-frame animation; Wendigo Hannibal 16-frame animation | Source images, processed PNG/DDS, static fallbacks, sheets, GIF previews, manifests, validation |
...
- Country flags: 195 TGA files for 65 current designs across three sizes.
- Portrait textures: 56 warlord DDS portraits, two ordinary Hannibal DDS textures, and two Wendigo Hannibal DDS textures.
- Animation runtime: every animation has real source frames, a sheet DDS, static fallback, GIF preview, manifest, and GFX handoff.
- Event 014 GFX closure: 812 path references, 598 unique runtime paths, 0 missing paths.
