#!/usr/bin/env python3
"""Process and validate Event 014 unified-decision icon rows 25-39 only."""
...
BRIEFS = PACKAGE / "prompts" / "icon_briefs_rows_25_39.tsv"
OUTPUTS = PACKAGE / "prompts" / "imagegen_output_paths_rows_25_39.tsv"
...
def load_rows() -> list[dict[str, str]]:
	...
	if [int(row["row"]) for row in output_rows] != list(range(ROW_START, ROW_START + EXPECTED_COUNT)):
		raise RuntimeError("Built-in output ledger row numbers do not match rows 25-39")
	if [row["decision_id"] for row in output_rows] != ids:
		raise RuntimeError("Built-in output ledger ids do not match the subset briefs")
	...
	if [decision_id for decision_id, _ in registered][ROW_START - 1:ROW_START - 1 + EXPECTED_COUNT] != ids:
		raise RuntimeError("Subset order no longer matches live unified GFX rows 25-39")
	for decision_id, texture_path in registered[ROW_START - 1:ROW_START - 1 + EXPECTED_COUNT]:
		expected = f"gfx/interface/decisions/014_cannibalism/decision_{decision_id}.dds"
		if texture_path != expected:
			raise RuntimeError(f"Registered path mismatch for {decision_id}: {texture_path!r}")
	return rows
...
def make_source_contact(rows: list[dict[str, str]]) -> None:
	...
	sheet.save(CONTACTS / "unified_decisions_rows_25_39_sources_contact.png", optimize=True)
...
def write_ledgers(rows: list[dict[str, str]], validations: list[dict[str, object]]) -> None:
	...
	with OUTPUTS.open("r", encoding="utf-8", newline="") as handle:
		output_by_id = {entry["decision_id"]: entry["built_in_output"] for entry in csv.DictReader(handle, delimiter="\t")}
	for index, row in enumerate(rows, start=ROW_START):
		...
		prompt_entries.append({
			...
			"built_in_output": output_by_id[row["decision_id"]],
	...
	(PACKAGE / "prompts" / "icon_prompt_ledger_rows_25_39.json").write_text(
		json.dumps(prompt_entries, indent=2, ensure_ascii=False) + "\n",
		encoding="utf-8",
	)

	with (VALIDATION / "icon_validation_rows_25_39.tsv").open("w", encoding="utf-8", newline="") as handle:
	...
	(PACKAGE / "manifest_rows_25_39.md").write_text("\n".join(manifest_lines) + "\n", encoding="utf-8")

	handoff_lines = [
		"# GFX handoff — unified decision icons rows 25-39",
		"",
	...
	(PACKAGE / "gfx_handoff_rows_25_39.md").write_text("\n".join(handoff_lines) + "\n", encoding="utf-8")
...
def main() -> None:
	...
	make_final_contact(rows, PROCESSED, ".png", "unified_decisions_rows_25_39_processed_checker_contact.png")
	make_final_contact(rows, LIVE, ".dds", "unified_decisions_rows_25_39_dds_decoded_checker_contact.png")
	...
	report = {
		"scope": "Event 014 unified decision icons rows 25-39",
		...
		"contact_sheets": [
			"contact_sheets/unified_decisions_rows_25_39_sources_contact.png",
			"contact_sheets/unified_decisions_rows_25_39_processed_checker_contact.png",
			"contact_sheets/unified_decisions_rows_25_39_dds_decoded_checker_contact.png",
		],
		"status": "complete",
	}
	(VALIDATION / "validation_report_rows_25_39.json").write_text(
