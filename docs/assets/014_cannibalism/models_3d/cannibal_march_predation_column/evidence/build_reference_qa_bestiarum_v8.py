"""Build source/final and black/white/checker alpha QA for the v8 reference."""
...
def checker(size: tuple[int, int], cell: int = 32) -> Image.Image:
	...
	values = np.where(pattern[..., None] == 0, 214, 244).astype(np.uint8)
	rgb = np.repeat(values, 3, axis=2)
	return Image.fromarray(rgb, "RGB").convert("RGBA")


def labelled_panel(image: Image.Image, title: str, width: int, height: int) -> Image.Image:
	panel = Image.new("RGBA", (width, height + 36), (24, 24, 24, 255))
	panel.alpha_composite(contain(image, (width, height), (24, 24, 24, 255)), (0, 36))
	ImageDraw.Draw(panel).text((12, 10), title, fill=(240, 240, 240, 255), font=ImageFont.load_default())
	return panel


def main() -> int:
	if len(sys.argv) != 4:
		raise SystemExit("usage: build_reference_qa_bestiarum_v8.py SOURCE FINAL OUTPUT_DIR")
	...
	comparison = Image.new("RGBA", (1600, 936), (18, 18, 18, 255))
	comparison.alpha_composite(labelled_panel(source, "OFFICIAL SOURCE — BESTIARUM FLESHMAD HUNTER 1", 800, 900), (0, 0))
	comparison.alpha_composite(labelled_panel(final, "FINAL MESHY INPUT — FAITHFUL MATERIAL INTERPRETATION", 800, 900), (800, 0))
	comparison_path = output_dir / "bestiarum_source_to_final_comparison_v8.png"
	...
	for name, background in backgrounds.items():
		...
		path = output_dir / f"meshy_input_alpha_{name}_v8.png"
	...
	metrics_path = output_dir / "bestiarum_reference_qa_v8.json"
