...
No interface edit is required. `interface/014_cannibalism.gfx` already registers the complete CBE-CBH set at lines 185-216.
...
The exact sprite-to-file mapping is recorded in `manifest.md`. The regenerated DDS filenames preserve every existing registration, so downstream character, cosmetic-tag, focus, decision, and GUI references require no rewiring.
