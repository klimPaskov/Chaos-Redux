...
This package supplies the five distinct missing ledger semantics used by Event 014 decision cost text: Larder Stores, consumed state population, Victory Receipt, Convoy-Hunt Receipt, and Enemy-Loss Receipt.

The package is asset-complete and intentionally does not edit Event 014 localisation, decisions, gameplay, or GUI files. The parent event owner must insert the `£` localisation tokens listed below wherever the corresponding cost ledger is shown.
