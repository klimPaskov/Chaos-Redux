...
| Small airframes in the Unified air operation | `£cannibalism_airframe_texticon` | Do not use a generic fighter icon; this token identifies the paid small-airframe reserve. |
| Transport aircraft in the Unified air operation | `£cannibalism_transport_aircraft_texticon` | Do not use a generic convoy or fighter icon; this token identifies the paid transport-aircraft reserve. |

## Localisation owner handoff

The Event14 localisation owner has inserted the matching `£` tokens in `localisation/english/014_cannibalism_l_english.yml` without changing the numeric values. Larder appears throughout the raise, warlord, unified, and Wendigo decision cost text. Consumed state population appears in the controlled-state, unified consumption, terminal-consumption, and Wendigo population costs. `Victory Receipt` is the battlefield-consumption receipt. `Convoy-Hunt Receipt` is the naval harvest receipt. `Enemy-Loss Receipt` is the Wendigo pack receipt earned from recorded enemy casualties. Small-airframe and transport-aircraft tokens appear in the Unified air-operation cost and requirement strings.
...
- The original five ledger sources are RGBA PNGs at 1254x1254. The small-airframe and transport-aircraft sources are RGBA at 1254x1254. Both have alpha extrema 0–255 and transparent unused canvas; no background-removal fallback was used.
...
- `gfx/texticons/014_cannibalism/cannibalism_airframe_texticon.dds`.
- `gfx/texticons/014_cannibalism/cannibalism_transport_aircraft_texticon.dds`.
