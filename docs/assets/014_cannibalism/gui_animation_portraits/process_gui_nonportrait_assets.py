...
def write_consolidated_runtime_inventory() -> None:
    ...
    portrait_specs = {
        "leader_CBL_hannibal": ((156, 210), 12),
        "leader_ZZZ_hannibal_wendigo": ((156, 210), 16),
    }
    for stem, (frame_size, frame_count) in portrait_specs.items():
        for kind, width in (("static", frame_size[0]), ("sheet", frame_size[0] * frame_count)):
            path = ROOT / "gfx" / "leaders" / "014_cannibalism" / f"{stem}_{kind}.dds"
            if not path.exists():
                raise RuntimeError(f"Missing consolidated portrait runtime: {path}")
            validate_dds(path, width, frame_size[1])
            rows.append([
                "portrait_animation", stem, f"{kind}_dds", path.relative_to(ROOT).as_posix(),
                f"{width}x{frame_size[1]}", sha256(path), str(path.stat().st_size), "complete",
            ])
    write_tsv(
...
def write_consolidated_gfx_handoff(handoff_rows: list[list[str]]) -> None:
    ...
    rows.extend([
        [
            "portrait_animation", "leader_CBL_hannibal", "GFX_cannibalism_revealed_portrait_static",
            "GFX_cannibalism_revealed_portrait_animated", "gfx/leaders/014_cannibalism/leader_CBL_hannibal_static.dds",
            "gfx/leaders/014_cannibalism/leader_CBL_hannibal_sheet.dds", "156x210", "12", "6", "yes", "yes",
            "cannibalism_reveal_complete",
        ],
        [
            "portrait_animation", "leader_ZZZ_hannibal_wendigo", "GFX_cannibalism_wendigo_portrait_static",
            "GFX_cannibalism_wendigo_portrait_animated", "gfx/leaders/014_cannibalism/leader_ZZZ_hannibal_wendigo_static.dds",
            "gfx/leaders/014_cannibalism/leader_ZZZ_hannibal_wendigo_sheet.dds", "156x210", "16", "6", "yes", "yes",
            "cannibalism_reveal_complete + cannibalism_wendigo_route_active",
        ],
    ])
    write_tsv(
