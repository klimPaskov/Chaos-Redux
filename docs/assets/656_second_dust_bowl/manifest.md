...
- Final DDS dimensions: `210x176`; one-level uncompressed BGRA 32-bit.
- Proposed sprite name: `GFX_report_event_fallout_second_dust_bowl`.
- Target `.gfx`: `interface/fallout_world_end.gfx`.
- Localisation key: not applicable to the asset package; parent-owned event localisation remains separate.
- Related consumer: Spec 66 event/report image (parent-owned event wiring).
- Animation/audio: not applicable; this is a static report image.
- Status: `handed_off`; source, processing, and runtime DDS are complete, while `.gfx` and event registration remain parent-owned handoff work.
