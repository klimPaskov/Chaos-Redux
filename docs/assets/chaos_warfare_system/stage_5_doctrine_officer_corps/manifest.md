...
Visible concept count: **61** — 5 doctrine adoption icons, 20 mastery rewards, 4 milestone concepts, 6 officer-corps spirits, 4 institutional role portraits, 1 prepared-command trait, 18 decision/mission icons, 1 decision-category icon, and 2 doctrine technology icons. Runtime DDS count is **45** because the 20 rewards are four 10-frame strips and the four milestones are four 2-frame sheets.
...
- Full generation prompt set: [prompts/asset_prompts.md](prompts/asset_prompts.md). Each row below records the concept-specific prompt delta and its source PNG. Existing captures used a uniform `#00ff00` chroma-key background; the new institutional-review capture used `#ff00ff` because the olive/teal subject made green keying unsafe. Both routes use the official `remove_chroma_key.py` helper with border auto-key, soft matte, thresholds 12/220, and despill to create the alpha-bearing `_master.png` source.
...
- DDS contract: standard 128-byte uncompressed 32-bit BGRA/B8G8R8A8-style DDS; header flags `135183`, pixel-format flags `65`, fourcc `0`, bit count `32`, masks `00FF0000/0000FF00/000000FF/FF000000`, texture caps `4096`, and no mipmaps. All final icons use real alpha.
...
| **Convene Institutional Review** — sealed olive-drab institutional-review ledger/dossier, restrained gas-mask/HQ readiness stamp, brass clasp, and inspection tabs | `source_png/decisions/cbrn_convene_institutional_review_source.png` -> `processed_png/decisions/cbrn_convene_institutional_review.png` | `gfx/interface/decisions/stage_5_chaos_warfare/cbrn_convene_institutional_review.dds` | `GFX_decision_cbrn_convene_institutional_review` / `interface/cbrn_doctrine.gfx` | Stage 5 institutional-review decision; 32x32, 1 frame; independently generated at decision scale; `#ff00ff` key used for olive/teal separation; source SHA-256 `500d48efcfc0aef783ff177e46975cae0239ae88e8718d9832879236a1afd4c0`; complete |
...
|---|---|---|---|---|
| **Mobile Decontamination Columns** — truck-mounted wash/decontamination column with pump, coiled hoses, spray manifold, and marked clean corridor | `source_png/technology/cbrn_mobile_decontamination_columns_source.png` -> `processed_png/technology/cbrn_mobile_decontamination_columns.png` | `gfx/interface/technologies/stage_5_chaos_warfare/cbrn_mobile_decontamination_columns.dds` | `GFX_mobile_decontamination_columns_medium` / `interface/cbrn_doctrine.gfx` | 64x64, 1 frame; truck/wash machinery emphasized over HQ command; complete |
| **Chemical Air Interdiction** — aircraft silhouette, sealed payload canister, protected air-ground board, exact-state target marker | `source_png/technology/cbrn_chemical_air_interdiction_source.png` -> `processed_png/technology/cbrn_chemical_air_interdiction.png` | `gfx/interface/technologies/stage_5_chaos_warfare/cbrn_chemical_air_interdiction.dds` | `GFX_chemical_air_interdiction_medium` / `interface/cbrn_doctrine.gfx` | 64x64, 1 frame; deliberate selected-state planning, not passive air-region contamination; complete |
