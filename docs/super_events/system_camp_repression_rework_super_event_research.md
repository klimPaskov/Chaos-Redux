...
chaosx_super_event.74.d: "Advancing troops and investigators have entered a site that official denials can no longer contain. Registers, physical traces, and testimony connect the local guards to the government that built and maintained the network. Its purpose and scale are now matters of public evidence.\n\nCopies are crossing borders, and the record will not close again."
```
...
chaosx_super_event.12.d: "The Auschwitz laboratory command has broken with Berlin. Josef Mengele's faction has seized the camp, biological-warfare facilities, and the experiment-linked network that answered to it. The chain of command that grew inside the regime now fields its own armies and issues its own orders.\n\nA rival state has emerged from Germany's own department."
```
...
chaosx_super_event.75.d: "Across the Soviet countryside, grain extraction, forced-labor quotas, political instability, and orders issued under fear have converged into a famine the ministries can no longer classify as a local shortage. Villages are losing people faster than reports can be rewritten, and hunger feeds every grievance already pulling at the Union.\n\nRelief, concealment, or an admission of collapse are the remaining choices. None can restore the bread already taken."
```
...
The original audio research pass did not edit the sound registry, gameplay, localisation, scripted effects, GUI, GFX, images, achievements, or package specs. That sentence describes the subagent's ownership boundary, not current repository status. Parent-owned integration has since registered ids `44` through `48`, wired the settings-aware sound helper, added final localisation and art, and connected all five live trigger paths. Exact definition and playback handoff is in `docs/assets/system_camp_repression_rework/audio_handoff.md`; file provenance and checksums are in `docs/assets/system_camp_repression_rework/audio_manifest.md`.
