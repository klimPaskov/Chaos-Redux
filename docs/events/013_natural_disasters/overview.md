...
`call_natural_disaster` is defined beside the Event 013 runtime in `common/scripted_effects/013_natural_disasters_effects.txt`; its caller contract and example are documented in this overview. The shared dynamic-effect registry only indexes helpers that are not owned by a single event.
