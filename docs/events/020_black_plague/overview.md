...
Route-specific decision surfaces are live in the existing rat categories. RTA can strip outer districts during a Hunger spike, establish timed Burrow Nodes in selected rat-held states, follow a real refugee road into a human enemy state, open a Citadel Stockpile, open Migration Lanes, issue a Tide Manifest, or order a Rail Breach after the matching capstone; RTX can collect a Crown Tithe, audit Council charters, or broadcast the Hierophant's ash liturgy. Feeding and Burrow Nodes are consumed by the live RTA pulse, and the refugee-road action uses the canonical exposure adapter with a state cooldown. The route operations use centralized meter costs and timed route consumers, so each choice changes state pressure, pulse capacity, or physically proven spread rather than only setting an unused flag.
