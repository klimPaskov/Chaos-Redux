...
Route-specific decision surfaces are live in the existing rat categories. RTA can open a Citadel Stockpile, Migration Lanes, Tide Manifest, or Rail Breach Order after the matching capstone; RTX can collect a Crown Tithe, audit Council charters, or broadcast the Hierophant's ash liturgy. Each operation uses centralized meter costs and timed route consumers, so the choices change pulse capacity or physically proven spread rather than only setting an unused flag.

The Rat King's court also exposes a state-targeted Royal Strike. During a war, RTX selects a human-controlled enemy state and spends Dominion and Brood Mass while reserving a civilian factory slot for a timed operation. Completion uses the canonical rat-occupation exposure path, reopening the shared disease lifecycle and mapmode provenance; a country cooldown and a separate state cooldown prevent repeated targeting.
...
Black Plague Unbound is implemented in the existing triggerable-scenario UI as `SCN-012`. Each intensity directly seeds established outbreaks across multiple eligible continents, creates several internal RTA warrens from infected-or-worse candidates without creating another tag, forces Evolutions I through IV, creates the Rat Nation carrier, creates the Rat King in a separate Royal Basin, grants a coexistence grace period, initializes AI, forces, disease values, decisions, logs, coronation, and performs one mapmode rebuild. Low intensity may begin with Infected rather than Severe states, so the RTA carrier uses a narrowly scoped bootstrap spawn gate and returns to the natural Severe/Collapsed gate as soon as setup clears. The setup reserves geography and checks the candidate/package postconditions before initializing the disease or rat registries, then uses a scoped bypass that is cleared after the transaction; repeat launches remain idempotent. It never grants Evolution V, `world_end`, or automatic victory and disqualifies ordinary-progression achievements.
