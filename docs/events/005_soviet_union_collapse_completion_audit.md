...
| No hardcoded external crisis hooks | Event 005 prior-crisis memory reads generic fired-event counts and shared world-threat state only. `no_hardcoded_external_crisis_hooks` scans Event 005 script and focus files for direct zombie or Holy Realm markers and fails the completion gate if any are present. | Implemented; verifier guard added |
...
| Foreign intervention categories and action-based aid | `soviet_collapse_foreign_patron_category` has 16 targeted decision blocks with `days_re_enable`; `soviet_collapse_breakaway_category` has 4 action blocks; the decision audit found no active timed foreign intervention missions outside the 128 Soviet crisis missions | Implemented for current layer; catalog row updated |
