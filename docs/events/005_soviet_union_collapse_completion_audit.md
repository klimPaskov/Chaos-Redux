...
| Non-linear focus structure, route locks, branch zones, focus filters, AI behavior | Focus files use route-specific branches, `search_filters`, `ai_will_do`, and mutual exclusions; parser audit over 46 focus trees and 1,500 focus IDs found no duplicate IDs, missing focus references, self-references, one-way mutual exclusions, unknown mutual targets, exact coordinate collisions, missing icons, missing `ai_will_do`, or missing/duplicated `completion_reward` blocks. Focus AI audit found 318 dynamic AI blocks overall and 143 dynamic mutually exclusive route-choice blocks after the latest republic and factory route AI pass; the republic and factory focus files now have zero flat mutually exclusive route AI blocks. The remaining 35 flat mutually exclusive choices are generated custom-splinter `*_settlement` stubs. | Implemented for current focus trees; custom-splinter settlement stubs remain compact fixed-weight choices |
