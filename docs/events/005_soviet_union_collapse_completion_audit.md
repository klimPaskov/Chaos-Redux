...
| Runtime focus trees for republics and breakaways | Focus counts after duplicate pruning: Ukraine 81, Belarus 38, Kazakhstan 57, fallback breakaway 36, Baltic 36, Caucasus 40, Central Asia 45, Moldova 23, internal republic 62 | Implemented for these trees |
...
| Hardcoded high-chaos successor activation removed | Mission failures, first-wave release, terminal collapse, and regional faction recruitment no longer call the fixed high-chaos successor spawn dispatcher or route custom tags into the active Event 005 runtime path. The current terminal verifier instead checks the supported ordinary/internal release surface, league formation surface, and terminal sequencing. | Implemented |
...
PASS event005_focus_integrity_surface: focuses=1013 bad_shape= missing_refs=
...
- `Fixed custom successor spawn helpers are not dispatched by the active Event 005 runtime path.
...
- The dormant high-chaos successor evolution writer is not called by the active Event 005 runtime path.

General Soviet Collapse mutation localisation remains available for future mutation tracks, but the current Event 005 script does not write baseline crisis stages or fixed successor activations into the evolution log.
...
focus_surface_audit focuses 1013
...
banned_low_dynamic_baseline_phrase_hits 0
focus_blocks 1013 duplicate_ids 0
...
focus_blocks 1013
focus_with_ai 1013 missing_ai 0
...
- focus counts and branch maps: 1013 retained focus blocks across the Event 005 runtime focus files, with no duplicate IDs or invalid focus references.
