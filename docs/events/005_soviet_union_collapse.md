...
The category totals mature into separate staged spirit tracks. Recognition grows from diplomatic contacts to missions to treaty backing, then to an Internationally Entrenched Republic when outside guarantees are deep enough. Investment grows from reconstruction offers to sponsored civil works; balanced two- or three-sponsor investment can mature into a Sovereign Reconstruction Board, while one-sided investment becomes a Dependent Reconstruction Network. Volunteer support grows from foreign cadres to brigades to an international defense corps, but a dominant sponsor lock turns the deepest stage into a Sponsor-Dominated Volunteer Command. Adviser support grows from observers to training missions to a joint staff mission, while dominant sponsorship turns the deepest stage into Sponsor-Directed Staff. Material aid still grows from supply contacts to corridors to a full supply network, and patronage grows from contacts to liaison to a patronage network before reaching a Foreign Dependency Network. These spirits can coexist because the republic may be recognized by one sponsor, supplied by another, rebuilt by several investors, and advised by a third.
