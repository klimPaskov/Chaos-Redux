...
1. `soviet_collapse_ukraine_focus_tree` in `common/national_focus/005_soviet_collapse_republics.txt`: 102 focuses, expanded from the previous 18-focus compact draft toward the 153-focus clean-spec target. The layout opens from the Emergency Rada into telegraph, grain, and depot side focuses, converges on the first republican line, then branches into six political route locks: Central Rada, socialist republic, military directory, Hetmanate file, Black Banner compact, and foreign provisional authority. The lower rows separate army doctrine, industry mobilization, diplomacy, League preparation, foreign-aid, late expansion policy, and hidden high-chaos lanes so Ukraine no longer reads as a single vertical ladder.
...
- Continue the Ukraine replacement from 102 focuses to the 153-focus clean-spec target, then expand Belarus, Kazakhstan, regional, and contingency breakaway runtime focus trees into full country packages with the larger focus counts mapped in the final clean specification.
