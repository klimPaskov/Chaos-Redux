...
Event-created Ukraine, Belarus, Kazakhstan, southern cascade republics, prepared regional tags, and any remaining event-created breakaway without a bespoke tree receive runtime focus trees through `load_focus_tree` after the release effect finishes. The loading effect only applies to countries with `soviet_collapse_event_created_republic`, and it does not use `keep_completed`, so it is intended for freshly released tags rather than replacing progress on existing countries. The high-chaos `CFR` and `MFR` successors are loaded directly from their setup effects with `keep_completed = no`, because they are custom crisis countries rather than standard released republics.
...
These tags define country files, history files, politics, basic technologies, leader portraits, localisation, opening decision mechanics, event spawn effects, and evolution-log entries. `CFR` has its full 58-focus Construction Directorate runtime tree, and `MFR` has its 46-focus Arsenal Board runtime tree; the larger `OGB` focus-tree package remains a separate implementation slice.
...
- Implement full focus-tree packages and longer event chains for remaining custom countries and serious splinters whose tag, history, localisation, ideas, decisions, leaders or councils, flags, opening spawn logic, and docs already exist, including the `OGB` high-chaos successor.
