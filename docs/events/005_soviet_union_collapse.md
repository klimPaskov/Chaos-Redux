...
The foreign patron category now covers the first full investment set: recognition, ideological liaison, equipment convoys, military advisers, intelligence channels, volunteer corps, trade missions, civilian construction, military construction, press and radio networks, aid corridors, republic conference sponsorship, League-mediated logistics aid, and an anti-puppet clause. Civilian construction adds a civilian factory and infrastructure to the target republic while applying a temporary consumer-goods and output burden to the sponsor. Military construction adds military industry and anti-air. Aid corridors open a target-side route flag for later support. Targeted aid and dependency steps require a real access route: war against Moscow, faction contact, a shared land border, a coastal convoy route, an opened aid corridor, or League conference access for a League member. They also pass a target-side acceptance gate: wartime need, weak stability or war support, rifle shortage, prior recognition, a trusted aid corridor, balanced sponsorship, low patronage risk, relief sponsorship, or League conference sponsorship can open the republic to aid, while a dominant sponsor lock, high patronage risk, direct client pressure, or strong League/faction protection can restrict direct patron missions. League-mediated logistics aid can target a Free Republics' League member through an existing route or a League aid channel, costs more negotiation and transport, improves League cohesion and independence resilience, and lowers patronage risk compared with direct patron corridors. Volunteer corps now spawns a republican field brigade in addition to the manpower and equipment package. Expanded patron action costs scale by target tier: regional republics and major republics require higher political, command, equipment, fuel, train, convoy, or stability commitments than ordinary breakaways, and the decision cost text displays only the active icon-value costs for the selected target.
...
High-chaos terminal collapse is additive, but the active spawn set is limited to grounded crisis actors. Chaos tier 4 and 5 attempt the active successor spawns after ordinary republic release, so factory boards, sailor councils, regional host councils, security zones, border authorities, and similar political or military breakaways can appear where their required territories remain under Soviet ownership and control. At chaos tier 5, the terminal pass treats the containment network as having broadly failed, unlocks the active successor gates used by the existing spawn helpers, and raises old-movement pressure to the high-chaos gate if it has not already reached it. Disabled hardcoded route packages are not called by the terminal spawn helper.

The same terminal effect closes the pre-collapse crisis board. `soviet_collapse_cleanup_terminal_collapse_missions` clears the active Soviet Collapse and opening-wave flags, clears one-use next-declaration flags, clears transient loyal-unit and district war-room helper flags, removes every defined active Soviet crisis mission with `remove_mission`, and leaves `soviet_collapse_terminal_collapse` as the outcome memory. Disabled hardcoded route entries are not queued by `soviet_collapse_activate_opening_objectives`. Because `is_soviet_collapse_active` now excludes the terminal flag, Soviet response, breakaway action, regional faction, special-actor, and foreign patron categories no longer remain visible after full collapse. The current foreign patron layer uses repeatable targeted decisions with `days_re_enable`, not active timed missions, so there is no separate foreign mission instance to remove in the terminal pass.
...
Event-created Ukraine, Belarus, Kazakhstan, southern cascade republics, prepared regional tags, and any remaining event-created breakaway without a bespoke tree receive runtime focus trees through `load_focus_tree` after the release effect finishes. The loading effect only applies to countries with `soviet_collapse_event_created_republic`, and it does not use `keep_completed`, so it is intended for freshly released tags rather than replacing progress on existing countries. The active high-chaos `CFR`, `MFR`, `KRS`, `FTH`, `BBH`, `BSC`, `TNC`, `ALA`, `UDC`, `SDZ`, `GAC`, `DHC`, `KHC`, `FEV`, `SZA`, `UWD`, `MRC`, `IUL`, `BAC`, `ARD`, and `NLC` successors are loaded directly from their setup effects with `keep_completed = no`, because they are custom crisis countries rather than standard released republics. The disabled hardcoded route packages are not loaded from Event 005 focus, decision, spawn, setup, or endgame script.
...
4. `soviet_collapse_baltic_focus_tree`: 21 focuses for Baltic restoration, border government, Baltic League, foreign-protection, port-sovereignty, and recognition routes.
5. `soviet_collapse_caucasus_focus_tree`: 20 focuses for Caucasus mountain compact, oil emergency, national restoration, foreign guarantee, Caspian watch, and survival routes.
6. `soviet_collapse_central_asia_focus_tree`: 14 focuses for Central Asian local republic, Turkestan federation, military border authority, and foreign patronage routes.
...
The reachable high-chaos successor tags are registered for spawn and mechanics work:
...
3. `KRS` - Kronstadt Free Soviet, led by the Sailors Assembly, with existing flag and leader assets.
4. `FTH` - Free Territory of Huliaipole, led by the Council of Free Soviets, with existing flag and leader assets.
5. `BBH` - Black Banner Host, led by the Black Banner War Council, with existing flag and leader assets.
6. `BSC` - Basmachi Confederation, led by the Oasis War Council, with existing flag and leader assets.
7. `TNC` - Turkestan National Council, led by the Turkestan Civic Council, with existing flag and leader assets.
8. `ALA` - Alash Restoration Authority, led by the Alash Restoration Council, with existing flag and leader assets.
9. `UDC` - Union Defense Committee, led by the Emergency Military Committee, with existing flag and leader assets.
10. `SDZ` - Security Directorate Zone, led by the Directorate Collegium, with existing flag and leader assets.
11. `GAC` - Green Army Congress, led by the Peasant Congress Council, with existing flag and leader assets.
12. `DHC` - Don Host Emergency Circle, led by the Host Emergency Circle, with existing flag and leader assets.
13. `KHC` - Kuban Host Provisional Council, led by the Kuban Provisional Council, with existing flag and leader assets.
14. `FEV` - Far Eastern Republic Revival, led by the Far Eastern Buffer Committee, with existing flag and leader assets.
15. `SZA` - Siberian Zemstvo Authority, led by the Siberian Regional Assembly, with existing flag and leader assets.
16. `UWD` - Ural Workers Directorate, led by the Factory Directorate, with existing flag and leader assets.
17. `MRC` - Mountain Republic of the Caucasus, led by the Mountain Republic Council, with existing flag and leader assets.
18. `IUL` - Idel-Ural League, led by the Volga-Ural Federal Council, with existing flag and leader assets.
19. `BAC` - Birobidzhan Autonomous Commune, led by the Settlement Defense Council, with existing flag and leader assets.
20. `ARD` - Arctic Naval Directorate, led by the Northern Port Directorate, with existing flag and leader assets.
21. `NLC` - Northern Lights Commune, led by the Aurora Survival Council, with existing flag and leader assets.
...
1. `CFR` receives Yaroslavl (`248`) and Ivanovo (`253`) if the required states are owned and controlled by `SOV` and the tag-specific crisis gate is open.
2. `MFR` receives Nizhny Novgorod (`252`) and Samara (`251`) if the required states are owned and controlled by `SOV` and the tag-specific crisis gate is open.
3. `KRS` receives Leningrad Area (`195`) if the required states are owned and controlled by `SOV` and the tag-specific crisis gate is open.
4. `FTH` receives Zaporozhe (`200`) and Dnipropetrovsk (`226`) if the required states are owned and controlled by `SOV` and the tag-specific crisis gate is open.
5. `BBH` receives Zaporozhe (`200`) and Dnipropetrovsk (`226`), with Stalino (`227`) when available if the required states are owned and controlled by `SOV` and the tag-specific crisis gate is open.
6. `BSC` receives Atyrau (`405`), Turkmenistan (`584`), and Stalinabad (`742`) if the required states are owned and controlled by `SOV` and the tag-specific crisis gate is open.
7. `TNC` receives Uzbekistan (`585`), with Kyrgyzstan (`586`) when available if the required states are owned and controlled by `SOV` and the tag-specific crisis gate is open.
8. `ALA` receives Kazakhstan (`583`) and Western Kazakhstan (`587`), with Some Mountains (`588`) when available if the required states are owned and controlled by `SOV` and the tag-specific crisis gate is open.
9. `UDC` receives Tula (`223`), Kursk Area (`220`), and Voronezh (`240`) if the required states are owned and controlled by `SOV` and the tag-specific crisis gate is open.
10. `SDZ` receives Yaroslavl (`248`) and Vologda (`351`) if the required states are owned and controlled by `SOV` and the tag-specific crisis gate is open.
11. `GAC` receives Orel (`222`) and Bryansk (`224`) if the required states are owned and controlled by `SOV` and the tag-specific crisis gate is open.
12. `DHC` receives Rostov Area (`218`) and Volgodonsk (`238`) if the required states are owned and controlled by `SOV` and the tag-specific crisis gate is open.
13. `KHC` receives Krasnodar (`234`) and Stavropol (`235`) if the required states are owned and controlled by `SOV` and the tag-specific crisis gate is open.
14. `FEV` receives Vladivostok (`408`) and Khabarovsk (`409`) if the required states are owned and controlled by `SOV` and the tag-specific crisis gate is open.
15. `SZA` receives Tomsk (`578`) and Tobolsk (`580`) if the required states are owned and controlled by `SOV` and the tag-specific crisis gate is open.
16. `UWD` receives Southern Urals (`582`) and Northern Urals (`581`) if the required states are owned and controlled by `SOV` and the tag-specific crisis gate is open.
17. `MRC` receives Dagestan (`232`), Chechnya-Ingushetia (`821`), Kabardino-Balkaria (`827`), and North Ossetia (`828`) if the required states are owned and controlled by `SOV` and the tag-specific crisis gate is open.
18. `IUL` receives Ufa (`651`) and Ulyanovsk (`250`) if the required states are owned and controlled by `SOV` and the tag-specific crisis gate is open.
19. `BAC` receives Birobidzhan (`657`) and Amur (`561`) if the required states are owned and controlled by `SOV` and the tag-specific crisis gate is open.
20. `ARD` receives Murmansk (`213`) and Arkhangelsk (`214`) if the required states are owned and controlled by `SOV` and the tag-specific crisis gate is open.
21. `NLC` receives Below Zero (`216`) and Nenetsia (`825`) if the required states are owned and controlled by `SOV` and the tag-specific crisis gate is open.

These are strict prerequisites, not contingency pools. If a required state has already left Soviet ownership or control, that successor is not created by the opening hook. A created high-chaos successor receives the normal breakaway support package, its tag-specific opening ideas, its tag-specific runtime focus tree with a clean focus state, and an event notice in `events/005_soviet_collapse_factory_ancient.txt` or `events/005_soviet_collapse_custom.txt`. The first eligible successor in each high-chaos tier records an actor-linked evolution-log entry under `Soviet Collapse: High-Chaos Successors`; later successor notices in the same tier remain normal reports so the crisis does not flood the evolution log.

Each active tag also has an opening decision board:
...
3. `KRS` uses `Kronstadt Council` with council consolidation, sailor-guard mobilisation, and the Every Port a Council endgame.
4. `FTH` uses `Free Territory` with commune consolidation, free-soviet mobilisation, and the World Without Capitals endgame.
5. `BBH` uses `Black Banner Host` with war-council consolidation, black-column mobilisation, and the World Without Prisons endgame.
6. `BSC` uses `Basmachi Confederation` with oasis consolidation, pass-guard mobilisation, and the Road Beyond the Steppe endgame.
7. `TNC` uses `Turkestan National Council` with civic consolidation, railway-guard mobilisation, and the New Turkestan endgame.
8. `ALA` uses `Alash Restoration Authority` with congress consolidation, cavalry-guard mobilisation, and the National Modernization Mandate endgame.
9. `UDC` uses `Union Defense Committee` with command consolidation, signature-force mobilisation, and the Provisional Soviet Command endgame.
10. `SDZ` uses `Security Directorate Zone` with archive consolidation, signature-force mobilisation, and the World in One Archive endgame.
11. `GAC` uses `Green Army Congress` with village-congress consolidation, signature-force mobilisation, and the Land and Bread Against the World endgame.
12. `DHC` uses `Don Host Emergency Circle` with host-circle consolidation, signature-force mobilisation, and the Southern Host Command endgame.
13. `KHC` uses `Kuban Host Provisional Council` with crossing-council consolidation, signature-force mobilisation, and the Kuban Line Command endgame.
14. `FEV` uses `Far Eastern Republic Revival` with buffer-committee consolidation, railway-guard mobilisation, and the Far Eastern Line Command endgame.
15. `SZA` uses `Siberian Zemstvo Authority` with zemstvo consolidation, city-guard mobilisation, and the Siberian Depth Command endgame.
16. `UWD` uses `Ural Workers Directorate` with factory-committee consolidation, worker-battalion mobilisation, and the Arsenal Autonomy endgame.
17. `MRC` uses `Mountain Republic of the Caucasus` with elder-council consolidation, pass-guard mobilisation, and the Mountain Republic Mandate endgame.
18. `IUL` uses `Idel-Ural League` with corridor-office consolidation, river-guard mobilisation, and the Federal Corridor Authority endgame.
19. `BAC` uses `Birobidzhan Autonomous Commune` with settlement-committee consolidation, militia mobilisation, and the Autonomous Commune Mandate endgame.
20. `ARD` uses `Arctic Naval Directorate` with port-directorate consolidation, naval-guard mobilisation, and the Port Neutrality Mandate endgame.
21. `NLC` uses `Northern Lights Commune` with polar-commune consolidation, survival-unit mobilisation, and the World After Capitals endgame.
