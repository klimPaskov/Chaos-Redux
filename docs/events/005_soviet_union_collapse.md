...
2. `soviet_collapse_belarus_focus_tree`: 16 focuses, replacing the compact skeleton with the clean-spec Minsk opening, rail map, forest committees, corridor guard, evacuation choice, route locks for the National Council, socialist autonomy, military transit, and foreign corridor administration, plus first route side focuses and the Belarusian Question Answered political finisher. The remaining rail sovereignty, forest defense, corridor, diplomacy, League logistics, and high-chaos rows still need expansion to reach the 83-focus clean-spec target.
...
- Continue the Belarus replacement from 16 focuses to the 83-focus clean-spec target, then expand Kazakhstan, regional, and contingency breakaway runtime focus trees into full country packages with the larger focus counts mapped in the final clean specification.
