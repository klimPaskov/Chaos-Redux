...
| Event 13 disaster calls | Event 12-only selected-enemy disaster wrapper call is audited against the unchanged Event 13 public contract, with strength derived from promotion, constitutional, ecological, war-readiness, payment, target-size, and completed priority-member nature-authority receipts | Accepted release-candidate evidence | Host and member callers share one action record and one Event 013 API. Exact target and result outputs are verified; no Event 013 source change was made. Severity never derives from ethnicity, a tribal label, or an unreviewed cultural stereotype. Priority-member AI willingness, the authority label, and the two decision descriptions now follow the same trace, low, medium, high, and ancestral ladder used by the impact bridge; each approved package also carries a distinct visible nature invocation. |
...
| `012_africa_runtime_core_audit_handoff.md` | Accepted release-candidate evidence, retained audit | The Event 12-only Event 13 selected-enemy disaster wrapper call is audited against unchanged Event 13 sources with exact target and output receipts. Strange-force consumers remain readiness-gated and model-free. |
...
2. The Event 13 wrapper call is now accepted release-candidate evidence: the Event 12-only call preserves the exact selected enemy and result outputs, and Event 13 source files remain unchanged.
