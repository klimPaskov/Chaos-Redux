...
| Sovereign portraits | Sixteen dedicated sovereign DDS files and matching sprite registrations exist; the accepted v3 package uses one dramatic decorated African king or queen per package on a plain, low-detail matte background in the HOI4 painted style | Installed, promotion-gated | The package recruits the portraits only after the bounded priority-member route reaches its gate. Court labels describe institutions and never replace the named sovereign portrait. The v3 source, processed PNG, decoded-DDS contact sheet, and prompt records are retained in the ignored asset workspace; the stable runtime sprite names and DDS paths are unchanged. |
