...
| Sovereign portraits | Sixteen dedicated sovereign DDS files and matching sprite registrations exist; five rows now use independently reviewed source-locked portraits for Shehu Sanda Kura, Emir Abdullahi, Pedro VII Afonso, Queen Ranavalona III, and Dinuzulu kaCetshwayo, while Buganda remains review-only and the other eleven historical rows remain source-, rights-, ownership-, or identity-gated | Partial installed runtime, promotion-gated | The five promoted rows preserve exact archival crops, source-visible regalia, plain painted backgrounds, stable sprite names, and 156x210 DDS paths. The runtime character effects now fail-closed: only those five reviewed package identities (or an explicit future source-approved flag) receive a sovereign leader role; held historical rows retain their mechanics but cannot display an unreviewed generated ruler. Buganda is not wired because the available Mutesa II evidence is a shadowed group crop; Asante is blocked by the Event 006 Prempeh II owner conflict and Prempeh I identity review, and the remaining source-gap/rights rows remain explicitly documented. Fictional, nonhuman, or supernatural high-chaos portraits are allowed only as a separate ImageGen asset family and remain dormant behind their package/model gates; six such subassets are documented for the existing model-required controlled-pool rows. |
