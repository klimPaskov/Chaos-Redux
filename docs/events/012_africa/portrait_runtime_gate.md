...
4. The shared portrait trigger remains an audit surface for the existing runtime sprite and source-backed identity map. The installed replacements do not suppress the sovereign settlement path, but strict provenance and independent likeness acceptance remain required before the grounded portrait package can be called final.
...
Fifteen historical priority-member rows use the supplied portrait replacements and one row, Buganda, remains an explicitly source-locked placeholder. The RSA branch uses the supplied Mgolombane Sandile portrait. The installed grounded replacements remain pending the strict portrait-worker acceptance package, including complete per-consumer provenance and independent historical-likeness review. The extra Radama I image is evidence-only because no matching runtime consumer exists. Do not add lion heads, masks, spears, face paint, crowns, or insignia that are not visible in the selected portrait. The six fictional portrait packages retain their separate evidence, while the nine Africa model consumers have mixed promotion status under their current manifests; final package, localisation, model, and live-consumer audits remain separate source or user-owned boundaries.
