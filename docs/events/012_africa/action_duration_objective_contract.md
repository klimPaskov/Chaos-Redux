...
The four targeted missions read `FROM.africa_active_action_duration_days`. The decision target is the `FROM` scope for a targeted decision, while vanilla confirms country-scoped mission-timer variables; this exact `FROM` timer form still requires runtime parser confirmation. It avoids `add_days_mission_timeout`, which has no target argument and would modify the wrong record when several target missions share one ID. Offer-response actions delay the existing `chaosx.nr12.210` event by the target's `africa_active_action_response_days` value and snapshot both action and host generations on the target. A timed `africa_action_response_callback_pending` receipt lasts one day beyond the response window; the event consumes it only when the active offer record, callback generation, and committed host generation all still match. A cancelled, superseded, transferred, or backlogged callback therefore fails closed, while only offer-response launches are blocked by an outstanding receipt. The Voluntary Return Registry (row 51) uses the medium mission with its row-specific 90–180-day contract and 135-day default, so registry opening is a timed policy outcome rather than an instant flag.
...
`africa_action_contract_should_cancel` handles target capitulation or loss of scope and routes through the existing cancellation cleanup. Event-active and generation guards remain in each mission's current cancel trigger. Cancellation deliberately leaves the target-owned callback receipt until its short expiry so an already queued event cannot unlock a later offer.
...
- Side effects: mission activation uses the target duration; response events are delayed; callback generations and a one-day-overrun receipt are target-owned; successful callback options clear that receipt before resolving; ordinary cleanup leaves it untouched until expiry.
...
The Charter Court action summary exposes the quoted objective through `GetAfricaCharterActionObjectiveName`, and each active mission repeats that objective in its description. When a mission resolves, `africa_resolve_action` snapshots the same value to `africa_last_action_objective` before creating the result event; idempotent cleanup repeats the snapshot before clearing the active record. The result event exposes it through `GetAfricaLastActionObjectiveName`, so the logged outcome remains tied to the matrix row that ran. Cancellation retains the snapshot for history but does not open a second result popup. Missing quote or result state falls back to the neutral `No timed review` label.
