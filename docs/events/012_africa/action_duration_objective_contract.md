...
The Charter Court action summary exposes the quoted objective through `GetAfricaCharterActionObjectiveName`, so the player can read whether a proposal resolves by peace or deadline, awaits a response, watches target validity, prepares a war, holds a review window, clears a terminal gate, or has no timed review. Missing quote state falls back to the neutral `No timed review` label.
