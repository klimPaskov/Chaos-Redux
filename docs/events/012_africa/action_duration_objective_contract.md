...
The Charter Court action summary exposes the quoted objective through `GetAfricaCharterActionObjectiveName`, and each active mission now repeats that objective in its description. When the mission resolves or is cancelled, cleanup copies the same value to `africa_last_action_objective`; the result event exposes it through `GetAfricaLastActionObjectiveName`, so the logged outcome remains tied to the matrix row that ran. Missing quote or result state falls back to the neutral `No timed review` label.
