...
- `africa_world_all_package_runtime_surfaces_are_certified` proves an exact six-entry, one-per-continent pending roster, live sovereign candidates, controlled non-African capitals, complete candidate-owned receipts, and no successor, exile, breakup, terminal, or high-chaos substitution state. `africa_world_review_runtime_surface_registry` records `africa_world_package_runtime_surfaces_reviewed` only after that proof succeeds, and `africa_world_certify_all_package_runtime_surfaces` then sets `africa_world_package_implementation_ready` across the frozen candidate array in one bounded pass. It records `africa_world_package_runtime_surfaces_certified` only after the pass; package installation and terminal presentation remain separately guarded.
