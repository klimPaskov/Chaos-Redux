...
World-order source, registrations, certified W5 runtime surfaces, four super-event roles, and the six package presentation surfaces are installed. The 239-row matrix remains the visual disposition authority for those surfaces, while current Africa model/audio promotion is governed by the per-package manifests and the fail-closed strange-force contract; no model or unique-package completion claim is made here. No fallback art or silent rights substitution is used.
...
- Keep the installed news, super-event, and animation rows aligned with the authoritative asset matrix, and preserve their source and rights evidence. Strange-force audio remains a mixed, per-package evidence set, so its runtime definitions, action synchronization, and live consumer review follow the current model/audio handoffs rather than a family-wide completion claim.
