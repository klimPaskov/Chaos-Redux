...
The sustained-pressure accumulator is `soviet_collapse_progressive_release_pressure`. It adds `constant:soviet_collapse_release_mtth.sustained_pressure_monthly_add` each eligible scheduler pass, caps at `constant:soviet_collapse_release_mtth.sustained_pressure_cap`, resets after a real release, and also resets if the candidate count is zero. Current tuning adds 4 per eligible monthly pass, caps at 24, and uses a 0.95 miss-weight factor after the first pressure month. This prevents empty-pool cooldowns while still making repeated unresolved pressure visible.
