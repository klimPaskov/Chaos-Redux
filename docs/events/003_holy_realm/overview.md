...
Detailed Buddhahood counters, concentration sequence, power gates, achievement hooks, GUI surface, and asset evidence live in [Holy Realm Buddhahood Progression](systems/buddhahood_progression.md).
...
- Evolutions: four special tracks record to the shared event-log evolution pipeline as evolution type `3`: Pattern of Suffering, Relic Mandala, False Buddha Schism, and Wrathful Protection. Normal stage advancement is handled by country flags and focus prerequisites, not event-log evolution rows. False Buddha Schism also refreshes the focus-tree layout so its conditional branch appears after the evolution is recorded.
...
- Event logs: event ID 3 has a dedicated event-detail description and four Holy Realm evolution preview entries covering the Pattern of Suffering, Relic Mandala, False Buddha Schism, and Wrathful Protection. The evolution list row shows the short evolution name and chaos tier. The detailed panes keep the evolution stage. Final Silence remains a stable cause-memory label, while the public world-end aftermath row belongs to Fallout.
...
- Super events: values `7`, `8`, `10`, `11`, `51`, and `61` display The Awakened One, non-terminal Final Silence, Mandala Breaks, Divine Sovereignty, Mandala of Nations, and Powers of the Awakened. The stable terminal Final Silence art and audio registration remains compatibility material. The current terminal transition is presented by Fallout.
- World-end scenarios: terminal Final Silence records `world_end_final_silence_completed` as stable cause memory and requests the shared Fallout aftermath before the world-end lock is written. Non-terminal Final Silence sets `holy_realm_final_silence_nonterminal_completed` without ending the world, while the retained interruption flags support legacy reconciliation.
- Settings: `world_end_disabled` prevents the terminal Fallout aftermath and leaves Final Silence on its non-terminal Empty Seat branch.
- Triggerable scenarios: the shared scenario registry preserves the legacy Final Silence selection identity, but selecting it redirects to Fallout. There is no separate normal or thermonuclear Final Silence launcher in the current scenario window.
...
- Condemnation: the current terminal branch hands world-end consequences to Fallout. Final Silence remains available as stable historical cause memory for compatibility and attribution.
- Deaths: the shared death ledger retains `chaos_meter_deaths_reason.final_silence` so previously recorded or compatibility-path deaths remain attributable. New terminal completion does not run the former `75%` or `95%` Final Silence state-death wave.
- Air cleanliness: terminal contamination, irreversibility, and treaty consequences are owned by the Fallout aftermath coordinator rather than a Final Silence-specific lock.
- Nuclear and wasteland logic: the regional strike-wave effects and scheduled callback events remain in place only for compatibility with previously queued chains and legacy flags. The current terminal ritual does not schedule those waves. It requests the maximum-intensity Fallout aftermath.
...
- Doctrine balance: the built-in balance of power uses `common/bop/003_holy_realm_bop.txt`, with Vow and Final Silence sides. It gives range modifiers, stores current doctrine pressure flags, and is moved by decisions, focuses, and teaching events. Final Silence launch requires the Final Silence side to dominate the balance.
...
Early identity and refuge focuses use short costs so the Realm becomes active quickly. Mountain Refuge, Bodhisattva government, refugee shelter, guarded passes, and the Seal of Refuge provide large stability, political organization, protected manpower, infrastructure, supply, bunkers, and legitimacy without early villain framing. Peaceful Himalayan unity adds `holy_realm_himalayan_unity`. Forced cleanup through `THR_complete_himalayan_mandala` instead adds Mandala Reach and Compassion Drift memory.
...
Governance choices have later route-specific follow-up focuses. `THR_abbot_examiners` lets Council of Abbots governance open the fourth active teaching seat before the wider teaching network is complete. `THR_regent_pass_watch` turns Protector Regent governance into capital-pass defenses, support equipment, and command capacity at the cost of a small Defilements increase. `THR_pilgrim_refuge_courts` lets Pilgrim Assembly governance widen the low-stability humanitarian compact path and adds refugee manpower, Mandala Reach, and Sangha Cohesion.

The meditation route continues after awakening. `THR_return_to_the_seat` adds a post-Buddhahood Dhyana payoff and unlocks `THR_renew_vow_under_fire`. That emergency decision is visible from the awakened meditation seat, requires the fourth Dhyana, a controlled capital, and a real chaos threat to the Realm, a neighbor, or a compact ally. It spends Command Power, support equipment, and a small amount of Stability to restore Meditation Charge faster, add Detachment, reduce Defilements, and record Wrathful Protection readiness.
...
If global Chaos is above `1000`, `world_end_disabled` is not set, and no other world-end is active, the terminal branch records Final Silence cause memory and requests the maximum-intensity Fallout aftermath before the shared world-end lock is written. Otherwise completion uses the non-terminal Empty Seat branch, leaves `world_end` unset, disables the armed doctrine, and shows super-event slot `8`. After `THR_empty_seat`, `THR_witnesses_keep_the_record` sends the aftermath back through prior teaching and compact contact: valid witnesses gain opinion, stability, and, for compact members, war support, while the Realm gains Sangha Cohesion if at least three witnesses answer.

The former thermonuclear Final Silence flag, regional payload constants, texture, and distinct audio wiring remain compatibility surfaces for retained callbacks and historical state. They are not a separate current triggerable-scenario or terminal-ritual route. Current terminal completion enters Fallout at maximum intensity.
...
Leader stage portrait aliases are registered in `interface/chaosx_characters.gfx`. The static fallback portrait DDS files live in `gfx/leaders/003_holy_realm/`. The former source and processed review files and contact sheet under `docs/assets/003_holy_realm_buddhahood/leader_portrait_static_fallbacks/` were deleted by Git commit `87d441ac75`. Retain that path as historical evidence because no replacement review archive is recorded here. Country leader portrait effects use the static aliases. The Buddha Mandate stage also has the animated sprite `GFX_portrait_THR_buddha_mandate_animated` backed by `gfx/leaders/003_holy_realm/portrait_THR_buddha_mandate_animated.dds`, and the Empty Seat aftermath has `GFX_portrait_THR_empty_seat_animated` backed by `gfx/leaders/003_holy_realm/portrait_THR_empty_seat_animated.dds` for event-log and UI presentation surfaces.
...
Each achievement uses completed, grey, and not-eligible DDS files in `gfx/achievements/`. The former source PNG, processed PNG, contact-sheet, and manifest archive `docs/assets/003_holy_realm_buddhahood/manifest.md` was deleted by Git commit `87d441ac75`. The runtime triplets remain under `gfx/achievements/`, and no current source archive is recorded here.
...
- `gfx/super_events/003_holy_realm/super_event_the_final_silence.dds`
  ...
  - registered in `interface/chaosx_super_events.gfx`. It currently shares the terminal Final Silence texture.
...
- `gfx/super_events/003_holy_realm/super_event_divine_sovereignty.dds`
  ...
  - registered in `interface/chaosx_super_events.gfx`. It fires when Divine Sovereignty is accepted under high crisis, high Final Silence Pressure, or prior coercive doctrine.
- `gfx/super_events/003_holy_realm/super_event_mandala_of_nations.dds`
  - referenced as `GFX_super_event_mandala_of_nations`
  - registered in `interface/chaosx_super_events.gfx`. It fires when the Mandala of Nations is formed.
- `gfx/super_events/003_holy_realm/super_event_powers_of_the_awakened.dds`
  ...
  - registered in `interface/chaosx_super_events.gfx`. It fires when anti-chaos Buddha powers are first demonstrated.

Super-event audio:

- The Buddha Mandate, audio ID `7`:
  - music file: `sound/003_holy_realm/super_event_7_buddha_mandate.wav`
  - sound definition: `chaosx_super_event_buddha_mandate_track`
- Final Silence, audio ID `8`:
  - music file: `sound/003_holy_realm/super_event_8_final_silence.wav`
  - sound definition: `chaosx_super_event_final_silence_track`
- Thermonuclear Final Silence, audio ID `9`:
  - music file: `sound/003_holy_realm/super_event_9_final_silence_thermonuclear.wav`
  - sound definition: `chaosx_super_event_final_silence_thermonuclear_track`
- The Mandala Breaks, audio ID `10`:
  - music file: `sound/003_holy_realm/super_event_10_mandala_breaks.wav`
  - sound definition: `chaosx_super_event_mandala_breaks_track`
- Divine Sovereignty, audio ID `11`:
  - music file: `sound/003_holy_realm/super_event_11_divine_sovereignty.wav`
  - sound definition: `chaosx_super_event_divine_sovereignty_track`
- Mandala of Nations, audio ID `28`:
  - music file: `sound/003_holy_realm/super_event_28_mandala_of_nations.wav`
  - sound definition: `chaosx_super_event_mandala_of_nations_track`
- Powers of the Awakened, audio ID `31`:
  - music file: `sound/003_holy_realm/super_event_31_powers_of_the_awakened.wav`
