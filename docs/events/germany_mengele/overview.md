...
The registry includes `weaponize_the_zombies`, `black_plague_weaponization_program`, Mengele's dedicated `sp_mengele_cloning`, and the nine non-cloning Event 016 prototypes: computation, advanced materials, biomedical acceleration, quantum transit, autonomous cognition, paleogenetics, xenobiological synthesis, alien arms, and temporal mechanics. Black Plague weaponization uses Event 020's native disease board, payload stockpile, delivery history, accidents, and aftermath without requiring the outbreak event to fire first. Mengele completion uses separate provider history and never creates a Kruger host, Event 016 capacity ledger, facility target, evolution, containment state, or Singularity state. Computation, materials, and biomedical prototypes grant Directorate-owned modifiers. The other six grant their provider-neutral operational technology through the shared custom-technology API, including its capped templates, equipment access, and Event 019 provider registration. Black Plague is also present in the reusable CBRN random/project registry, so future special projects must be reviewed against those registries before acceptance rather than creating parallel project or disease ledgers.
