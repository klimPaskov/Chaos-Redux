...
The `sp_mengele_cloning` special project is unlocked by the full-authorization proposal or by the Directorate special-project registry. Completing it sets `germany_mengele_cloning_project_completed`, grants the Mengele refinement of the shared clone system, and lets each controlled `biowarfare_facility` produce `clone_equipment_1` every week. Physical clone equipment in the stockpile provides the shared reserve-manpower output and can be captured or transferred like other equipment. If Germany completes the project while the Mengele program is still active, `germany_mengele.24` fires and calls the same `germany_mengele_start_coup` path used by the ordinary coup chain.
...
`make_random_directorate_special_project_researchable` and `make_all_directorate_special_projects_researchable` are the reusable special-project registry for the Directorate. They set research-availability flags instead of completing projects directly. Earlier research focuses reveal one uncompleted project at a time with equal family weights; `MCL_black_archive_release` and `MCL_directorate_project_registry` expose the complete portfolio, so Directorate progression cannot permanently miss a family.

The registry includes `weaponize_the_zombies`, `black_plague_weaponization_program`, Mengele's dedicated `sp_mengele_cloning`, the D’Rhondan envoy craft, and the nine non-cloning Event 016 prototypes: computation, advanced materials, biomedical acceleration, quantum transit, autonomous cognition, paleogenetics, xenobiological synthesis, alien arms, and temporal mechanics. Black Plague weaponization uses Event 020's native disease board, payload stockpile, delivery history, accidents, and aftermath without requiring the outbreak event to fire first. Mengele completion uses separate provider history and never creates a Kruger host, Event 016 capacity ledger, facility target, evolution, containment state, or Singularity state. Computation, materials, and biomedical prototypes grant Directorate-owned modifiers. The other six grant their provider-neutral operational technology through the shared custom-technology API, including its capped templates, equipment access, and Event 019 provider registration. The D’Rhondan craft gate maps completed Directorate Alien Arms, Materials, and Computation work together with `rocket_engines` and `atomic_research` to the five required operational domains. This is a D’Rhondan-specific bridge for the two families absent from Mengele’s cloned Event 016 portfolio, not an identical Event 016 stage-ledger receipt. Black Plague and the guarded D’Rhondan craft completion are also present in the reusable CBRN random/project registry, so future special projects must be reviewed against those registries before acceptance rather than creating parallel project or disease ledgers.

`sp_mengele_cloning` remains the sole Mengele cloning project and grants the Mengele refinement of the shared clone system. The parallel Event 016 cloning prototype remains restricted to Kruger's host. Strategic Singularity components remain restricted to the Kruger State because they are a terminal world-end weapon rather than a reusable technology family. Standard biowarfare projects such as anthrax, ordinary plague, tularemia, and smallpox keep their normal special-project definitions; Black Plague weaponization remains a distinct project even though its offensive completion shares the plague delivery technology.
...
- music file: `sound/003_holy_realm/super_event_12_angel_directorate.wav`
- sound definition: `chaosx_super_event_angel_directorate_track`
- sound derivative: `sound/003_holy_realm/super_event_12_angel_directorate.wav`
