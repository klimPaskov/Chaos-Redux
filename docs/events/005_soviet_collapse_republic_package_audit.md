...
The tags are vanilla country tags. Event 005 does not redefine their base country files; it uses vanilla tag registration and vanilla history, keeps vanilla country localisation where present, adds Event 005 country-name localisation where vanilla coverage is missing for active release ideology, and adds the crisis package at runtime.
...
- all 23 audited tags have country localisation coverage from vanilla or Event 005
- all 23 audited vanilla-supported tags use vanilla default flags; Event 005 has no mod-side `gfx/flags/<TAG>*.tga` overrides for them
- explicit existing-tag flag changes are route/cosmetic only, currently represented by `UKR_BLACK_BANNER` from the Ukraine black-banner focus route
