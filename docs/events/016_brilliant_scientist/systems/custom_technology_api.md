...
An external operational grant enables the matching generic battalion, recreates its locked example template without an artificial division cap, reopens production of the matching custom equipment where that family uses equipment, and registers the existing Event 019 neutral provider row. Equipment, fuel, manpower where applicable, and production capacity remain the scaling constraints. Rebuilding Event 016 clears and reconstructs ordinary project-derived runtime state while preserving the independent external knowledge ledger. Alien Infantry remains the explicit exception: an alien grant enables laser production and contact consumers but never exposes the battalion to ordinary recruitment.
