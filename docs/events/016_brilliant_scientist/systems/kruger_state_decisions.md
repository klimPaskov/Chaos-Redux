...
The current Event 016 biological implementation surface is documented in [`biological_operations.md`](biological_operations.md). Ordinary industry creates the concrete equipment; the eighteen agent-and-method raid IDs reserve it through native raid payment and dispatch guarded country-scope outcomes. Event 016 has no separate biological production, staging, or attack-decision receipt. Tranche 4 source remains under review and MCP transaction acceptance is unresolved, so this documentation is not a final acceptance claim.
