...
The native raid surface itself is active independently of that queued ledger. Countries may manually launch every ordinary biological, battlefield, captured-facility recovery, hostile weaponized-zombie, friendly weaponized-zombie, and anti-zombie-cure raid whenever the existing native authority, policy, staging, target, aircraft or formation, and payload checks pass. A living Kruger in the current host or active Kruger State multiplies the native AI willingness for those operations without auto-launching them or changing their strength. Portal Warfare from the weaponization stage additionally exposes `brilliant_scientist_portal_facility_raid` in the native `brilliant_scientist_raids` category; it consumes the existing `kruger_portal_equipment` archetype, requires `kruger_portal_raider`, an authenticated transit network with an owned terminal, and strikes a hostile strategic-facility state from a supply node. Native preparation, reservation, cancellation, expiry, outcome selection, and raid history remain authoritative, while Event 016 records persistent actor and target history for later follow-up decisions.
