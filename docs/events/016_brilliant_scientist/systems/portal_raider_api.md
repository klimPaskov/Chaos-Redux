...
The evidence package is rooted at `docs/assets/shared_portal_raider_system/models_3d/portal_raider/`, but it is rejected for runtime use because the authorized Meshy 6 task omitted the mandatory ray rifle and no user-approved paid recovery has been authorized. The package therefore does not hand accepted model, entity, animation, or sound files to runtime consumers.
