...
`alien_infantry_spawn_landing_cohort` rejects materialization after world end and validates an existing ordinary reservation before using its committed equipment.
If a non-deferred public spawn call discovers contact loss, state loss, or another invalid reservation, it calls `alien_infantry_cancel_landing_reservation` before clearing the target.
That cancellation returns the one 2,000-gun reservation immediately and clears its pending receipt, so a later mission cancellation or reconciliation cannot refund it again.
Event 019 deferred materialization keeps its separate outer transaction ownership and does not enter this ordinary-reservation cleanup branch.
