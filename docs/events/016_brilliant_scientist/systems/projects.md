...
## D’Rhondan envoy craft integration

`sp_dhrondan_envoy_craft` is a separate shared air-specialization project built after Alien Arms, Rocketry, Advanced Materials, High Energy, and Computation are operational. The Kruger and KRG gate reads the existing deployment triggers and disabled-family arrays rather than accepting prototype history alone. The project is registered in `grant_random_chaos_special_project_available_tech` with its exact route and operational-work gate. The Mengele registry can expose the same craft through a separate availability receipt, using its documented D’Rhondan-specific bridge rather than creating Event 016 stage history. Full expedition, pact, landing callback, and rebellion behavior is documented in `dhrondan_contact.md`.
...
## Advanced Materials plus Rocketry qualification

Once Advanced Materials reaches Deployment, Rocketry reaches Prototype, the primary prototype works are expanded, and both facilities and ledgers remain healthy, the current host can select `brilliant_scientist_prepare_high_speed_materials_trial` on any owned, controlled, core, non-impassable state. The 180-day test corridor consumes 100 Political Power, 25 Air Experience, 150 Support Equipment, 200 Motorized Equipment, 5,000 fuel, 3,000 manpower, and three civilian factories. The selected state is saved as a global event target, so host transfer, lost control, project damage, containment, terminal commitment, and world-end cleanup clear the active corridor and paid transaction without refund.
...
| Alien Arms | `sp_brilliant_scientist_alien_arms` | Antarctic or spacecraft evidence plus Materials and High Energy | Interface-chamber history and `brilliant_scientist_alien_infantry_tech`; Event 025's one-time artifact report and Event 036's spacecraft report record evidence without replacing their physical evidence gates |
