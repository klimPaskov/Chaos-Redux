...
The institutional-conflict pair records the university network's final governance choice. `brilliant_scientist_protect_dissenting_scientists` is a one-time, 60-day decision that spends political power, support equipment, and manpower, then sets `brilliant_scientist_dissent_protected`, applies a slower-but-safer dynamic modifier, and reduces hidden Grievance while increasing Independent Capacity. `brilliant_scientist_dismiss_ethics_chair` is the mutually exclusive one-time alternative; it spends political power and support equipment, sets `brilliant_scientist_ethics_chair_dismissed`, accelerates special projects, and increases hidden Grievance while reducing Independent Capacity. Both decisions impose a temporary consumer-goods and factory-efficiency burden during their timed action, cancel when the host loses control, and feed the containment score: protected dissent strengthens the government's case, while a dismissed ethics chair strengthens Kruger's case. The two policy flags are carried with the exact host-directorate history during a verified Kruger State formation, but their dynamic modifiers remain host-only.

The university network also chooses one mutually exclusive appointment method. `brilliant_scientist_award_national_science_chair` spends 200 support equipment, 1,000 manpower, political power, and 90 days for public replication, regional prestige, and a visible foreign-intelligence target. `brilliant_scientist_rotate_grants_among_institutions` spends 100 support equipment, 500 manpower, political power, and 60 days to lower Dependence, increase Independent Capacity, and make the network resilient at the cost of slower concentrated progress. `brilliant_scientist_let_kruger_select_institutions` spends the same logistics allocation over 45 days for faster project delivery and higher Mandate, but increases Dependence and Grievance and leaves excluded scholars vulnerable to recruitment. Each route has a persistent host-only modifier, an AI policy preference, and a carried history flag; the appointment-method lock prevents stacking the three governance styles into an unearned reward ladder.

Containment reads these institutional receipts as causal history rather than flavour: the national chair and rotating grants strengthen the government's sovereignty score, while Kruger's personal selection strengthens his opposing score. This keeps public legitimacy, network resilience, and personal appointment authority relevant when the later board compares release, arrest, exile, shutdown, concession, and sovereignty outcomes.

Electronics and Teleportation also have one explicit cross-project countermeasure. Once both ledgers reach Prototype, the host may run `brilliant_scientist_establish_portal_calibration_network` from this category. The 150-day programme requires both valid facilities, an idle project board, healthy non-stolen records, 85 Political Power, 250 Support Equipment, 125 Motorized Equipment, 2,000 fuel, 2,500 manpower, and two civilian factories. It writes one `brilliant_scientist_portal_calibration_network_established` receipt and the matching Kruger character-history flag; it does not advance a project, grant capacity, create a unit, or add an Event Log entry. While the receipt remains active on the current host, Teleportation accident pressure is reduced and foreign operations targeting Teleportation face a success penalty and higher detection. Transfer, containment, facility loss, incidents, and terminal locks cancel unfinished work without refund; ordinary transfer and Kruger State formation retain the completed history without creating a free terminal or active host countermeasure.

Advanced Materials and Rocketry have a separate geographic qualification action. At Materials Deployment, Rocketry Prototype, expanded prototype works, two valid facilities, and healthy ledgers, the host can target any owned, controlled, core, non-impassable state with `brilliant_scientist_prepare_high_speed_materials_trial`. The trial occupies three civilian factories for 180 days and consumes 100 Political Power, 25 Air Experience, 150 Support Equipment, 200 Motorized Equipment, 5,000 fuel, and 3,000 manpower. Its saved state target and transaction flags are cleared without refund on transfer, lost control, damaged ledgers, containment, or terminal cleanup. The resulting `.195` report lets the AI and player choose a national qualification board or Kruger's proprietary flight envelope. The national board lowers Dependence and Project Capacity, raises Independent Capacity, improves air safety, and remains a former-host institution; the proprietary envelope raises range and air mission efficiency, increases Rocketry accident pressure, and follows Kruger through ordinary transfer or fixed-tag formation. Only the existing ten settlement receipts bias the AI and facility-clause wording; no new country route, model, CBRN callback, Event Log row, or spreadsheet row is introduced.
...
The accepted addendum promotes ten conditional choices inside the existing assistant-conflict event `chaosx.nr16.5`. The generic `.5.a`, `.5.b`, and `.5.c` options remain unchanged. The national options cover `ENG` and `FRA` in public-science or distributed-research context, `USA` and `CZE` in industrial-mobilization or distributed-research context, `SOV`, `JAP`, `GER`, `ITA`, and `CHI` in strategic-security or industrial-mobilization context, and `POL` in public-science, distributed-research, or strategic-security context. Unnamed countries retain the complete generic option pool.

Each national option clears `brilliant_scientist_context_assistant_conflict_pending`, sets `brilliant_scientist_assistant_conflict_resolved`, calls its named base resolver and settlement resolver, and calls `brilliant_scientist_try_schedule_impossible_lecture` once. The settlement receipt is host-local institutional history and is not a Kruger character flag. It is intentionally absent from `brilliant_scientist_transfer_to_target`, the ordinary context-copy block, and the Kruger State formation-copy block. Transfer before `.5` uses the recipient's tag; transfer after `.5` leaves the former host receipt local and gives the recipient only already-supported generic assistant state.

The accepted total vectors are British research associations `(+5, -10, +15, +5, +20, -15)`, American federal contracts `(+10, +5, +10, +15, +5, -5)`, the Soviet Academy plan `(+5, +20, -5, +15, -5, +20)`, the Japanese RIKEN council `(+15, +5, +5, +10, +5, +5)`, German research board `(+10, +20, -10, +20, -15, +20)`, French laboratories `(+10, -10, +15, +5, +25, -20)`, Italian procurement compact `(+20, +10, +15, +15, +5, -5)`, Chinese technical bureau `(+5, +25, -10, +20, -20, +15)`, Polish university shelter `(+15, 0, +10, +5, +15, 0)`, and Czechoslovak research charter `(+5, -15, +15, +15, +20, -15)` in Mandate, Dependence, Exposure, Project Capacity, Independent Capacity, and Grievance order. Numeric inputs belong in `common/script_constants/016_brilliant_scientist_country_settlement_constants.txt`, and the resolvers use the existing bounded `brilliant_scientist_change_*` effects.

The `.7` and `.8` player-facing three-way choices remain unchanged. A receipt adds only its facility or custody description clause and the named AI factors. The British receipt prefers civic compact and public trust and cautions toward restricted district and executive reserve; the American receipt prefers industrial charter and patent pool; the Soviet receipt prefers restricted district and executive reserve and cautions toward civic compact and public trust; the Japanese receipt prefers industrial charter and executive reserve; German and Italian receipts prefer state-directed industrial choices but become cautious under high Exposure or Dependence; French and Polish receipts prefer public or university custody but become cautious under high Exposure or Grievance; Chinese and Czechoslovak receipts prefer industrial or technical custody but become cautious under high Exposure or Dependence. Existing high-pressure factors can reverse every preference. The finite layer reuses the registered `.5`, `.7`, and `.8` images and adds no model, GUI, audio, animation, sprite, flag, portrait, super-event, or separate country chain. Runtime transfer, cleanup, and quantitative balance evidence remain pending, so this documentation promotion is not a completion claim.
...
The accepted settlement layer centralizes its tuning in `brilliant_scientist_country_settlement_ai`: `option_base = 10`, `option_preferred_factor = 2.25`, `option_cautious_factor = 0.50`, `reaction_preferred_factor = 1.50`, and `reaction_cautious_factor = 0.70`. The national `.5` choices use the base and preferred factor only in their compatible context, then apply the existing host-archetype factor and the named war, Exposure, Dependence, or Grievance caution checks. No factor is zero, country identity never forces an outcome, and the existing high-pressure modifiers remain able to reverse the preference. These values are accepted for implementation but are not yet runtime evidence.
