...
`africa_clear_runtime_context` clears the global event targets, country flags, and runtime variables owned by this foundation API, including the unifier, Charter League leader, triggerable scenario actor/target markers, RSA branch targets, RSA branch state, and World Is One gate state. It does not clear terminal world-end flags after the World Is One scenario starts.
...
Scenario intensity changes manpower, stockpiles, colonial alarm, and regional-authority seeding. The Fragile Unifier type selects a small African-capital host where possible or seeds the West African Congress fallback, then starts with lower authority and higher restoration strain. The Ally Under Attack type seeds a Charter League authority under outside attack, stores the ally and selected holder as global scenario targets, flags both countries for AI posture and audit visibility, and opens aid, corridor, defensive, and liberation responses immediately. If no holder can be selected despite the launch gate, the unifier receives `africa_triggerable_scenario_ally_no_holder_found` so scenario validation can see the failed setup. The Liberation League type opens the Liberation War Office, starts the liberation mission state, registers outside African holders, and declares one opening anti-colonial war if a valid outside holder exists. High-Chaos Covenant opens the Authority Atlas and unlocks Bestiary package activity. Continental Pole opens the post-unification Scramble, sponsor staff, proof ledger, one-charter, and route-validation logistics surface. At high intensity it also fills the dossier, regional-authority, living-core, and high-chaos validation counters that those late decisions expect. At maximum intensity it adds the external continent-ready hooks and Totalen Chaos tier needed for the proof/certification route. It does not set proof-verified flags, `all_continent_unifiers_world_end_ready`, `africa_world_is_one_gate_prepared`, `world_end_africa_world_is_one`, or any terminal World Is One flag, so the terminal branch remains behind the visible proof, certification, and final gate decisions.

## Selected Unifier Origin Profiles

`africa_prepare_runtime_context_from_this` now applies `africa_apply_unifier_origin_package` immediately after the Event 012 core values are initialized. The helper clears any stale origin profile, classifies the selected host, applies one visible origin spirit, moves the mapped Event 012 values, grants a small matching force/logistics package, and then clamps the visible values. The classification is tag-based so manual scenario seed hosts and existing releasables use only registered countries:

- `ETH`: Highland Legacy.
- `LIB`: Atlantic Return Route.
- `SAF`: Union Rupture.
- `EGY`, `SUD`, `ERI`, `DJI`, `SOM`: Nile Sea Gate.
- `WAC`, `GHA`, `MLI`, `SEN`, `NGA`, `GNA`, `VOL`, `DAH`, `IVO`, `SIE`, `GAM`, `TOG`, `NGR`: Western Congress Ports.
- `CBC`, `COG`, `ANG`, `CMR`, `EQG`, `GAB`: Congo River-Forest Mandate.
- `IOC`, `MAD`, `MZB`, `KEN`, `TZN`: Indian Ocean Gate.
- Any other valid selected unifier receives the General Congress Mandate general profile.

The profile sets `africa_unifier_origin_profile`, one `africa_origin_profile_*` flag, and one `africa_origin_*_spirit`. The Continental Congress category header shows `GetAfricaUnifierOriginProfileName`, so the selected host identity is visible beside the live value board. Cleanup calls `africa_clear_unifier_origin_package` from `africa_clear_runtime_context` to remove origin flags, ideas, and variables. Tuning lives in `africa_unifier_origin_profile`, `africa_unifier_origin_delta`, and `africa_unifier_origin_force` inside `common/script_constants/012_africa_constants.txt`.

The origin profile also opens a one-active-at-a-time mandate case after `AFR_the_charter_mandate`. `africa_open_origin_mandate_case` spends political power, support equipment, and profile-specific logistics: rifles and manpower for highland, RSA, west-Atlantic, and general congress hosts; convoys and command power for Atlantic Return and Indian Ocean hosts; trains and command power for Nile/Red Sea hosts; and motorized equipment plus army experience for Congo/forest hosts. The decision starts `africa_origin_mandate_case_mission`, a 120-day deadline that requires the capital to remain controlled and the profile's visible value gates to hold. Success applies a second profile-specific value movement and marks the case filed; timeout lowers Legitimacy and Authority while raising Colonial Alarm and Restoration Debt, then leaves the case retryable. The standard decision header and the Continental Congress scripted GUI expose `GetAfricaOriginMandateCaseStatus`, and `GetAfricaOriginMandateRequirementSummary` shows the tuned gates through global constant mirrors.
...
Core values are visible in the Event 012 decision category descriptions and move through the selected unifier origin profile, focuses, decisions, wars, state control, foreign influence, dossier work, high-chaos packages, and AI. Covenant Pressure is surfaced in the high-chaos decision header and moves through Bestiary unlocks, habitat terms, omen review, actor binding, package-specific outcomes, actor-specific Bestiary actions, non-actor Bestiary actions, warnings, and containment mission outcomes.

The Continental Congress scripted GUI is attached to the main congress decision category. It shows the selected origin profile through the category header, plus the core value board, current dossier, selected dossier seat, dossier profile, current Bestiary case, active warning/status line, a regional seats card, a Bestiary seat card, a dossier target card, a current-operations target card, a warning docket card, and action buttons for Congress, Register, Dossier, Sponsor, Seats, and Terms. These buttons call scripted GUI handlers, spend the same political-power, equipment, manpower, convoy, and command-power layers as their decision equivalents, and use timed recent-action flags to prevent repeat-click loops. The Seats action binds controlled regional authority seat states into Charter subjects; the Terms action negotiates the one-time Bestiary habitat terms from the Congress panel. A visual strip below the cards uses static fallback sprites and route-gated animated overlays for the Charter banner, Authority Atlas seal, and Bestiary warning seal.
...
The 25 created tags keep direct country localisation where a region or polity name is the public state identity: West Africa, Sahel, Maghreb Coast, Nile-Horn, East Africa, Great Lakes, Congo Basin, Zambezi-Stone Cities, South Africa, Indian Ocean, Gorilla Highlands, Baobab Senate, Tidemark Dominion, Ananse Web, Orisha/Vodun Nature Courts, Crocodile Rivers, Chimpanzee Telegraph League, Okapi Court, Termite Citadel Engineers, Honeyguide Commons, Great Herds, Bonobo Kinship Congress, Hyena Radio Dominion, Bird of the Walls, and Sao Terracotta Host. Institutional labels such as congresses, councils, charters, courts, hosts, and line offices remain appropriate for leader/court display names, party names, staff roles, focus groups, and mechanics where they represent internal bodies rather than replacing a direct public country name.
