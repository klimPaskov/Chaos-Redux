...
- settled historical dossiers can start a one-at-a-time local resistance watch while the register continues to the next case: observer settlements must keep the stored seat secure while maintaining Regional Trust and League Cohesion, while direct Archive settlements must keep Authority high and Restoration Debt below the listed cap. The active watch keeps its own stored dossier id and seat state so later selected dossiers do not overwrite the report context, and new dossier settlements are blocked until the current watch resolves. Observer watches can be resolved through a timed Congress mediation action that spends support equipment, manpower, and profile logistics; direct Archive watches can be resolved through a timed enforcement action that spends infantry equipment, support equipment, manpower, command power, and profile logistics. Profile logistics reuse the dossier survey geography: convoys for Nile, river, lake, and ocean cases; trucks and army experience for Sahel cases; infantry escorts for desert and western-crown cases; and trains for southern stone-seat cases. These interventions are one-at-a-time, block the passive success path while active, record whether the case was mediated or enforced, and apply profile-specific value movement before firing the visible local report. Failure records local resistance, raises sovereignty/debt pressure, and fires a visible dossier report.
...
`common/ai_strategy/012_africa.txt` gives the unifier broad route postures and differentiates Event 012 created subjects by role and by tag. The unifier has route bands for liberation, General Staff, Crown Congress, diaspora return, RSA emergency handling, Authority Atlas survey work, observer and direct-Archive resistance watches, eight historical dossier profiles, and world-order sponsorship; the sponsor band prioritizes convoys, support equipment, dockyards, and restraint once the sponsor office, export office, charter staff, cross-continent charters, proof ledger, certification, or one-charter terminal preparation are active. The historical dossier profile bands react to the active selected dossier profile and to opened profile flags, nudging Nile/Red Sea and Indian Ocean records toward convoys and dockyards, Sahel records toward motorized logistics, western/Great Lakes records toward support and restraint, central river records toward convoys and habitat infrastructure, and southern stone records toward infrastructure, factories, and infantry. Opened-profile flags intentionally stack as institutional memory: once a profile has entered the Archive, its logistics posture can continue to influence the unifier while later profiles are surveyed. The Authority Atlas decisions also weight local offices, guards, observer settlements, and direct Archive settlements by the active route and dossier profile. Regional authorities have separate AI bands for West/Sahel mobility, coastal/rail networks, interior guard states, and southern industry plus one per-tag posture for all ten created regional authorities. High-chaos actors have separate forest/covenant, river/coast, Ananse support, signal/court, citadel-engineer, and Great Herds supply postures plus one per-tag posture for all fifteen explicit Bestiary actors. These are still shared companion trees with tag-specific capstones rather than full bespoke country focus trees, but they prevent every created actor and every dossier profile from using a single identical build, production, and settlement priority.
...
- selected-dossier survey missions, local office, old-seat guard, guarded settlement, and local resistance mediation/enforcement;
