...
`chaosx.nr11.900` is a hidden, triggered-only AI test entry. It can be invoked only from a valid AI country scope and temporarily opts that country into the normal target initializer. The initializer records the run as forced through `achievement_secret_alliance_forced_origin`, which disqualifies all six Event 011 achievements. The event is not a player route, never appears in player-facing content, and does not replace SCN-009.
...
The final outcomes are target victory, coalition victory, negotiated settlement, internal rupture, leadership split, dissolution, or a continuing regional bloc. Coalition victory does not automatically preserve the faction. Sponsor collapse and major rivalry can split it. Spoils doctrine fractures over unresolved promises unless the coalition remains unusually coherent. Containment normally closes after terms are imposed but can retain a narrow security council when Resolve, membership, and leadership remain strong. A major-led regime-pressure coalition can survive, while a weak punitive coalition dissolves.
