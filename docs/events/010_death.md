...
Death reveals itself only when it consumes a mainland coastal state above the configured population threshold, and the mainland route remains locked until the first island-report evolution has been recorded and enough island pressure has built up. The reveal pans player cameras to the revealed state, fires super-event `62`, makes Death a world threat source, declares war on neighboring countries, immediately consumes unguarded neighboring states from the mainland wasteland, and enables containment decisions. After reveal, Death runs daily public pressure from its tag-scoped on-action: living units in active wastelands are withered every day, unguarded neighboring states fall immediately from any mainland Death wasteland, and scheduled pulses are shortened for stronger ghost refreshes, wither checks, and cooldown-limited coastal jumps.
