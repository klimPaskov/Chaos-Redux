...
World-end starts only when Death has consumed a whole continent and Chaos is above the Death world-end threshold. This fires super-event `63`, swaps Zol to the world-end portrait, creates one coastal foothold on each continent that does not already contain Death, and unlocks the strongest host behavior. World-end footholds prefer empty non-capital coasts, then relax watch/quarantine and defense filters before falling back to any valid coastal state; a successful foothold pulls its owner/controller into war if needed. If every populated state is consumed, super-event `65` fires.
