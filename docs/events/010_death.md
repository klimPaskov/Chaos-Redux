...
- Event log: Death actor mapping, event details, and four milestone evolutions in the shared event-log scripted effects/localisation. `The First Silence` and delayed Missing Island reports are baseline crisis flow, not evolution entries. The Event Details catalog lists the Island Pattern, mainland reveal, Last Shores, and whole-world-consumed milestones, while the actual evolution records are still written only when their stage and Chaos-tier gates are reached. Early Death evolution detail pages use the normal Zol portrait; Last Shores and whole-world-consumed detail pages use the subtle animated world-end Zol portrait.
...
Missing Island decisions are limited to countries that receive delayed reports. They can send a survey boat, check records, issue quiet quarantine instructions, or dismiss the matter. Surveying can expose Death before the world learns its name and preserves quiet-defeat tracking.
...
Dark Methods are a living-country desperation route. A threatened country can open Black Book offices, bind unburied names into weak capped hosts, interrogate empty roads for Death pressure intelligence, seal names in iron, and burn the Black Book before exposure becomes a domestic scandal. The route uses exposure, bound names, and mourning debt variables, keeps AI use conservative, and records the hidden Book Burner predicates only when the country used a Dark Method, burned the book before the high-exposure threshold, and later helped defeat Death.
...
After Death is defeated, active war and compact decisions close. Recaptured wasteland decisions remain available so living countries can survey dead zones and build outposts. Dead-zone outpost tracking starts only after the custom defeat path has fired.
