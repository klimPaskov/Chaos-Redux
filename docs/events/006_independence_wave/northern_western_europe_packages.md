...
The current whole-event authority is 32 content-attested packages across 29 compatible reservation groups with 161 selectable rows still unattested, served by 40 central/runtime adapters. Eight adapter-only rows remain fail-closed: IW-013 NAV, IW-015 GLC, IW-043 CHU, IW-058 ASY, IW-093 DOX, IW-098 SOK, IW-177 FIJ, and IW-179 FSM; older arithmetic in the dated regional snapshots below is historical traceability only. IW-045 Bashkiria is now admitted outside this regional reference.
...
## Historical regional admission snapshot (2026-08-03; superseded for event-wide counts by the 2026-08-05 Transcaucasus amendment)
...
The current portrait archive authority is `docs/assets/portraits/006_independence_wave/`: original source files are kept directly in the parent, exactly one child folder `processed/` contains processed assets and metadata, and no `156x210` image files are retained in either archive location. CHU Karim Tinchurin v3, the other CHU evidence rows, the two MNT masters, and ASY Haydo remain `needs_user_review` for rights/date or provenance evidence; ASY Barsoum v93 passes its independent audit and v94 promotes only the existing concordat-council DDS consumer, without wider IW-058 admission.
...
The current Event 006 authority is `docs/plans/006_independence_wave_plans/006_source_of_truth_map.md` and the current resume packet. Nine Northern and Western Europe packages are statically content-attested: IW-001, IW-002, IW-004, IW-006, IW-007, IW-008, IW-009, IW-010, and IW-012, across their compatible reservation groups. Event-wide, the current allocator boundary is 32 content-attested packages across 29 compatible reservation groups. The central dispatcher exposes 40 package adapters, with eight adapter-only fail-closed rows: IW-013 NAV, IW-015 GLC, IW-043 CHU, IW-058 ASY, IW-093 DOX, IW-098 SOK, IW-177 FIJ, and IW-179 FSM. IW-038 RUT, IW-040 KUB, IW-044 TAT, and IW-045 BSK are admitted under their vanilla-carrier package contracts and documented asset authorities. FORM-07 and FORM-16 remain separate fail-closed formable surfaces. The dated portrait-shelf paragraphs remain historical traceability only and do not override the current package and portrait authorities.

The IW-012 route-arbitration source closure is recorded after commits `3570ed8ff` and `72d8549e3` in `subagent_handoffs/006_iw012_decision_ai_reaudit_2026_07_28.md` and `subagent_handoffs/006_iw012_formal_route_ai_closure_implementation_2026_07_28.md`. These static findings do not close live allocator, host-survival, force, save/load, route-AI timing, shared-focus visibility, scenario, or synchronized transaction evidence. The regional package remains under the overall **HOLD / PARTIAL** runtime disposition. The v105 focus receipt closes the former Event 006 geometry blocker under normal spacing, while focus visibility and runtime selection remain separate bounded evidence.
...
Scotland uses the sourced R. B. Cunninghame Graham character token `SCO_independence_wave_civic_convention` for its civic routes and the sourced Victor Morven Fortune token `SCO_independence_wave_territorial_commandant` as its emergency head and corps commander. Wales uses the independently approved and runtime-wired J. H. Thomas token `WLS_independence_wave_national_council` as its civic leader and the independently approved and runtime-wired Major George Frederick Myddleton Cornwallis-West token `WLS_independence_wave_mountain_commandant` as its mountain commandant.

These are guarded runtime Event 6 characters, not replacements for vanilla characters. Setup retains stable `156x210` sprite consumers with `set_portraits`; Event 6 defines no commander miniature, advisor-card, dossier, or `_small` portrait. Scotland's two sourced real-male treatments and Wales's two named treatments are approved and wired. The Lewis Pugh Evans trials remain historical evidence only and do not represent the current runtime identity. A package cannot pass content attestation while one of its required identities is source-blocked or its complete portrait set has not passed visual review.
...
| Wales | Major George Frederick Myddleton Cornwallis-West | `portrait_WLS_independence_wave_mountain_commandant.dds` | archival source, source-locked HOI4 commander repaint, independent likeness/style/provenance pass, and byte-matched runtime DDS; Welsh-born Scots Guards role wording retained |
...
Gameplay adapters, projects, focus hooks, ideas, AI, localisation, exact setup validation, source-backed Scottish leaders, asset-neutral advisor boards, and FORM-01/02 integration are implemented. IW-001 Scotland and IW-002 Wales are admitted after their complete sourced rosters and post-wiring package audits. The rejected Lewis Pugh Evans commandant trials remain evidence-only; no portrait tranche alone admits a package. The runtime allocator still enforces host survival, unique anchors, reservations, Event 5 separation, and wave capacity. Wales's installed vanilla flag retains its 1959-layout caveat in the asset audit. No dormant vanilla history file receives a content-readiness flag.

Admission correction (2026-08-01): IW-002 Wales is admitted after the independently approved J. H. Thomas civic and Cornwallis-West commander promotions. The stable runtime token `WLS_independence_wave_mountain_commandant` carries Cornwallis-West; the rejected Lewis Pugh Evans trials remain evidence-only and do not reopen the package gate.
