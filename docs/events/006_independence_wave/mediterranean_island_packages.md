...
IW-018 Sardinia now has unchanged sourced originals for Emilio Lussu, Sardinian-born royal-court statesman Luigi Arborio Mella di Sant'Elia, and Sardinia-linked Italian commander Vittorio Verne, each with an explicit head-and-shoulders crop, deterministic `156x210` source-placeholder candidate, and runtime DDS under the stable ARX_gavino_piras compatibility key where applicable. The three promoted runtime consumers, corrected `.gfx` sprites, characters, localisation, and post-wire package audit remain aligned. Exact Pala and Piras identities remain blocked and are not relabelled. No advisor icons or portrait derivatives were created.
