# FORM-09 Balkan Federation

## Purpose

FORM-09 turns a successful Balkan regional ambition into a visible federation without allocating another country tag. A researched `BBX` Epirus or `BAX` Thrace carrier retains its Event 006 origin package and adopts the `BLX` cosmetic identity.
...
The carrier gains the `Federal General Staff` idea after enough additional consenting members have been processed. The army-integration receipt opens `Ratify the Federal Border Board`, a paid 120-day project that funds surveyors, legal delegations, and military liaison offices. Completion improves the public Event 006 ledgers without granting claims, states, units, or cores outside the frozen settlement.

## Assets and localisation

The `BLX` flag package is stored under `docs/assets/006_independence_wave/form09_balkan_federation_flag_2026_08_09/`. Runtime flags cover all three HOI4 sizes and all ideology suffixes. The ImageGen source is retained, while the final runtime design is mechanically flattened into four solid colours so it remains vexillology rather than artwork.
