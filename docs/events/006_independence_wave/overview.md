...
006 portrait shelf now records exactly 56 original-size pre-resize masters directly
...
The canonical content gate now gives unadmitted automatic candidates zero weight and rejects a direct or stale reservation before anchor allocation. Twelve attested IDs across eleven compatible reservation groups provide a source-level exact-ten path: IW-008 RHI and IW-010 AJX share `RG-RHINE-SAAR` but are the only reciprocal pair admitted at that group's capacity of two, with distinct anchors 51 and 42. The current design decision doubles the automatic ladder to 6/8/10/14/20, including a twenty-country World Collapse band. The 6/8/10 bands are conditionally viable only when their source-level host, anchor, reservation, Event 005 collision, force, chaos-band, and transaction preflight checks pass; the 14/20 design targets have no currently admitted package or reservation capacity route and remain source fail-closed until the pool and disjoint reservation capacity are expanded. Live execution, save/load, and player-owned transaction evidence are optional future QA.

FORM-48 gameplay and its HBX carrier plus sovereign HAW/FSM member contract remain implemented, but IW-173 HAW and IW-179 FSM are not admitted, so FORM-48 is currently unreachable through a compliant carrier/member set despite IW-184 HBX admission. FORM-06 through FORM-47 remain fail-closed, including FORM-42. The current whole-event completion authority is `docs/plans/006_independence_wave_plans/subagent_handoffs/006_event_completion_audit_v33_2026_07_29.md`, which supersedes the historical v32, v31, v30, v23, and v28 whole-event snapshots for the current disposition and carries forward their bounded findings where still applicable. The f8 focus candidate was rejected and reverted by `8fddaeea3`; the later retained professional-defense coordinate reflow keeps 184 focuses and 223 connectors while reducing crossings from 49 to 45 and node intersections from 18 to 7, with 28 long connectors and the same 14 blocking diagnostics. The focus framework and whole event therefore remain **HOLD / PARTIAL**.
...
The automatic wave counts are 6, 8, 10, 14, and 20. World Collapse is also 20; its force level, instability, package rarity, and regional ambition rise with the count instead of replacing it. The repaired planner applies the canonical content-attestation trigger during weight calculation and anchor reservation, so unadmitted candidates receive zero automatic weight and a direct stale reservation fails before any host, country, or state is reserved. The 6/8/10 bands are conditionally viable from the twelve attested IDs when all source-level host, country, state, reservation, force, chaos-band, and transaction preflight checks pass; IW-008 RHI and IW-010 AJX share RG-RHINE-SAAR but are the only reciprocal pair permitted at capacity two, with distinct anchors 51 and 42; all other groups remain one-package-only. The 14/20 bands remain source fail-closed until the attested pool, reservation groups, same-host protected-remnant cases, Event 005 collisions, rollback, and synchronized source evidence are expanded. Live execution, save/load, and player-owned transaction observation are optional future QA. Overlay-only, community-variant-only, formable-or-route-only, scenario-variant-only, and unbound rows remain available only through their explicit owning systems; Open Sovereignty cannot promote them merely to fill a wave.
