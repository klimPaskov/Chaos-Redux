...
Focus content follows the creating event. Event 006-owned tags use the full `independence_wave_focus_tree` only after the frozen release transaction and package attestation. Registered carriers keep their meaningful vanilla tree and receive a reviewed additive overlay or package decisions. The additive loader is fail-closed when a carrier has not registered an owning-tree import; do not set an overlay flag and assume the engine will inject a shared focus by itself.
