...
The package-dispatch section in `common/scripted_effects/006_independence_wave_effects.txt` applies the common finalization barrier. If a package adapter reports success without either contract, its final validation is changed to failure and the frozen release cannot commit. This keeps the generic tree universal without overwriting a living meaningful carrier tree.
