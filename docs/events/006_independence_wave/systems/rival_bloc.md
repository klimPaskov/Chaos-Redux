...
- `independence_wave_rival_bloc_active`, `independence_wave_rival_bloc_league_rivalry_active`, and `independence_wave_rival_bloc_invitation_open` are contract flags. The invitation target is held by `independence_wave_rival_bloc_pending_invitation_target`. The recipient owns a separate response mission bound to the inviter, contract generation, and recipient Event 006 generation; acceptance, decline, expiry, invalidation, origin cleanup, reconciliation, reunification, reopening, and dissolution clear both sides of the invitation state.
...
| `independence_wave_rival_bloc_issue_invitation` | Leader scope with `active_target_country` pointer | Opens one generation-locked invitation, stores the target pointer for cleanup, and activates the recipient-owned response deadline after delivery | Targeted invitation decision timeout |
...
- `independence_wave_rival_bloc_duration`: invitation delivery, recipient response, acceptance, reserve, leadership, patron, and host timers.
...
3. The leader can invite one network country at a time. Delivery, recipient response, and ratification are separate timed stages. The target must accept explicitly; an unanswered response deadline records a decline through the same central cleanup path. Main-league members, founders, clients, stale generations, and countries below the network-standing gate cannot be invited.
...
The bloc intentionally uses a registry rather than a vanilla faction because the accepted Event 006 league already exposes registry values and generation-safe event-detail surfaces. Event Details shows the active route, leader, member and region counts, all six aggregate values, and a Previous/Next inspection route for every surviving member's contribution and confidence. The decision category exposes the same aggregate values to participating countries. No map rewrite, automatic world scan, or new visual asset is required.
