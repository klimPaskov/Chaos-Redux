...
Gameplay is in `common/scripted_effects/006_independence_wave_join_effects.txt` and the Join predicates in `common/scripted_triggers/006_independence_wave_triggers.txt`. Callback wiring is in the `on_actions` block sourced from `common/on_actions/006_independence_wave_join_on_actions.txt` inside `common/on_actions/006_independence_wave_on_actions_registry.txt`; the former file is removed to keep the callback registry compact. Reports and the scoped retry event (`chaosx.nr6.36` through `chaosx.nr6.40`) are in the shared `events/006_independence_wave_support_events.txt` registry, with player-facing localisation in the shared `localisation/english/006_independence_wave_l_english.yml` registry. Retry timing is centralized at `constant:independence_wave_join.retry_days`. Shared zero-host exceptions are narrow edits in `common/scripted_effects/chaosx_liberation_release_effects.txt`, `common/scripted_triggers/chaosx_liberation_release_triggers.txt`, and `common/scripted_effects/006_independence_wave_package_planner_effects.txt`.
