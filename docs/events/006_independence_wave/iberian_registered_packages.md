...
- `common/script_constants/006_independence_wave_iberian_constants.txt` owns politics, ledger, duration, and AI tuning.
...
NAV and GLC have bounded runtime `source_placeholder` outputs using unchanged attributed sources, exact crops, deterministic `156x210` processing, and DDS conversion. The consolidated ComfyUI source shelf keeps NAV's original at `docs/assets/portraits/006_independence_wave/iw013_nav_jose_antonio_aguirre_source_placeholder_2026_08_13__portrait_NAV_jose_antonio_aguirre_original.jpg`; its crop, provenance, and corps-commander role review are in the single `processed/` subfolder. Its rights remain `PASS_WITH_CAVEAT` because the source page's CC BY-SA version metadata disagrees. No explicit `styled_final` request is recorded, so a provider-backed replacement is optional rather than automatically pending. NAV projects now fail closed after an IW-013 founding-crisis failure through the shared project-ready trigger; rehabilitation would require a distinct authored recovery mechanic. Independent source, identity, rights, flag, role, package, central-attestation, and gameplay gates remain open, and no Event 006 advisor icons are authorized.

## Future depth

The next approved tranche is the independent country-package and AI/probability audit for the existing NAV and GLC rosters. The current command consumers use archived source-placeholder portraits; NAV is `source_placeholder` without a styled-final request. A styled replacement is only an explicit-request work item; optional NAV extension states should only be added through the current-map binding after a fresh host-survival and reservation-group check.
