...
NAV and GLC have bounded runtime `source_placeholder` outputs using unchanged attributed sources, exact crops, deterministic `156x210` processing, and DDS conversion. NAV's durable archive and corps-commander role review are co-located at `docs/assets/portraits/006_independence_wave/iw013_nav_jose_antonio_aguirre_source_placeholder_2026_08_13/`; its rights remain `PASS_WITH_CAVEAT` because the source page's CC BY-SA version metadata disagrees. No explicit `styled_final` request is recorded, so a provider-backed replacement is optional rather than automatically pending. NAV projects now fail closed after an IW-013 founding-crisis failure through the shared project-ready trigger; rehabilitation would require a distinct authored recovery mechanic. Independent source, identity, rights, flag, role, package, central-attestation, and gameplay gates remain open, and no Event 006 advisor icons are authorized.
