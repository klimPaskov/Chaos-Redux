...
The exact compile-time content-attestation set is IW-001 Scotland, IW-004 Brittany, IW-006 Wallonia, IW-007 Frisia, IW-008 Rhineland, IW-009 Bavaria, IW-010 Saar, IW-012 Iceland, IW-017 Corsica, IW-019 Sicily, and IW-184 California. These eleven packages have the current package and sourced-portrait or approved-vanilla-identity evidence required by the static runtime gate.
...
006 portrait shelf now records exactly 54 original-size pre-resize masters directly
...
The canonical content gate now gives unadmitted automatic candidates zero weight and rejects a direct or stale reservation before anchor allocation. Eleven attested IDs provide a source-level exact-ten path: IW-008 RHI and IW-010 AJX share `RG-RHINE-SAAR` but are the only reciprocal pair admitted at that group's capacity of two, with distinct anchors 51 and 42. The current design decision doubles the automatic ladder to 6/8/10/14/20, including a twenty-country World Collapse band. The 6/8/10 bands are conditionally viable only when their live host, anchor, reservation, Event 005 collision, force, chaos-band, and transaction checks pass; the 14/20 design targets have no currently admitted capacity route and remain runtime fail-closed until the pool, disjoint reservation, host-survival, rollback, save/load, and synchronized execution evidence are expanded accordingly.

FORM-48 gameplay and its HBX carrier plus sovereign HAW/FSM member contract remain implemented, but IW-173 HAW and IW-179 FSM are not admitted, so FORM-48 is currently unreachable through a compliant carrier/member set despite IW-184 HBX admission. FORM-06 through FORM-47 remain fail-closed, including FORM-42. The current whole-event completion authority is `docs/plans/006_independence_wave_plans/subagent_handoffs/006_event_completion_audit_v31_2026_07_28.md`, which supersedes the historical v30, v23, and v28 whole-event snapshots for the current disposition and carries forward their bounded findings where still applicable. The f8 focus candidate was rejected and reverted by `8fddaeea3`; the restored baseline remains at 184 focuses, 223 connectors, 49 crossings, 18 node intersections, 27 long connectors, and 14 blocking diagnostics. All preserve the overall **HOLD / PARTIAL** disposition.
...
The AGX North Sea Coastal Conference lifecycle and three-factory cost surface receive bounded **PASS** findings in `006_agx_conference_post_repair_reaudit_2026_07_26.md` and `006_agx_conference_cost_localisation_reaudit_2026-07-26.md`; the prerequisite wording repair is recorded in `006_agx_conference_description_clarity_repair_2026_07_26.md`. The IW-007 package audit remains **PARTIAL** because shared-tree geometry, live allocation evidence, and broader package/runtime acceptance remain open. The current bounded localisation audit is **PASS** for the scoped Event 006 English surfaces and mirror wording; this does not close missing future package localisation or whole-event runtime validation. HAW retry 03 remains a documentation-only **blocked** rights result with no crop, repaint, DDS, or runtime override.
...
FORM-06 through FORM-47 remain fail-closed, including FORM-42. This historical portrait-gated snapshot predates the v31 whole-event authority and the IW-012 admission. Its v6/v7/v9/v10 counts and authority labels are retained only as at-time evidence; use the v31 audit, current source-of-truth map, and resume packet for current routing. The overall **HOLD / PARTIAL** disposition remains open.

The allocator and installed-scan figures in this historical snapshot are at-time evidence only. The current v31 audit records 149 publishers, 126 automatic/high-chaos selectable packages, 138 SCN-008 ranked packages, eleven exact attestations across ten compatible reservation groups, and Event 005-first joint reservation ordering. Its earlier 3/4/5/7/10 ladder is superseded by the later 6/8/10/14/20 design decision. The older 2026-07-18 installed scan covers 108 Event 006-owned identifiers, 122 Workshop directories, three sibling local mods, archives, and reports zero collisions; it is not a current environment scan.
...
The automatic wave counts are 6, 8, 10, 14, and 20. World Collapse is also 20; its force level, instability, package rarity, and regional ambition rise with the count instead of replacing it. The repaired planner applies the canonical content-attestation trigger during weight calculation and anchor reservation, so unadmitted candidates receive zero automatic weight and a direct stale reservation fails before any host, country, or state is reserved. The lower bands are conditionally viable from the eleven attested IDs when all live gates pass; IW-008 RHI and IW-010 AJX share RG-RHINE-SAAR but are the only reciprocal pair permitted at capacity two, with distinct anchors 51 and 42; all other groups remain one-package-only. The 10/14/20 bands are therefore source-level count targets but remain runtime fail-closed until the attested pool, reservation groups, same-host protected-remnant cases, Event 005 collisions, rollback, save/load, and synchronized execution evidence are expanded. Overlay-only, community-variant-only, formable-or-route-only, scenario-variant-only, and unbound rows remain available only through their explicit owning systems; Open Sovereignty cannot promote them merely to fill a wave.

The host-facing Independence Wave crisis is a scoped, costed mission rather than a second release system. A country sees it when stability falls below 35%, when an enemy controls one of its owned states above 50 resistance, or when it controls a foreign-owned state above 50 resistance. Opening the 120-day emergency spends the standard security commitment and lowers stability. If pressure still exists when the mission resolves, `chaosx.nr6.3` queues the ordinary current-band wave and retries the shared coordinator for the bounded fourteen-day window; an invalid plan records a blocked outcome, applies the failure pressure, and changes no ownership. The source now persists queued, committed, blocked, cancelled, exhausted, and requester-lost resolution receipts, writes distinct Event Log resolution rows for queued, blocked, cancelled, committed, and requester-lost outcomes, records host/cause history for both queued and cancelled requests, recovers an annexed requester at the `on_annex` boundary, attributes payload 6006 to the lost requester, and discloses the concrete failure consequences in the mission and Event Details. Live mission timing, retry, requester-loss, save/load, and Event Log rendering remain unproved. The crisis uses no world-wide daily loop and never bypasses host survival, unique anchors, reservation, or synchronized execution.
...
The source-level doubled-ladder and crisis implementation handoff is `docs/plans/006_independence_wave_plans/subagent_handoffs/006_pre_wave_crisis_and_doubled_ladder_2026_07_28.md`; the focused completion audit and parent closure handoff record the subsequent receipt, requester-loss, wording, workbook, and scoped tag-audit repairs. These remain implementation evidence only: live mission timing, AI selection, retry cleanup, requester-loss and non-annex removal, save/load, allocator execution, Event Log rendering, and the 32-cell scenario/runtime matrices remain open under the whole-event **HOLD / PARTIAL** boundary.

SCN-008 uses the same anchor-first transaction but attempts every viable ranked candidate at every intensity. Intensity selects anchor/compact/extended territory and fragile/viable/armed/high-chaos forces; scenario type independently selects league, host-war, belligerence, patron, and partition rules. The implementation retains six numeric scenario families, while Universal Belligerence contributes three independently selectable rules, giving the player eight selectable modes. The runtime acceptance matrix is eight modes by four intensities, or 32 cells. Great Partition may advance the territory tier but never the candidate count. Universal Belligerence keeps a bounded target array only for the duration of its launch, preventing duplicate targets inside one incident while clearing every target mark after successful or failed war declarations.
