...
| File | Focus id | Current x/y | Current prerequisite(s) | Expected visual/logical parent | Suggested patch |
| --- | --- | ---: | --- | --- | --- |
| `005_soviet_collapse_ancient_restorations.txt` | `APX_returned_names_endgame` | `10/8` | `APX_khazar_charter`; `available` requires flag from `APX_expansionist_steppe_levy` | Visually sits under `APX_expansionist_steppe_levy` at `10/6`, while the visible pathline comes from the charter at `7/7`. | If parent wants clean route lines, make the prerequisite `APX_expansionist_steppe_levy` and keep `APX_khazar_charter` as an `available` requirement or a second explicit prerequisite depending on desired tooltip/pathline behavior. |
| `005_soviet_collapse_ancient_restorations.txt` | `SOG_returned_names_endgame` | `10/8` | `SOG_sogdian_city_charter`; `available` requires flag from `SOG_expansionist_merchant_claims` | Visually sits under `SOG_expansionist_merchant_claims` at `10/6`. | Same pattern as APX: align the visible parent with the expansionist route while preserving charter requirement explicitly. |
| `005_soviet_collapse_ancient_restorations.txt` | `ANX_returned_names_endgame` | `10/8` | `ANX_khwarazmian_water_charter`; `available` requires flag from `ANX_expansionist_water_claims` | Visually sits under `ANX_expansionist_water_claims` at `10/6`. | Same pattern as APX. |
| `005_soviet_collapse_ancient_restorations.txt` | `ABX_returned_names_endgame` | `10/8` | `ABX_alan_pass_charter`; `available` requires flag from `ABX_expansionist_mountain_claims` | Visually sits under `ABX_expansionist_mountain_claims` at `10/6`. | Same pattern as APX. |
