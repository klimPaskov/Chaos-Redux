...
4. **Ancient restoration branches are real but too shallow.** The APX example shows the pattern: politics opens at `APX_restore_itil_council` (`005_soviet_collapse_ancient_restorations.txt:29`), industry is mostly `APX_ferry_house_registers` and two children (`56`, `70`, `84`), military is `APX_volga_toll_guard` plus children (`98`, `115`, `129`), and expansion is mainly `APX_old_border_files` into `APX_expansionist_steppe_levy` (`176`, `239`). The final route choice is only one symbolic focus versus one expansion focus (`213`, `240`) before `APX_khazar_charter` (`287`). SOG/ANX/ABX repeat this compact shape.

5. **Some high-chaos trees are aggressive enough at endpoints but not along the route.** Strong endpoint examples exist: `PRA_rails_over_capitals` creates a Soviet war goal and AI hostility at `005_soviet_collapse_custom_splinters.txt:1739`, `TSC_starfall_mandate` does the same at `2266`, `DSC_congress_of_the_dead_army` escalates to war at `3268`, and `APX_expansionist_steppe_levy` does it at `005_soviet_collapse_ancient_restorations.txt:240`. The problem is that many earlier focuses on those paths still give stockpiles, single-state construction, or variable ticks before the endpoint instead of continuously opening attacks, raids, mandate decisions, or aggressive neighbor pressure.
...
2. Convert clustered stockpile rewards in ARD, KHC, GAC, AOX, UWD, ADX, NLC, AEX, DHC, and FEV into route mechanics: staged raids, port/rail/convoy missions, claim-and-core packages, named unit templates, or decision tiers.
...
- Read required repo skills: `chaos-redux-subagents`, `chaos-redux-focus-trees`, and `chaos-redux-decisions-missions`.
