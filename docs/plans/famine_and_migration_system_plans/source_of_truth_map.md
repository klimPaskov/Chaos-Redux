...
- The famine replay-boundary audit found and closed one exact adapter defect: state and actor proof tokens are now bound to the current state and saved actor target, and all six famine adapter exits reset the complete temporary proof envelope while preserving the public result. Generic persisted adapter deduplication remains intentionally absent because the current Camp and CBRN date-based identities are not proven unique; source-owner transaction guards remain authoritative.
