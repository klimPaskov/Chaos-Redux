...
5. Spawn the bounded counter artist with `fork_context=false` and the exact future handoff path above. Require seven distinct large strips and seven distinct on-map strips; the current byte-identical skull/Z files are not acceptable finals.
...
Validation intentionally skipped: Internet sound sourcing and license verification, audio transformation/listening acceptance, animation-frame synchronization, counter generation and contact-sheet approval, specialized model/entity creation, runtime edits, source-to-runtime synchronization, provider work, Blender work, and in-game validation. Those activities are outside this read-only/non-paid intake or require artifacts that do not yet exist.
