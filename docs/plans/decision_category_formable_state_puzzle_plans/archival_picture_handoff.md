...
The source archive, processing previews, review contact sheet, and provenance evidence remain under `docs/assets/decision_category_formable_state_puzzle/` while the parent completes integration. The archival comparison sheet is `docs/assets/decision_category_formable_state_puzzle/archival_contact_sheet.png` and is not a runtime asset. The shared `docs/assets/decision_category_formable_state_puzzle/contact_sheet.png` is a separate generated-art review sheet owned by another package and is intentionally preserved unchanged.
## Manifest rows
