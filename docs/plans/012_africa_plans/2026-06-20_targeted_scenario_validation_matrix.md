...
| `World Is One` | Static coverage present as direct manual terminal setup, live proof pending | `triggerable_scenario_africa_type.world_is_one`; `africa_apply_triggerable_world_is_one_opening`; `africa_ensure_triggerable_world_is_one_external_unifier_seats`; `africa_spawn_triggerable_external_continent_unifiers`; `africa_create_triggerable_external_unifier_guard_template`; `africa_start_triggerable_world_is_one_terminal_wars`; `africa_force_triggerable_world_is_one_terminal`. | Launch at Low, Medium, High, and Maximum. Confirm the Africa Is One opening still occurs, every external region has at least one scenario actor, actors receive intensity-scaled stockpiles and guard divisions, terminal wars start, World Is One flags are set intentionally by the scenario helper, accepted super-event/audio slots fire, and compatibility with an existing world-end state behaves as documented. |
...
| External-unifier support imbalance | External support scales through `africa_apply_triggerable_external_unifier_scaled_support`, and each supported actor receives Continental Unifier Guard divisions from the same intensity layer. | Compare Low, Medium, High, and Maximum support values, guard counts, and resulting wars; confirm Maximum is severe but not broken by missing equipment, manpower, or target setup. |
