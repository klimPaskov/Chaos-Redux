...
## Sovereign portrait reconciliation (2026-08-03)

The installed sovereign portrait paths remain the stable runtime contract for these sixteen packages, but the male-only presentation rule now governs promotion. A promoted row must use one decorated male African sovereign on a plain, low-detail matte background in the HOI4 painted style, with dramatic but restrained regalia and bounded natural or supernatural symbolism. It does not use a council, delegation, crowd, committee, interior, battle scene, sacred-object motif, or institutional body as the player-facing leader portrait. The stable sprite names and existing-carrier/tag contract are unchanged.

The historical source review remains separate from package loading, but the accepted settlement path always installs the package's stable sovereign role; no politics-only fallback suppresses promotion. The portrait worker must still replace every source-locked placeholder with an accepted male ruler/person crop and record a defensible period or alternate-history contract, including the former artifact/map rows and pre-1936 rulers, before release. The stable sprite, character, and carrier identifiers do not change during that handoff. The six fictional high-chaos portrait subassets for Pan, Gorilla Kingdom, The Green, Living Rivers, Stoneborn, and Ancient Hosts are separate dormant model-required assets and must never be attached to these grounded sovereign characters.
