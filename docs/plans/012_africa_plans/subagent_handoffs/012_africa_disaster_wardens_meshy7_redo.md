...
Status: `blocked_provider_rig_retry_failed_90_percent_refunded`.
...
One and only one failure-driven retry was then submitted as rig task `01a038d2-b191-7f51-baa0-dd00a1d41daf` against the accepted geometry. It reached terminal `FAILED` at 90% with `unexpected_error`, produced no rig or walk/run download, and returned `consumed_credits=0` after the provisional 5-credit reservation was refunded; the current balance is 13. Provider work is stopped at this boundary, and no custom animation or local rig/action fallback was attempted.
...
The package's confirmed attributed total is 180 credits, comprising the 150 credits of quarantined old lineage and the 30-credit active generation; both rig attempts refunded their transient 5-credit estimates.
