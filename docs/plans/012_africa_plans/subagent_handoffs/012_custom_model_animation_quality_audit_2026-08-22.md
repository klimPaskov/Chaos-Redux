...
- Nine runtime model packages expose 46 required animation files: five each for eight strange-force models and six for the elephant model.
- None of the 46 action surfaces has verified provider-action or professional-source lineage.
...
- The expected elephant evidence root docs/assets/012_africa/models_3d/elephant_shared_base/ is absent. The runtime elephant mesh and six .anim files exist, but the dated handoff is the only surviving source for the claimed action and reimport package.
- No action-phase visual evidence was found. Each of the eight non-elephant packages has seven static geometry previews only, and the elephant has no package preview root.
...
| Elephant runtime actions | 6 | UNVERIFIABLE | Runtime files exist, but the evidence root, provider action lineage, Blender route, reimport proof, and visual proof are absent. |
| Required runtime actions overall | 46 | PASS 0 | No action meets the full acceptance rule. |
| Evidence packages missing entirely | 1 | MISSING | elephant_shared_base is absent from docs/assets/012_africa/models_3d/. |
...
| gfx/entities/chaosx_elephants.asset | Runtime state-to-animation bindings for the shared elephant entity. | Attack/defend/support_attack 10-34, move 37-42, deploy 45-50, supply_load 53-59, retreat 62-67, death 70-76, idle/training 79-90. |
| gfx/entities/chaosx_elephants.gfx | Runtime elephant mesh and animation type registration. | The pdxmesh block and six animation types are at lines 4-21. |
| gfx/models/units/chaosx_elephants/animation_chaosx_elephants.asset | Maps the six elephant animation types to .anim files. | Six definitions span lines 3-30. |
...
| docs/plans/012_africa_plans/subagent_handoffs/012_africa_elephant_shared_model_2026-08-05.md | Only surviving elephant package handoff. | Lines 3 and 7-14 claim a missing package root, six actions, custom 17-bone rig, and a Meshy task id. |
...
| chaosx_elephants; gfx/models/units/chaosx_elephants/ | chaosx_elephant_idle, move, deploy, supply_load, attack, impact; all six runtime .anim files are present and registered by animation_chaosx_elephants.asset lines 3-30. | Only the missing-package handoff claims Meshy task 019fd212-8909-765a-a4f0-70294b8ff7f3 at 012_africa_elephant_shared_model_2026-08-05.md:13. No provider request/response, rig task id, action id, Blender action report, or professional-source receipt survives under the expected root. | UNVERIFIABLE. The expected docs/assets/012_africa/models_3d/elephant_shared_base/ root does not exist, so the Blender authoring route cannot be audited. | No evidence-root export parser, validation JSON, or reimport .blend survives. Runtime files alone do not establish reimport proof. | No elephant evidence preview root survives. | UNVERIFIABLE/MISSING evidence. Runtime mesh and rig reuse cannot be confirmed; recover the package first, otherwise custom rig and regeneration are likely. |
...
- The elephant runtime mesh is present, but its provider source, rig report, and package root are missing. Its geometry and rig must be treated as unrecoverable until the dated handoff claims are backed by actual files; if no rig package can be recovered, custom rigging and likely geometry regeneration are required before action work.
...
1. Keep all nine packages out of animation-quality PASS and runtime activation until every required role has approved source lineage and visual evidence.
2. Recover or regenerate the missing elephant package evidence before touching its six runtime actions.
...
- Confirm runtime presence and exact semantic coverage with: Get-ChildItem gfx/models/units/012_africa_* -Recurse -File -Filter *.anim and Get-ChildItem gfx/models/units/chaosx_elephants -File -Filter *.anim.
- Confirm runtime bindings and semantic aliases with: rg -n 'state =|animation =|pdxmesh =' gfx/entities/012_africa_strange_forces.asset gfx/entities/chaosx_elephants.asset.
...
- Verify the elephant evidence root exists and contains the six claimed action reports, exports, parser files, validation files, reimport checkpoints, provider receipts, and previews before classifying any elephant role.

## Risks and blockers

### Confirmed blockers

- No verified Meshy action or professional-source lineage exists for any of the 46 required runtime actions.
...
- The elephant evidence root is absent even though runtime files and a dated handoff claim it existed.
...
- The locked meshy_animate schema requires rig_task_id and action_id, but the eight package jobs record only image-to-3D geometry task ids and the elephant package has no surviving machine-readable job.
...
Keep all nine runtime model packages blocked from animation PASS. Recover the elephant evidence package and run one representative strange-force creature through the verified Meshy7 meshy_animate or explicitly approved professional-source route with complete action lineage, reimport, and five-phase visual evidence before batching the remaining 45 actions.
