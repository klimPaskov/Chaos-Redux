...
Status: this is an asset-production handoff, not an in-game completion claim. Current local package evidence is mixed: Oracle Recon has a complete Meshy 7 local PDX mesh/five-action export and reimport package awaiting parent runtime copy, wiring, and live validation; Forest Giants is blocked; Disaster Wardens reuses vanilla infantry with parent override work pending; Plague Carriers is blocked before mesh/action promotion; and the current redo records for the other four packages do not support the former all-complete claim.
...
| `oracle_recon` | local PDX export/reimport complete; parent runtime copy, wiring, and live validation remain | `export/mesh/chaosx_oracle_recon.mesh` (local package only) | `chaosx_oracle_recon_entity` | accepted idle, move, recon, observation, and death actions | Meshy 7 generation `01a03996-6938-71f8-aec7-1d2b9c4b854c`; accepted action tasks are in the linked handoff |
| `riverborn` | blocked; current Meshy 7 generation rejected for identity and topology | no accepted current `.mesh`; legacy export is evidence only | `riverborn_entity` is a proposed consumer only | idle, move, attack, water transition, death remain blocked | Meshy 7 generation `01a033eb-7cad-7cd5-adfc-76f4b97dbe59` rejected |
