...
The tranche covers row 24 recognised rival confederation, row 28 measured local project ownership, row 28 local-ownership loss, and the weather-call backfire correction used by row 38.
...
Hostile weather actions should continue through `africa_call_hostile_natural_disaster_from_action`; its accepted selected-country result is not a civilian or campaign-victory proof, and only exact backfire/rejection branches set the new backfire disqualifier.
...
The Event 013 wrapper does not expose a delayed campaign-victory or army-defeat callback for the weather call, so row 38's readiness gate remains closed.
...
Targeted `rg` checks confirmed one definition and the intended callsite for each new helper, no remaining call to the removed hostile-civilian proxy, no writer for `africa_achievement_rain_command_owner_ready`, and no writer for `africa_achievement_ecological_covenant_owner_ready`.
...
Rows 30, 32, 33, and 38 are intentionally incomplete because their exact owner lifecycles are not exposed by the current sources.
