...
- Achievements: exact owners were added for the rival-confederation and measured diaspora cases, and the owner kernel now routes development, socialised-resource, diaspora-owned, and disease-containment counters through declared recorders. Disaster backfire evidence was corrected. The full 44-achievement requirement is not satisfied because several exact lifetime and ownership proofs remain unavailable.
...
The previous static reachability scan found 51 Event 012 helpers with no non-definition text reference. This does not create a parser or load error. The owner-kernel tranche below closes four achievement-recorder callsites; a fresh workspace-wide helper count remains a separate audit task.
...
- `africa_confirm_all_required_first_proof_regions`, `africa_confirm_first_proof_domestic_settlement`, and `africa_confirm_first_proof_reform`, whose orchestration ownership is not proven. `africa_select_mapped_first_proof_action` has an exact focus-opening caller.
- Nine achievement recorders without an exact caller, covering civilian disaster weaponisation, model-based elephant proofs, forced-relocation/scenario proof, other-world-end proof, and the terminal super-event. Development, socialised-resource, diaspora-owned, and disease-containment recorders now have exact full-action callers. The common-reserve deployment recorder now has an exact Action 80 and protection-war caller, but its six-war live acceptance is still open. Weather-army defeat and weather-war ownership now have an exact accepted-target plus direct-host-capitulation caller, while row 38's three-target live proof remains open.
