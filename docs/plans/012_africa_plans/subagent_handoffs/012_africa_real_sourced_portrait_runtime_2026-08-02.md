...
Asante remains blocked: Prempeh II is already owned by Event 006, while the locally retained Prempeh I candidate still needs independent identity confirmation. Sokoto, Luba, and Lunda remain rights/provenance review cases. Oyo, Manden, Aksum, Kilwa, Nubia, and Great Zimbabwe remain source-gap cases. The Zulu row is now wired to Dinuzulu kaCetshwayo only; it is not a Solomon kaDinuzulu substitution. The eleven remaining held rows therefore retain the existing generated source mode; no unsupported lion head, spear, ritual mask, face paint, crown, or animal regalia was added to them.

The source masters, exact crop PNG/JSON proofs, raw source-locked outputs, processed PNGs, DDS derivatives, and source manifest remain in the ignored workspace `docs/assets/012_africa_priority_portraits_real_sources/` for audit and future replacement. They were not silently relabelled as fictional art. The parent-owned character effects now fail-closed: only the five independently reviewed source rows receive a promoted sovereign role, while held historical rows retain their package politics and remain role-dormant until a source-approved portrait is available. No model production was attempted.
