...
| --- | --- | --- | --- |
| AI profiles, 64 rows | `docs/specs/012_africa_specs/matrices/012_africa_ai_route_matrix.csv` | `subagent_handoffs/012_africa_ai_profile_audit_2026-07-29.md`, profile registry, controller, and bounded action assignments | All 64 remain `blocked`. Static source coverage is 64 profiles and 102 prepared action profiles. Campaign simulation, independent scenario audit, and country-specific profile to focus-plan binding remain open. |
...
- AI: 64 rows, all still blocked. The bounded controller covers 86 early branches for Actions 1 through 76 and 93 through 102 plus 16 late branches for Actions 77 through 92, giving 102 of 102 static action branches.
...
| --- | --- | --- |
| `012_africa_ai_profile_audit_2026-07-29.md` | Accepted audit evidence | Static profile and 102-action coverage is current. Parent owns campaign and focus-plan validation. |
| `012_africa_full_action_ai_dispatcher_handoff.md` | Accepted bounded implementation evidence | Its 86-action early dispatcher closes the former missing-range claim. Campaign acceptance and focus-plan differentiation remain queued. |
...
- Static AI dispatch coverage does not prove campaign outcomes, weighted behavior, or profile-specific focus-plan priorities.
...
1. Keep all 64 AI rows blocked until campaign simulation, independent scenario audit, and a reviewed country-profile to focus-plan binding are available.
...
Remaining risks are the open campaign and focus-plan AI validation, owner-system achievement callsites and disqualifier cleanup, deferred model and world packages, and the absence of live valid and invalid achievement scenarios.
