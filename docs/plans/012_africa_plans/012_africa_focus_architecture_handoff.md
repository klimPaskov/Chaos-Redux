...
4. Repository Event 6 proves the limitation in practice. `006_independence_wave_focus_effects.txt` has full and nominal additive assignment modes, but the additive root is not statically included by owning trees. Its own focus-framework handoff records that the additive definitions are detached until each owning tree imports the root.
