...
> Superseded current-state notice, 2026-08-11: the historical all-blocked and open-owner verdict in this handoff is no longer current. The acceptance ledger now records all 44 achievement rows as `implemented`; current source semantics, including rows 10, 16, 29, 30, and 40, are reconciled in `docs/plans/012_africa_plans/subagent_handoffs/012_africa_final_documentation_reconcile_2026-08-11.md`. Retain this file for implementation provenance only and do not use its historical disposition table to reopen completed owner work.

> Release-candidate correction (2026-07-29): all 44 achievement icon triplets are installed as 132 DDS files, and every row below records `triplet installed`. The owner-system review described below is historical; current acceptance is recorded in the 2026-08-11 ledger reconciliation.

## Status

The complete 44-row achievement matrix has 44 registered definitions, 44 completion triggers, one English name/description/tooltip triplet per row, central thresholds, normal-launch eligibility, successor-safe lifetime ledgers, live member proof, duration deadlines, and explicit positive and disqualifying conditions. The 2026-07-29 callsite labels below are historical classifications. The current ledger records all 44 rows as source-implemented, with accepted structural invariants where the engine does not expose a separate callback and no current gameplay omission identified.

`RUNTIME-EVIDENCE-GATED` is a historical label for the pre-model-acceptance snapshot. The current elephant package and its achievement owner contract are source-present; user-owned movement, supply, destruction, and war-purpose playback remains separate evidence and is not a ledger blocker.

The common runtime hooks for initial host registration, action start, full and failed action resolution, immutable diaspora parameters, protection-war starts and settlements, Scramble expedition defeat, continental-war victory and settlement, post-unification sponsorship, negotiated two-continent integration, terminal-formation evidence, pre/post unification snapshots, valid RSA succession, Tier A player-origin validation, and the bounded weather-target/capitulation owner are integrated. The current owner handoffs reconcile the restoration, maritime, Horn, development, reserve, high-chaos, and terminal disqualifier/cleanup contracts without proxy writers.

Event 012 has no registered triggerable-scenario route. Forced-scenario achievement checks remain structural documentary invariants only; there is no Event 012 writer or callable launch path.

All 44 unique three-state DDS sets are installed as 132 DDS files under `gfx/achievements/`. They accompany the current source-complete owner contracts; the dated row table below remains provenance, not a second acceptance gate.

## Current row semantics

Rows 10, 16, 29, 30, and 40 use the exact regional-congress, settled-Scramble partition, per-mode network, Event 018 local-processing, and symmetric Stoneborn/human-war contracts recorded in the final documentation handoff. The retired triggerable-scenario placeholder is not an Event 012 owner or acceptance path.
...
| Scramble settlement owner | `africa_achievement_record_scramble_settlement = yes` | Called only by `africa_scramble_ratify_aftermath` and `africa_scramble_close_continental_docket` after the defensive-war winner and settled response barriers are true; a full `break_intervention_coalition` action alone is not achievement evidence. |
...
- Pairwise hooks integrate last-convoy settlement, protection wars, expedition/intervention defeat, member capitulation, continental-war victory, continental settlement, and the direct-host-capitulation weather proof. The reserve owner remains caller-pending, while the elephant package now has static host and Action 102 consumers; its achievement helpers remain uncalled until the named terrain, supply, destruction, and protection-victory evidence is final. Weather-army counting now uses the accepted-target flag plus this bounded host winner check.
...
- Existing explicit disqualifier helpers cover coercive annexation, broken guarantees, member capitulation, congress expulsion/coerced accession, member cascade, burden/confidence/ceiling breach, forced relocation, trust collapse, network split, concession breach, ecological bargain breach, nonhuman-rights violation, and other world ends. Forced-scenario checks for rows 1, 4, 27, and 44 are structural invariants because no Event 012 triggerable scenario is registered; no writer is claimed. Other matrix-specific terminal owners must set the identically named `africa_achievement_*` disqualifier flag at the authoritative failure transition.

## Historical row-by-row coverage and disposition

The table below preserves the 2026-07-29 callsite classifications and icon directions as implementation provenance. It is superseded by the current ledger and final documentation handoff and must not be read as a current blocked-row list.
...
| 4 | `africa_the_interveners_left` | The defensive-war winner writes Scramble victory only through the settled aftermath/close owners, with one recorded expedition defeat and the hostile-state snapshot. A full coalition-breaking action alone is not a victory receipt. Capitulation/partition owners set DQs. | REACHABLE/PARTIAL | Broken expedition helmet before continental shield — triplet installed. |
| 5 | `africa_archive_of_the_living_state` | Evacuation actions plus `record_archive_restoration` count three independent/autonomous restorations. The bounded pre-Africa-is-One Archive Disposition category offers public/custodial custody or explicit sale/suppression receipts against a selected evacuated government; destruction, sale/suppression, and later annexation require the matching DQ transition. | REACHABLE/PARTIAL | Archive chest carried past a burning capital — triplet installed. |
...
| 10 | `africa_every_region_speaks` | Each full `convene_regional_congress` completion appends only its selected overlay to a lifetime unique congress-region ledger; current live members and settled overlaps remain separate proof sets, and formation requires all nine of each. Region-loss/unresolved owner sets DQ. | REACHABLE/PARTIAL | Nine regional emblems around a speaking staff — triplet installed. |
...
| 13 | `africa_republic_of_many_capitals` | Five authored focus rewards record distinct republican institution regions; election/succession actions activate succession; five-year deadline is direct. Civil war remains the current-state gate. Suspension/centralisation have no authored route, so their dead technical checks were removed; the positive placement/owner-ready/succession contract is authoritative, and no military-transition proxy is used. | REACHABLE/PARTIAL | Five civic buildings linked by a republic star — triplet installed. |
...
| 16 | `africa_order_without_partition` | Military profile/route, `record_major_intervention_defeated`, representation action, postwar/commander review, and the settled Scramble core snapshot prove bounded emergency rule without partition. `africa_achievement_african_core_ceded` is the exact foreign-cession DQ; genocide and permanent-rule owners remain source-owned. | REACHABLE/PARTIAL | Sheathed sword beside open charter — triplet installed. |
...
| 24 | `africa_member_who_said_no` | The bounded `on_capitulation` owner records a rival priority member's victory when the current host capitulates after the explicit rival-bloc departure route. Exact Tier A player origin, four-member count, independence, recognised-alternative milestone, and lifetime DQs remain required; ordinary peace is non-qualifying. | ACTIVE/PARTIAL | Small restored seal before broken chain — triplet installed. |
...
| 28 | `africa_capital_without_capture` | Distinct local diaspora projects and the measured local-ownership threshold are implemented; housing or bond failure owns local-loss and corruption DQs, while host-only `on_puppet`, `on_release_as_puppet`, and `on_subject_autonomy_level_change` witnesses classify foreign versus participating diaspora government capture. The ten-project count and clean live campaign remain required. | REACHABLE/PARTIAL | Coin and factory held in an open hand — triplet installed. |
...
| 33 | `africa_common_reserve_answers` | Action 80 now opens a stockpile-backed posture; defender-only protection-war callbacks debit the existing reserve, copy a deployment sequence, and close on peace, capitulation, or annexation. Deadline/capital/offensive-abuse owners set DQs. | ACTIVE/PARTIAL | Reserve standard before six shields — runtime owner kernel installed; six-war live acceptance remains open. |
...
| 36 | `africa_elephants_crossed_the_desert` | Elephant formation, unique terrain enum including desert, supply proof, and protection-victory helpers exist; the `chaosx_elephant` unit/entity and host/Action 102 consumers are statically wired, but achievement writers and live witnesses remain open. | RUNTIME-EVIDENCE-GATED | Armoured elephant with rail and water drums — triplet installed; live movement, supply, destruction, and war-purpose proof remain required. |
...
| 38 | `africa_rain_on_command` | Distinct hostile-army weather helper counts three; an accepted target that capitulates directly to the current host records the weather-war milestone. Accepted member/neutral-African targets and canonical wrath-threshold collapse set the three weather DQs. | ACTIVE/BLOCKED | Storm held by an oracle staff — target classification, wrath owner, and host-capitulation owner installed; live three-target acceptance remains open. |
...
- Declaration-level positive flag and helper references resolve; current owner reachability is recorded by the 2026-08-11 reconciliation, while the 2026-07-29 callsite labels remain historical.
...
- Duration deadline predicates avoid recurring world scans, and accepted structural invariants fail closed where the engine exposes no separate callback.
...
- The HOI4 MCP routes remain adapter-bounded: prior lint was blocked by `ARTIFACT_STORAGE_LIMIT`, and current event/probability evidence is exact only for isolated pools or partial/bounded for dynamic surfaces. This limitation does not identify a gameplay omission.

## Simplifications, omissions, and blockers

- All 44 unique achievement icon triplets are installed. No fallback art was used.
- The elephant, restoration, weather, reserve, high-chaos, and terminal packages retain their exact source contracts and accepted readiness gates. User-owned playback and adapter-limited MCP evidence remain separate from the source disposition and are not reported as gameplay omissions.
