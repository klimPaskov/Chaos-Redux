...
- The elephant package is statically wired through the shared unit/entity body, host guard, and Action 102 member guards, but its achievement route still requires owning systems to call the exact formation, terrain, supply, protection-victory, and disqualifier helpers after live movement, supply, destruction, and war-purpose evidence exists. Restoration milestone families and the terminal super-event still require their owning gameplay systems to call the exact helper or set the exact DQ flag identified above. The weather route has source owners but is not completion-ready until three distinct hostile targets capitulate to the current host. The common-reserve owner kernel is now wired, but its achievement is not completion-ready until six separate defensive-war scenarios and the failure paths are accepted in play.
