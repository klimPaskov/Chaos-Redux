...
- Pairwise hooks integrate last-convoy settlement, protection wars, expedition/intervention defeat, member capitulation, continental-war victory, continental settlement, and the direct-host-capitulation weather proof. Reserve and elephant owners still call their helpers only after the named deadline, capital-control, terrain, supply, and victory evidence is final; weather-army counting now uses the accepted-target flag plus this bounded host winner check.
...
| 28 | `africa_capital_without_capture` | Distinct local diaspora projects and the measured local-ownership threshold are implemented; housing or bond failure owns local-loss and corruption DQs, while host-only `on_puppet` and `on_release_as_puppet` witnesses classify foreign versus participating diaspora government capture. The ten-project count, clean live campaign, and unclassified direct-autonomy paths remain required. | REACHABLE/PARTIAL | Coin and factory held in an open hand — triplet installed. |
...
- The elephant route, restoration milestone families, and terminal super-event still require their owning gameplay systems to call the exact helper or set the exact DQ flag identified above. The weather route has source owners but is not completion-ready until three distinct hostile targets capitulate to the current host. The common-reserve owner kernel is now wired, but its achievement is not completion-ready until six separate defensive-war scenarios and the failure paths are accepted in play.
