...
The historical visual counts and no-model statement below predate the shared armoured elephant tranche. Current source-of-truth status is `52 installed_runtime`, `28 installed_dormant`, `12 deferred_runtime_gated`, `133 deferred_controlled_pool`, `14 deferred_model_required`, and `0 deferred_unique_package_required`; matrix rows 203–204 are installed through one shared `chaosx_elephant` body with static host and Action 102 consumers. The elephant achievement remains runtime-evidence-gated until its achievement owners and live movement, supply, destruction, and war-purpose witnesses are accepted. Preserve the dated counts below as provenance rather than current release status.
...
| Shared acceptance ledger | `docs/plans/012_africa_plans/012_africa_acceptance_ledger.csv` | All 239 `asset_item` rows now match the authoritative matrix status in matrix order with the existing UTF-8 BOM; action, focus, host, polity, priority, achievement, and AI dispositions remain separate. |
...
The accepted Event 012 identity is tier 4, Minor Fire-Once, Formables cluster 6, Severe member severity, and entry event `chaosx.nr12.1`. The current implementation ledger records 51 host playbooks, 22 full and 29 compact, a generic-tree-only host focus loader, 102 parameterized action IDs with 90 implemented and 12 gated rows, Africa-only Scramble aftermath closure, 16 existing-tag priority packages with 13 conditionally reachable and DYX/DZX/EMX dormant and unbound, and the explicit runtime gates `africa_strange_formation_package_ready` and `africa_world_package_implementation_ready`.

The current visual matrix has 239 rows and no `planned` or `pending_runtime_blocker` values: 50 `installed_runtime`, 21 `installed_dormant`, 12 `deferred_runtime_gated`, 133 `deferred_controlled_pool`, 16 `deferred_model_required`, and 7 `deferred_unique_package_required`.

At the historical 2026-08-01 boundary, the filesystem and registered `.gfx` evidence covered 23 Event 012 report/news/super-event pictures, 12 Charter static textures, two rebuilt semantic Charter frame sheets with static fallbacks, 13 focus-family textures, 132 achievement DDS files, 16 sovereign portraits, seven route flag ladders and emblem packages, and the selected priority-member decision and report textures. Those assets were installed, dormant, promotion-gated, model-required, unique-package-required, or controlled-pool deferred according to the matrix and current owner gates; the statement that no model package existed is retained only as historical provenance and is superseded by the current mixed model-production ledger.
...
| `subagent_handoffs/012_africa_country_rc_audit_2026_07_29.md` | Historical count corrected | Sixteen sovereign portraits and seven carrier flag ladders have installed evidence; carrier ownership, reachability, and package promotion risks remain open. |
...
| `012_africa_final_improvement_loop_addendum_2026-08-01.md` | Active final integration plan | Broad expansion is closed. W0-W4 source is implemented, W5 initial certification is blocked by absent pre-install receipts, B2 retains the six late/episodic families as list-only, B3 closes row 24/28 owners plus the weather correction, and B4 retains two AI evidence blockers. |
| `subagent_handoffs/012_africa_world_w5_atomic_certification_blocker_2026-08-01.md` | Queued blocker evidence | No authoritative pre-install receipt identifiers exist for all-six certification; no readiness setter is authorized. |
| `subagent_handoffs/012_africa_action_duration_objective_d1_handoff_2026-08-01.md` | Implemented action contract | All 102 rows have per-row duration/objective contracts; 90 are implemented and rows 73-76 and 85-92 remain explicitly gated. |
| `subagent_handoffs/012_africa_final_focus_audit_2026-08-01.md` | Accepted focus evidence | 77 of 78 payoff rows are implemented; one external-world row remains queued with owner. Branch-unaware layout diagnostics and live scenario proof remain open. |
| `subagent_handoffs/012_africa_charter_animation_repair_v2_2026_08_01.md` | Installed runtime asset evidence | Two semantic Charter frame sheets were rebuilt and installed; no new GUI page or duplicate late-family buttons are authorized. |
| `subagent_handoffs/012_africa_ai64_scenario_audit_2026-08-01.md` | Static registry evidence with two blockers | All 64 profiles have predicates, loaders, and registry calls. Material-readiness Scramble evidence and strategy-plan probability evidence remain open. |
| `subagent_handoffs/012_africa_b3_achievement_owner_closure_2026-08-01.md` | Narrow owner closure | Row 24 and measured row 28 owners are wired and the weather-call correction is recorded; rows 30, 32, 33, 37, and 38 remain fail-closed where exact owners are absent. |
...
7. The shared acceptance ledger previously marked all asset rows `queued` or `blocked`; its 239 asset rows now mirror the matrix's six statuses exactly. Remaining non-asset rows retain their current accepted, queued, or blocked dispositions.
8. The July 30 world-package addendum's W1-W4 implementation claims are superseded only where it described them as future source work; W0-W4 are source-implemented, while W5 initial certification remains queued because pre-install receipts are absent.
9. The old Charter GUI and route-identity pending-art wording is superseded by the rebuilt semantic animations and seven installed route flag/emblem packages. Runtime package acceptance and provenance remain open.
...
- Older world-order prose that says W1-W4, the full action dispatcher, or dynamic union wars are future work is superseded by `docs/events/012_africa/world_order.md`; W5 remains explicitly queued.
- The historical final-completion and full-goal-delta reports retain their dated counts and must not be reused as current release status.
...
3. Decide whether the broader per-final-focus icon requirement remains queued or is narrowed to the current family consumers.
4. Supply or explicitly reject the pre-install receipt contract required before the six-package W5 certification setter can be designed; do not set `africa_world_package_implementation_ready` from static existence.
...
6. Obtain native/full-string review and placement for the two deferred Afaan Oromoo strings, or explicitly reject them without substitutes.
...
Targeted path and registration checks confirmed the current image, Charter, focus, achievement, portrait, decision, report, route-identity, and carrier-flag evidence described above. The final 16/16 Charter exact-path review, rebuilt semantic animation handoff, focus and decision reaudits, and registration scan report no active registration blocker. The matrix and shared ledger were imported after editing and confirmed 239 matching keys in row order, preserved ledger BOM, and exact status counts of 50 `installed_runtime`, 21 `installed_dormant`, 133 `deferred_controlled_pool`, 16 `deferred_model_required`, 12 `deferred_runtime_gated`, and 7 `deferred_unique_package_required`. The action, focus, B3, AI64, route-identity, and W5 handoffs were read as evidence; no gameplay claim was inferred from them.

No in-game launch, save test, workbook/export edit, binary visual re-authoring, or gameplay audit was performed because those surfaces remain parent-owned or outside this documentation scope. The working tree contains concurrent unrelated changes; no commit was created.

## Remaining risks

The documentation is internally cross-referenced for the release-candidate status, but gameplay acceptance, owner-system achievement proof beyond the B3 closure, Event 013 live integration, carrier provenance, GUI runtime scaling and interaction, thirteen priority-package scenarios, two AI evidence blockers, W5 pre-install certification, world-package runtime readiness, audio rights/wiring, native language review, and broad controlled-pool asset production remain open. The parent should use this handoff and `docs/events/012_africa/overview.md` before starting another implementation tranche.
