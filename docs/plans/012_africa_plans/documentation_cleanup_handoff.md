...
The filesystem and registered `.gfx` evidence covers 23 Event 012 report/news/super-event pictures, 12 Charter static textures, two rebuilt semantic Charter frame sheets with static fallbacks, 13 focus-family textures, 132 achievement DDS files, 16 sovereign portraits, seven route flag ladders and emblem packages, and the selected priority-member decision and report textures. These assets are installed, dormant, promotion-gated, model-required, unique-package-required, or controlled-pool deferred according to the matrix and current owner gates. No model package exists.
