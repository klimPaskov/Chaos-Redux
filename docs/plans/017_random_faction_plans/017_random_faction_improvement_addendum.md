...
| Canonical documentation and GFX consumer | Resolved by the documentation curator | `docs/events/017_random_faction/overview.md` matches the live dynamic selection, local cascade budget, exact history result, decisions, cleanup, achievements, and asset wiring. `docs/assets/017_random_faction/gfx_handoff.md` names `random_faction_bloc_pressure_category`. |
...
- `docs/events/017_random_faction/overview.md` says manual settings dispatch first prefers the current player. The live shared dispatch correctly uses the weighted eligible-country pool for automatic, manual, Event Details, and cluster routes.
