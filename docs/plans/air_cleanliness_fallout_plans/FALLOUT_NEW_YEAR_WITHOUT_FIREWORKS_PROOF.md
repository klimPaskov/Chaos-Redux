...
Status: dormant reviewed tranche with static wiring and a generation-reset stale-path repair, with runtime and wider scheduler blockers remaining.
...
Current-generation cleanup authenticates the candidate, route, branch, token, ticket, payment, and commitment receipts. A dedicated generation-reset abort route now runs before country runtime arrays are rebuilt. It cancels stale opening receipts, refunds only uncommitted branch costs, terminalizes issued result or callback rows, prepares and releases matching cleanup tombstones, and clears the country-only frozen state. Runtime execution of that route and save-recovery acceptance remain unproven.
...
| Stale cleanup after a generation change | A dedicated abort route is statically wired from country runtime cleanup. It cancels stale opening and delayed rows before array rebuild. Runtime execution and save recovery remain unproven. |
...
The candidate, event, Event Log, workbook, and asset wiring have been reviewed statically. Exact branch affordability, normalized callback grading, and the generation-reset abort route are implemented in source. Runtime stale-row execution, save recovery, live event delivery, multiplayer host authority, normal-map presentation, and the exact all-valid-province thermonuclear sweep remain runtime boundaries. No HOI4 process was launched. The Fallout release-floor count does not claim this dormant tranche until activation review and user validation pass.
