...
- state population, live category, historical Air Winter category, buildings, supply, resources, and contamination memory
...
Applies one-time population mutation, permanent state-building level loss, category conversion, resource, supply, and province memory effects from the stored grade.
...
The accepted ordering requires a committed nine-resource survival ledger after final allocation. The survivor-allocation phase now stages a formula-neutral identity transaction and enforces the committed-ledger barrier before player continuation. It covers every assigned successor and every included state through aligned generation and physical-index arrays. State rows freeze snapshot, Air Winter, grading, population, rewrite, supply, and owner provenance. Country rows freeze allocation, state-count, region, archetype, and country-memory provenance. An exact single-error recovery wrapper can reset and restage only a malformed uncommitted identity payload during survivor allocation. Numerical row producers, aggregation, validation, row commits, and the ready setter remain absent. [FALLOUT_SURVIVAL_NUMERICAL_CONTRACT_PROPOSAL.md](FALLOUT_SURVIVAL_NUMERICAL_CONTRACT_PROPOSAL.md) records the current proposed contract and is not accepted implementation authority.
...
The live formula-neutral stage runs after successor allocation and package assignment but before player continuation. It rebuilds an absent or structurally invalid uncommitted identity payload from the current allocation. An owned `survival_ledger_incomplete` retry requires an exact one-error signature and cannot run after survivor allocation or player continuation commits. The future numerical producer must fill state raw rows and country raw, immutable initial, and mutable values for all nine resources. The global commit must match the current transition generation, exact finalized successor count, and exact included-state set. The proposed contract defines suggested 0 to 100 formulas, aggregation, immutable and mutable initialization, validation order, and the ready-last commit sequence. It remains unaccepted, so no implementation writes the values, row receipts, or ready flag.
...
The current schema is version 11 and migration is deliberately fail closed.

- Completed schema-10 and older saves are promoted non-destructively and marked as lacking current-schema and supply-network receipts. No state receipt is fabricated.
- Active schema-7, schema-8, schema-9, and schema-10 transitions may rebuild only in the snapshot phase, before snapshot application or destruction, and without an unrelated error.
- An active schema-10 phase 2 transition may promote before grading mutation.
- An active schema-10 phase 3 through 6 transition may promote only before allocation, with current grading, and with exact live-to-frozen supply-network equality for every destructive-grade state.
...
- An incomplete terminal save with no schema remains under blackout with migration blocked.
...
- permanent loss of a small share of state-building levels
...
- broad permanent infrastructure and industrial level loss
...
All state population loss goes through the shared Deaths system with reason `chaos_meter_deaths_reason.fallout_aftermath`, numeric reason `19`, and state-level records. Fallout first applies its bounded state mutation, observes the exact live delta, and only then registers that observed amount. The Deaths call is told not to apply population again. It still writes the state Deaths map ledger when Deaths tracking is enabled.

The three destructive rewrite phases use separate proof ledgers. Grading proves the deterministic score band, grade, subtype, survival range, and generation for every state. Population loss stores frozen intent, live availability, committed loss, observed loss, Deaths totals, Deaths sequence movement, state-ledger movement, exclusive applied-or-zero result, and generation. Physical collapse proves exact permanent level removal across five state-building families, exact grade modifier, altered-biosphere modifier when applicable, a category target no higher than the frozen live category, the supply-collapse flag, and rewrite generation. A stale completion flag is cleared when its ledger is no longer current. Missing idempotent rows can re-enter only through the exact error signature owned by their current phase.
