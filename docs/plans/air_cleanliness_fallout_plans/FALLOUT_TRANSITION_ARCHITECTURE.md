...
The live two-stage transaction runs after successor allocation and package assignment but before player continuation. Identity staging rebuilds an absent or structurally invalid uncommitted identity payload from the current allocation. Numerical staging fills state raw rows and country raw, immutable initial, and mutable values for all nine resources. Every numerical row is recomputed into separate replay arrays before its row flag is written. The global precommit proof matches the current transition generation, exact finalized successor count, exact state set, frozen ownership, current source generations, arithmetic bounds, and current-equals-initial rule. The numerical coordinator clears the building flag and writes `fallout_survival_ledger_ready` as its final success write. Identity and numerical recovery each require their exact one-error signature and cannot run after survivor allocation or player continuation commits.
...
The current schema is version 12 and migration is deliberately fail closed.
...
- Completed schema-11 saves are promoted non-destructively and marked as lacking the schema-12 survival inputs. No opening ledger is fabricated.
- Active schema-11 transitions may rebuild only in the snapshot phase before snapshot application or destruction. A later schema-11 phase remains blocked.
...
Schema 12 retains the schema-11 snapshot and destructive proof while adding the frozen specialty sources and coal required by the survival ledger. Player and world rows are captured and retried as one epoch. Air Winter opens a distinct producer generation for every attempt. Valid states must pass canonical initialization, normalization, and exact live-to-frozen comparison. Invalid states receive a typed N/A row without initialization. Every state also freezes its live category through a separate enum and `has_state_category` match. Exact live owner, controller, Air Winter payload, live category, specialty-source, and deposit receipts are required while capture is accepted. Population, building, category, and supply-network issue flags are written before mutation. Population uses the accepted grade ladder of 90 through 95 percent and preserves one person in every nonempty state. Recovery observes and reconciles issued transactions without applying loss again. Schema 12 keeps the approved market-access cancellation proof and binds province supply-network collapse receipts to the current generation before the diplomacy, survival, and map-return gates can pass.
