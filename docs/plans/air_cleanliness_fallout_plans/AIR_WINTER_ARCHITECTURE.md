...
5. register event, evacuation, and island-refugee source owners during that pass
...
The live recurring seasonal layer uses the same monthly state pass and bounded owner array. The cycle opener snapshots the documented current engine year once. State processing captures complete durable marker rows before the cooldown gate, so an observed condition does not disappear while its country is cooling down. A marker retains its origin year, origin cycle, origin owner, presentation class, score, and typed event id until a validated dispatch consumes it. First frost also stores a typed route subtype that separates the exact Desert City and generic regional uses of event 13. The same state pass records at most one live coastal refugee-source state per owner in a separate bounded array. That registry is rebuilt for the current cycle and cleared after dispatch.
...
The implemented Phase 2 island-refugee route checks the exact engine `is_island_state` or `is_one_state_island` topology after mountain-capital identity and before the generic city route. A 131-point route bonus lets it outrank another same-phase candidate without overtaking a later phase. Dispatch compares only the bounded source-owner array, excludes the receiving country, and chooses the highest phase and pressure score with lower state id breaking an exact tie. If no live source exists, the route writes no cooldown, seen flag, or seasonal receipt. A positive opening moves a capped share of destination population from the exact source state, preserves 1,000 people at the source, and adds exactly the applied source loss to the destination. Rescue, quarantine, and exclusion record separate state and country memories and resolve through six deterministic results after 30 days. Full proof is recorded in `AIR_WINTER_PHASE_2_ISLAND_REFUGEE_SOURCE_AND_POPULATION_PROOF.md`.

The implemented Phase 2 Desert City route checks exact arid urban identity after mountain capitals and engine islands but before the generic city route. Its 131-point same-phase bonus, typed `desert_city` subtype, first-frost row, country candidate, and owner-bound opening receipt preserve that identity without changing the generic arid or Mediterranean event 13 fallback. Municipal works, railway tankers, and motor water columns use distinct payments, ledgers, AI projections, and temporary modifiers. Event 49 resolves the three branches through nine exhaustive results after 30 days. Casualties use Deaths, failures damage only an operational repairable target, and the result independently proves the complete pending-owner row. A malformed branch without its pending receipt cancels during the existing state reconciliation pass. Full proof is recorded in `AIR_WINTER_PHASE_2_DESERT_CITY_EVENT_PROOF.md`.

The Phase 2 seed-ledger route retains its regional selection and resolves every opening choice through `chaosx.fallout.18` after 45 days. Each valid opening click refreshes the 46-day country cooldown before scheduling that result. Guarded seed plots divert 10 percent of local factory availability for a 46-day review window. Protected plots and breeding stock use disclosed ledger thresholds with exact pre-choice AI translations. Emergency herd slaughter has a fixed later depletion cost. All three branches share the established pending owner and cleanup transaction. The Fallout numerical resource contract is accepted and implemented. Post-Fallout food recovery remains outside this route until a separate reviewed consumer contract and its implementation exist. Full proof is recorded in `AIR_WINTER_PHASE_2_SEED_LEDGER_EVENT_PROOF.md`.
