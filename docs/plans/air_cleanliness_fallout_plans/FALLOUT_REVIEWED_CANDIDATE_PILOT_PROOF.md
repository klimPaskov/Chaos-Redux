...
The reviewed candidate producer now covers `74` ordinary dormant rows through The Clean Certificate candidate `719`, with `600` defined event blocks through `chaosx.fallout.725` and `0 of 660` countable release-floor blocks. The reviewed route upper bound is `7181`. Candidate 719 uses transaction `710074`, route `7180`, and survivor Event Log history `9180`. The producer selects the lowest eligible native Quarantine Air Winter state after a closed Returning Disease memory and preserves it in the dispatch envelope. Each opening gate rehydrates and rechecks that issued target, so this is static source evidence rather than an accepted runtime path.
