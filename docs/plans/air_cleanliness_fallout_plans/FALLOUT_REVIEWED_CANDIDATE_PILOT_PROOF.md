...
The reviewed candidate producer now covers `76` ordinary dormant rows through Children of the Cordon candidate `733`, with `614` defined event blocks through `chaosx.fallout.739` and `0 of 660` countable release-floor blocks. The reviewed route upper bound is `7185`. Candidate 733 uses transaction `710076`, route `7184`, and survivor Event Log history `9182`. The producer admits a current country with a completed Border Inspection Crisis memory, generation-change evidence, durable survival rows, and one affordable branch. The country-only opening has no fabricated state target, so this is static source evidence rather than an accepted runtime path.
...
Current tranche note: The Clean Certificate is the latest reviewed candidate at `719`, transaction `710074`, route `7180`, events `719` through `725`, and history `9180`. The producer now carries seventy-four ordinary rows and 600 defined event blocks. The older count and constant paragraphs below are historical snapshots retained for audit traceability.
