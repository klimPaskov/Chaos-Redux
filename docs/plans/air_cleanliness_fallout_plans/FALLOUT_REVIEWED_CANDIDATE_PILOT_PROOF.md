...
The reviewed candidate producer now covers `84` ordinary dormant rows through The Pirate Amnesty candidate `789`, with `670` defined event blocks through `chaosx.fallout.795` and `0 of 660` countable release-floor blocks. The reviewed route upper bound is `7201`. Candidate 789 uses transaction `710084`, route `7200`, and survivor Event Log history `9190`. The producer selects the lowest valid West African Port Confederacies coastal or naval-base state with a closed Harbor Without a City memory and a foreign neighbor after authenticating the maritime-remnant memory, then admits durable Air Winter and survival rows with raider ledgers and one affordable branch. Conditional amnesty, privateer service, public trial, and reef exile share deterministic `35` day result and `270` day callback contracts, hidden-AI parity, bounded Deaths failure, Air Winter, raider-ledger, opinion, Supply Access effects, cleanup, dedicated localisation, survivor Event Log routing, dedicated report art, a reviewed source specification, and `FALLOUT_PIRATE_AMNESTY_CHAIN_PROOF.md`. This is static source evidence rather than an accepted runtime path.

## Current reconciliation after The Harbor Without a City

The reviewed candidate producer now covers `83` ordinary dormant rows through The Harbor Without a City candidate `782`, with `663` defined event blocks through `chaosx.fallout.788` and `0 of 660` countable release-floor blocks. The reviewed route upper bound is `7199`. Candidate 782 uses transaction `710083`, route `7198`, and survivor Event Log history `9189`. The producer selects the lowest valid West African Port Confederacies coastal or naval-base state with a closed Ghost Convoy memory and a foreign neighbor after authenticating the maritime-remnant memory, then admits durable Air Winter and survival rows with harbor ledgers and one affordable branch. Port republic, inland alliance, military occupation, and evacuation share deterministic `45` day result and `330` day callback contracts, hidden-AI parity, bounded Deaths failure, Air Winter, harbor-ledger, opinion, and Supply Access effects, cleanup, dedicated localisation, survivor Event Log routing, dedicated report art, a reviewed source specification, and `FALLOUT_HARBOR_WITHOUT_CITY_CHAIN_PROOF.md`. This is static source evidence rather than an accepted runtime path.

## Current reconciliation after The Ghost Convoy

The reviewed candidate producer now covers `82` ordinary dormant rows through The Ghost Convoy candidate `775`, with `656` defined event blocks through `chaosx.fallout.781` and `0 of 660` countable release-floor blocks. The reviewed route upper bound is `7197`. Candidate 775 uses transaction `710082`, route `7196`, and survivor Event Log history `9188`. The producer selects the lowest valid West African Port Confederacies coastal or naval-base state with a closed Captain's Articles memory and a foreign neighbor after authenticating the maritime-remnant memory, then admits durable Air Winter and survival rows with convoy ledgers and one affordable branch. Approach, shadow, avoid, and signal publicly share deterministic `42` day result and `300` day callback contracts, hidden-AI parity, bounded Deaths failure, Air Winter, convoy-ledger, opinion, and Supply Access effects, cleanup, dedicated localisation, survivor Event Log routing, dedicated report art, a reviewed source specification, and `FALLOUT_GHOST_CONVOY_CHAIN_PROOF.md`. This is static source evidence rather than an accepted runtime path.
...
The Returning Disease follows Doctor's Coup. Its four branches resolve after twenty-four days and schedule a two hundred ten day callback. The chain freezes current Air Winter, Supply Access, public-health, grievance, treatment capacity, border trust, aid dependence, concealment pressure, and cause-memory ledgers. Seal the Infected District, Send Targeted Care Teams, Request the Aid Convoy, and Keep the Outbreak Off the Books have distinct Food, Medicine, Fuel, Scrap, Power, and Recognition costs. Results and callbacks change the surviving country's medicine, recognition, cohesion, public health, grievance, Air Winter, supply access, treatment capacity, border trust, aid dependence, concealment pressure, and cause memories. Failures route bounded losses through Deaths, damage the target infrastructure, write survivor-country Event Log memory, and clear both delayed tickets through authenticated cleanup. The chain preserves the Quarantine government archetype, uses dedicated report art, and remains dormant because neither scheduler activation flag is set.

The Clean Certificate follows Returning Disease. Its four branches resolve after thirty days and schedule a one hundred eighty day certification review. The chain freezes current Air Winter, Supply Access, public-health, grievance, inspection capacity, public trust, route dependence, appeal backlog, and cause-memory ledgers. Public Service, Paid Inspection, Political Leverage, and Regional Standard have distinct Food, Medicine, Scrap, Power, Fuel, Recognition, and Cohesion costs. Results and reviews change the surviving country's Medicine, Recognition, Cohesion, public health, grievance, Air Winter, Supply Access, inspection capacity, public trust, route dependence, appeal backlog, and cause memories. Failures route bounded losses through Deaths, write survivor-country Event Log memory, and clear both delayed tickets through authenticated cleanup. The chain preserves the Quarantine government archetype, uses dedicated report art, and remains dormant because neither scheduler activation flag is set.

Border Inspection Crisis follows Clean Certificate. Its four branches resolve after thirty days and schedule a one hundred eighty day seasonal review. The chain freezes current Air Winter, Supply Access, public-health, grievance, border guard capacity, border trust, trade dependency, crossing backlog, cause memory, and a foreign neighboring state receipt. Close the Crossing, Admit a Joint Team, Issue Relief Exemptions, and Deploy a Military Screen have distinct Food, Medicine, Scrap, Power, Fuel, Recognition, and Cohesion costs. Results and reviews change the surviving country's Medicine, Recognition, Cohesion, public health, grievance, Air Winter, Supply Access, border ledgers, bilateral opinion, and cause memories. Failures route bounded losses through Deaths, write survivor-country Event Log memory, and clear both delayed tickets through authenticated cleanup. The chain preserves the Quarantine government archetype, uses dedicated report art, and remains dormant because neither scheduler activation flag is set.

Children of the Cordon follows Border Inspection Crisis. Its four branches resolve after thirty-two days and schedule a two hundred forty day cohort callback. The chain freezes country Cohesion, Food, Shelter, Recognition, generation count, youth trust, and movement law. Rewrite the Movement Law, Open Service Travel, Continue the Strict Cordon, and Give the Wards Their Own Boards have distinct survival costs and AI priorities. Results and callbacks change Food, Medicine, Power, Recognition, Cohesion, Stability, War Support, future-leader pool, youth trust, movement law, government route, and branch memory. Failures route bounded losses through Deaths, write survivor-country Event Log history `9182`, use dedicated report art, and clear both delayed tickets through authenticated cleanup. The chain remains dormant because neither scheduler activation flag is set.

Open the Sealed City follows Border Inspection Crisis into a recovery-year city or large-city state. Its four branches reopen civic rooms, grant scientific access, send a civilian-inspected salvage command, or establish a permanent memorial district. The chain freezes Shelter, Supply Access, Reclamation, Exposure, Disease, infrastructure, population, country Food, Medicine, Power, Recognition, and Cohesion. Results arrive after thirty-five days and the recovery callback after two hundred seventy days. Results and callbacks change Air Winter, supply, infrastructure evidence, salvage access, memorial readiness, public trust, faction pressure, Stability, War Support, and bounded Deaths. Failures route through Deaths, write survivor-country Event Log history `9183`, use dedicated report art, and clear the state reservation through authenticated cleanup. The chain remains dormant because neither scheduler activation flag is set.

Health Without Borders follows Open the Sealed City into the fifth campaign year. Its four branches establish a shared health compact, license client inspections, adopt an isolation doctrine, or expand humanitarian service. The chain freezes Shelter, Supply Access, Reclamation, Exposure, Disease, infrastructure, population, country Food, Medicine, Power, Recognition, and Cohesion. Results arrive after forty-five days and the regional review after three hundred sixty-five days. Results and callbacks change Air Winter, supply, clinic capacity, regional trust, inspection capacity, isolation readiness, humanitarian reach, Stability, War Support, and bounded Deaths. Failures route through Deaths, write survivor-country Event Log history `9184`, use dedicated report art, and clear the state reservation through authenticated cleanup. The chain remains dormant because neither scheduler activation flag is set.

The Envoy at the Gate follows the Congo Green Basin memory. Its four branches use concrete Food, Medicine, Scrap, Power, Fuel, Recognition, and Cohesion costs. The state selector requires a current foreign neighbor and the owner memory for the mutant polity. The registry freezes civic legitimacy, external pressure, Corridor Security, Partner Trust, Refugee Pressure, Border Fatigue, Air Winter, and the neighbor receipt. Results arrive after forty-two days and the callback after three hundred days. Results and callbacks update the envoy ledgers, Air Winter, Supply Access, bilateral opinion, Stability, War Support, and bounded Deaths. The chain writes Event Log history `9186`, uses dedicated report art, and remains dormant because neither scheduler activation flag is set.
...
Regional Health Compact follows Health Without Borders. Its four branches share a forty-two day result and three hundred day callback, freeze a foreign neighboring state, update health-authority, corridor, refugee, and fatigue ledgers, apply dedicated bilateral opinion memories, route failures through Deaths, record survivor Event Log payloads, and release both state reservations through authenticated cleanup. Its dedicated proof is `FALLOUT_REGIONAL_HEALTH_COMPACT_CHAIN_PROOF.md`.
...
Current tranche note: Health Without Borders is the latest reviewed candidate at `747`, transaction `710078`, route `7188`, events `747` through `753`, and history `9184`. The producer now carries seventy-eight ordinary rows and 628 defined event blocks. The older count and constant paragraphs below are historical snapshots retained for audit traceability.

The Fallout scheduler now owns a reviewed ordinary-candidate producer for the current global-survival pilot chains. The producer is deliberately narrower than the future 660-block release surface. It exists to prove the candidate-array contract, target-shape contract, and deterministic state selection without opening either scheduler activation flag.
...
- The candidate pilot constants define seventy-three candidate ids, seventy-three transaction keys, and seventy-three route ids in a dedicated Fallout namespace. The Returning Disease candidate `712` is emitted by the candidate producer after the current-generation Quarantine Air Winter state gate and a closed Doctor's Coup memory, and carries a typed state target, a global-survival route with no false regional claim, Quarantine government proof, Air Winter pressure proof, Medicine resource proof, controller proof, campaign-day proof, and at least one affordable branch.
...
Bowl now has a static generation-reset abort route for pre-result stale
cancellation and generation-drift cleanup, while runtime execution and save
recovery remain unproven. Same-generation committed cleanup uses the generic
country registry. The manual survival contract remains a
...
Its country gate requires a current Fallout registry row, current-generation East Asian regional identity, durable Survival resources, campaign days from `300` through `2599`, minimum Food, Clean Water, Fuel, Medicine, Shelter Capacity, Recognition, and Cohesion, an affordable branch, and no conflicting ordinary receipt. Its target shape is subject type `none` with target `0`. The selected human or AI branch is rechecked for exact affordability before payment.

Four authored branches cover Quiet Remembrance, Hold a Ration Feast, Stage a Military Ceremony, and Leave the Night to Local Festivals. The result uses frozen survival and Air Winter values with branch thresholds. The callback uses a weighted normalized 0 through 100 score over current Food, Clean Water, Recognition, Cohesion, Calendar Trust, Civic Participation, and inverse Ration Strain. The chain uses exact 21-day and 330-day delays, bounded Deaths effects, durable branch memories, hidden-AI parity, Event Log history `9170`, dedicated report art, authenticated current-generation cleanup, and a statically wired generation-reset abort route that clears stale country-only opening and delayed rows before runtime rebuild. Runtime execution and save recovery remain unproven. The chain does not schedule itself again.
...
The hidden-AI route uses the same target proof, branch costs, grade, deterministic tie order, delayed receipts, and cleanup. The chain writes only post-consequence survivor memory history. It does not register the Fallout transition, blackout, population sweep, or Air Cleanliness shutdown as a public event log or evolution row. Dedicated report art and static sprite wiring are present under `docs/assets/670_work_for_rations/` and `interface/fallout_consolidated.gfx`.
...
## The Engineer Franchise correction

The Engineer Franchise is the sixty-sixth reviewed ordinary candidate.

It uses candidate id `796`, transaction `710085`, route `7202`, event blocks `796` through `802`, and Event Log history `9191`.

Its country gate requires a current Fallout registry row, East Asian Technate identity, Manchurian Reactor Keeps memory `46`, closed Reactor Without a Country memory, durable Medicine, Cohesion, Recognition, engineering legitimacy, class pressure, an affordable branch, and no conflicting ordinary receipt. Its state selector chooses the lowest eligible owner-controlled power or industrial state with a foreign neighboring state, produced Air Winter values, surviving population, native infrastructure, and bounded Disease Pressure.

Four authored branches cover Engineer Vote, Public Council, Military Oversight, and Universal Vote. The result and callback use frozen state, neighbor, Air Winter, Supply Access, and technical ledgers. The result resolves after exactly `35` days and schedules a `270` day technical callback. Success routes the government archetype, while partial and failed outcomes retain contested power-board memories. Failure uses bounded Deaths requests against the authenticated target state, records Event Log history `9191`, preserves hidden-AI parity, and closes through authenticated cleanup.
...
The asset package is `docs/assets/796_engineer_franchise/` with source, processed PNG, runtime DDS, manifest hashes, and sprite `GFX_report_event_fallout_engineer_franchise`. The authoritative workbook is `docs/spreadsheets/chaos_redux_events_catalog.xlsx`. The exported `FALLOUT-796` row covers four branch choices, twelve result outcomes, three callback outcomes, authenticated cancellation, and visible budget cost `3` with `Needs Testing` status.

## The Load Shedding tranche

Load Shedding is the next reviewed ordinary candidate after The Engineer Franchise. It uses candidate id `803`, transaction `710086`, route `7204`, event blocks `803` through `809`, and Event Log history `9192`.

Its country gate requires a current Fallout registry row, East Asian Technate identity, Manchurian Reactor Keeps memory `46`, a durable Engineer Franchise branch memory, durable Medicine, Cohesion, Recognition, load legitimacy, sector pressure, an affordable branch, and no conflicting ordinary receipt. Its state selector chooses the lowest eligible owner-controlled power or industrial state with a closed Engineer Franchise state memory, foreign neighboring state, produced Air Winter values, surviving population, native infrastructure, and bounded Disease Pressure.

Four authored branches cover Hospital Priority, Food Priority, Industry Priority, and Defense Priority. The result and callback use frozen state, neighbor, Air Winter, Supply Access, and load and material ledgers. The result resolves after exactly `28` days and schedules a `240` day first-winter callback. Failure uses bounded Deaths requests against the authenticated target state. The hidden-AI route uses the same affordability, grade, delayed receipts, Event Log payloads, and cleanup. The chain writes only post-consequence memory and does not register Fallout itself as an event, evolution, or ordinary Event Log entry.

The chain remains dormant and contributes zero countable release-floor blocks. Scheduler activation, host authority, save recovery, delayed queue delivery, multiplayer behavior, runtime Event Log rendering, and player-visible art remain unproven. Static proof is recorded in `FALLOUT_LOAD_SHEDDING_CHAIN_PROOF.md`, and the asset package is `docs/assets/803_load_shedding/`.

## The Reactor Shift tranche

The Reactor Shift is the next reviewed ordinary candidate after Load Shedding. It uses candidate id `810`, transaction `710087`, route `7206`, event blocks `810` through `816`, and Event Log history `9193`.

Its country gate requires a current Fallout registry row, East Asian Technate identity, Manchurian Reactor Keeps memory `46`, a durable Load Shedding branch memory, durable Medicine, Cohesion, Recognition, safety legitimacy, operator fatigue, an affordable branch, and no conflicting ordinary receipt. Its state selector chooses the lowest eligible owner-controlled power or industrial state with a closed Load Shedding state memory, foreign neighboring state, produced Air Winter values, surviving population, native infrastructure, and bounded Disease Pressure.

Four authored branches cover safe negotiation, compelled duty, controlled shutdown, and a foreign repair team. The result and callback use frozen state, neighbor, Air Winter, Supply Access, and reactor safety ledgers. The result resolves after exactly `35` days and schedules a `270` day first-winter callback. Failure uses bounded Deaths requests against the authenticated target state. The hidden-AI route uses the same affordability, grade, delayed receipts, Event Log payloads, and cleanup. The chain writes only post-consequence memory and does not register Fallout itself as an event, evolution, or ordinary Event Log entry.

The chain remains dormant and contributes zero countable release-floor blocks. Scheduler activation, host authority, save recovery, delayed queue delivery, multiplayer behavior, runtime Event Log rendering, and player-visible art remain unproven. Static proof is recorded in `FALLOUT_REACTOR_SHIFT_CHAIN_PROOF.md`, and the asset package is `docs/assets/810_reactor_shift/`.
