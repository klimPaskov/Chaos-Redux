...
The reviewed candidate producer now covers `73` ordinary dormant rows through The Returning Disease candidate `712`, with `593` defined event blocks through `chaosx.fallout.718` and `0 of 660` countable release-floor blocks. The reviewed route upper bound is `7179`. Candidate 712 uses transaction `710073`, route `7178`, and survivor Event Log history `9179`. The producer selects the lowest eligible native Quarantine Air Winter state after a closed Doctor's Coup memory and preserves it in the dispatch envelope. Each opening gate rehydrates and rechecks that issued target, so this is static source evidence rather than an accepted runtime path.
...
Current tranche note: The Returning Disease is the latest reviewed candidate at `712`, transaction `710073`, route `7178`, events `712` through `718`, and history `9179`. The producer now carries seventy-three ordinary rows and 593 defined event blocks. The older count and constant paragraphs below are historical snapshots retained for audit traceability.
