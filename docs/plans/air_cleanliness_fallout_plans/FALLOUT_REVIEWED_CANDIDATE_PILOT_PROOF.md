...
The reviewed candidate producer now covers `77` ordinary dormant rows through Open the Sealed City candidate `740`, with `621` defined event blocks through `chaosx.fallout.746` and `0 of 660` countable release-floor blocks. The reviewed route upper bound is `7187`. Candidate 740 uses transaction `710077`, route `7186`, and survivor Event Log history `9183`. The producer selects the lowest valid owned city or large-city state after a completed Border Inspection Crisis memory, then admits a current Quarantine country with durable Air Winter and survival rows and one affordable branch. This is static source evidence rather than an accepted runtime path.
...
Current tranche note: Open the Sealed City is the latest reviewed candidate at `740`, transaction `710077`, route `7186`, events `740` through `746`, and history `9183`. The producer now carries seventy-seven ordinary rows and 621 defined event blocks. The older count and constant paragraphs below are historical snapshots retained for audit traceability.
