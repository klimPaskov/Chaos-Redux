...
The reviewed candidate producer now covers `75` ordinary dormant rows through Border Inspection Crisis candidate `726`, with `607` defined event blocks through `chaosx.fallout.732` and `0 of 660` countable release-floor blocks. The reviewed route upper bound is `7183`. Candidate 726 uses transaction `710075`, route `7182`, and survivor Event Log history `9181`. The producer selects the lowest eligible native Quarantine Air Winter state after a closed Clean Certificate memory, requires a foreign neighbor, and preserves both states in the dispatch envelope. Each opening gate rehydrates and rechecks the issued target and crossing partner, so this is static source evidence rather than an accepted runtime path.
