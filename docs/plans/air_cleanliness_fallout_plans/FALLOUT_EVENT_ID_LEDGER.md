...
The living-world pilot now contains `607` defined event blocks across the reviewed ordinary range through `732`, including the dormant Border Inspection Crisis tranche at `726` through `732`. The current reviewed ordinary row count is `75`. Border Inspection Crisis uses candidate `726`, transaction `710075`, route `7182`, and survivor Event Log history `9181`. The reserved route upper bound is `7183`. Every row remains dormant and uncounted, so the release-floor total is still `0 of 660`. Historical count snapshots below are retained as corrections and are superseded by this paragraph.
