...
The living-world pilot now contains `593` defined event blocks across the reviewed ordinary range through `718`, including the dormant The Returning Disease tranche at `712` through `718`. The current reviewed ordinary row count is `73`. The Returning Disease uses candidate `712`, transaction `710073`, route `7178`, and survivor Event Log history `9179`. The reserved route upper bound is `7179`. Every row remains dormant and uncounted, so the release-floor total is still `0 of 660`. Historical count snapshots below are retained as corrections and are superseded by this paragraph.
