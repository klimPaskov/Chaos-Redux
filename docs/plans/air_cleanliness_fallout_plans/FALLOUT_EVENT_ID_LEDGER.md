...
The living-world pilot now contains `600` defined event blocks across the reviewed ordinary range through `725`, including the dormant The Clean Certificate tranche at `719` through `725`. The current reviewed ordinary row count is `74`. The Clean Certificate uses candidate `719`, transaction `710074`, route `7180`, and survivor Event Log history `9180`. The reserved route upper bound is `7181`. Every row remains dormant and uncounted, so the release-floor total is still `0 of 660`. Historical count snapshots below are retained as corrections and are superseded by this paragraph.
