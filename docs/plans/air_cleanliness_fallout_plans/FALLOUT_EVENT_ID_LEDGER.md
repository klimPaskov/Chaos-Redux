...
The living-world pilot now contains `621` defined event blocks across the reviewed ordinary range through `746`, including the dormant Open the Sealed City tranche at `740` through `746`. The current reviewed ordinary row count is `77`. Open the Sealed City uses candidate `740`, transaction `710077`, route `7186`, and survivor Event Log history `9183`. The reserved route upper bound is `7187`. Every row remains dormant and uncounted, so the release-floor total is still `0 of 660`. Historical count snapshots below are retained as corrections and are superseded by this paragraph.
