...
The living-world pilot now contains `614` defined event blocks across the reviewed ordinary range through `739`, including the dormant Children of the Cordon tranche at `733` through `739`. The current reviewed ordinary row count is `76`. Children of the Cordon uses candidate `733`, transaction `710076`, route `7184`, and survivor Event Log history `9182`. The reserved route upper bound is `7185`. Every row remains dormant and uncounted, so the release-floor total is still `0 of 660`. Historical count snapshots below are retained as corrections and are superseded by this paragraph.
