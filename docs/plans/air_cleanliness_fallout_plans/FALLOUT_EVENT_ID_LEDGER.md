...
The living-world pilot now contains `628` defined event blocks across the reviewed ordinary range through `753`, including the dormant Health Without Borders tranche at `747` through `753`. The current reviewed ordinary row count is `78`. Health Without Borders uses candidate `747`, transaction `710078`, route `7188`, and survivor Event Log history `9184`. The reserved route upper bound is `7189`. Every row remains dormant and uncounted, so the release-floor total is still `0 of 660`. Historical count snapshots below are retained as corrections and are superseded by this paragraph.
