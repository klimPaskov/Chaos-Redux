# Fallout The Machine Suggests chain proof

## Scope

This proof covers the dormant ordinary Fallout chain The Machine Suggests. It does not claim that Fallout itself is an ordinary event, that the blackout is active, or that the Fallout-owned scheduler has been accepted by the live engine.

## Registry and identity

- Candidate id: `845`.
- Transaction key: `710092`.
- Route: `7216`.
...
The candidate requires a current-generation East Asian Technate with Manchurian Reactor Keeps memory, a current state carrying the closed Laboratory Citizenship memory, produced Air Winter and Supply Access receipts, surviving population and native infrastructure, a foreign neighbor, bounded Disease pressure, durable machine ledgers, and one affordable branch. It selects the lowest eligible owner-controlled industrial or arms state and uses the same Fallout ordinary-request envelope as the preceding reviewed chains.

## Deterministic chain

The opening freezes the country, state, owner, controller, generation, Air Winter values, Supply Access, and lowest authenticated foreign neighbor before payment. The four branches are Follow Recommendation, Sealed Protocol Trial, Human Ledger Review, and Public Advisory Seat. The result is graded from the frozen state values and machine ledgers after exactly `63` days. The callback runs after exactly `390` days and writes durable machine protocol memory. Both stages apply bounded state and country effects, including Deaths requests on failure paths, then record authenticated Event Log payloads.
...
Static inspection covers the owned event ids, localisation references, helper calls, dynamic modifiers, opinion modifiers, constants, braces, quote parity, and forbidden punctuation. Focused Event Inspector lint is the intended engine-facing review surface for `chaosx.fallout.845`. No HOI4 process was launched. Scheduler activation, host authority, save recovery, multiplayer delivery, live Event Log rendering, full-screen blackout integration, and exact engine-native world sweep remain outside this proof.
