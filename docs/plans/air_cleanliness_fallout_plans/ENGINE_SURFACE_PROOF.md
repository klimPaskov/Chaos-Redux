...
The scheduler does not initialize survival values because the accepted specs do not define numeric source formulas. Only the schema and nine resource identities are reserved. The required state and country receipt transaction and pre-player-continuation transition barrier are absent. Schema 8 now authenticates each frozen Air Winter input as either a current produced row or an explicit N/A row. Active arcs, delayed tickets, bilateral reservations, invalid-target handling, hidden AI resolution, cleanup recovery, and bounded candidate selection remain absent. `FALLOUT_EVENT_SCHEDULER_PROOF.md` records the full scheduler contract. `FALLOUT_AIR_WINTER_SNAPSHOT_PROVENANCE_PROOF.md` records the producer and lock contract.
...
World transition schema 8 captures player and world rows through one effect-chain epoch. The shared epoch records the transition generation and capture date. Air Winter opens a distinct producer generation for every attempt, including a retry on the same date. Valid state rows initialize and normalize through the Air Winter-owned producer, then copy only after the complete canonical row passes. Invalid states receive a typed N/A row without initialization. The synchronous capture proof compares the frozen produced payload to the live producer receipt. State rows also prove that stored owner and controller scopes equal the live owner and controller during capture. Player rows prove that every currently owned state has one matching origin row and that no extra origin row points back to the player. A retry discards and rebuilds both snapshot halves together. Blackout and world-end ownership are committed only after both halves pass. After capture, grading and destructive receipts use the frozen structural snapshot row and do not require live ownership or live Air Winter values to remain unchanged. The three destructive state ledgers remain durable after valid allocation changes ownership. Each row carries the active transition generation. A phase completion flag is written only when no state fails its phase-local current-row trigger.

Grading uses only frozen request, nuclear, Air Winter, contamination, population, category, building, resource, coastal, war, owner, and controller inputs. A calculation helper recomputes the expected score and survival value from those frozen inputs. The row requires both reconciliation receipts, the exact score band, grade from 0 through 5, altered-biosphere subtype consistency, survival range, and current generation. A failed grading row is safe to clear and rebuild because no destructive state effect has started.

Population loss calls the shared `apply_exact_state_civilian_population_loss` transaction with Deaths reason `fallout_aftermath`. Before the call, the row records observed population, the request recomputed from frozen population and grade percentage, population available above the protected floor, and the expected clamped loss. After the call, it records observed population, the helper output, and the before-and-after difference. The phase-local row is current only when frozen request, expected loss, helper output, observed difference, and current `state_population_k` agree. Applied and zero flags are mutually exclusive. Recovery never calls the Deaths transaction a second time. It re-reads population and heals only an exact existing receipt. Once accepted, the durable row preserves the same stored transaction and generation without requiring later `state_population_k` to remain fixed.

Physical collapse records an immediate pre-damage baseline, available level, request, post-damage value, and observed delta for infrastructure, civilian factories, military factories, air bases, and dockyards. Each family must match independently before the phase-local receipt passes. The official effects catalogue documents `damage_building` in state scope. The official dynamic-variable catalogue documents `damaged_building_level` and `non_damaged_building_level` with a building-type target. Recovery re-reads the five families without issuing a second damage effect. Once accepted, the durable building receipt retains the stored per-family request and observed delta without requiring later repair state to match. Exact grade modifiers are verified with `has_dynamic_modifier` and can be removed and rebuilt safely. Wasteland, one-step category results, and unchanged category results are verified with `has_state_category`. Category recovery changes receipt flags only when the live category already matches the frozen deterministic result. Grade 3 and above requires the supply-collapse receipt.

State-scoped `damage_building` is not used for `supply_node` or `rail_way`. The official effect documentation says that a provincial building call without a province searches for the first matching province. It cannot prove aggregate network loss across a state. Supply collapse therefore uses its exact state flag and modifier receipt. Province-level network destruction remains blocked until a deterministic province registry and per-province receipt exist.

The official documentation does not state whether `state_population_k` and `damaged_building_level@type` must expose a mutation immediately later in the same effect chain. No Hearts of Iron IV run was authorized. The next coordinator attempt can reconcile a delayed read from its stored baseline without repeating population or building loss. A phase-local value that still differs remains under blackout with its exact owned error. Map return then uses durable population and building transaction receipts. Normal later demographic change or building repair does not invalidate already accepted destruction. Immediate read timing remains an unresolved runtime acceptance obligation.
