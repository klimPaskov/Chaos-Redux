...
The official scripted-GUI schema binds `window_name` to an independent `containerWindowType`. The current `interface/fallout_world_end.gui` blackout root is parentless, independent, full-screen, percentage-sized, and uses a Fallout-owned opaque tile plus a full-screen button blocker. Offline GUI inspection and renders returned structural artifacts for 1920 by 1080 and 2560 by 1440. Root parentlessness and percentage geometry do not prove priority over every hardcoded popup.
