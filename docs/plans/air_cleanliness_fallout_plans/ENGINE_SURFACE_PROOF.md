...
World transition schema 7 captures player and world rows through one effect-chain epoch. The shared epoch records the transition generation and capture date. State rows prove that stored owner and controller scopes equal the live owner and controller during capture. Player rows prove that every currently owned state has one matching origin row and that no extra origin row points back to the player. A retry discards and rebuilds both snapshot halves together. After capture, grading and destructive receipts use the structural snapshot row and do not require live ownership to remain unchanged. The three destructive state ledgers remain durable after valid allocation changes ownership. Each row carries the active transition generation. A phase completion flag is written only when no state fails its phase-local current-row trigger.
