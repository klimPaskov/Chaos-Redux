# Fallout First Streetlight chain proof
...
operators, and no non-ASCII script tokens. Event ids `317` through `323` are
...
`chaosx.fallout.317`, with helper expansion disabled and bounded nodes and
...
First Streetlight is a dormant reviewed pilot and contributes zero countable
blocks to the 660-block release floor. It is suitable for deeper expansion
only after the engine-sensitive coordinator surfaces are proven or the blocker
is resolved.
