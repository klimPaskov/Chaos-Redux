...
The current documented ordinary pilot total is seventy-five rows and 607 defined blocks, all dormant and outside release-floor credit. The named global-survival subset above is retained as the original country-memory and recovery reference list. The latest reviewed row is Border Inspection Crisis candidate `726`, with dedicated proof in `FALLOUT_BORDER_INSPECTION_CRISIS_CHAIN_PROOF.md`.
...
The current reviewed ordinary pilot has `75` dormant rows and `607` defined event blocks through Border Inspection Crisis candidate `726`, event ids `726` through `732`, transaction `710075`, route `7182`, and survivor Event Log history `9181`. The reviewed route upper bound is `7183`. Both scheduler activation flags remain unset. Candidate 726 has static proof for a current Quarantine owner, completed Clean Certificate memory, native Air Winter state, foreign neighbor, all-branch affordability, deterministic grading, branch-aware hidden AI, bounded Deaths loss, bilateral opinion, survivor Event Log memory, dedicated report art, border memories, and authenticated cleanup. The wider pilot still has stale cleanup and option-competition priority gaps in earlier reviewed rows. The exact engine-native all-valid-province thermonuclear sweep remains blocked, the manual Fallout survival contract remains a direct `90` to `95` percent prestrike population-loss band, and Hearts of Iron IV was not launched.
...
Border Inspection Crisis is the current reviewed row at candidate `726`, transaction `710075`, route `7182`, events `726` through `732`, and history `9181`. The ordinary pilot now has seventy-five dormant rows and 607 defined blocks. Countable release-floor credit remains 0 of 660. Older count paragraphs below are historical snapshots.

The Border Inspection Crisis workbook row is recorded with identity `FALLOUT-726` and `Needs Testing` status.

## Proof index

The reviewed-candidate proof index above retains historical count wording for earlier snapshots. The current ordinary pilot is seventy-five rows and 607 defined blocks through Border Inspection Crisis, with the current chain proofs recorded in `FALLOUT_BORDER_INSPECTION_CRISIS_CHAIN_PROOF.md`, `FALLOUT_CLEAN_CERTIFICATE_CHAIN_PROOF.md`, `FALLOUT_RETURNING_DISEASE_CHAIN_PROOF.md`, `FALLOUT_DOCTORS_COUP_CHAIN_PROOF.md`, `FALLOUT_NEW_YEAR_WITHOUT_FIREWORKS_PROOF.md`, `FALLOUT_SECOND_DUST_BOWL_PROOF.md`, `FALLOUT_GREAT_LAKES_LOCK_WINTER_PROOF.md`, `FALLOUT_WORK_FOR_RATIONS_CHAIN_PROOF.md`, and `FALLOUT_HUNGRY_BATTALION_CHAIN_PROOF.md`.
