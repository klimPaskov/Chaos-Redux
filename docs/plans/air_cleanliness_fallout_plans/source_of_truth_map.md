...
Status: current implementation map, reconciled 2026-07-28 after Great Lakes Lock Winter. Fallout remains incomplete and dormant where the proof ledger says so.
...
| Air Winter phases and consequences | `docs/air_cleanliness_winter.md` and tranche proofs | `common/scripted_effects/air_cleanliness_winter_effects.txt`, `common/scripted_effects/air_cleanliness_winter_visual_effects.txt`, Air Winter constants, decisions, mapmodes, and ordinary-map entities |
...
Air Winter and post-consequence survivor event definitions belong in `events/fallout_world_end_events.txt` under `chaosx.fallout`. Fallout is a terminal consequence transition with its own blackout and scheduler. The consequence itself has no public Event Details row, evolution entry, ordinary event-log registration, or ordinary super-event slot. Zombie Apocalypse retains its own ids, files, assets, audio, sprites, and paths. New Fallout work must not borrow them.
...
The current documented ordinary pilot total is sixty-five rows and 544 defined blocks, all dormant and outside release-floor credit. The named global-survival subset above is retained as the original country-memory and recovery reference list. The latest reviewed row is Great Lakes Lock Winter candidate `663`, with dedicated proof in `FALLOUT_GREAT_LAKES_LOCK_WINTER_PROOF.md`.
...
The current reviewed ordinary pilot has `65` dormant rows and `544` defined event blocks through Great Lakes Lock Winter candidate `663`, event ids `663` through `669`, transaction `710066`, route `7166`, and Event Log history `9172`. The reviewed route upper bound is `7167`. Both scheduler activation flags remain unset. Great Lakes now has static proof for result-only callback scheduling cleanup and explicit hidden-AI option competition, while New Year Without Fireworks and Second Dust Bowl have explicit hidden-AI priority scoring. The wider pilot still has stale cleanup and live option-competition priority gaps in earlier reviewed rows. The exact engine-native all-valid-province thermonuclear sweep remains blocked, the manual Fallout survival contract remains a direct `90` to `95` percent prestrike population-loss band, and Hearts of Iron IV was not launched.
...
The Nile Pump Congress paragraph below is a historical snapshot. The current reviewed ordinary pilot extends through Great Lakes Lock Winter at candidate `663`, events `663` through `669`, transaction `710066`, route `7166`, and history `9172`. The source contract selects the lowest eligible current-generation Great Lakes state, carries Air Winter, survival, hydrology, food, shelter, Deaths, Event Log, and cleanup ledgers, and remains dormant with the scheduler activation flags unset. Great Lakes has static proof for result-only callback scheduling cleanup and explicit option competition. Runtime delivery and exact sweep surfaces remain open for this tranche, while wider pilot rows retain stale cleanup and option-competition gaps. The preceding Second Dust Bowl row remains documented at candidate `656` through `662`.

## Proof index

The reviewed-candidate proof index above retains historical count wording for earlier snapshots. The current ordinary pilot is sixty-five rows and 544 defined blocks through Great Lakes Lock Winter, with the current chain proofs recorded in `FALLOUT_NEW_YEAR_WITHOUT_FIREWORKS_PROOF.md`, `FALLOUT_SECOND_DUST_BOWL_PROOF.md`, and `FALLOUT_GREAT_LAKES_LOCK_WINTER_PROOF.md`.
...
- `FALLOUT_MANUAL_POPULATION_RECONCILIATION_PROOF.md` and `FALLOUT_CONSEQUENCE_BOUNDARY_AND_DEATHS_PROOF.md` record the generation-bound 90 to 95 percent manual and standard population contract, Deaths routing, and the durable Air Cleanliness shutdown.
