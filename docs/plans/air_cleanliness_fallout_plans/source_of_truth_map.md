...
The current documented ordinary pilot total is fifty-two rows and 446 defined blocks, all dormant and outside release-floor credit. The named global-survival subset above is retained as the original country-memory and recovery reference list. The latest reviewed row is Ashline Firebreak candidate `554`, with dedicated proof in `ASHLINE_FIREBREAK_CHAIN_PROOF.md`.
...
The B7 successor pilot is recorded in `FALLOUT_SUCCESSOR_PLAYER_CONTINUATION_B7_PROOF.md` and `docs/specs/air_cleanliness_fallout_specs/specs/53_successor_allocation_player_continuation_b7.md`. It adds a USA federal-continuity package and a deterministic fragmentation candidate probe without setting the global allocation completion or scheduler activation flags. NZL is the only reviewed package-aware fragmentation candidate, but its assignment and Samoa/Aotearoa conflict receipts have no producer, so package-aware selection and materialization remain blocked.

The current reviewed ordinary pilot extends through Ashline Firebreak at candidate `554`, events `554` through `564`, transaction `710052`, route `7152`, and history `9157`. The row selects a deterministic owner-controlled wildfire or ash-heavy aftermath state, carries Air Winter, disease, Supply Access, hazard-memory, building, military, and Deaths ledgers, and remains dormant with the scheduler activation flags unset. The preceding A New Funeral row remains documented at candidate `541` through `553`.
