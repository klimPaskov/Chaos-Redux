...
The current documented ordinary pilot total is forty-nine rows and 415 defined blocks, all dormant and outside release-floor credit. The named global-survival subset above is retained as the original country-memory and recovery reference list. The latest reviewed missing-patrol row is The Missing Patrol candidate `527`, with dedicated proof in `FALLOUT_MISSING_PATROL_CHAIN_PROOF.md`.
...
The current reviewed ordinary pilot extends through The Missing Patrol at candidate `527`, events `527` through `533`, transaction `710049`, route `7149`, and history `9154`. The row selects a deterministic exposed Air Winter state, carries intelligence, fear, reputation, contact, and cause-memory ledgers, and remains dormant with the scheduler activation flags unset. The preceding Captain Refuses row remains documented at candidate `520` through `526`.
