...
The current documented ordinary pilot total is seventy-two rows and 586 defined blocks, all dormant and outside release-floor credit. The named global-survival subset above is retained as the original country-memory and recovery reference list. The latest reviewed row is Doctor's Coup candidate `705`, with dedicated proof in `FALLOUT_DOCTORS_COUP_CHAIN_PROOF.md`.
...
The current reviewed ordinary pilot has `72` dormant rows and `586` defined event blocks through Doctor's Coup candidate `705`, event ids `705` through `711`, transaction `710072`, route `7176`, and survivor Event Log history `9178`. The reviewed route upper bound is `7177`. Both scheduler activation flags remain unset. Candidate 705 has static proof for a current Quarantine owner, completed Permit Market memory, native Air Winter state, all-branch affordability, deterministic grading, branch-aware hidden AI, bounded Deaths loss, survivor Event Log memory, dedicated report art, government-archetype memory, and authenticated cleanup. The wider pilot still has stale cleanup and option-competition priority gaps in earlier reviewed rows. The exact engine-native all-valid-province thermonuclear sweep remains blocked, the manual Fallout survival contract remains a direct `90` to `95` percent prestrike population-loss band, and Hearts of Iron IV was not launched.
...
Doctor's Coup is the current reviewed row at candidate `705`, transaction `710072`, route `7176`, events `705` through `711`, and history `9178`. The ordinary pilot now has seventy-two dormant rows and 586 defined blocks. Countable release-floor credit remains 0 of 660. Older count paragraphs below are historical snapshots.

## Proof index

The reviewed-candidate proof index above retains historical count wording for earlier snapshots. The current ordinary pilot is seventy-two rows and 586 defined blocks through Doctor's Coup, with the current chain proofs recorded in `FALLOUT_DOCTORS_COUP_CHAIN_PROOF.md`, `FALLOUT_NEW_YEAR_WITHOUT_FIREWORKS_PROOF.md`, `FALLOUT_SECOND_DUST_BOWL_PROOF.md`, `FALLOUT_GREAT_LAKES_LOCK_WINTER_PROOF.md`, `FALLOUT_WORK_FOR_RATIONS_CHAIN_PROOF.md`, and `FALLOUT_HUNGRY_BATTALION_CHAIN_PROOF.md`.
