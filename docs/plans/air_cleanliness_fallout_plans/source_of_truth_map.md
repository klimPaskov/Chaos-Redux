...
The current documented ordinary pilot total is forty-eight rows and 408 defined blocks, all dormant and outside release-floor credit. The named global-survival subset above is retained as the original country-memory and recovery reference list. The latest reviewed command-fracture row is The Captain Refuses candidate `520`, with dedicated proof in `FALLOUT_CAPTAIN_REFUSES_CHAIN_PROOF.md`.
...
The current reviewed ordinary pilot extends through The Captain Refuses at candidate `520`, events `520` through `526`, transaction `710048`, route `7148`, and history `9153`. The row selects a deterministic exposed Air Winter state, carries captain loyalty, mission mandate, field autonomy, and refusal memory ledgers, and remains dormant with the scheduler activation flags unset. The preceding Weapons in the Nursery row remains documented at candidate `513` through `519`.
