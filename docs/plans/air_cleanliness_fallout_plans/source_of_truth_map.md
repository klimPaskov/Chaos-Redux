...
The current documented ordinary pilot total is fifty-three rows and 453 defined blocks, all dormant and outside release-floor credit. The named global-survival subset above is retained as the original country-memory and recovery reference list. The latest reviewed row is River Ration League candidate `565`, with dedicated proof in `FALLOUT_RIVER_RATION_LEAGUE_CHAIN_PROOF.md`.
...
The current reviewed ordinary pilot extends through River Ration League at candidate `565`, events `565` through `571`, transaction `710053`, route `7153`, and history `9158`. The row selects two deterministic current-generation Danube corridor states, carries Air Winter, survival, Supply Access, trust, tension, and cleanup ledgers, and remains dormant with the scheduler activation flags unset. The preceding Ashline Firebreak row remains documented at candidate `554` through `564`.
...
The current reviewed total is fifty-three rows and 453 defined blocks, all dormant and outside release-floor credit.
