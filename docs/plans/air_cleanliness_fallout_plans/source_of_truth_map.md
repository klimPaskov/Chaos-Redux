...
The current documented ordinary pilot total is forty-seven rows and 401 defined blocks, all dormant and outside release-floor credit. The named global-survival subset above is retained as the original country-memory and recovery reference list.
...
The current reviewed ordinary pilot extends through The Weapons in the Nursery at candidate `513`, events `513` through `519`, transaction `710047`, route `7147`, and history `9152`. The row selects a deterministic occupied urban Air Winter shelter state, carries arms discipline, accident risk, civic trust, and nursery memory ledgers, and remains dormant with the scheduler activation flags unset.
