...
The current documented ordinary pilot total is seventy-three rows and 593 defined blocks, all dormant and outside release-floor credit. The named global-survival subset above is retained as the original country-memory and recovery reference list. The latest reviewed row is The Returning Disease candidate `712`, with dedicated proof in `FALLOUT_RETURNING_DISEASE_CHAIN_PROOF.md`.
...
The current reviewed ordinary pilot has `73` dormant rows and `593` defined event blocks through The Returning Disease candidate `712`, event ids `712` through `718`, transaction `710073`, route `7178`, and survivor Event Log history `9179`. The reviewed route upper bound is `7179`. Both scheduler activation flags remain unset. Candidate 712 has static proof for a current Quarantine owner, completed Doctor's Coup memory, native Air Winter state, all-branch affordability, deterministic grading, branch-aware hidden AI, bounded Deaths loss, survivor Event Log memory, dedicated report art, treatment and border memories, and authenticated cleanup. The wider pilot still has stale cleanup and option-competition priority gaps in earlier reviewed rows. The exact engine-native all-valid-province thermonuclear sweep remains blocked, the manual Fallout survival contract remains a direct `90` to `95` percent prestrike population-loss band, and Hearts of Iron IV was not launched.
...
The Returning Disease is the current reviewed row at candidate `712`, transaction `710073`, route `7178`, events `712` through `718`, and history `9179`. The ordinary pilot now has seventy-three dormant rows and 593 defined blocks. Countable release-floor credit remains 0 of 660. Older count paragraphs below are historical snapshots.

## Proof index

The reviewed-candidate proof index above retains historical count wording for earlier snapshots. The current ordinary pilot is seventy-three rows and 593 defined blocks through The Returning Disease, with the current chain proofs recorded in `FALLOUT_RETURNING_DISEASE_CHAIN_PROOF.md`, `FALLOUT_DOCTORS_COUP_CHAIN_PROOF.md`, `FALLOUT_NEW_YEAR_WITHOUT_FIREWORKS_PROOF.md`, `FALLOUT_SECOND_DUST_BOWL_PROOF.md`, `FALLOUT_GREAT_LAKES_LOCK_WINTER_PROOF.md`, `FALLOUT_WORK_FOR_RATIONS_CHAIN_PROOF.md`, and `FALLOUT_HUNGRY_BATTALION_CHAIN_PROOF.md`.
