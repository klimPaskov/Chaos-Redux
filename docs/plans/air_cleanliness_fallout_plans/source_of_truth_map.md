...
The current documented ordinary pilot total is seventy-four rows and 600 defined blocks, all dormant and outside release-floor credit. The named global-survival subset above is retained as the original country-memory and recovery reference list. The latest reviewed row is The Clean Certificate candidate `719`, with dedicated proof in `FALLOUT_CLEAN_CERTIFICATE_CHAIN_PROOF.md`.
...
The current reviewed ordinary pilot has `74` dormant rows and `600` defined event blocks through The Clean Certificate candidate `719`, event ids `719` through `725`, transaction `710074`, route `7180`, and survivor Event Log history `9180`. The reviewed route upper bound is `7181`. Both scheduler activation flags remain unset. Candidate 719 has static proof for a current Quarantine owner, completed Returning Disease memory, native Air Winter state, all-branch affordability, deterministic grading, branch-aware hidden AI, bounded Deaths loss, survivor Event Log memory, dedicated report art, inspection and certificate memories, and authenticated cleanup. The wider pilot still has stale cleanup and option-competition priority gaps in earlier reviewed rows. The exact engine-native all-valid-province thermonuclear sweep remains blocked, the manual Fallout survival contract remains a direct `90` to `95` percent prestrike population-loss band, and Hearts of Iron IV was not launched.
...
The Clean Certificate is the current reviewed row at candidate `719`, transaction `710074`, route `7180`, events `719` through `725`, and history `9180`. The ordinary pilot now has seventy-four dormant rows and 600 defined blocks. Countable release-floor credit remains 0 of 660. Older count paragraphs below are historical snapshots.

The Clean Certificate workbook row is `docs/spreadsheets/chaos_redux_events_catalog.csv:773` with identity `FALLOUT-719` and `Needs Testing` status.

## Proof index

The reviewed-candidate proof index above retains historical count wording for earlier snapshots. The current ordinary pilot is seventy-four rows and 600 defined blocks through The Clean Certificate, with the current chain proofs recorded in `FALLOUT_CLEAN_CERTIFICATE_CHAIN_PROOF.md`, `FALLOUT_RETURNING_DISEASE_CHAIN_PROOF.md`, `FALLOUT_DOCTORS_COUP_CHAIN_PROOF.md`, `FALLOUT_NEW_YEAR_WITHOUT_FIREWORKS_PROOF.md`, `FALLOUT_SECOND_DUST_BOWL_PROOF.md`, `FALLOUT_GREAT_LAKES_LOCK_WINTER_PROOF.md`, `FALLOUT_WORK_FOR_RATIONS_CHAIN_PROOF.md`, and `FALLOUT_HUNGRY_BATTALION_CHAIN_PROOF.md`.
