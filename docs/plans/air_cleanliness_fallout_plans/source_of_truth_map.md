...
Status: current implementation map, reconciled 2026-07-29 after the Fallout namespace and consequence-boundary audit. Fallout remains incomplete and dormant where the proof ledger says so.
...
The current documented ordinary pilot total is seventy-six rows and 614 defined blocks, all dormant and outside release-floor credit. The named global-survival subset above is retained as the original country-memory and recovery reference list. The latest reviewed row is Children of the Cordon candidate `733`, with dedicated proof in `FALLOUT_CHILDREN_OF_CORDON_CHAIN_PROOF.md`.
...
The current reviewed ordinary pilot has `76` dormant rows and `614` defined event blocks through Children of the Cordon candidate `733`, event ids `733` through `739`, transaction `710076`, route `7184`, and survivor Event Log history `9182`. The reviewed route upper bound is `7185`. Both scheduler activation flags remain unset. Candidate 733 has static proof for a current Quarantine country, completed Border Inspection Crisis memory, durable survival rows, generation-change evidence, all-branch affordability, deterministic grading, branch-aware hidden AI, bounded Deaths loss, survivor Event Log memory, dedicated report art, youth-trust and movement-law ledgers, and authenticated cleanup. The wider pilot still has stale cleanup and option-competition priority gaps in earlier reviewed rows. The exact engine-native all-valid-province thermonuclear sweep remains blocked, the manual Fallout survival contract remains a direct `90` to `95` percent prestrike population-loss band, and Hearts of Iron IV was not launched.
...
Children of the Cordon is the current reviewed row at candidate `733`, transaction `710076`, route `7184`, events `733` through `739`, and history `9182`. The ordinary pilot now has seventy-six dormant rows and 614 defined blocks. Countable release-floor credit remains 0 of 660. Older count paragraphs below are historical snapshots.

The Children of the Cordon workbook row is recorded with identity `FALLOUT-733` and `Needs Testing` status.

## Proof index

The reviewed-candidate proof index above retains historical count wording for earlier snapshots. The current ordinary pilot is seventy-six rows and 614 defined blocks through Children of the Cordon, with the current chain proofs recorded in `FALLOUT_CHILDREN_OF_CORDON_CHAIN_PROOF.md`, `FALLOUT_BORDER_INSPECTION_CRISIS_CHAIN_PROOF.md`, `FALLOUT_CLEAN_CERTIFICATE_CHAIN_PROOF.md`, `FALLOUT_RETURNING_DISEASE_CHAIN_PROOF.md`, `FALLOUT_DOCTORS_COUP_CHAIN_PROOF.md`, `FALLOUT_NEW_YEAR_WITHOUT_FIREWORKS_PROOF.md`, `FALLOUT_SECOND_DUST_BOWL_PROOF.md`, `FALLOUT_GREAT_LAKES_LOCK_WINTER_PROOF.md`, `FALLOUT_WORK_FOR_RATIONS_CHAIN_PROOF.md`, and `FALLOUT_HUNGRY_BATTALION_CHAIN_PROOF.md`.
