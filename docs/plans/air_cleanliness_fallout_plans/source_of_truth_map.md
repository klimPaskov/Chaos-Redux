...
The current documented ordinary pilot total is fifty-five rows and 467 defined blocks, all dormant and outside release-floor credit. The named global-survival subset above is retained as the original country-memory and recovery reference list. The latest reviewed row is Nile Pump Congress candidate `579`, with dedicated proof in `FALLOUT_NILE_PUMP_CONGRESS_CHAIN_PROOF.md`.
