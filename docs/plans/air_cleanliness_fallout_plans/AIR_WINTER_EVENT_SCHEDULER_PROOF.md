...
Every initial event validates its typed targets before opening. Every effect-bearing option repeats target or response-target validation at click time. All 67 delayed-result schedules call the shared country-cooldown helper or the island commit helper immediately before the child timer begins. A stale delayed-result click cancels only its matching pending branch, while a stale island opening rolls back only its uncommitted offer. Phase 1 opening popups use a separate notice-only rejection path that never clears state transaction data because a newer transaction may already own the same state. Each valid stale path opens `chaosx.fallout.203` as a recovery notice. The notice is suppressed during the Fallout transition and active Fallout. It has one effect-free acknowledgement.
...
The five Phase 1 openings reject any generic pending row or existing Air Winter branch before display and at click time. Four conditional alternatives also repeat their operational factory, operational port, Manpower, Command Power, or operational transport gates. Their paired alternative remains executable. Each successful click writes exactly one of ten branches, refreshes the pending-owner row, reanchors the 46-day cooldown, and schedules `chaosx.fallout.6` after 21 days. Event 6 independently proves both regular targets, the saved event country, current ownership, the generic pending flag, stored-owner equality, and exact one-of-ten branch cardinality. Its result predicates read live ledgers and buildings without requiring the state to remain in Phase 1. Twenty options partition the ten branches into success and direct inverse failure. Casualties use Deaths. No Phase 1 option damages a building. Its three mutually exclusive result modifiers last 21 days and survive opening cleanup, normal transaction finalization, and generic pending cancellation until timed expiry or full state reset.
...
21. click-time cooldown reanchoring for all 67 delayed-result schedules
...
42. exact one-of-ten Phase 1 branch cardinality and malformed-branch reconciliation
43. notice-only Phase 1 opening rejection without transaction cleanup
44. live Phase 1 result thresholds after ordinary phase movement
45. ten success and direct inverse failure partitions with exact pre-choice AI projections
46. Phase 1 Deaths routing without direct building damage
47. 21-day Phase 1 result-modifier persistence through ordinary finalization and full-reset cleanup

The 52 current Air Winter event blocks have unique `chaosx.fallout` ids and matching localisation. They contain 191 options. One hundred ninety effect-bearing options have click-time target guards, while the remaining stale-order acknowledgement has no effect. This pilot is separate from the Fallout living-world scheduler and does not satisfy the 660-block Fallout release floor. Phase 1 regional return proof is in `AIR_WINTER_PHASE_1_REGIONAL_RETURN_EVENT_PROOF.md`. Island source and population proof is in `AIR_WINTER_PHASE_2_ISLAND_REFUGEE_SOURCE_AND_POPULATION_PROOF.md`. Desert City route, result, and dynamic-picture proof is in `AIR_WINTER_PHASE_2_DESERT_CITY_EVENT_PROOF.md`. Seed-ledger proof is in `AIR_WINTER_PHASE_2_SEED_LEDGER_EVENT_PROOF.md`. Dam, refinery, and reactor proof is in `AIR_WINTER_PHASE_3_INFRASTRUCTURE_EVENT_PROOF.md`. Heavy-industry proof is in `AIR_WINTER_PHASE_3_HEAVY_INDUSTRY_EVENT_PROOF.md`. Mountain-capital proof is in `AIR_WINTER_PHASE_2_TUNNEL_SCHOOL_EVENT_PROOF.md`. Dead-city salvage proof is in `AIR_WINTER_PHASE_5_DEAD_CITY_SALVAGE_EVENT_PROOF.md`.

## Unobserved engine boundary

The installed documentation supports the current-year dynamic variable, meta-effect text, scope-valued variables, array iteration, regular event targets, delayed event syntax, state dynamic modifiers, state population reads, positive state `add_manpower`, damaged and non-damaged building-level variables, concrete equipment grants, and repairable building damage. Vanilla also proves conditional event pictures through scripted localisation. A live session has not observed the year snapshot assignment, generated event dispatch, delayed regular-target retention, Phase 1 callback timing, Phase 1 timed-modifier display and expiry, popup ordering, AI resolution, exact Desert City route and picture selection, subtype persistence, balanced migration readback, dead-city route selection, control-loss reconciliation, modifier arithmetic and expiry, equipment readback, damage repair, annual receipt persistence, or save-resume behavior. HOI4 was not launched. These surfaces are not claimed as runtime proven.
