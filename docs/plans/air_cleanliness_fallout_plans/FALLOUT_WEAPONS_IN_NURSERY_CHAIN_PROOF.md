...
The fictional generated source, prompt, processed PNG, preview, runtime DDS, manifest, and handoff are under `docs/assets/air_cleanliness_fallout/fallout_weapons_in_nursery/`. The mod runtime copy is `gfx/event_pictures/fallout_world_end/report_event_fallout_weapons_in_nursery.dds` and the sprite is registered in `interface/fallout_world_end.gfx`.
