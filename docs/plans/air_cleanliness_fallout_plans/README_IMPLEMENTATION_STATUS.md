...
## Current reconciliation after The Returning Disease

The latest dormant tranche is The Returning Disease at `chaosx.fallout.712` through `chaosx.fallout.718`, candidate `712`, transaction `710073`, route `7178`, and survivor Event Log history `9179`. Its source package defines one Quarantine state row that requires a completed Doctor's Coup memory, a produced Air Winter snapshot, current Shelter Capacity, Supply Access, Adaptation, bounded Disease Pressure and Exposure, current generation evidence, and at least one complete affordable branch. Seal the Infected District, Send Targeted Care Teams, Request the Aid Convoy, and Keep the Outbreak Off the Books share deterministic `24` day result and `210` day callback contracts, hidden-AI parity, bounded Deaths failure, infrastructure damage, Air Winter and Supply Access effects, treatment-capacity, border-trust, aid-dependence, concealment, and cause-memory ledgers, cleanup, dedicated localisation, survivor Event Log routing, dedicated report art, a reviewed source specification, and `FALLOUT_RETURNING_DISEASE_CHAIN_PROOF.md`. The authoritative workbook carries exported row `FALLOUT-712` with `Needs Testing` status. The reviewed ordinary pilot now has `73` rows and `593` defined blocks through `chaosx.fallout.718`. Countable release-floor credit remains `0 of 660` while both scheduler activation flags are unset. The wider pilot still has stale cleanup and live option-competition priority gaps in earlier reviewed rows. Scheduler dispatch, host authority, save recovery, delayed queue delivery, multiplayer behavior, runtime Event Log delivery, Deaths readback, player-visible art, and the exact all-valid-province sweep remain unproven.
...
Current content snapshot: The Returning Disease is the latest reviewed row at candidate `712`, transaction `710073`, route `7178`, events `712` through `718`, and history `9179`. The ordinary pilot has seventy-three dormant rows and 593 defined blocks. The older historical status paragraphs below retain their original counts for audit traceability.
