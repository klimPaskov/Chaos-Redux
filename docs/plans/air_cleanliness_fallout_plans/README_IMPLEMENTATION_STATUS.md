...
`FALLOUT_EVENT_SCHEDULER_NUMERICAL_CONTRACT_PROPOSAL.md` is a pending approval gate for the missing cadence, visible-budget, fatigue, scoring, batching, and queue-cap values. Its proposed values are not implemented. The dormant constants and effects remain unchanged until explicit user approval.
