...
## Current reconciliation after Regional Health Compact

The latest dormant tranche is Regional Health Compact at `chaosx.fallout.754` through `chaosx.fallout.760`, candidate `754`, transaction `710079`, route `7190`, and survivor Event Log history `9185`. Its source package defines one state-targeted Quarantine bilateral row with a completed Health Without Borders memory, a deterministic state selector with a foreign neighbor, current Air Winter and survival ledgers, and at least one complete affordable branch. Share the Clinic Registers, Exchange Clinic Teams, Grant Corridor Authority, and Enforce a Border Quarantine share deterministic `42` day result and `300` day bilateral review contracts, hidden-AI parity, bounded Deaths failure, Air Winter, health-authority, corridor, refugee, fatigue, opinion, and Supply Access effects, cleanup, dedicated localisation, survivor Event Log routing, dedicated report art, a reviewed source specification, and `FALLOUT_REGIONAL_HEALTH_COMPACT_CHAIN_PROOF.md`. The authoritative workbook carries exported row `FALLOUT-754` with `Needs Testing` status. The reviewed ordinary pilot now has `79` rows and `635` defined blocks through `chaosx.fallout.760`. Countable release-floor credit remains `0 of 660` while both scheduler activation flags remain unset. The wider pilot still has stale cleanup and live option-competition priority gaps in earlier reviewed rows. Scheduler dispatch, host authority, save recovery, delayed queue delivery, multiplayer behavior, runtime Event Log delivery, Deaths readback, player-visible art, and the exact all-valid-province sweep remain unproven.
...
Current content snapshot: Regional Health Compact is the latest reviewed row at candidate `754`, transaction `710079`, route `7190`, events `754` through `760`, and history `9185`. The ordinary pilot has seventy-nine dormant rows and 635 defined blocks. The older historical status paragraphs below retain their original counts for audit traceability.
