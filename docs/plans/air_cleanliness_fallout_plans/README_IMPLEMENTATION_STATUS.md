...
The old-world diplomacy proof gate also remains unresolved. Market access is no longer one of its missing surfaces. Lend lease now has an official ordered-pair detector and current-generation proof when the surface is empty. Docking validation, active lend-lease cancellation, ordinary trade, intelligence, and expeditionary return still block map return until their required postconditions are genuinely satisfied.
