...
- a versioned transition envelope with schema 9
...
Schema-9 migration is fail closed:

- completed saves are promoted non-destructively
- active schema-7 and schema-8 saves rebuild only while still in the snapshot phase, before snapshot application or destruction, and without an unrelated error
