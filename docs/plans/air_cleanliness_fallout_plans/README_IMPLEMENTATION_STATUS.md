...
The latest dormant tranche is The First Red Line at `chaosx.fallout.684` through `chaosx.fallout.690`, candidate `684`, transaction `710069`, route `7170`, and survivor Event Log history `9175`. Its source package defines one Quarantine state row that requires a produced Air Winter snapshot, current Shelter Capacity, Supply Access, Adaptation, bounded Disease Pressure and Exposure, current generation evidence, and at least one complete affordable branch. Strict Cordon, Medical Checkpoints, Controlled Evacuation, and Local Self-Control share deterministic `21` day result and `180` day callback contracts, hidden-AI parity, bounded Deaths failure, infrastructure damage, Air Winter ledger effects, public-health memory, cleanup, dedicated localisation, survivor Event Log routing, dedicated report art, and a reviewed source specification. The reviewed ordinary pilot now has `69` rows and `565` defined blocks through `chaosx.fallout.690`. Countable release-floor credit remains `0 of 660` while both scheduler activation flags are unset. The wider pilot still has stale cleanup and live option-competition priority gaps in earlier reviewed rows. Scheduler dispatch, host authority, save recovery, delayed queue delivery, multiplayer behavior, runtime Event Log delivery, Deaths readback, player-visible art, and the exact all-valid-province sweep remain unproven.
