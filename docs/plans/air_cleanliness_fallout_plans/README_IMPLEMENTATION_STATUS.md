...
Status reviewed against the live working tree on 2026-07-16.
...
Air Winter has phases 0 through 6, exposure and survival ledgers, Deaths integration, building and supply damage, category degradation, disease, state movement and attrition, a controller-owned air-operations burden, three map modes, reviewed regional ordinary-map visuals, response decisions, and a 35-block event pilot. The air burden is the deterministic equal-state mean phase across controlled states with a working airbase. It scales mission efficiency, detection, accidents, and weather penalties, clears stale controllers through the bounded country registry, and is removed when Fallout pauses the monthly winter pass. The event scheduler carries durable first-frost, dark-harvest, ash-thaw, second-winter, and terminal-season markers through cooldowns and calendar boundaries. Annual receipts and nine regional severe-year memories keep recurrence deterministic. All three mapmode tooltips use four viewer-specific monitoring levels. A foreign state exposes only its current phase. Ownership, control, or treaty reports add the one-month trend. Roof sampling creates a national Atmospheric Office. Terminal atmospheric classification appears at 90 percent global contamination or for a monitored major power. It does not write the committed Fallout grade.

Air Winter remains incomplete. The monthly host owns a bounded treaty coordinator with member, violator, invitation, active-donor, and relief-route ledgers. Membership and sanctions change only on lifecycle edges. Global Cleaning Day spends support equipment and convoys while reserving civilian factories before applying pooled cleanup. Joint Filter Convoy targets a treaty member, reserves one eligible controlled state, spends equipment and convoys, and creates a temporary state relief route that lowers Air Winter pressure. Invalid routes clear before the state pressure pass. Founder succession runs once after batch cleanup. Annexation and membership loss cancel donor and recipient projects through the active-donor registry. Delayed invitation failures remain retryable, while declines are terminal for the current treaty generation. Fallout preserves treaty memory while silently ending operational projects, invitations, and routes. Founder succession, annex cleanup, cancellation, delayed arrival, mapmode reporting, and idempotent route expiry are wired. Treaty verification, evacuation corridors, direct Fallout-era successor memory, and the wider manually reviewed treaty event package remain absent. Additional Air Winter pressure from active ordinary combat or recent strategic bombing is absent. General air-operation fields are available only as a country-scoped runtime modifier, so exact strategic-region confinement remains an engine boundary. Tooltip rendering, viewer scope, normal-map visuals, the country air modifier, event dispatch, seasonal save persistence, treaty targeting, treaty project completion, and Fallout pause timing remain runtime observation gates.
