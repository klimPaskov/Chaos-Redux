...
The latest dormant regional tranche is Mine Generator at `chaosx.fallout.642` through `chaosx.fallout.648`, candidate `642`, transaction `710063`, route `7163`, and Event Log history `9169`. It adds one Sub-Saharan African resource-works row with four human and hidden-AI-parity branches, deterministic result and operating-season callback grading, native infrastructure or industrial damage on failure, Deaths-backed failure, durable ledgers, cleanup, dedicated localisation, Event Log routing, and dedicated report art. The reviewed ordinary pilot now has `63` rows and `523` defined blocks. Countable release-floor credit remains `0 of 660` while scheduler activation is unset. The chain remains dormant while host authority, save recovery, delayed delivery, multiplayer behavior, and runtime Event Log delivery remain unproven.
...
- a scheduler activation gate with no setter, a dormant release audit receipt, and a reviewed-candidate pilot producer for sixty-three ordinary global-survival rows
