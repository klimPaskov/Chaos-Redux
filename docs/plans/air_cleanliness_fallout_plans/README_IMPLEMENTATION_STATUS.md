...
Air Winter remains incomplete. The live monthly cleanup disables the legacy treaty lifecycle, so relief routes and the broader treaty package have no active owner. Additional Air Winter pressure from active ordinary combat or recent strategic bombing is absent. General air-operation fields are available only as a country-scoped runtime modifier, so exact strategic-region confinement remains an engine boundary. Tooltip rendering, viewer scope, normal-map visuals, the country air modifier, event dispatch, and seasonal save persistence remain runtime observation gates.
