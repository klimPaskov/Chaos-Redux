...
The latest dormant tranche is Doctor's Coup at `chaosx.fallout.705` through `chaosx.fallout.711`, candidate `705`, transaction `710072`, route `7176`, and survivor Event Log history `9178`. Its source package defines one Quarantine state row that requires a completed Permit Market memory, a produced Air Winter snapshot, current Shelter Capacity, Supply Access, Adaptation, bounded Disease Pressure and Exposure, current generation evidence, and at least one complete affordable branch. Accept Medical Rule, Build a Medical Coalition, Arrest the Medical Leadership, and Call a Public Vote share deterministic `30` day result and `240` day callback contracts, hidden-AI parity, bounded Deaths failure, infrastructure damage, Air Winter and Supply Access effects, government-archetype memory, civil-authority ledgers, cleanup, dedicated localisation, survivor Event Log routing, dedicated report art, a reviewed source specification, and `FALLOUT_DOCTORS_COUP_CHAIN_PROOF.md`. The authoritative workbook carries exported row `FALLOUT-705` with `Needs Testing` status. The reviewed ordinary pilot now has `72` rows and `586` defined blocks through `chaosx.fallout.711`. Countable release-floor credit remains `0 of 660` while both scheduler activation flags are unset. The wider pilot still has stale cleanup and live option-competition priority gaps in earlier reviewed rows. Scheduler dispatch, host authority, save recovery, delayed queue delivery, multiplayer behavior, runtime Event Log delivery, Deaths readback, player-visible art, and the exact all-valid-province sweep remain unproven.
...
Current content snapshot: Doctor's Coup is the latest reviewed row at candidate `705`, transaction `710072`, route `7176`, events `705` through `711`, and history `9178`. The ordinary pilot has seventy-two dormant rows and 586 defined blocks. The older historical status paragraphs below retain their original counts for audit traceability.
