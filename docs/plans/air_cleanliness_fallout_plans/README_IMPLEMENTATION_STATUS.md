...
The user approved `FALLOUT_ASH_WEEK_ORIENTATION_CONTRACT_PROPOSAL.md` and `FALLOUT_ORIENTATION_LIVE_LEDGER_NUMERICAL_CONTRACT_PROPOSAL.md` on 2026-07-18. Their accepted design is incorporated into the Fallout source specs. Suffixes `62` through `84` are reserved for the five-component orientation sequence, its visible and hidden results, both closures, and authenticated cleanup. The dormant transaction substrate, six dedicated assets, sprite registrations, shared localisation, schema-3 Cohesion and State Supply Access ledgers, native local-supply translation, authenticated live Air Winter state results, the manually written national pilot at `62` through `65`, and the dormant closure and cleanup events at `82` through `84` are implemented. All seven defined blocks remain blocked and uncounted. Cause memory is separated from successor memory, state pointers are country-owned, the source is the exact assigned capital, and country-proportion Deaths traverse the stable state ledger. The closure restores the durable identity row before issuing its authenticated event envelope. The cleanup writes the closure receipt only after re-authenticating event `84`. The other sixteen blocks, caller, logs, details, complete coverage rows, and registry-backed late components remain absent. Exact one-level infrastructure repair remains blocked because the official script surface has no documented repair effect. Construction, absolute level setting, remove-then-rebuild, and repair-speed modifiers do not prove the accepted operational mutation. Both scheduler activation flags remain unset. The living-world count remains 0 of 660 until the full tranche is wired, logged, detailed, manually reviewed, and audited.
...
11. Complete the accepted Ash-week orientation contract with the other sixteen blocks, exact capital repair, implementation registries for the approved 108-cell source matrix, country-memory coverage, government and character registries, callers, logs, details, and audits. Do not wire its caller until successor allocation, player continuation, and the required candidate registries are proven. Do not enable the living-world scheduler. The schema-3 live ledger, national pilot, and closure package remain dormant.
