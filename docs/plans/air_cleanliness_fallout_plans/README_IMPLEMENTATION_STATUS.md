...
## Current reconciliation after source-bound reset repairs

The latest dormant tranche is Great Lakes Lock Winter at `chaosx.fallout.663` through `chaosx.fallout.669`, candidate `663`, transaction `710066`, route `7166`, and Event Log history `9172`. Its source package defines one North American coastal Great Lakes row with four human and hidden-AI-parity branches, deterministic 35-day result and 270-day thaw-inspection callback contracts, bounded Deaths failure, six durable lock and thaw ledgers, cleanup, dedicated localisation, Event Log routing, dedicated report art, and a reviewed source specification. Its static cleanup gate now covers result-only callback scheduling errors, generation-reset cancellation, state-reservation release, and hidden-AI branch competition with deterministic tie order. Radio Island, Rail Spine Vote, Canal Schedule, Great Lakes Lock Winter, Tunnel Ward Committees, Metro Republic Below, Mine Generator, and Old Airfield now have source-bound reset opening paths, while wider reviewed rows retain known stale-cleanup gaps. The reviewed ordinary pilot now has `65` rows and `544` defined blocks. Countable release-floor credit remains `0 of 660` while both scheduler activation flags are unset. The wider pilot still has stale cleanup and live option-competition priority gaps in earlier reviewed rows. Scheduler dispatch, host authority, save recovery, delayed queue delivery, multiplayer behavior, runtime Event Log delivery, and player-visible art remain unproven.
...
The Fallout consequence boundary now has one idempotent post-admission Air shutdown effect. The locked transition, save migration, and validated manual sweep call `fallout_disable_air_cleanliness_after_fallout_admission` after the relevant snapshot or prestrike ledger is durable. Rejected request envelopes retain their separate temporary pause rollback. The durable flag continues to gate Air Winter, natural sources, treaty operations, contamination deltas, and stale Deaths callbacks.
...
Overall system status reviewed against the live working tree on 2026-07-27. The numerical approval gate was reconciled on 2026-07-18.
...
Fallout request intake now disables Air Cleanliness durably before host reconciliation and clears already-applied state modifiers and country pressure ideas immediately. A rejected pending request restores the ordinary modifier surface, while a locked Fallout transition keeps the durable shutdown. An invalid-intensity manual launch never acquires the shutdown, while a valid admitted sweep keeps it through a later dormant error so a destructive attempt cannot reopen Air Cleanliness.

Late nuclear callbacks consume the same boundary. Ordinary nuke drops, Final Silence fallout helpers, and Air Winter reactor failures cannot add `nuclear_fallout_state` after Fallout owns Air Cleanliness, and the ordinary nuke callback removes stale residue before it reaches the disabled Air source.
...
- a scheduler activation gate with no setter, a dormant release audit receipt, and a reviewed-candidate pilot producer for sixty-five ordinary global-survival rows
