...
## Current reconciliation after Open the Sealed City

The latest dormant tranche is Open the Sealed City at `chaosx.fallout.740` through `chaosx.fallout.746`, candidate `740`, transaction `710077`, route `7186`, and survivor Event Log history `9183`. Its source package defines one state-targeted Quarantine recovery row with a completed Border Inspection Crisis memory, a deterministic city or large-city selector, current Air Winter and survival ledgers, and at least one complete affordable branch. Reopen the Civic Blocks, Grant Scientific Access, Send a Salvage Command, and Build a Permanent Memorial District share deterministic `35` day result and `270` day recovery callback contracts, hidden-AI parity, bounded Deaths failure, Air Winter, infrastructure, salvage, memorial, public-trust, and faction-pressure effects, cleanup, dedicated localisation, survivor Event Log routing, dedicated report art, a reviewed source specification, and `FALLOUT_OPEN_THE_SEALED_CITY_CHAIN_PROOF.md`. The authoritative workbook carries exported row `FALLOUT-740` with `Needs Testing` status. The reviewed ordinary pilot now has `77` rows and `621` defined blocks through `chaosx.fallout.746`. Countable release-floor credit remains `0 of 660` while both scheduler activation flags remain unset. The wider pilot still has stale cleanup and live option-competition priority gaps in earlier reviewed rows. Scheduler dispatch, host authority, save recovery, delayed queue delivery, multiplayer behavior, runtime Event Log delivery, Deaths readback, player-visible art, and the exact all-valid-province sweep remain unproven.
...
Current content snapshot: Open the Sealed City is the latest reviewed row at candidate `740`, transaction `710077`, route `7186`, events `740` through `746`, and history `9183`. The ordinary pilot has seventy-seven dormant rows and 621 defined blocks. The older historical status paragraphs below retain their original counts for audit traceability.
