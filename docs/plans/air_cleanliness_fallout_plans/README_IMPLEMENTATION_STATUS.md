...
The latest dormant tranche is Children of the Cordon at `chaosx.fallout.733` through `chaosx.fallout.739`, candidate `733`, transaction `710076`, route `7184`, and survivor Event Log history `9182`. Its source package defines one country-only Quarantine generation row with a completed Border Inspection Crisis memory, durable survival ledgers, generation-change evidence, and at least one complete affordable branch. Rewrite the Movement Law, Open Service Travel, Continue the Strict Cordon, and Give the Wards Their Own Boards share deterministic `32` day result and `240` day cohort callback contracts, hidden-AI parity, bounded Deaths failure, future-leader and government-route effects, cleanup, dedicated localisation, survivor Event Log routing, dedicated report art, a reviewed source specification, and `FALLOUT_CHILDREN_OF_CORDON_CHAIN_PROOF.md`. The authoritative workbook carries exported row `FALLOUT-733` with `Needs Testing` status. The reviewed ordinary pilot now has `76` rows and `614` defined blocks through `chaosx.fallout.739`. Countable release-floor credit remains `0 of 660` while both scheduler activation flags are unset. The wider pilot still has stale cleanup and live option-competition priority gaps in earlier reviewed rows. Scheduler dispatch, host authority, save recovery, delayed queue delivery, multiplayer behavior, runtime Event Log delivery, Deaths readback, player-visible art, and the exact all-valid-province sweep remain unproven.
...
Current content snapshot: Children of the Cordon is the latest reviewed row at candidate `733`, transaction `710076`, route `7184`, events `733` through `739`, and history `9182`. The ordinary pilot has seventy-six dormant rows and 614 defined blocks. The older historical status paragraphs below retain their original counts for audit traceability.
