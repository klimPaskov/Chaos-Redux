...
The stable `world_end_scenario_id.fallout = 2` entry remains in `common/script_constants/world_end_scenario_registry_constants.txt`. It is used by internal request and settings gates and is not inserted into the public registry. The generic `GetWorldEndScenario` selector no longer exposes a Fallout title. The Chaos Meter and world-end tooltip use the direct `chaos_tier_end_fallout` consequence label, while the dedicated blackout GUI uses its own sequential display text. Fallout blackout audio remains owned by `fallout_dispatch_blackout_audio` in `common/scripted_effects/fallout_world_end_effects.txt`.
