...
The standard path sets `fallout_air_cleanliness_disabled` in `fallout_lock_transition` after `fallout_snapshot_epoch_is_ready_to_lock` succeeds. The coordinator immediately calls `air_contamination_apply_state_modifier` so existing country pressure ideas and state modifiers do not linger until the next monthly pass. `fallout_world_end_migrate_save` restores the flag and performs the same immediate modifier cleanup for saves with `world_end_fallout`. The manual path sets the flag in `fallout_manual_initialize_sweep` after intensity validation, clears current modifiers before native strikes, and clears the flag only on a pre-Fallout sweep failure, where it refreshes the ordinary Air Cleanliness modifiers together with the temporary Air Winter shutdown receipt.
