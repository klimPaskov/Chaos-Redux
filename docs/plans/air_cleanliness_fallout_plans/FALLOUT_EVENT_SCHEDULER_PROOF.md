...
The row remains dormant and contributes zero countable blocks because both scheduler activation flags, host authority, save recovery, delayed queue delivery, multiplayer behavior, full-screen Fallout blackout, runtime Event Log delivery, and player-visible art remain unproven. Stale-generation cleanup is also blocked by the shared delayed dispatcher current-generation gates. The asset manifest records the static sprite registration. The read-only event-inspector result is unresolved tooling evidence and is not a validation pass. The exact chain proof is recorded in `FALLOUT_NEW_YEAR_WITHOUT_FIREWORKS_PROOF.md`.
