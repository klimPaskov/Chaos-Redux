...
The Year Zero pilot adds the `major_arc_stage` dispatch source and maps the existing identity to human token `401` or hidden-AI token `402`. Its opening consumer advances the exact ticket through conflict, choice, delayed result, consequence, callback, and cleanup, then releases the row. The engine-sensitive boundary is recorded in `FALLOUT_YEAR_ZERO_MAJOR_ARC_STAGE_PILOT_PROOF.md`. Orchard Flowers, Skilled List, and False Spring Losses still have no stage consumer. All four rows remain dormant behind the unset activation flags, and no release-floor credit is added.
...
- Relationship candidates fail closed until reviewed rows freeze the complete reciprocal payload. Orchard Flowers, Skilled List, and False Spring Losses remain dormant until their authored major-arc stage consumers exist.
