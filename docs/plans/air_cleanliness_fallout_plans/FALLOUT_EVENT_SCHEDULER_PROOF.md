...
The dormant reviewed producer now covers `77` ordinary Fallout rows and `621` defined event blocks through `chaosx.fallout.746`. Open the Sealed City is the latest ordinary row with candidate `740`, transaction `710077`, route `7186`, and survivor history `9183`. It is admitted only from a current-generation Quarantine country with a completed Border Inspection Crisis memory, a deterministic city or large-city state target, durable Air Winter and survival ledgers, and an affordable branch. Its `35` day result and `270` day recovery callback remain behind the unset activation flags. The countable release-floor total remains `0 of 660`.
