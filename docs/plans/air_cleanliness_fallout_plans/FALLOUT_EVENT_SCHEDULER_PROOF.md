...
The dormant reviewed producer now covers `64` ordinary Fallout rows and `530` defined event blocks through `chaosx.fallout.655`. The New Year Without Fireworks is the East Asian country-only row with candidate `649`, transaction `710064`, route `7164`, and history `9170`. It is admitted only from a current-generation East Asian Fallout country with durable survival resources, minimum branch floors, an affordable branch, and an authenticated subject type `none` target `0`, and remains behind the unset activation flags. The countable release-floor total remains `0 of 660`.
