...
The Year Zero, Orchard Flowers, and Skilled List pilots use the `major_arc_stage` dispatch source and map their existing identities to human and hidden-AI tokens `401` and `402`, then `408` and `409`, then `429` and `430`. Each opening consumer advances its exact ticket through conflict, choice, delayed result, consequence, callback, and cleanup, then releases the row. The engine-sensitive boundaries are recorded in `FALLOUT_YEAR_ZERO_MAJOR_ARC_STAGE_PILOT_PROOF.md`, `FALLOUT_ORCHARD_FLOWERS_MAJOR_ARC_STAGE_PILOT_PROOF.md`, and `FALLOUT_SKILLED_LIST_MAJOR_ARC_STAGE_PILOT_PROOF.md`, with the second and third pilot contracts in `2026-07-26_orchard_flowers_major_arc_stage_consumer_addendum.md` and `2026-07-26_skilled_list_major_arc_stage_consumer_addendum.md`. False Spring Losses still has no stage consumer. All four rows remain dormant behind the unset activation flags, and no release-floor credit is added.
...
- Structural transaction families and deterministic crisis, routine, and broadcast selection are implemented but dormant. The reviewed candidate pilot producer is present for sixteen ordinary global-survival rows, while the full candidate matrix, living-world event caller, human-choice caller, hidden AI result caller, content-owned cleanup execution, and scheduler debug presentation remain absent.
...
- Relationship candidates fail closed until reviewed rows freeze the complete reciprocal payload. False Spring Losses remains reservation-only until its authored major-arc stage consumer exists. Year Zero, Orchard Flowers, and Skilled List have dormant stage consumers, but they remain outside release-floor credit.
