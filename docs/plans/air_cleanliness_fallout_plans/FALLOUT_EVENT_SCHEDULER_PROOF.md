...
The four branches use authored resource costs and deterministic result and callback grading. The effects freeze the country survival, Air Winter, Cohesion, Stability, War Support, and seven New Year ledgers, consume the exact opening receipt after delayed-result allocation, refund and cancel on scheduling or receipt-consume failure, resolve after exactly `21` days, schedule the anniversary callback after exactly `330` days, apply bounded Deaths failure, preserve hidden-AI parity, record Event Log payloads, and close through authenticated cleanup.
...
The row remains dormant and contributes zero countable blocks because both scheduler activation flags, host authority, save recovery, delayed queue delivery, multiplayer behavior, full-screen Fallout blackout, runtime Event Log delivery, and player-visible art remain unproven. The asset manifest remains `handed_off` even though the parent-owned `.gfx` file contains the static sprite registration. The read-only event-inspector result is unresolved tooling evidence and is not a validation pass. The exact chain proof is recorded in `FALLOUT_NEW_YEAR_WITHOUT_FIREWORKS_PROOF.md`.
