...
The Year Zero and Orchard Flowers pilots use the `major_arc_stage` dispatch source and map their existing identities to human and hidden-AI tokens `401` and `402`, then `408` and `409`. Each opening consumer advances its exact ticket through conflict, choice, delayed result, consequence, callback, and cleanup, then releases the row. The engine-sensitive boundaries are recorded in `FALLOUT_YEAR_ZERO_MAJOR_ARC_STAGE_PILOT_PROOF.md` and `FALLOUT_ORCHARD_FLOWERS_MAJOR_ARC_STAGE_PILOT_PROOF.md`, with the second pilot contract in `2026-07-26_orchard_flowers_major_arc_stage_consumer_addendum.md`. Skilled List and False Spring Losses still have no stage consumer. All four rows remain dormant behind the unset activation flags, and no release-floor credit is added.
...
- Relationship candidates fail closed until reviewed rows freeze the complete reciprocal payload. Skilled List and False Spring Losses remain reservation-only until their authored major-arc stage consumers exist. Orchard Flowers now has a dormant stage consumer, but it remains outside release-floor credit.
