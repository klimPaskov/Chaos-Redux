...
The living-world scheduler has a schema-2 dormant transaction-integrity substrate with a backward-compatible fixed ordinary-opening receipt. It records the reveal timeline, freezes a stable post-allocation country registry, initializes country runtime rows, and exposes five-part orientation, anti-repetition, ordinary-opening, arc, delayed-result, bilateral, cancellation, cleanup, routing, exact dispatch-issuance, and issued reciprocal terminalization contracts. It does not select a candidate or schedule an event. Visible and hidden AI issuance remain unreachable because both activation flags have no setter. No live gameplay path can reach hidden cleanup, and no living-world cleanup event is defined.
