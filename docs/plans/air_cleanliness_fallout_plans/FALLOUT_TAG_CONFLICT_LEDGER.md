...
- Air Winter historical category memory is copied into `fallout_pretransition_air_winter_original_category` only with a schema-10 produced or N/A provenance row. The live category is frozen separately for grading and rewrite. Later Air Winter cleanup cannot change bunker, food, or maritime origin classification.
...
No active allocator calls the begin or finalize effect. A dormant B7 NZL pilot now writes one exact fragmented-footprint transfer, five source retirement receipts, and the existing-tag conversion provenance after the live ledger is current. A second B7 path materializes one reviewed dynamic NZL output only when the live conflict row, exact five-state footprint, source retirement receipts, and package gates are current. The dynamic result records `created_dynamic`, a current source-to-output link, a capital row, and a same-generation rollback effect that restores the five source states and releases the reserved dynamic slot. It does not resolve the general successor matrix or apply a global package wave. Map return therefore remains blocked.

The future allocator must use a persistent generation-bound plan, cursor, and per-row committed stage because Clausewitz exposes no atomic rollback. The current initializer is only the transaction boundary and does not yet supply that resumable allocator. The allocator must reserve humans and event packages first, prove every target, state, and capital unique before mutation, capture or materialize the target, transfer exact planned states, validate ownership and control, set the capital, apply identity and gameplay packages, write the resolution-specific provenance and reciprocal conflict receipts, then call the guarded finalizer. Static existing tags and approved releasables may be used only after the live conflict proof. Dynamic creation remains limited to the dormant NZL pilot until capacity, reservation, recycling, save, and multiplayer behavior are proven. The documented engine surface proves creation and unreservation, but not destruction, save persistence, or multiplayer delivery of a newly materialized country.
...
The accepted specifications under `docs/specs/air_cleanliness_fallout_specs/` remain unchanged. This ledger now records the dormant NZL exact-transfer pilot and its conflict-ledger-gated dynamic materialization as bounded implementation handoffs. It does not approve a reduced package, authorize global allocation, or declare the general country roster ready for activation.
