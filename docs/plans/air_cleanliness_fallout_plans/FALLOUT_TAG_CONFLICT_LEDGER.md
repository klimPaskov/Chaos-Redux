...
Government classifier schema 1 assigns a provisional origin archetype to every surviving live country before ownership changes. It aggregates frozen state inputs, uses a fixed specificity order, and stops the transition with error 16 when a surviving country has no valid identity.
...
Successor allocation schema 1 separates the frozen input ledger from the mutated output world.

- `fallout_begin_successor_allocation_transaction` may consume only a fully current inventory generation and records the source generation and schema.
- Every frozen live-country conflict needs a current resolution and cleanup owner.
- Every surviving landholder needs one committed country row, one unique capital row, current country, focus, archetype, regional, and country-memory package generations, and current conflict and cleanup receipts.
...
No active allocator calls the begin or finalize effect. No active producer creates assignment rows or applies the required package layers. Map return therefore remains blocked.
...
- The accepted 99-successor matrix remains a design catalogue. It does not provide stable runtime candidate identifiers, exact state packages, ordered capital choices, and a complete package assignment for allocation.
