...
- Air Winter historical category memory is copied into `fallout_pretransition_air_winter_original_category` only with a schema-9 produced or N/A provenance row. The live category is frozen separately for grading and rewrite. Later Air Winter cleanup cannot change bunker, food, or maritime origin classification.
