...
No active allocator calls the begin or finalize effect. A dormant B7 NZL pilot now writes one exact fragmented-footprint transfer, five source retirement receipts, and the existing-tag conversion provenance after the live ledger is current. It does not materialize a tag, resolve the general successor matrix, or apply a global package wave. Map return therefore remains blocked.
...
The accepted specifications under `docs/specs/air_cleanliness_fallout_specs/` remain unchanged. This ledger now records the dormant NZL exact-transfer pilot as a bounded implementation handoff. It does not approve a reduced package, authorize global allocation, authorize dynamic materialization, or declare any country package ready for activation.
