...
| SCN-012 scheduler continuation | Finished for a fresh launch, partial for an active-crisis edge case | `black_plague_scenario_start_scheduler` at `common/scripted_effects/020_black_plague_scenario_effects.txt:465-480` selects a new scenario anchor, saves `black_plague_scheduler_anchor_state`, sets the anchor state flag, and calls `black_plague_schedule_next_pulse`. The launch calls it before clearing scenario arrays at lines 482-531. The helper first clears the existing scheduler anchor, however, and the preflight does not prove that an unestablished scenario anchor can be selected. A late active crisis with no eligible unestablished anchor can therefore lose its existing scheduler. Save and reload behavior is also unproven. |
...
| Evolution detail and ordinary log plumbing | Partial | `black_plague_evolution_record_stage` at `common/scripted_effects/020_black_plague_evolution_effects.txt:74-89` writes through the shared evolution logger, and Event Details registers five previews at `common/scripted_effects/chaosx_events_log_effects.txt:2523-2550`. The five active flags, per-stage setting gates, report dispatches, and a due-date check exist. Pacing isolation, actor ownership, Evolution IV transaction safety, and the Evolution V lifecycle remain incorrect. |
...
| Evolution dynamic pacing | Partial and completion-blocking | `black_plague_evolution_check_is_due` at `common/scripted_triggers/020_black_plague_evolution_triggers.txt:58-67` now consumes `global.black_plague_rat_next_evolution_check_date`, and `black_plague_evolution_runtime_pulse` schedules the next MTTH-backed check after a due pass at `common/scripted_effects/020_black_plague_evolution_effects.txt:371-385`. The due pass still evaluates I, II, III, and IV sequentially. Activating one stage can make the next stage ready in the same effect chain before the next date is scheduled, so one callback can still cascade several evolutions with zero spacing. The accepted next-unrecorded-stage pacing is not satisfied. |
| Evolution actor ownership | Incorrect | `black_plague_evolution_select_actor` at `common/scripted_effects/020_black_plague_evolution_effects.txt:47-71` prefers the origin controller before `RTX`. Evolution II therefore does not log the first overseas destination owner. `black_plague_rat_activate_evolution_iv` explicitly saves the source `RTA` as actor at `common/scripted_effects/020_black_plague_rat_effects.txt:1157-1163`, even after creating `RTX`. A later natural Evolution V normally selects the origin controller, or the same-chain RTA target if stages cascade, rather than the Rat King. The matrix requires III to use RTA and IV and V to use RTX. |
| Evolution IV activation transaction | Partial and completion-blocking | `black_plague_rat_activate_evolution_iv` sets IV active and recorded before proving that a Royal Basin was selected or that `black_plague_rat_transfer_to_king` succeeded at `common/scripted_effects/020_black_plague_rat_effects.txt:1157-1163`. `black_plague_rat_transfer_to_king` can instead set `black_plague_rat_king_activation_failed` at lines 1116-1155, yet the outer activation still records and reports IV. Natural play and SCN-012 can therefore record a Rat King evolution without a Rat King. |
| Evolution V and world-end order | Accepted design gap | `black_plague_activate_evolution_v` at `common/scripted_effects/020_black_plague_evolution_effects.txt:355-368` records Evolution V and immediately calls the terminal takeover. Its ready trigger already requires the completed royal route and continent control at `common/scripted_triggers/020_black_plague_evolution_triggers.txt:156-233`. The accepted source spec requires two gates: Evolution V unlocks the world-end lane and target selection, then route completion and continent control trigger terminal takeover. See `docs/specs/020_black_plague_specs/specs/020_black_plague_spec_part_7_world_end_and_aftermath.md:7-73,263-275`. |
...
| SCN-012 active-crisis preservation and atomicity | Partial and completion-blocking | Candidate collection now counts existing established states at `common/scripted_effects/020_black_plague_scenario_effects.txt:102-105`, every continent anchor excludes established states at lines 116-171, and the top-up pool excludes established states at lines 260-276. Existing disease values are therefore no longer overwritten by the seed effect. The transaction is still not atomic for actors: `black_plague_scenario_assign_king_army` at lines 380-383 resets an existing RTX's Dominion, Sentience, Cohesion, and Hunger to scenario values and creates another Royal Brood Guard package rather than preserving values and topping up to floors. The launch writes its permanent launched flag before Royal Basin and King creation are proved at lines 482-505, so a failed IV transfer can leave a one-shot partial scenario. The scheduler can also be cleared without replacement when no new anchor is available. |
