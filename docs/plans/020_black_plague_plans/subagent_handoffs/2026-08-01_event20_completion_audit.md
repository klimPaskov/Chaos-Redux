...
| Mission family | Missing | No audited Event 020 decision file contains `days_mission_timeout`, `mission_timeout`, or `activate_mission`. The accepted mission rows remain plan-level content without an implemented mission owner, timeout, success, failure, or duplicate-risk contract. |
