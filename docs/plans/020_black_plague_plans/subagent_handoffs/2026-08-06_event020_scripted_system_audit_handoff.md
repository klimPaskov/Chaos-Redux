...
This pass audited the shared Event 020 scripted effects, scripted triggers, script constants, event-target lifecycle, scheduler, disease phases, spread ledger, mortality adapter, response/countermeasure lanes, Rat Nation/Rat King runtime, SCN-012 triggerable launch, weaponization, and the Doctor Wu bridge.
...
| `black_plague_weaponization_*` and `doctor_wu_*` | Country/any | Native payload, approach costs, host eligibility, foreign request targets, and countermeasure inputs | Uses the ordinary exposure path for delivery/accidents; Doctor Wu stores one global host target and transfers it atomically | `020_black_plague_weaponization_effects.txt`, `163_doctor_wu_effects.txt`, and Event 163 event chain |
...
The source audit found 1,332 `constant:category.key` references across Event 020 and Doctor Wu with zero unresolved keys against `common/script_constants/`.
...
Doctor Wu and weaponization now use the shared dynamic stockpile debit helpers documented in `common/scripted_effects/chaosx_dynamic_effects.md`; those parent-owned edits were observed but not modified by this subagent.
...
Source hygiene checks found zero duplicate top-level helper names across the Event 020/Doctor Wu effect, trigger, and constant files and zero unresolved constant references. No new world-iterating on_action was introduced; explicit `every_state`/`every_country` effects remain bounded to initialization, terminal execution, one-shot Doctor Wu selection, or aftermath notification paths.
