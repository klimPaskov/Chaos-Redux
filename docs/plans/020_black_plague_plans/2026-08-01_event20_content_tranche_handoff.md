...
- The RTX court's Royal Strike now uses the dedicated `GFX_decision_black_plague_strike_royal_node` sprite, matching the shared human Royal Node counterstrike without creating another icon family or category.
- The Rat King defeat archive branch now makes the `.73` Preserve the royal records choice operational: it registers the response country, exposes the existing shared Publish Findings action, and leaves the recovered findings latent until ordinary countermeasure setup transfers them.
- SCN-012 now launches directly from the existing scenario button when its normal availability trigger passes. The shared confirmation window remains unchanged for other scenario entries, while the Black Plague terminal lock still disables the launch button.
- The shared RTA category retains a paid, cooldown-gated `Absorb a Weaker Brood` command. It invokes the bounded state-marker merger helper, transfers one weaker marker's surviving unit budget through the reusable RTA carrier, and never creates an additional country tag.
- The ordinary phase report surface now includes one-shot Collapsed and Recovery notices (`chaosx.nr20.12` and `chaosx.nr20.13`) alongside the existing Threatened, Severe Crisis, Containment, relapse, Cured, and Rat-Controlled reports. These are ordinary phase reports only; they do not create event-log evolutions or alter the five-evolution ledger.
- The ordinary phase report surface now also includes a one-shot Incubating notice (`chaosx.nr20.14`). It is emitted from the same state-owned milestone path as the other phase reports, remains suppressed during SCN-012 bootstrap, and does not create an event-log evolution or alter the five-evolution ledger.
- The existing RTA hierarchy report (`chaosx.nr20.45`) now resolves to route-specific descriptions for Distributed Instinct, Dominant Beast, and Emergent Cunning, with the previous neutral wording retained as a guarded fallback.
...
- The shared Royal Burrows cleanup decision and linked last-response refuge surfaces now consume the five-frame authored seal through `GFX_decision_black_plague_seal_royal_burrows_animated`, while the existing static seal remains registered as a fallback. Source frames, processed frames, review previews, DDS dimensions, and hashes are recorded in `docs/assets/020_black_plague/royal_burrows_seal_animation/manifest.md`.
- The shared Royal Node counterstrike now consumes the dedicated `GFX_decision_black_plague_strike_royal_node` icon instead of the generic military-support sprite. Source, alpha-intermediate, processed PNG, runtime DDS, and provenance are recorded in `docs/assets/020_black_plague/manifests/royal_node_decision_icon_manifest.md`.
- Natural Evolution III now rebuilds a live basin-state count before each event-owned evolution readiness pass and requires at least three human-controlled states in a connected Infected/Severe/Collapsed basin. The spawn selector and both readiness gates use the shared `black_plague_rat_state_has_connected_basin_signal` trigger and `minimum_basin_states_for_iii`, while SCN-012 keeps its scoped bootstrap path for deliberate multi-basin setup.
