...
| Unit audio | per-job manifests and `sound/014_cannibalism_units_sound.asset` | Five installed packages contain 35 converted 44.1 kHz WAV files; source-side packages for other foot families remain evidence only | Bone Riders playback and remaining family runtime sound definitions remain open. |
| Super-event media | `docs/specs/014_cannibalism_specs/matrices/super_event_matrix.md` and `docs/super_events/014_cannibalism/audio_research.md` | Four audio/image packages are source-side documented | Sound definitions, settings-aware dispatch, catalog confirmation, and final wiring remain parent-owned. |
...
- Complete parent-owned entity, model, action, sound-definition, super-event wiring, and live consumer validation before any final package-completion claim.

## Current parent handoff state

The gameplay/documentation design is current at nine consumers, three origins, no cluster, no Prison Host, reveal-gated Hannibal identity, separate vanilla elephantry, no animation toggle, and a 27-texture counter package. Five model/action/entity/audio packages are parent-installed with source-checked evidence. Bone Riders, Island Reavers, Scavenger Warband, Network Cadre, final super-event sound wiring, and live consumer validation remain open or blocked.
