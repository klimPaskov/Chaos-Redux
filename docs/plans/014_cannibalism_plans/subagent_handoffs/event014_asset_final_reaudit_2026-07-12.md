...
For all 14 packages, expected source and processed frame counts match; every frame is hash-distinct within its package; sheet dimensions equal frame width multiplied by the declared frame count; the runtime DDS sheets and fallbacks match the PNG dimensions; and source frames, processed frames, sheet PNG, sheet DDS, fallback PNG/DDS, GIF preview, contact sheet, manifest, and GFX handoff are present. `interface/014_cannibalism_frontline_hunger.gui` consumes every animated sprite and its static fallback. `common/scripted_guis/014_cannibalism_scripted_gui.txt` switches all 14 pairs through `cannibalism_gui_animations_disabled` and reveal/route gates.
