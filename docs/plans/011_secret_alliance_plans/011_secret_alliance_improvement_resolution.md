...
| B: operation dossiers and pacing | One substantial operation is owned at a time with actor, family, surface, risk, evidence class, readiness layer, recent-family penalty, and adaptive recovery. Commitment weights actor selection, and each result changes the actor's later commitment. MTTH cadence responds to evolution, readiness, alertness, preparedness, recovery, and controlled channels. | `secret_alliance_launch_weighted_operation`, commitment ticket helpers, operation dossier variables, `common/mtth/011_secret_alliance_mtth.txt` |
...
- Coalition victories follow the saved doctrine and recorded war facts. Only qualifying bloc outcomes retain the Event 011 faction, renamed and converted to the postwar security manifest; all other owned Event 011 factions are dismantled.
