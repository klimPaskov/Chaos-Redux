...
- All twenty-seven exact-agent Lightweight Installation, Long-Range Payload, and combined aircraft-rack variants have independent source art, 64-by-64 transparent processed previews, BGRA DDS archive/runtime pairs, a contact sheet, manifest, validation note, and exact `GFX_EMI_chem_air_bomb_<agent>_<variant>` wiring in `interface/chaosx_equipment.gfx`. No base rack, cross-agent icon, or resized cross-type substitute is used.
- Each aircraft-rack variant is unlocked by an exact agent technology and the matching completed Aerosol and Air Delivery Bureau trait through a strict AND gate. Lightweight, long-range, and combined effects exist only on the installed exact module, so no ordinary CAS or tactical-bomber bonus, payload-production proxy, all-agent unlock, or designer fallback remains. Current-version live validation still needs to confirm how multiple installed range-bearing racks aggregate native aircraft range; no estimator will be added if the engine differs from the design-screen arithmetic.
