...
- each super-event needs one unique final WAV under `sound/<event_id>_<event_slug>/...`
- each final WAV must be registered in `sound/chaosx_sound.asset` and played through the settings-aware sound helper
...
- each super-event must set `global.current_super_event_audio_id` before calling the settings-aware sound helper
- `music/chaosx_music_track_list.html` remains the canonical audio catalogue, and the parent implementation owns its row updates
...
- `chaosx_super_event_audio_researcher`: unique audio research, licensing, final WAV conversion
