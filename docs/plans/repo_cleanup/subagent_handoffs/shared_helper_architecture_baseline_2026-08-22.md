...
This document is retained evidence from the 2026-08-22 snapshot. Its helper counts, candidate classifications, MCP artifacts, and proposed follow-ups describe that audit only and do not establish current approval or implementation status.
...
The clearest documentation defect in the snapshot was ownership drift in `common/scripted_effects/chaosx_dynamic_effects.md`: its table of contents named the 17 helpers declared in `chaosx_dynamic_effects.txt`, but later sections also documented APIs declared in Event 006, Event 016, the clone system, alien infantry, Mengele bridge, and Event 019 provider files. Commit `775099cbfa` later clarified ownership in the current `chaosx_dynamic_effects.md` preamble and owner index. The dated finding is retained, and no replay of that documentation patch is needed.

The clearest proof-gated cleanup candidates in the snapshot were the uncalled dynamic helpers `modify_value_based_on_chaos_tier` and `damage_buildings_in_random_states`, plus the commented `clear_special_chaos_country_civilian_effects` hook. The first helper was later removed by commit `2f529cc54c`, while the remaining two still require dynamic/meta-effect and generated-reference searches before any removal.

The event log scripted GUI repeats tab reset, detail-close, and view-rebuild logic, but this baseline recommends no GUI patch. A future functional binding cleanup may be considered only with a bounded MCP before/after inspection and without changing `interface/*.gui`, GUI assets, or visual layout and coordinates.
...
| --- | --- | --- |
| `modify_value_based_on_chaos_tier` | Declaration at `chaosx_dynamic_effects.txt:17` in the audited snapshot. No direct active callers were found. | Removed after the snapshot by commit `2f529cc54c`, as recorded in `events_1_20_catalog_dead_localisation_cleanup.md`. No replay of the old orphan-removal instruction is needed. |
...
In the audited snapshot, `modify_value_based_on_chaos_tier` encoded tier values 0-3 and a greater-than-3 branch with factors 0-4, while its documented inputs were `base_value` and `add_value` and its output was `modified_value`. The hardcoded ladder was a tuning candidate only if the helper remained an API. Commit `2f529cc54c` later removed the helper, so no constants migration or replay is needed.
...
This was a documentation/source-of-truth mismatch, not proof that those helpers were stale. The dated recommendation was to split the API index into declared dynamic helpers and subsystem-owned contracts, with links to each authoritative file. Commit `775099cbfa` later clarified that ownership in `common/scripted_effects/chaosx_dynamic_effects.md` and its owner index.
...
`interface/chaosx_events_log_popup.gui` is the visual consumer with tabs, dynamic lists, details windows, world-end scenario details, and cluster details. It remained untouched and is excluded from all patch recommendations in this baseline.
...
The dated audit identified `docs/systems/world_threat_mechanic.md:81` and `:154` as stale references to `common/scripted_effects/014_cannibalism_core_effects.txt`. A targeted current path check found that both lines now name `common/scripted_effects/014_cannibalism_effects.txt`. The narrow path correction is present, while the broader Black Plague wrapper wording and runtime behavior reconciliation remain unresolved and require owner and MCP review.
...
These candidates are narrow and preserve the behavior observed in the audited snapshot.
...
The dated audit recommended updating `docs/systems/world_threat_mechanic.md:81,154` from `014_cannibalism_core_effects.txt` to `014_cannibalism_effects.txt`. A targeted current path check confirms that both lines already use `014_cannibalism_effects.txt`, so no owner patch is needed for this narrow correction. Broader wrapper wording and runtime behavior reconciliation still require owner and MCP review.
...
`modify_value_based_on_chaos_tier` was removed after this snapshot by commit `2f529cc54c`. Before deleting the remaining `damage_buildings_in_random_states` or `clear_special_chaos_country_civilian_effects`, search all source, markdown, generated Qoder/Codex metadata, scripted localisation, `meta_effect`, `meta_trigger`, collection, and data-driven references.
...
| --- | --- | --- | --- | --- | --- |
| `refresh_world_threat_state` | Country/global state flow observed in the source snapshot | Source flags and global threat state recorded in the snapshot | `global.world_threat_source_count`, `world_in_threat` | Sets or clears global aggregate flag | Events 2, 3, 5, 7, 10, 14, 16, 18, 20 and shared/21+ effects listed above |
| `union_compatible_researched_technologies_from_donor` | Country with donor event target | `event_target:technology_union_donor.researched_techs` | Compatible researched technologies on the scoped country | Adds allowed technologies while skipping branch conflicts | Event 14 `014_cannibalism_effects.txt:12401,12577,18595` |
| `call_natural_disaster` | Country/event scenario | Selected disaster token and scenario inputs recorded in the snapshot | Natural-disaster event/effect dispatch | Fires the selected disaster flow | Event 13 and cluster/scenario callers listed above |
...
The future GUI helper must be designed around the existing shell's functional state contract, not around visual layout changes.
...
No interface layout, coordinate, asset, or visual behavior recommendation is made by this baseline.
