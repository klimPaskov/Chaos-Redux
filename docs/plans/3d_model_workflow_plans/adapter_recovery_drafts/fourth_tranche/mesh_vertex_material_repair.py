...
def finish_verified(req,h,job,source,output,report):
    ...
    if before['sha256']!=after['sha256']:
        raise RuntimeError('Repair checkpoint save/reopen fingerprint mismatch; checkpoint is not accepted. Changed sections: '+repr([key for key in before['sha256'] if before['sha256'][key]!=after['sha256'].get(key)]))
