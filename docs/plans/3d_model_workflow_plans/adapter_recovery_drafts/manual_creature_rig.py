...
def inspect_mesh_landmarks(req,h):
    ...
    for n in names:
        ...
        meshes.append(obj)
    data={"source":p["blend_rel"],"source_sha256":expected,"meshes":{obj.name:{"vertices":[{"index":v.index,"world":list(obj.matrix_world@v.co)} for v in obj.data.vertices],"triangles":[list(f.vertices) for f in obj.data.polygons],"materials":[m.name if m else None for m in obj.data.materials]} for obj in meshes},"rigs":{rig.name:[{"name":b.name,"parent":b.parent.name if b.parent else None,"head":list(b.head_local),"tail":list(b.tail_local),"matrix_local":[list(v) for v in b.matrix_local],"pose_matrix_basis":[list(v) for v in rig.pose.bones[b.name].matrix_basis],"rotation_mode":rig.pose.bones[b.name].rotation_mode} for b in rig.data.bones] for rig in h["armatures"](False)}}
