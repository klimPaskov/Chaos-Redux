...
def _finish(req, h, job, source, output, report):
    ...
    if check_pixels:
        ...
        if fingerprint['sha256'] != persisted['sha256']:
            raise RuntimeError('Empty-path image checkpoint save/reopen fingerprint mismatch; output is not accepted.')
...
def repair_explicit_mesh_winding(req,h):
    ...
    for face in mesh.polygons:
        ...
        for index in face.loop_indices:
            desired[index]=tuple(sign*x for x in original_normals[(face.index,mesh.loops[index].vertex_index)])
    mesh.normals_split_custom_set(desired)
    mesh.update()
    normal_proof=_verify_normal_signs(mesh,original_normals,selected_faces)
    ...
    mesh_name=obj.name
    report=_finish(req,h,job,source,output,{'operation':'repair_explicit_mesh_winding','mesh':obj.name,'face_indices':ids,'face_indices_sha256':digest(ids),'topology_before':topology_before,'topology_after':topology_after,'positions_preserved':True,'corner_uvs_preserved':True,'rig_weights_actions_materials_preserved':True,'normal_policy':'preserve every unaffected corner normal; negate only explicitly flipped face corners','normal_sign_proof':normal_proof,'status':'saved_pending_reopen_normal_proof'})

    bpy.ops.wm.open_mainfile(filepath=str(output),use_scripts=False)
    report['reopened_normal_sign_proof']=_verify_normal_signs(bpy.data.objects[mesh_name].data,original_normals,selected_faces)
