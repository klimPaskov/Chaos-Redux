...
def _verify_normal_signs(mesh, original, flipped):
    ...
    for key, value in actual.items():
        ...
        error = math.sqrt(sum((x-y)**2 for x, y in zip(value, wanted)))
        if error > maximum:
            worst = {"face_vertex":key,"wanted":wanted,"actual":value,"selected_face":key[0] in flipped}
        maximum = max(maximum, error)
    if maximum > 0.001:
        raise RuntimeError(f"Winding correction changed custom normals beyond sign and native quantization: {maximum}; worst_corner={worst!r}.")
    return {"corners": len(actual), "maximum_vector_error": maximum, "tolerance": 0.001, "policy": "original corner vector times selected face sign; bounded native custom-normal quantization"}
        ...
        for obj, row in zip(targets, report["meshes"]):
            ...
            for face in mesh.polygons:
                ...
                for index in face.loop_indices:
                    desired[index] = tuple(sign*x for x in originals[obj.name][(face.index, mesh.loops[index].vertex_index)])
