...
p=Path('docs/plans/3d_model_workflow_plans/adapter_recovery_drafts/fifth_tranche').resolve();sys.path.insert(0,str(p));sys.path.insert(1,str(p.parent));sys.path.insert(2,str(Path('.tools/3d_pipeline/tests').resolve()))
...
class Extra(unittest.TestCase):
 def image(self,**kw):
  args=dict(name='FixtureImage',packed_files=[],filepath='',source='GENERATED',has_data=True,size=[1,1],channels=4,pixels=[.2,.3,.4,1],alpha_mode='STRAIGHT',colorspace_settings=types.SimpleNamespace(name='sRGB'));args.update(kw);return types.SimpleNamespace(**args)
 ...
 def test_lazy_file_cache_stable(self):
  class Lazy:
   def __init__(self, base): self.__dict__.update(base.__dict__);del self.__dict__['size']
   @property
   def size(self): self.has_data=True;return [1,1]
  with tempfile.TemporaryDirectory() as td:
   path=Path(td)/'image.dds';path.write_bytes(b'fixture')
   obj=Lazy(self.image(filepath=str(path),has_data=False))
   a=ns['_promotion_image_record'](Path(td),obj);b=ns['_promotion_image_record'](Path(td),obj)
   self.assertEqual(a,b);self.assertIsNone(a['has_data']);self.assertIsNotNone(a['file_sha256'])
 def test_missing_sentinel(self):
  a=ns['_promotion_image_record'](Path('.').resolve(),self.image(has_data=False,pixels=[],size=[0,0],source='FILE'));self.assertTrue(a['missing_data_sentinel']);self.assertIsNone(a['pixel_sha256']);self.assertEqual(a['name'],'FixtureImage')
...
suite=unittest.TestSuite(unittest.defaultTestLoader.loadTestsFromModule(x) for x in [vr,mp,mr]);suite.addTests(unittest.defaultTestLoader.loadTestsFromTestCase(Extra));result=unittest.TextTestRunner(verbosity=1).run(suite);sys.exit(not result.wasSuccessful())
