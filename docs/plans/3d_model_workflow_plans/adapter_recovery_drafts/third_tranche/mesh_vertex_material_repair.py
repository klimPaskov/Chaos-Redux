...
def finish_verified(req,h,job,source,output,report):
    ...
    if before['sha256']!=after['sha256']:
        raise RuntimeError('Repair checkpoint save/reopen fingerprint mismatch; checkpoint is not accepted.')
    report['save_reopen_proof']={'passed':True,'sha256':after['sha256'],'source_blend_sha256':h['file_sha256'](source),'reopened_checkpoint_sha256':h['file_sha256'](output)}
...
def bind_existing_pdx_material(req,h):
    ...
    for section in ('materials','images'):
        for n,row in before['sections'][section].items():
            ...
            if row!=other: raise RuntimeError('Material rebind changed an original material/image datablock.')
