...
def _finish(req, h, job, source, output, report):
    ...
    fingerprint = h['_promotion_fingerprint'](job,[],include_sections=True)
    check_pixels = any(row.get('pixel_sha256') for row in fingerprint['sections']['images'].values())
    h["save_blend"](output)
    if check_pixels:
        ...
        report['empty_path_image_save_reopen_proof'] = {'passed':True,'image_sha256':persisted['sha256']['images'],'source_blend_sha256':h['file_sha256'](source)}
