...
def _promotion_image_record(job, image):
    ...
    if not packed and path is not None:
        ...
        if image.source not in {"FILE", "GENERATED"} or not image.has_data:
            raise ValueError("Empty-path image requires local loaded FILE/GENERATED pixels.")
    ...
    return {"filepath":image.filepath,"size":list(image.size),"source":image.source,"alpha_mode":image.alpha_mode,"colorspace":image.colorspace_settings.name,"packed_hashes":packed,"file_sha256":file_hash,"pixel_sha256":pixel_hash,"pixel_encoding":"little_endian_float32" if pixel_hash else None,"settings":{key:_promotion_value(getattr(image,key)) for key in ("channels","is_float","generated_type","generated_width","generated_height","generated_color","use_generated_float","use_view_as_render") if hasattr(image,key)},"properties":_promotion_properties(image)}
