...
def repair_explicit_vertex_remap(req,h):
    ...
    for f in bm.faces:f[fl]=f.index
    ...
    for index,tri in changed.items():
        ...
        for loop,old in zip(new.loops,original.loops):
            loop.copy_from(old)
    ...
    for f in mesh.polygons:
        ...
        for i in f.loop_indices:
            ...
            corner=list(f.loop_indices).index(i);source_corner=faces[old_face][corner]
    ...
    mesh.normals_split_custom_set(desired);mesh.update();normal_proof=_verify_normal_signs(mesh,expected_normals,set())
    ...
    for f,old in zip(mesh.polygons,face_map):
        target=changed.get(old,faces[old]);expected_positions=[positions[sources[i]] for i in target]
        if [list(mesh.vertices[i].co) for i in f.vertices]!=expected_positions:raise RuntimeError('Face corner position/order changed.')
