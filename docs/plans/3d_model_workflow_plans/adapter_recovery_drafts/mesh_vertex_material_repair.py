...
def edit_explicit_mesh_vertices(req,h):
    ...
    mesh=obj.data; edits=p['vertex_edits']; selected=validate_vertex_edits(edits,len(mesh.vertices),set(rig.data.bones.keys()),set(obj.vertex_groups.keys()))
    ...
    return _finish(req,h,job,source,output,{'operation':'edit_explicit_mesh_vertices','mesh':obj.name,'target_armature':rig.name,'vertex_edits_sha256':digest(edits),'changed_vertices':sorted(selected),'position_edited_vertices':sorted(changed_position),'unchanged_vertices_uvs_topology_rig_actions_materials_preserved':True,'geometry_before':before['mesh_counts'],'geometry_after':after['mesh_counts'],'normal_policy':'retained source corner normals; changed-position faces require normal review','status':'edited_requires_contact_deformation_and_export_reimport_review'})
...
def bind_existing_pdx_material(req,h):
    ...
    before=h['_promotion_fingerprint'](job,[],include_sections=True)
    material=_component_material(req,h,job,new_name,p['material_spec'])
    ...
    for section in ('materials','images'):
        for n,row in before['sections'][section].items():
            if row!=after['sections'][section].get(n): raise RuntimeError('Material rebind changed an original material/image datablock.')
    return _finish(req,h,job,source,output,{'operation':'bind_existing_pdx_material','source_material':source_name,'material':material.name,'material_spec_sha256':digest(p['material_spec']),'targets':{obj.name:slots for obj,slots in targets},'geometry_uvs_weights_rig_actions_and_old_materials_preserved':True,'status':'bound_requires_preview_and_export_texture_reference_review'})
