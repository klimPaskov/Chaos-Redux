...
def repair_explicit_vertex_remap(req,h):
    ...
    vl=bm.verts.layers.int.new('chaosx_remap_source_vertex');fl=bm.faces.layers.int.new('chaosx_remap_source_face');cl=bm.loops.layers.int.new('chaosx_remap_source_corner')
    for v in bm.verts:v[vl]=v.index
    for f in bm.faces:
        f[fl]=f.index
        for loop in f.loops:loop[cl]=loop.vert.index
    ...
    loop_sources={f.index:[loop[cl] for loop in f.loops] for f in bm.faces}
    final_indices={key:v.index for key,v in by_id.items() if v.is_valid}
    bm.verts.layers.int.remove(vl);bm.faces.layers.int.remove(fl);bm.loops.layers.int.remove(cl);bm.to_mesh(mesh);bm.free();mesh.update()
