...
| Fracture Cascade SCN deterministic review | SCN-01 through SCN-07 map to verified ID 18, four types, four intensities, centralized target shares, the Low 4:1 minor-to-major, Medium 2:2, and High 1:3 target-ticket ladder, severity bands, normal-human eligibility, same-tag fallback, and immediate setup. Maximum now freezes its eligible pool before committing any crisis, so countries created during the run cannot join that run. Every type selects the prepared same-tag route first for unsafe one-state or all-island countries, and High/Maximum setup can enter the bounded multi-front transaction without a globally active Evolution I. | Current source-backed deterministic evidence and demonstrated defects fixed; Low/High observed distributions, full Maximum commit guarantee, exact probability baseline/compare cycle, and live timing remain pending |
| Fresh archetype probability refresh | Current parent `hoi4.probability_inspect` resolves the six-entry pool at source line 1132 with six candidates, six required inputs, and zero unresolved inputs; the matching three-fixture evaluation resolves 18 rows with zero unresolved inputs and seven intentional zero-route/dominance warnings | Current parent MCP evidence recorded in `subagent_handoffs/probability_parent_refresh_2026-09-19.md`; independent specialist certificate and full named matrix remain pending |
...
The scenario commit path still uses the immediate selected-set preparation and commit phases. `chaosx.nr21.19` is host-bound for each selected target, and `chaosx.nr21.15` consumes an explicit frozen commit-target array after all selected targets have completed preparation. Maximum adds an opening-state preflight to its normal-human enumeration, and the controller records preflight-selected, frozen commit-eligible, and preflight-skipped counts without post-mutation route search or replacement selection.
...
## 2026-09-19 front-plan and documentation repair

Evolution I additional-front callers now require pre-commit route, anchor, objective, and package receipts; the ordinary and Event 006 helpers no longer search the changed post-opening map for a replacement front. A missing receipt is a bounded skip, preserving the primary crisis and avoiding an unplanned state collision.
