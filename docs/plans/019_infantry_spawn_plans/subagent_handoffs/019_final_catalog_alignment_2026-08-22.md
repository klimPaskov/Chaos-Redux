...
- `Events!F20` was updated to retain the exact **Command Fracture** title and body while adding the current decision-only Formation Ledger behavior and closure rule. The text now states that ordinary decisions handle requests, claimant files, demands, coups, and revolt paths, that the shared relevance gate hides the Formation Management and claimant categories after a completed takeover or achievement-marked claimant or derivative revolt even if another claimant row remains, and that peaceful closeout remains available while obligations, claimants, and operations are still active. It also records the takeover, failed-coup, and derivative-revolt closure outcomes in player-facing language.
