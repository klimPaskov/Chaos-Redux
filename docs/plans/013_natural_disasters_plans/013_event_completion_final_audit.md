...
2026-07-26 direct-interface addendum: the Event 013 abnormal-path window was re-rendered with the HOI4 MCP at 1280x720, 1920x1080, and 2560x1440 (1.25 UI scale), across twelve interaction states. The sequence, family-card, milestone, path-control, and footer button hitboxes now use the native sprite dimensions before their declared scale, so source-coordinate spacing has zero overlaps and each clickable rectangle follows its visible sprite. The header now exposes the selected family through the mapped category picture, severity band, and scheduled impact date, while the bottom legend consumes eight icon-and-label identities including the foreign-relief badge. The MCP's remaining global overlap and `player_context` diagnostics are project-wide offline-analyzer findings; `player_context` is the documented vanilla context for this scripted GUI, and the direct source graph has no Event 013-specific unresolved font or GIF reference. The general shared super-event window and its scripted GUI/localisation remain byte-identical to the pre-Event 013 baseline.
