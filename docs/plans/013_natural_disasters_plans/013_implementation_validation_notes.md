...
The earlier targeted Event 013 abnormal-path MCP artifact covered 1280x720, 1920x1080, and 2560x1440 with twelve interaction states, but its layout data exposed 17 click-region intersections caused by legacy hitbox sizes. The source-coordinate correction now uses scaled dimensions for the sequence controls, five family cards, six milestones, three path controls, and three footer controls; an independent bounded parser reports zero source-coordinate overlaps across all 25 buttons. A fresh MCP inspect/render completed at all three resolutions and twelve states at source revision `2452b17255026c63b7d59f61594eec80bbea9fc3fcc8374acc3e06c2e890063`. Its four remaining card-3/milestone intersections are offline-model false positives because the renderer multiplies source offsets by each control's scale; the vanilla interface contract and the `airselectionview.gui` nuclear-button overlay establish that scale changes button size while position remains the declared screen coordinate. The fresh direct scene reported no element-level long-text overflow or clipping; only the close-button shader and DDS decoding renderer warnings remained. The header exposes the selected family picture, severity band, and scheduled impact date, and the passive legend consumes impact, warning, report, aftermath, chain-risk, closed, foreign-relief, and blocked-action icons. The offline analyzer's whole-project overlap and context findings remain separate from the direct source-coordinate proof, and no direct Event 013 font or GIF reference remains. The general shared super-event window was not changed and remains at its pre-Event 013 baseline.

## User-reported firing failure

Scope note: the Event 013-owned delta for the general shared super-event surfaces remains at the pre-Event 013 baseline; unrelated Brilliant Scientist/Rat King edits in shared scripted files were preserved outside this task.
