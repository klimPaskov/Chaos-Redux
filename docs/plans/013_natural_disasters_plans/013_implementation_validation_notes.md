...
| Delayed dates | Every state job passes through the global sequence/date reservation ledger and clamps to at least one day. | Passed static queue review. |
...
| Rejected no-target call | The provisional sequence id is restored or cleared when no primary hit schedules; caller last-sequence id, hit counts, and anchor flag are written only after acceptance. | Passed fail-closed state review. |
