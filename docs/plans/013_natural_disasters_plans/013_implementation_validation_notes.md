...
The broader concurrent `docs/assets/` cleanup removes all 1,035 tracked files in the Event 013 binary source-art/audio archive from this working tree. All registered live DDS/OGG/WAV files remain, and the separate super-event audio production document retains the replacement cue's source, rights, and hashes. The former archive included the event asset manifest, GFX handoff, animation provenance/build records, and many source/processed files; the current `docs/assets/013_natural_disasters/manifest.md` is only a retention record, so complete provenance is not present. Restoring the archive or explicitly accepting its removal is queued pending the repository-wide asset-retention decision; it was not silently reconstructed or treated as present.
