...
The earlier targeted Event 013 abnormal-path MCP artifact covered 1280x720, 1920x1080, and 2560x1440 with twelve interaction states, but its layout data exposed 17 click-region intersections caused by legacy hitbox sizes. The source-coordinate correction now uses scaled dimensions for the sequence controls, five family cards, six milestones, three path controls, and three footer controls; an independent bounded parser reports zero source-coordinate overlaps across all 25 buttons. A fresh post-correction MCP inspect/render is queued because the configured HOI4 MCP transport returned `Transport closed`, so the static parser result is not represented as a new MCP render. The header exposes the selected family picture, severity band, and scheduled impact date, and the passive legend consumes impact, warning, report, aftermath, chain-risk, closed, foreign-relief, and blocked-action icons. The offline analyzer's earlier whole-project overlap and context findings remain separate from the direct source-coordinate proof, and no direct Event 013 font or GIF reference remains. The general shared super-event window was not changed and remains at its pre-Event 013 baseline.
