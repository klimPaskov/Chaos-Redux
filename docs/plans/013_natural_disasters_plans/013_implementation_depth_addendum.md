...
- Turn the route sets above into family-conditioned alternatives rather than one default chain per family. Route choice must use target context, warning action, severity, and recovery state. `political_shock` remains an accepted chain family and should be used only for high-death/failed-recovery conditions, not added to every disaster.
