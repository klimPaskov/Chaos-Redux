...
- Read-only `hoi4_probability_inspect` for `events/016_brilliant_scientist_synthesis_events.txt` returned `status = ok` with no blockers or diagnostics, but reported an incomplete candidate pool and no normalized probability result; no balance claim is based on that analyzer output.
