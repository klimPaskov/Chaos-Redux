...
| Unit, equipment, template, tactics, and hidden technologies | Source-evidenced; current MCP comparison blocked | The exact battalion/equipment values, ten-battalion locked template, two alien-only tactics, predictive dependency, equipment gates, and enum registration are present. A prior same-day technology inspect/render is partial; fresh current technology inspection timed out and no before/after technology comparison exists. |
...
| Expedition and pact chain `.40-.47` | Source-evidenced; current full-chain MCP blocked | Events, costs, 180-day missions, Kruger suspension/restore, exact Directorate deltas, Mengele separation, pact receipts, landing report, failure, warning, and revolt bridge exist. Only `.47` has a retained partial same-day MCP artifact; fresh `.40` inspect/render calls timed out, so the whole chain is not currently rendered or compared. |
...
| Caller-owned landing history across independent countries/providers | Incomplete documentation; source-evidenced; runtime blocked | Current effect code and dynamic API docs are corrected. The binding addendum and acceptance scenarios omit the ownership contract and the required cross-country matrix. |
...
- This auditor's `hoi4.focus_inspect` and `hoi4.focus_render` calls for `dhrondan_focus_tree` timed out, but the named probability auditor later obtained a current inspect at revision `e751d1b2a47f0b39bae57a1686b5f41e1a99bbb3a4918a3df4b133a244af47ce`: 88 focuses, 102 connectors, and zero layout diagnostics, together with current HTML, SVG, and JSON renders.
- `hoi4.tech_inspect` for the Alien Infantry technology timed out.

After repeated event adapter timeouts, no responsible `event_compare` could be run because there was no successful fresh current revision/baseline pair. Source diffs are not substituted for MCP comparison evidence.

A retained same-day handoff provides partial artifacts at revision `f588a2607444400ec9fa9d102943fc0e10dc4482ebca9935232a4df2966f59d5` for `chaosx.nr16.47` and `chaosx.nr19.1`, a successful 88-focus inspect/render, and a partial Alien Infantry technology inspect/render. Those artifacts are useful historical/post-registry evidence, but they cover only one contact event and the Event 019 root, they predate V13 runtime promotion, and they do not provide a current full-chain comparison. They are not treated as engine proof for `.40-.46`, `.48-.52`, the Event 016 root/evolution chain, deferred material conservation, or current model consumers.
