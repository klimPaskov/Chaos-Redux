...
| Decisions, missions, and ideas | Static pass | Ten KRG decision categories and the foundation, clone/machine, paleo/xeno, portal/temporal, safeguard, foreign, canonical/exotic, and terminal decision files are present. Four universal starting liabilities plus the conditional Scientific Exodus spirit for enclave/rebellion, and their lifecycle upgrades, are defined and guarded by KRG sovereignty. Quantitative decision balance remains unvalidated. |
...
4. `docs/assets/016_brilliant_scientist/manifest.md` contains mixed historical status sections and tracked legacy portrait-source deletions even though active DDS/TGA runtime paths resolve; documentation/asset-provenance reconciliation is still needed.
