...
| Portraits and flags | Pass for active runtime package | KRG and cosmetic route flags exist under `gfx/flags/`; stage 0–4 leader/advisor DDS files are registered in `interface/016_brilliant_scientist.gfx` and resolve on disk. The current asset manifest is reconciled to the active runtime package, while external redistribution rights for the copied Stage-0 source remain unresolved. |
...
| Decisions, missions, and ideas | Static gameplay and visual pass; quantitative validation pending | Ten KRG decision categories and the foundation, clone/machine, paleo/xeno, portal/temporal, safeguard, foreign, canonical/exotic, and terminal decision files are present. Four universal starting liabilities plus the conditional Scientific Exodus spirit for enclave/rebellion, and their lifecycle upgrades, are defined and guarded by KRG sovereignty. The country idea file defines 28 visible KRG ideas with 28 explicit `picture` assignments, and the bounded 21-icon extension has complete static asset evidence. Quantitative decision balance remains unvalidated. |
...
The portrait and flag row's older manifest warning and the decisions row's older 21-icon gap wording are historical observations from the pre-reconciliation audit. The current manifest and the 21-icon handoff close those production gaps. The remaining limits are external Stage-0 portrait rights, targeted or quantitative or live acceptance, the partial technology-tree proof, the native CBRN callback, and the intentional no-model boundary.
...
4. The current asset manifest and KRG idea-icon package close the former 21-icon production gap. External redistribution rights for the copied Stage-0 portrait source remain unresolved, and tracked legacy portrait-source deletions remain a provenance or ownership review item even though active DDS/TGA runtime paths resolve.
5. Event 016-specific 3D packages remain intentionally deferred outside the current no-model scope. This is a scope boundary, not a missing active runtime package.
...
Parent review should carry the open limits above into the final Event 016 completion report and schedule targeted validation rather than adding a new KRG identity or fallback CBRN implementation.

## Correction — 2026-08-03 KRG country-idea icon status refresh

This dated correction was verified against the Event 016 documentation baseline at `6377fdbdc` and current working-tree files. It supersedes only the earlier 21-icon gap statements in this handoff.
