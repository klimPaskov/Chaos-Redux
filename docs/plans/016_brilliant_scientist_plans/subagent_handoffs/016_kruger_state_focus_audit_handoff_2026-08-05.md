...
| Alien technology and biological containment | `KRG_build_an_independent_reactor_grid` through `KRG_authorize_agents_of_last_resort` (083-088) | Reactor, delivery architecture, interface specialists, paid alien cohorts, containment doctrine, and last-resort agents are gated by high-energy, contact, delivery, and containment triggers. | Alien-arms and biological plans exist. Alien cohorts use the shared exact-cost landing API; biological delivery remains governed by its native lifecycle. |
