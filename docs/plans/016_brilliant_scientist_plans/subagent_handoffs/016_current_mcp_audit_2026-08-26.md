...
These MCP artifacts do not replace user-owned in-game acceptance. They also do not close the remaining custom-unit runtime blockers: Alien Infantry now has all seven promoted V13 action consumers and four static state sound references, but it still lacks a supported stable muzzle locator, particle/light binding, strict selection/acknowledgement/impact/special audio coverage, positional playback proof, and live acceptance, while Portal Raider remains without an accepted firearm-bearing model/entity package. The Portal beachhead state-marker lifecycle and five D’Rhondan support-route consumers remain queued because no accepted lifecycle owner exists. The custom `chaosx_ai_probability_auditor` route and same-scenario `hoi4.probability_compare` remain unavailable in this runtime.
