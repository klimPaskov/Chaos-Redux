...
| Early and country-specific flavor events | `chaosx.nr16.4` through `.9` implement one bounded host-context, assistant-conflict, facility, custody, and named foreign-operation slice without a separate chain for every tag. Broader country-specific flavour remains queued. |
