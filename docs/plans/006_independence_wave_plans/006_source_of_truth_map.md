...
| IW-018 ARX | Gameplay retained; runtime admission closed | Emilio Lussu's 1916 source-locked trial 01 passed broad style/framing gates but failed likeness because it reconstructed the source-soft eye and regularized the face; the candidate is unwired and the older stable DDS is not approval. A source-locked HOI4-style Vittorio Vernè commander candidate has exact crop evidence, raw repaint, deterministic 156x210 processing, and bounded separate likeness/style/provenance PASS findings in `006_arx_verne_visual_audit_v13_2026_07_26.md`; it remains candidate-only and does not change the existing consumer or runtime admission. Pietro Pinna Parpaglia has a source-locked HOI4-style crown-route candidate, but `006_arx_pinna_source_v14_2026_07_26.md` records an exact Kaiserreich owner (`SRD_pietro_pinna_parpaglia`) and only partial strict crown-role fit, so it is also blocked. Adalberto di Savoia-Genova is vanilla-owned, and no collision-free crown/council replacement was established. The complete ARX roster still requires a cleared crown role and parent package re-audit. FORM-05 gameplay remains implemented while IW-018 stays outside runtime attestation |
