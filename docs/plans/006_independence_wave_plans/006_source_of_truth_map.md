...
| Installed-mod tag collision audit | `ef256c2c2` |
...
| MFX collision evidence says the proposed candidate is unused while the current tag audit rejects it | Both findings are retained with different scopes. The dated collision snapshot is historical evidence; the current audit fails closed because MFX is an unreviewed Event 006 identity, so no readiness or collision PASS is claimed |
