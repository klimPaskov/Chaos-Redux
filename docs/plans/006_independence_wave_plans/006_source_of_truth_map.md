...
The ten-country automatic band has a source-level capacity path: IW-008 RHI and IW-010 AJX share RG-RHINE-SAAR, but the central reservation effect admits that exact pair after one sibling is selected because their anchors (51 and 42) are distinct and each row still passes the ordinary per-host protected-remnant check. Other groups remain one-package-only. The band remains fail-closed until live disjoint reservation, host-survival, and synchronized-transaction proof is recorded.
