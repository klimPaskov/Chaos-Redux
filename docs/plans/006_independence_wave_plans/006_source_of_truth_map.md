...
Curated: 2026-07-22 after the sourced-portrait policy withdrew noncompliant grounded packages from runtime admission
...
| FORM-48 Pacific Regional Federation | System implemented; currently unreachable through a compliant carrier set | Family 48 still binds HBX as carrier and sovereign HAW/FSM members without annexation fallback. HBX and FSM package admission is withdrawn until sourced portraits pass, so the formable cannot presently satisfy its complete admitted-member contract |
| Static visual package | Non-portrait assets materially produced; grounded portrait replacement active | Static icons, report scenes, researched/generated flat flags, formable flags, and super-event images remain. Scotland, Brittany, Frisia, Rhineland, Corsica, and Sicily have independently approved and wired sourced real-male, identity-preserving HOI4 repaints and complete package audits; protected Rupprecht and Matthes remain unchanged. Every other grounded generated, generic, or photographic runtime portrait remains rejected as readiness evidence until a complete sourced roster passes independent review. Adenauer, Epp, and Ironside remain rejected because vanilla actively owns them. Exactly zero custom Event 006 advisor icons/assets/sprites/portrait blocks are present |
| Super-event `6002` | Runtime implemented; hidden-formable branch currently unreachable | Final audio, wrappers, station entry, slot-24 dispatch, Event Log payload, settings-aware FIFO, and predicates exist. FORM-48 has no currently compliant admitted carrier/member set; FORM-42 remains fail-closed |
...
| --- | --- | --- | --- |
The exact compile-time set contains IW-001 Scotland, IW-004 Brittany, IW-007 Frisia, IW-008 Rhineland, IW-009 Bavaria, IW-017 Corsica, and IW-019 Sicily.
...
Seven attested IDs can supply the three-, four-, five-, and seven-country automatic waves only when the required number independently pass their live host, anchor, reservation, Event-5 collision, chaos-band, force, and transaction gates.
...
The ten-country automatic bands remain unavailable until at least three additional packages are admitted.
...
| Current sourced male HOI4 portrait authority | The per-subject archival source/refinish manifests, independent visual/provenance audits, and fresh post-wire country-package audits are authoritative. The 2026-07-18 fictional-person refresh and every army-small package are historical evidence only. Scotland, Brittany, Frisia, Rhineland, Corsica, and Sicily are the current fully sourced and admitted rosters. No Event 006 advisor portrait or icon package exists |
...
are currently unreachable under the canonical seven-package gate, and no full
...
| Pacific IW-173/IW-179/IW-184 and FORM-48 audit chain | Implemented gameplay/system evidence; portrait admission superseded | Country-package, focus/icon, architecture, decision/mission, localisation, flag, and postformation PASS handoffs remain bounded implementation evidence. The later sourced-only rule withdraws HAW/FSM/HBX runtime admission until grounded sourced portraits pass. HAW trial 01 is explicitly rejected on its independent likeness gate despite passing style; FORM-48 is therefore currently unreachable through a compliant admitted carrier set |
...
| Older audio notes say `6002` derivatives, wrappers, or gameplay firing are absent | Partly superseded. Slot 24, audio 6002, five predicates, Event Log payload, and queued settings-aware playback are implemented. Its hidden-formable and ten-country paths remain unreachable under the current seven-package gate, while FORM-42 remains fail-closed. Audio 6001 and all slot-23 runtime dispatch remain absent, while its dormant ASSET-005 DDS and sprite registration are retained |
| Earlier AFX source says its Level 2 implementation restored runtime admission | Historical implementation evidence only. IW-006 remains outside the canonical seven-package content-attestation set until its grounded roster and complete package receive a fresh audit |
...
| Older curator status for BRI and AJX | BRI is admitted under the current source-locked portrait and package audits. AJX implementation evidence remains useful, but IW-010 is outside the canonical seven-package gate until both grounded roles and the complete package pass current audits |
| Pacific admission handoffs say HOLD/FAIL-CLOSED | Current admission authority agrees with those bounded verdicts. HAW, FSM, and HBX remain outside the canonical seven-package gate; their gameplay, formable-contract, audit, and validation findings remain implementation evidence only |
...
| IW-043/IW-058 improvement addendum says neither package is admitted and all assets are absent | Partly superseded implementation evidence. Final CHU/ASY assets and operational transactions exist, but IW-043 and IW-058 remain outside the canonical seven-package content-attestation set |
...
2. Preserve the canonical seven-package content-attestation set until additional
