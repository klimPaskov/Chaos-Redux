...
The current flat shelf check is `root_pngs=51`, `child_dirs=0`, and `normalized=0`; the manifests and README now describe 51 original-size masters, including the CHU Galimzhan v2 repaint. The normalized Galimzhan v2 candidate remains in `docs/assets/006_independence_wave/iw043_iw058_portrait_source_research_2026_07_28/` and has an independent likeness/style/crop/provenance audit PASS with recorded source uncertainty. DDS and runtime wiring remain parent-owned promotion work and have not been performed. The other five CHU/ASY grounded rows retain their source-only, blocked, or needs-review statuses. No Event 006 advisor icons were created.
...
- The IW-043/IW-058 package manifests record final reviewed non-portrait assets:
  ...
  provenance PASS, while the other five institutional route
...
| IW-043 CHU | Signature gameplay retained; visual admission withdrawn | Mirsaid Sultan-Galiev remains the source-cleared opening portrait. Galimzhan Ibrahimov now has an independently reviewed source-locked HOI4-style v2 repaint with exact prompt and provenance records and a visual/provenance PASS; no DDS/runtime wiring is admitted yet. Preserve CHU identity, route, forces, focus, decisions, centralized politics, exact vanilla guard, and FORM-12/13 contracts. Keep execution closed until the Bolgar row, river-security row, and full package re-audit pass |
