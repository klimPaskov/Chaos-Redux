...
| COR and package set | Retain COR in the exact thirteen-package static set while carrying its v66 PARTIAL limitations. Do not promote any additional package from a shell, adapter, portrait, or bounded audit. |
...
- The thirteen attested packages can conditionally supply the six-, eight-, and ten-country automatic waves only when the required number pass their source-level host, anchor, reservation, Event-5 collision, chaos-band, force, and transaction gates; the fourteen- and twenty-country bands are design targets with no currently admitted capacity route and remain source fail-closed until the pool grows. Live execution and save/load observation are optional future QA.
...
| IW-018 ARX | Gameplay retained; visual admission withdrawn | Emilio Lussu is sourced, processed, and wired. Preserve the Sardinian identity, anchor 114, Italian host survival, Mediterranean mechanics, and FORM-05 access. The source-locked Vittorio Vernè commander candidate remains candidate-only with bounded separate visual/provenance PASS findings in `006_arx_verne_visual_audit_v13_2026_07_26.md`. The source-locked Pietro Pinna Parpaglia crown-route candidate is preserved under `arx_pinna_crown_v1/`, but `006_arx_pinna_source_v14_2026_07_26.md` records an exact Kaiserreich owner (`SRD_pietro_pinna_parpaglia`) and only partial strict crown-role fit. Adalberto is vanilla-owned, so no collision-free crown/council replacement is currently available. Keep execution closed and do not wire either candidate; the full package still requires a cleared roster and parent re-audit |
...
| Other package IDs | Fail-closed | Implement and audit individually before promotion. The current compile-time content-attestation set contains IW-001, IW-002, IW-004, IW-006, IW-007, IW-008, IW-009, IW-010, IW-012, IW-017, IW-019, IW-173, and IW-184 |
...
must still satisfy exact source-level host, anchor, reservation, Event 005 collision,
  chaos-band, and wave-size gates. The current thirteen-ID set exposes twelve compatible reservation-group IDs because RHI and AJX share RG-RHINE-SAAR, but that pair now has a narrow two-package capacity rule because anchors 51 and 42 are distinct
...
- The thirteen exact package attestations remain binding runtime gates; a broad
