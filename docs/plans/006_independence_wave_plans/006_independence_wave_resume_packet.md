...
Updated: 2026-07-26 after the Event 006 v10 completion audit, the DM-58 injective preflight commit `cf2316a9a`, the witness-before-cost repair `0b6289c4b`, the coordinator-bound expiry cleanup `37c4f6036`, and the rejected/reverted shared-focus candidate `f8ca54d24`
...
The current whole-event completion authority is `subagent_handoffs/006_event_completion_audit_v10_post_cf2316a9a_f8ca54d24_2026_07_26.md` and remains **HOLD**. It supersedes v9 only for its bounded DM-58, focus-validation, and ARX findings, while `0b6289c4b`, `37c4f6036`, and `subagent_handoffs/006_dm58_witness_resolver_2026_07_26.md` supersede its DM-58 execution finding. The resolver now binds an exact three-member/three-state/three-distinct-owner witness before claims, finite war goals, or costs, and the coordinator-bound `chaosx.nr6.309` callback clears the shared receipts after natural expiry; live success, invalidation, save/load, and no-witness performance matrices remain open. `f8ca54d24` was a failed coordinate-only shared-focus candidate and was reverted by `8fddaeea3`; `006_shared_focus_geometry_reversion_2026_07_26.md` records the fresh restored-baseline MCP inspection of 184 focuses, 223 connectors, 49 crossings, 18 node intersections, 27 long connectors, and 14 blocking diagnostics. The v5-v9 audits remain historical where their bounded findings differ from these later records.
...
Commit `37c4f6036` supersedes the earlier DM-58 expiry-gap wording: the activating coordinator schedules `chaosx.nr6.309` for the finite operation duration, and the callback clears shared receipts only when its coordinator target is still current. Live execution and save/load evidence remain open.
...
- The ten attested packages can satisfy the three-, four-, five-, and seven-country automatic waves only when the required number pass their live host, anchor, reservation, Event-5 collision, chaos-band, force, and transaction gates.
  ...
  The ten-country bands remain fail-closed because IW-008 RHI and IW-010 AJX share RG-RHINE-SAAR, leaving only nine mutually compatible reservation groups; a tenth unique-group package or valid map rebinding is required before a ten-country runtime proof.
...
- The current portrait authority is the sourced-only workflow in the event-asset
  ...
  `docs/assets/006_independence_wave/portraits_generated_png/` now preserves 43
  larger pre-resize masters and 80 normalized pre-DDS PNGs with byte/hash
...
| IW-018 ARX | Gameplay retained; visual admission withdrawn | Emilio Lussu is sourced, processed, and wired. Preserve the Sardinian identity, anchor 114, Italian host survival, Mediterranean mechanics, and FORM-05 access. A source-locked Vittorio Vernè commander repaint candidate is preserved under `docs/assets/006_independence_wave/sourced_portrait_replacements_2026_07_26/arx_verne_commander_v1/` with exact crop/equality evidence, deterministic 156x210 output, and bounded separate visual/provenance PASS findings in `006_arx_verne_visual_audit_v13_2026_07_26.md`; Adalberto is vanilla-owned, the crown/council role remains rights-blocked, and Giuseppe Pizzorno is only an unprocessed source lead. Keep execution closed until the crown-route leader is rights-cleared, the candidate passes independent review, and the full package is re-audited |
