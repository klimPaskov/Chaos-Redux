...
The current final bounded audits are `subagent_handoffs/006_event6_focus_final_audit_v70_2026_08_01.md`, `subagent_handoffs/006_iw017_cor_country_package_audit_v66_2026_08_01.md`, `subagent_handoffs/006_event6_decision_mission_final_audit_v71_2026_08_01.md`, and `subagent_handoffs/006_event6_localisation_final_audit_v72_2026_08_01.md`. v70 keeps the focus framework **PARTIAL / HOLD** with fourteen Event 006 MCP layout blockers and identifies the CAT additive/full-framework contract mismatch. COR v66 finds no local package defect but remains **PARTIAL** because shared focus diagnostics, runtime compile/allocation evidence, and Technology Tree Viewer evidence are unavailable. v71 finds no source-level decision or mission defect but leaves the 32-cell scenario, package, and live evidence gaps. v72 passes static localisation and catalog wording checks while leaving live GUI/event-log and parent-owned catalog/source acceptance open. These bounded audits do not change the exact fourteen-package set or the whole-event **HOLD / PARTIAL** status.
...
| Focus and CAT | Treat the fourteen v70 layout diagnostics as Event 006 blockers. Keep CAT fail-closed until its additive/full-framework carrier contradiction is resolved by a reviewed parent decision. |
...
The current whole-event authority is `subagent_handoffs/006_event_completion_audit_v33_2026_07_29.md` with status **HOLD / PARTIAL**. The current static Event 006 content-attestation set is fourteen packages (`IW-001`, `IW-002`, `IW-004`, `IW-006`, `IW-007`, `IW-008`, `IW-009`, `IW-010`, `IW-012`, `IW-017`, `IW-018`, `IW-019`, `IW-173`, and `IW-184`) across thirteen compatible reservation groups and fourteen distinct anchors. The current portrait evidence shelf authority records 78 original-size PNG masters directly under `docs/assets/006_independence_wave/portraits_generated_png/` after the CHU Bolgar v90 v1/v2/v3, ASY Haydo, and ASY Barsoum original-size repaint copies; the current authority declares 73 indexed rows, with five older physical PNGs outside that index (four ARX masters and the CHU Mirsaid master). Normalized 156x210 candidates remain in their package workspaces, and no Event 006 advisor or dossier derivatives are authorized. The two Montenegro masters remain rights-gated evidence only. The v90 Karim Tinchurin source/repaint evidence is `docs/assets/006_independence_wave/iw043_bolgar_civic_presidium_source_v90/` and `subagent_handoffs/006_iw043_chu_bolgar_civic_portrait_source_v90_2026_08_01.md`; it remains `needs_user_review` with no DDS, `.gfx`, character, or content-attestation promotion. The package-local v3 prompt is `docs/assets/006_independence_wave/iw043_bolgar_civic_presidium_source_v90/prompts/karim_tinchurin_hoi4_repaint_v3.txt` (SHA-256 `c0c370e6346e79c6f3b1373e8b18754edaae3afcfa49be19d7a6bf74ffd435ba`); v3 passes identity/style/provenance under v95 but remains rights/date HOLD (`needs_user_review`), while v2 remains HOLD/evidence-only under the v94 audit. ASY Shamoun Hanne Haydo's source/repaint evidence is under `docs/assets/006_independence_wave/asy_portrait_source_research_v91_2026_08_01/`; its v92 audit passes visual likeness, HOI4 style, and provenance, but rights/date remains HOLD (`needs_user_review`), so no DDS, `.gfx`, character, or content-attestation promotion occurred. The ASY Barsoum v2 evidence package is under `docs/assets/006_independence_wave/asy_portrait_source_retry_v92_2026_08_01/` pending independent audit and has no runtime admission.
...
Current routing authority is `subagent_handoffs/006_event_completion_audit_v33_2026_07_29.md`; older v32/v31/v30 references below are retained only as dated traceability. The current shelf authority records 78 original-size PNG masters in one flat directory after the CHU Bolgar v90 v1/v2/v3, ASY Haydo, and ASY Barsoum copies; the current authority declares 73 indexed rows, with five older physical PNGs outside that index (four ARX masters and the CHU Mirsaid master). The v90 source/repaint handoff remains `needs_user_review` with no wire or admission. The package-local v3 prompt is retained at `docs/assets/006_independence_wave/iw043_bolgar_civic_presidium_source_v90/prompts/karim_tinchurin_hoi4_repaint_v3.txt`; v3 passes identity/style/provenance under v95 but remains rights/date HOLD. The ASY Haydo repaint evidence passes visual likeness, HOI4 style, and provenance under v92 but remains rights/date HOLD. The Barsoum v2 evidence package is under `docs/assets/006_independence_wave/asy_portrait_source_retry_v92_2026_08_01/` pending independent audit and has no runtime admission.
...
IW-014 Catalonia is documented as an additive vanilla-carrier package draft, not as a current Event 006 admission. It reuses vanilla `CAT`, capital state 165, reservation group `RG-165`, and the vanilla real-male `CAT_lluis_companys` identity and portrait. The draft package covers industrial cohesion and assembly-legitimacy ledgers, p14 navy-plus-air inheritance, five reinforcement pathways, five route governments, six connected focuses, a 420-day founding mission, eleven concrete-cost decisions, AI response layers, and generation-safe cleanup without a duplicate country, history, flag, leader, portrait, advisor, or custom asset. The v70 focus audit identifies an unresolved contract mismatch because the documentation calls this carrier additive while the current setup requests `full_framework`; no admission or silent documentation rewrite is authorized.
...
The v47 focus audit closes a shared-tree import gap without changing route semantics: the IW-093 Asante and IW-098 Sokoto terminal and sibling focuses are now explicit root imports, so prerequisite traversal reaches all 111 shared focuses and all 184 direct focuses. The v70 final focus audit confirms that the same rendered tree still has fourteen Event 006 MCP layout blockers, 45 crossings, 7 through-node intersections, and 28 long connectors; the geometry reflow remains a separate parent-owned follow-up rather than a package-admission shortcut. Evidence is `subagent_handoffs/006_event6_focus_audit_v47_2026-08-01.md`, `subagent_handoffs/006_event6_focus_final_audit_v70_2026_08_01.md`, and commit `36a1261e0`.
...
content-attestation promotion. The ASY Barsoum v2 evidence package is under
`docs/assets/006_independence_wave/asy_portrait_source_retry_v92_2026_08_01/`
pending independent audit and has no runtime admission.
...
The current whole-event completion authority is `subagent_handoffs/006_event_completion_audit_v33_2026_07_29.md` and remains **HOLD / PARTIAL** for static blockers. It carries forward the bounded DM-58, Statehood Ledger, focus, package, runtime-matrix, asset, AI, balance, formable, super-event, and achievement gaps while recording the later static IW-002, IW-012, and IW-173 admissions, 149 publishers, 126 automatic/high-chaos selectable packages, 138 SCN-008 ranked packages, fourteen exact attestations across thirteen compatible reservation groups and fourteen distinct anchors, current 78-PNG/73-indexed shelf authority after the CHU Bolgar v90 v1/v2/v3, ASY Haydo, and ASY Barsoum copies, corrected Pinna hash, 138/55 selectable-pool distinction with 13 route-only overlays, SCN-008 stale-ledger reset, four deadline-mission availability repairs, the participant-exit cleanup policy, the Samuel Wilder King and Galimzhan v2 visual/provenance audits, the CHU Spasov visual PASS that remains package-unadmitted, and the IW-014 CAT implementation draft that remains outside attestation pending its missing adapter path. The CHU v3 prompt is retained at `docs/assets/006_independence_wave/iw043_bolgar_civic_presidium_source_v90/prompts/karim_tinchurin_hoi4_repaint_v3.txt` (SHA-256 `c0c370e6346e79c6f3b1373e8b18754edaae3afcfa49be19d7a6bf74ffd435ba`); v3 passes identity/style/provenance under v95 but remains rights/date HOLD (`needs_user_review`), while v2 remains HOLD/evidence-only under v94. ASY Shamoun Hanne Haydo's raw repaint and deterministic candidate are retained under `docs/assets/006_independence_wave/asy_portrait_source_research_v91_2026_08_01/`; v92 passes visual likeness, HOI4 style, and provenance, but rights/date remains HOLD (`needs_user_review`), with no DDS, `.gfx`, character, or content-attestation promotion. The ASY Barsoum v2 evidence package is retained under `docs/assets/006_independence_wave/asy_portrait_source_retry_v92_2026_08_01/` pending independent audit and has no runtime admission. Commit `61d059841` and `006_dm58_cost_cleanup_reconciliation_2026_07_27.md` prove source-level validation-before-cost, payment-before-claim/wargoal mutation, coordinator-origin cleanup, and the new recorded-participant invalidation cleanup; static witness, invalidation, dense/no-witness performance, and AI/source audits remain required. FORM-39 remains fail-closed around six writerless inputs and an unadmitted MFX package; the current installed-mod audit reports zero reserved/custom-cosmetic collisions for MFX. IW-157/IW-178 remain research HOLDs, and `6001` remains blocked after v22 research. Historical v5-v32 audits remain preserved where their bounded findings differ from v33. Live witness, save/load, runtime consumer, and player-owned observation are optional future QA under the controlling authority.
...
- The current portrait authority is the sourced-only workflow in the event-asset
  ...
  promotion. The ASY Barsoum v2 evidence package is under
  `docs/assets/006_independence_wave/asy_portrait_source_retry_v92_2026_08_01/`
  pending independent audit and has no runtime admission.
...
7. Preserve the verified Event Details, five evolution mirrors, Liberations
   ...
   or formable IDs.
8. Preserve the current v66 COR, v70 focus, v71 decision/mission, and v72
   localisation audits as bounded evidence. Do not rerun them merely to repeat
   a completed source scan. Re-audit only after the owning source changes, and
