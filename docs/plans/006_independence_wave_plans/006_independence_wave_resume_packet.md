...
The v47 focus audit closes a shared-tree import gap without changing route semantics: the IW-093 Asante and IW-098 Sokoto terminal and sibling focuses are now explicit root imports, so prerequisite traversal reaches all 111 shared focuses and all 184 direct focuses. The rendered tree still has the accepted authored-layout diagnostics (14 blocking MCP diagnostics, 45 crossings, 7 node intersections, and 28 long connectors); the geometry reflow remains a separate parent-owned follow-up rather than a package-admission shortcut. Evidence is `subagent_handoffs/006_event6_focus_audit_v47_2026-08-01.md` and commit `36a1261e0`.
