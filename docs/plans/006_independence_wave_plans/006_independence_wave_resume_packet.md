...
The v47 focus audit closes a shared-tree import gap without changing route semantics: the IW-093 Asante and IW-098 Sokoto terminal and sibling focuses are now explicit root imports, so prerequisite traversal reaches all 111 shared focuses and all 184 direct focuses. The v82 static after-state retains 184 regular nodes and 223 prerequisite connectors with 53 straight-segment crossings, 2 point-on-segment hits, 28 long connectors, 5 same-row gaps under two units, and zero duplicate coordinates. Current MCP inspect/render returns `SCAN_BYTE_LIMIT` with no diagnostics; the v70 fourteen-blocker/45-crossing/7-intersection figures are historical baseline evidence. Geometry and probability follow-up remain separate parent-owned work rather than a package-admission shortcut. Evidence is `subagent_handoffs/006_event6_focus_audit_v47_2026-08-01.md`, `subagent_handoffs/006_event6_focus_final_audit_v70_2026_08_01.md`, `subagent_handoffs/006_event6_focus_geometry_ownership_audit_v97_2026-08-02.md`, and commit `36a1261e0`.
...
8. Preserve the current v97 focus-ownership audit, v66 COR, v70 historical focus
   ...
   evidence. Record the v82 static focus after-state and current MCP
   `SCAN_BYTE_LIMIT`; do not rerun byte-limited inspections merely to repeat a
   completed source scan. Re-audit only after the owning source changes, and
