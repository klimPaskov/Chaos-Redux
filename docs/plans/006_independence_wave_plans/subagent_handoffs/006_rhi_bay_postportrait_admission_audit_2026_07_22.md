...
Status: `static_gameplay_audit_passed_portrait_ownership_failed_runtime_admission_blocked`
...
mechanics, and asset wiring, but neither portrait roster is admissible. A later
exact vanilla ownership check found that Germany recruits both
`GER_konrad_adenauer` and `GER_franz_ritter_von_epp` at game start. Event 006
must not duplicate either active vanilla person. The Adenauer and Epp treatments
remain rejected provenance evidence until different sourced real male
replacements are found. Both packages remain fail-closed and must **not** be
re-admitted to automatic runtime execution or SCN-008 at this time. The
...
The runtime and scenario preflights require that attestation (`:52`, `:140`), so the adapter registry and capacity branches are present but cannot execute either package. The empty attestation remains correct, and the active-character portrait conflicts are additional package-admission blockers.

| Package | Country | Static package audit | Runtime / SCN-008 admission | Recommendation |
| --- | --- | --- | --- | --- |
| IW-008 | RHI, Rhineland | Static gameplay pass; portrait ownership fail | Adenauer is active vanilla GER; attestation empty | Replace Adenauer with a different sourced male Rhenish civic leader, then re-audit |
| IW-009 | BAY, Bavaria | Static gameplay pass; portrait ownership fail | Epp is active vanilla GER; attestation empty | Replace Epp with a different sourced male Bavarian military leader, then re-audit |
...
| Command roster | Pass | Portrait ownership fail | Package effects roster setup and package triggers `:46-55`; both commandants are `is_corps_commander = yes`, but Epp is already an active vanilla GER character. |
...
- RHI currently creates sourced Konrad Adenauer as
  `RHI_independence_wave_provisional_directorate` and sourced Josef Harpe as
  `RHI_independence_wave_river_commandant`; both are explicitly `gender = male`.
  Harpe remains valid, but Adenauer is rejected because vanilla
  `common/characters/GER.txt:155` defines him and
  `history/countries/GER - Germany.txt:887` recruits him for GER.
- BAY currently creates sourced Heinrich Held as
  `BAY_independence_wave_state_council` and sourced Franz Ritter von Epp as
  `BAY_independence_wave_mountain_commandant`; both are explicitly
  `gender = male`. Held remains valid, but Epp is rejected because vanilla
  `common/characters/GER.txt:1067` defines him and
  `history/countries/GER - Germany.txt:1017` recruits him for GER.
...
- All characters are male, with no opposite-gender name-pool or portrait metadata pairing. These are real sourced historical identities; no fictional random-name pool is used. Source compliance does not cure the Adenauer/Epp active-character ownership conflict.
...
1. **Portrait ownership blocker:** Adenauer and Epp are already active vanilla GER characters. Different sourced real male replacements are mandatory; neither a generated face nor a localisation-only rename is acceptable.
2. **Admission blocker:** the authoritative runtime content attestation is intentionally fail-closed (`always = no`), and both runtime and SCN-008 preflight require it. The parent must update that source-of-truth attestation only after the independent package and visual-review gates are satisfied, then rerun this admission audit.
3. **Provenance contract:** the four processor metadata JSON files must remain `candidate_requires_visual_approval`; the separate sourced manifest and `visual_review.md` are the approval authority. This is the required two-record workflow, not an unresolved metadata issue.
4. **Validation boundary:** no live HOI4/MCP renderer or Technology Tree Viewer was available here. Source-level proof is strong, but runtime admission should still be followed by the parent’s normal live-load/SCN-008 checks after attestation changes.
