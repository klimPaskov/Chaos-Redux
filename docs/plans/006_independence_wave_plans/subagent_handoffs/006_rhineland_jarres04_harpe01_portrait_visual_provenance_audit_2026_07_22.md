...
The Kaiserreich character is a real person owner even though the portrait treatment differs. There is no explicit guarded transfer/availability contract that removes `GER_karl_jarres` ownership before the RHI target takes the identity, and the RHI package does not document one. The subject must therefore remain blocked; do not silently rely on the fact that players normally load one mod at a time.
...
**`FAIL` (blocked); no runtime advancement.** Trial 04 is a better identity-preserving attempt than all three earlier rejected revisions, but it does not yet clear full/native likeness. More importantly, Kaiserreich has an active `GER_karl_jarres` owner and no guarded transfer contract exists. A new visual pass cannot cure the ownership collision by itself. Do not convert, copy, register, or wire this candidate; do not substitute a generic or generated Rhenish officeholder.
...
4. Resolve Karl Jarres's Kaiserreich ownership collision with an explicit guarded transfer/availability contract before reusing the identity, or keep the role blocked. No fallback, renamed clone, or generic substitute is authorised.
...
- Jarres is **blocked** by the Kaiserreich active owner in addition to the remaining visual likeness gap. The source/rights chain itself remains defensible.
