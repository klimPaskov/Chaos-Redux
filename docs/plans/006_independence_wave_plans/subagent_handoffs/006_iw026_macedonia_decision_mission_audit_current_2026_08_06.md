...
`independence_wave_setup_iw_026_macedonia` invokes the hidden roster event `chaosx.nr6.350`; that event idempotently attaches `MAC_independence_wave_vardar_presidium` and sets the MAC roster checkpoint only after the same roster trigger passes. The setup effect only marks `independence_wave_iw_026_setup_complete` after that gate passes. This preserves the vanilla carrier history while keeping the dynamic attachment in an authoritative event scope. It is sound static lifecycle proof for the newly added roster/planner contract. It does not remove the decision-surface findings below, and source inspection alone cannot prove a live runtime setup sequence.
