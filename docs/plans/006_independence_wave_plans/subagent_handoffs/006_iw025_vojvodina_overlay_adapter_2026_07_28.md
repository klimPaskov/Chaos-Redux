...
The previously orphaned paid `independence_wave_iw025_vojvodina_start_watch_mission` effect now has the explicit `independence_wave_iw025_mobilize_border_watch` decision caller with anchor and material-cost gates, matching the existing overlay mission pattern. The watch-success legitimacy writer now uses the centralized `watch_legitimacy_gain = 34`, which closes the municipal and federal settlement threshold after the documented depot and mounted-reserve chain (`28 + 8 - 5 + 34 = 65`). This does not promote the overlay to a complete Event 006 package and does not resolve the existing route-loss pause, focus, network, league, formable, source, or runtime boundaries.
