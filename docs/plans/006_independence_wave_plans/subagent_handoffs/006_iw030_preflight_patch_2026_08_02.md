...
- Added phase-aware exact IW-030 branches to `is_independence_wave_runtime_package_preflight_ready` and `is_independence_wave_scenario_package_preflight_ready`: dormant candidates use the exact candidate wrapper, while an explicitly live scope uses the runtime setup/force/cleanup receipt trigger.
