...
- Kept both normal and SCN-008 preflight triggers strictly candidate-scoped (`exists = no`); their callers run before release and cannot evaluate active setup. The active runtime trigger is consumed by the MNT final-validation adapter after setup and activation, so no dead live branch is advertised.
...
- Targeted trigger/effect assertions found the IW-030 wrapper, planner replacement, candidate-only normal/scenario branches, the runtime setup/force receipts consumed by MNT final validation, and lifecycle cleanup exclusion; they also confirmed no `iw_030` content-attestation branch was added.
