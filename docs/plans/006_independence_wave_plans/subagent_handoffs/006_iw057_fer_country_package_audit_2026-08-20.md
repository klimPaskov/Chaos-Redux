...
| Mission and decisions | One 420-day founding mission and ten project decisions exist with package localization; project-ready now requires the existing 408/409 anchor-owned trigger. | No remaining package-local anchor-visibility defect was found in this tranche. |
...
## Post-`5eda32a6f` and current-HEAD re-audit

Commit `5eda32a6f` changes dormant carrier scope checks and the `chaosx.nr6.1` entry-flow receipt handling, but does not touch the IW-057 Region 05 planner, exact FER wrapper, Region 05 automatic-weight effect, or central attestation list. Current `HEAD` is `bd8e436c2` (`Harden IW-057 FER package reachability`), which contains the exact IW-057 planner wrapper and anchor-owned project-readiness guard described above. The working tree has no additional FER source delta.

The exact FER automatic path is present end to end: `can_plan_independence_wave_package_iw_057` calls `is_independence_wave_exact_package_iw_057_tag_available`; `independence_wave_prepare_weight_iw_057` calls that planner and then `independence_wave_calculate_candidate_allocation_weight`; Region 05 includes the resulting weight in `independence_wave_region_05_total_weight`; and `independence_wave_select_region_05_automatic_package` includes `independence_wave_reserve_package_iw_057`. The IW-057 loader selects 408 first or 409 second and passes the selected anchor to the reservation effect.

The exact path does not bypass central admission. `independence_wave_calculate_candidate_allocation_weight` and `independence_wave_begin_package_reservation` still require `has_independence_wave_runtime_package_content_attestation_for_execution_id`. IW-057 remains absent from `has_independence_wave_runtime_package_content_attestation_for_execution_id`, so the exact path reaches the scoring/reservation code structurally but resolves to zero allocation weight or a rejected reservation under the current source.

The stale generic gate remains globally present and is still selection-blocking for packages that use it. `is_independence_wave_candidate_tag_available` is defined in `common/scripted_triggers/006_independence_wave_package_triggers.txt` and requires `independence_wave_package_content_ready`; no setter exists in `common/events` or `common` text. A current static scan found 99 `can_plan_independence_wave_package_iw_*` blocks still calling that generic predicate, including Region 05 IW-047, IW-048, and IW-050 through IW-055. There are 102 total source call sites because two IW-093/IW-098 preflights and the IW-046 exact wrapper also retain the generic dependency. IW-057 is not among those remaining generic planner blocks. This is a broad fail-closed boundary for other unattested packages, not permission to grant the withheld flag or widen central admission.

The refreshed event lint artifact is `hoi4-agent://workspace/mod_chaos_redux_ea3b2d67c2c0/artifact/1ea1c1c6eada40d1cccc293fe2299bc3487b0edd1427a13628ccbab201543e44/b434afe3156b4d5ea4a6cb72ff238fc966ab5fb56e733c1052d39bc6402dd360/event-lint-98ac244e0b19.json`. It reports `EVENT_INSPECTED_PARTIAL`, `blockingDiagnostics=0`, and the same deferred large-workspace helper/lifecycle projection. The refreshed event render bundle is rooted at manifest `hoi4-agent://workspace/mod_chaos_redux_ea3b2d67c2c0/artifact/05495a9b087fb4dac5250966cfc2681fe699f6e478886e1ff8f8f62f6dd0d801/ea8b52b8f276dd6a2235b222f5221ea410f1aeef628c41808a221736e26cb45d/event-state-98ac244e0b19-manifest.json`, with PNG `hoi4-agent://workspace/mod_chaos_redux_ea3b2d67c2c0/artifact/ebe6c3bcce05f3bcd6e9b070bb6aa7e59b4dd4cea9c4a6378c5c3a707208eb18/5bc897e72e5e65cd28925e1a3ae695d8de104682947892caae9cfdbd98944340/event-state-98ac244e0b19.png`; it also reports `blockingDiagnostics=0` with the documented deferral.

The refreshed structural probability artifact is `hoi4-agent://workspace/mod_chaos_redux_ea3b2d67c2c0/artifact/2bc2805a68877a46c802f38a17c3051b276074068a99e1316b95e7daab90c8b4/0bdbf8226295af93c04308d1599203352349a88c598c407e3cf23c4e855f76b4/probability-inspect-998f95c632e0.json`; it reports `poolComplete=false`, `candidates=0`, and `availableCandidates=0` for the custom planner pool. AI strategy discovery reports `no_weighted_surfaces` in `hoi4-agent://workspace/mod_chaos_redux_ea3b2d67c2c0/artifact/95432e93e8d0f16857c13e840bd1900afa13368551aaff299e268fc69dfc6bfe/14a57e664be7fd73bfa0182bc5ef36bf25f4ca911ecae7a1acb2e923cfe746ef/probability-inspect-4b1b9d0035ee.json`; the direct decision adapter returned an MCP `INTERNAL_ERROR`, so neither result is a balance claim. The mandatory `chaosx_ai_probability_auditor` remains absent from the callable inventory.

The refreshed map inspection artifact is `hoi4-agent://workspace/mod_chaos_redux_ea3b2d67c2c0/artifact/ca404169072961c817be345519127c0354db74f1abbaf3073f95771268a16717/11443e9f21f1796561ad6459cca75b3a15d805c70c0230774dc234ee762d57a8/map-inspect.7c0b0196de715c91.json`; states 408, 409, and 563 and Region 05 membership were inspected. State/region membership, geometry, and networks passed; only the same global unrelated building-position and floating-harbor diagnostics remain. The bounded map render passed with the existing `map-state.png` artifact above. The shared focus inspect/render also completed; the shared tree has no FER-specific diagnostic, while the same 14 unrelated continuous-focus icon/layout diagnostics remain. The Technology Tree Viewer is still unavailable.
...
The parent should review the planner and anchor-owned readiness changes and keep IW-057 centrally HOLD until the accepted identity/rights, command-roster, portrait, flag/symbol, and runtime attestation evidence exists. The exact wrapper now reaches automatic allocation structurally, but the central attestation gate still makes the package unavailable. No probability-bearing surface was changed, so the refreshed probability receipts remain structural only and no quantitative balance claim is made.
