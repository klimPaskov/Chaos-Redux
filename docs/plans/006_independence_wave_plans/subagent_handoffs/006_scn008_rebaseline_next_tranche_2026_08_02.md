...
> Superseded for current SCN-008 routing by commit `a4cf1eff4` and the static receipts `006_scn008_32_cell_static_acceptance_2026_08_02.md` plus `006_scn008_edge_case_static_matrix_2026_08_02.md`. The findings below are historical pre-tranche exploration; the former missing-artifact finding and recommended artifact build are no longer current. The receipts are bounded non-live source evidence only and do not change package admission, 14/20 capacity, live execution, or the whole-event **PARTIAL / HOLD** status. Use `006_event6_narrowed_generic_focus_completion_audit_v98_2026_08_02.md`, the source map, and the Event 006 overview for current routing.
...
3. The highest-impact safe next tranche was a non-live SCN-008 acceptance-matrix artifact. That tranche is now complete as bounded static evidence under commit `a4cf1eff4`; the paired 32-cell and edge-case receipts prove the eight modes, four intensities, required edge cases, and ledger alignment without admitting packages, changing focus trees, or requiring live HOI4.
...
| `docs/plans/006_independence_wave_plans/006_source_of_truth_map.md` | Current source-of-truth and queued work order. | The current map records the paired 32-cell and edge-case receipts as static PASS evidence, keeps live playback optional, and distinguishes current 14-package authority from the wider registry. |
| `docs/plans/006_independence_wave_plans/006_independence_wave_resume_packet.md` | Resume boundary and no-reopen rules. | The shared allocator/core is source-closed, the paired SCN-008 receipts pass non-live source validation, and 14/20 capacity remains fail-closed. |
| `docs/plans/006_independence_wave_plans/subagent_handoffs/006_event6_narrowed_generic_focus_completion_audit_v98_2026_08_02.md` | Current whole-event matrix and recommended order. | Its continuation records `STATIC PASS; LIVE/CAPACITY BOUNDARY RETAINED` for the 32-cell and edge-case receipts; the original pre-tranche recommendation is historical. |
...
| `.tools/audit_event6_allocator.py` | Existing static source audit to reuse/extend. | It checks 149 publishers, the exact 14 attested set, 138 ranked IDs excluding overlays, all four intensity mappings, type effects, Great Partition tuning, Universal Former Hosts cleanup, Wars-of-Separation policy, and Event 005-first ordering. The committed companion `.tools/audit_event6_scenario_matrix.py` now emits the paired 32-cell and edge-case receipts. |
...
5. The allocator audit and companion matrix validator were run by the committed tranche, with outputs preserved under `docs/plans/006_independence_wave_plans/subagent_handoffs/`. The catalog remains conservative until a separate accepted QA decision changes it; this explorer does not authorize workbook edits.
...
- Historical pre-tranche finding: no 32-case static/MCP scenario artifact existed at exploration time. The catalog remains `Needs Testing` under the source authority even though the committed non-live receipts now pass.
...
This section is historical and superseded by commit `a4cf1eff4`; the paired static receipts are now the current evidence. Preserve them and do not rerun the former artifact-build recommendation as new work.
