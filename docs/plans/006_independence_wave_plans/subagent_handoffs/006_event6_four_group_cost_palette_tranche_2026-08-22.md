...
The FORM-39 civil-service cost strings were then reconciled with their current payment path. The stale War Support label was removed, the paid palette now shows stability, the combined civic command commitment, and the major-equivalent two-transport reserve, while the tooltip and blocked text retain the spare-civilian-factory availability gate. This is a localisation alignment only; no factory assignment is claimed as a paid resource.
