...
| --- | --- | --- | --- |
| `MNT_blazo_jovanovic` | Blažo Jovanović, 1942 Livno group photograph | `source_ready_needs_pipeline` | High-resolution 1121x1509 Commons source; caption explicitly identifies him as the central subject in the three-person group, and the face matches the separately catalogued 1942 individual record. The source is contemporary wartime material and Commons applies a public-domain rationale through znaci.net. |
| `MNT_blazo_dukanovic` | Blažo Đukanović, estimated 1938–1940 military portrait | `source_ready_needs_rights_review` | Single 443x599 male military portrait with explicit subject caption, period fit near the 1936 start, book/source credit, and Commons `PD-old`/Public Domain Mark. Photographer is unknown, so independent rights review remains mandatory. |
...
This audit does not authorize DDS conversion or `.gfx` wiring. The Jovanović candidate is visually acceptable but remains provenance/source-rights review pending; the Đukanović candidate is visually acceptable but remains explicitly rights/provenance `NEEDS_USER_REVIEW`. Both still require a producer-side repaint prompt/source-lock record and a deterministic 156x210 candidate before any runtime admission.
