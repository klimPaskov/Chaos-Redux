...
| Origin separation | **Event 012 owns the active UGA identity path; Event 006 origin path absent.** | Event 012 writes `africa_priority_origin_buganda` at `common/scripted_effects/012_africa_priority_member_effects.txt:68` and `africa_priority_player_origin_buganda` at `:610`; dispatch preflight rejects active foreign origins in `common/scripted_triggers/006_independence_wave_package_dispatch_triggers.txt:207-214`, but has no IW-108 admission branch. |
...
5. `common/scripted_effects/006_independence_wave_join_effects.txt` contains no `iw_108` or `independence_wave_package_id.iw_108` match. Join must not bypass the admission boundary.
