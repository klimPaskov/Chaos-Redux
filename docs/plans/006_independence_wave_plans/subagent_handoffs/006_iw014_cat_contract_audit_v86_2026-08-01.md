...
| Flag and country identity | `docs/events/006_independence_wave/catalonia_package.md:5,41` and the implementation handoff state that vanilla CAT history, flag, capital, Companys portrait, and carrier identity remain authoritative; vanilla flag assets are `gfx/flags/CAT.tga`, `gfx/flags/medium/CAT.tga`, and `gfx/flags/small/CAT.tga`. | Covered at source level. No new flag, cosmetic tag, or identity fallback is allowed. |
...
No CAT advisor, high-command, theorist, or officer character is defined in vanilla `common/characters/CAT.txt` or `history/countries/CAT - Catalonia.txt`, and the Event 006 adapter adds none. The absence is therefore an explicit reuse boundary, not an unresolved portrait reference; any future advisor addition would require a separate sourced identity and asset review.
