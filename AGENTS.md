...
- Use `chaos-redux-3d-model-pipeline` for Chaos Redux HOI4 3D models and required sound/counter handoffs: Meshy 7 body generation, Blender component modeling, sourced unit audio, and vanilla-green counters. Try Meshy rigging once per model and animation once per required action; on first failure, unusable output, or unsupported stage, use manual Blender rigging and animation under the skill's Blender route without renewed approval. Run the pipeline worker on the default `deepseek-flash` subagent route. Retain valid Meshy outputs and do not repeat paid rig/animation attempts. Planned geometry generation and recovery remain authorized while live balance permits them. Route to `chaosx_3d_model_pipeline`.
...
Stage 2, introduced in the 3.1.0 candidate, adds opt-in bounded event and technology helper-expansion pages, persistent jobs, `hoi4.job_inspect`/`hoi4.job_cancel`, and negotiated MCP task adapters. Use these only after verifying the installed server version, live tool schemas, and client negotiation; until then follow the installed routes. Jobs and helper pages are delivery and coverage tools, not substitutes for the domain-specific inspect, render, compare, and scenario evidence below.
...
- GUI work: follow `chaos-redux-scripted-gui`, create the intended reference before implementing a new or redesigned layout, inspect and render the linked layout, states, resolutions, and click regions, apply the authorized reviewed edit directly or optionally through `hoi4.gui_rewrite`, then compare matching before-and-after scenarios and source artifacts. The rewrite transaction's automatic post-write/index validation is not a mandatory completion gate, and its failure or rollback alone does not require another fallback approval for direct application. Treat the production MCP render as the one-to-one in-game visual view for review: every visible alignment, spacing, clipping, overflow, background, asset, text, state, and click-region defect must be fixed, and an agent may not dismiss a bad GUI as a renderer discrepancy.
...
All project custom subagents must be spawned with a fully explicit, self-contained prompt and no inherited parent-thread context. With the current Codex `collaboration.spawn_agent` tool, set `fork_turns="none"`. In the Qoder, Cursor, opencode, Claude Code, and DSH runtimes subagents are isolated by design, and the parent prompt must still carry every needed input. In Cursor, spawn the generated hyphen-case agent from `.cursor/agents/` through the Task tool or a `/name` invocation. In Claude Code, spawn the generated hyphen-case agent from `.claude/agents/` through the `Agent` tool. DSH composes subagents at runtime through its delegation tool and loads no agent definition files, and it does not read `.codex/agents/*.toml`, so a named role does not arrive with the model its TOML names. Treat the canonical identifiers in `chaos-redux-subagents` as routing and ownership contracts and write the role boundaries into the delegation prompt. DSH layout, skill roots, model selection, and MCP registration are documented in `docs/runtimes.md`. In DSH, every subagent runs on `deepseek-official/deepseek-flash`; `chaosx-subagent-models` is the source of truth for that model policy, and no subagent is ever spawned on an OpenAI model.
...
11. When adding a Chaos Redux land sub-unit, concrete equipment type, special project, special facility, doctrine, or general system, register one package-owned idempotent CXT setup effect through the dynamic extension contract in `docs/testing/chaosx_test_country.md` in the same change. Give the package a modifier-free hidden-idea carrier whose id matches the setup effect before its `_apply` suffix. An additive `on_startup` block must register that carrier token through one bounded existing-country scope so the first console setup consumes it, and an additive `on_daily_CXT` fallback may register the token and mark it pending without granting content. CXT grants, replenishment, and pending-package application after initial setup belong to player-triggered debug decisions; do not add periodic CXT grant hooks.
